...
export function getMemoryRoot(agentDir: string, cwd: string): string {
	// HERMES-OMP PATCH (memory consolidation): under MERCURY_HOME,
	// project-scoped memory lives in the ENGINE-NEUTRAL shared tree
	// ($MERCURY_HOME/memories/projects/<encoded-cwd>) — both engines
	// read and write the same store; no engine name in a shared path
	// (user decision 2026-09-05). Stock behavior
	// (agentDir/memories/<encoded-cwd>) when unset.
	const mercuryHome = process.env.MERCURY_HOME?.trim();
	if (mercuryHome) {
		return path.join(mercuryHome, "memories", "projects", encodeProjectPath(cwd));
	}
