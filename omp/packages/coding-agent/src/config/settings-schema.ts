...
export const SETTINGS_SCHEMA = {
	...
	"delegateFallback": {
		...
		ui: {
			tab: "model",
			group: "Delegation (Mercury)",
			label: "Delegate Fallback",
			description: "Required fallback for the delegate model",
		},
	},
	...
	"theme.dark": {
		type: "string",
		// HERMES-OMP PATCH: mercury theme (orangered identity, matches the
		// hermes-side mercury default skin)
		default: "mercury",
		ui: {
			tab: "appearance",
			group: "Theme",
	...
	"theme.light": {
		type: "string",
		// HERMES-OMP PATCH: mercury-light (white-legible orangered lifts)
		default: "mercury-light",
		ui: {
			tab: "appearance",
			group: "Theme",
			label: "Light Theme",
	...
	"tools.approvalMode": {
		...
		// HERMES-OMP PATCH (unified approvals): safe default — no yolo unless
		// the user explicitly opts in (unified approvals.mode: off, written by
		// the bridge render, or the installer's permanent-yolo option).
		default: "write",
		ui: {
			tab: "interaction",
			group: "Approvals",
	...
	"task.softRequestBudget": {
		...
		ui: {
			...
			description:
				"Soft per-subagent request budget (assistant requests per run). Crossing it injects a wrap-up steering notice (see task.softRequestBudgetNotice); at 1.5x the budget the run is force-stopped and the agent must yield its partial findings. 0 disables the guard. The single bundled agent (subagent) uses the default budget.",
			options: [
				{ value: "0", label: "Disabled" },
