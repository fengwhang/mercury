...
export class Settings {
	...
	async #loadYamlIfPresentForWriteLocked(
		...
		// HERMES-OMP PATCH (unified config): under MERCURY_CONFIG the locked
		// re-read must yield the `omp:` subtree, not the whole shared file —
	...
	async #readExistingMainYaml(quarantineInvalid: boolean): Promise<MainYamlReadResult> {
		...
		// HERMES-OMP PATCH (unified config): when MERCURY_CONFIG points at the
		// one Mercury config (~/.mercury/config.yaml), omp's main config is
		// the `omp:` SUBTREE of that file — never a whole separate config.yml.
		// Reads extract the subtree; writes go through #persistMercurySubtree.
		const mercuryPath = process.env.MERCURY_CONFIG?.trim();
		if (mercuryPath) {
	...
	async #writeYamlAtomically(filePath: string, settings: RawSettings): Promise<void> {
		...
		// shared file with omp-only content.
		let toWrite: RawSettings = settings;
		const mercuryPath = process.env.MERCURY_CONFIG?.trim();
		if (mercuryPath && path.resolve(filePath) === path.resolve(mercuryPath)) {
		...
		try {
			...
			removeTemp = true;
			try {
				await handle.writeFile(stringifyYamlConfig(toWrite), "utf8");
				await handle.sync();
			} finally {
				await handle.close();
			}
