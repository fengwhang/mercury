/** Default session-title model: the online tiny-model path (no local download / on-device inference). */
...
export const AUTO_THINKING_MODEL_OPTIONS = [
	{
		...
		label: "Online (tiny model)",
		description:
			"Classify prompt difficulty online with the online tiny model; no local download or on-device inference.",
	},
	...TINY_MEMORY_LOCAL_MODELS.map(model => ({
		value: model.key,
		label: model.label,
