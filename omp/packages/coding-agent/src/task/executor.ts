 ...
 * {@link installSubagentRetryFallbackChain}), so a legacy role-alias request (
	...
	if (hasYield) {
		...
		if (fallback) {
			...
			if (!result.success) {
				...
				try {
					// HERMES-OMP PATCH (quoted-reply artifact): a scalar string
					// yield must NOT be JSON-serialized — JSON.stringify("hello")
