...
if "__omp_prelude_loaded__" not in globals():
    ...
    def output(
        *ids: str,
        ...
        Args:
            *ids: Output IDs to read (e.g., 'subagent_0', 'subagent_1')
        ...
        Examples:
            output('subagent_0')  # Read as raw text
            output('reviewer_0', format='json')  # Read with metadata
            output('subagent_0', query='.files[0]')  # Extract JSON field
            output('subagent_0', offset=10, limit=20)  # Lines 10-29
            output('subagent_0', 'subagent_1')  # Read multiple outputs
        """
