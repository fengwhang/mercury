...
- Added detailed, non-summarized findings for subagent agents through the report definition field, and subagent result relay so read-only agents can return data to their originating agent.
...
- Fixed read-only subagents (`subagent`, restricted-tool custom agents) crashing before their first prompt when extensions register callable tool schemas.
...
- Fixed `task.softRequestBudget` configuration having no effect on bundled subagent and subagent subagents.
