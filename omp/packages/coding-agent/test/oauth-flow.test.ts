...
describe("mcp oauth flow", () => {
	it("uses mercury client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("mercury");
			...
			if (url === "https://cropwise.example/oauth/register") {
				return new Response(
					JSON.stringify({
						...
						error_description: "client_name 'mercury' is not on the approved list.",
					}),
