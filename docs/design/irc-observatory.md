...
| D11 | Tailscale pin: the bouncer listener (only) pins to the tailnet IP on request; the agent listener stays localhost. `mercury setup observatory` offers it when the tailnet is up. |
