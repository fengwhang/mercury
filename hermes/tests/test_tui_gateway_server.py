...
from mercury_cli.active_sessions import active_session_registry_snapshot
from mercury_cli.browser_connect import ChromeDebugLaunch
...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    ...
    from mercury_cli import mcp_startup
...
def test_wake_owner_is_sticky_and_routes_detection_to_first_transport(monkeypatch):
    ...
    try:
        ...
        assert emitted == [(
            "wake.detected",
            "first-session",
            {"phrase": "hey mercury", "profile": None, "start_new_session": True},
            first,
        )]
...
def test_wake_toggle_persists_enabled_flag_only_on_explicit_gesture(monkeypatch):
    ...
    config = {"enabled": False, "phrase": "hey mercury", "surface": "auto",
              "start_new_session": True}
    persisted = []
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # _get_platform_tools (a non-configurable platform toolset in mercury-cli's
    ...
    from mercury_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mercury_state.SessionDB", lambda db_path=None: profile_db)
    monkeypatch.setattr(server, "_get_db", lambda: launch_db)
    monkeypatch.setattr(server, "_enable_gateway_prompts", lambda: None)
    monkeypatch.setattr(server, "_set_session_context", lambda target: [])
    monkeypatch.setattr(server, "_clear_session_context", lambda tokens: None)
...
def test_stored_session_runtime_overrides_skips_bare_billing_provider(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.config.load_config", lambda: cfg)
...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/mercury-test "}}
    )

    assert server._resolve_model() == "nous/mercury-test"
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `mercury --tui -m <model>` sets HERMES_MODEL/HERMES_INFERENCE_MODEL as a
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "mercury_cli.model_switch.resolve_persist_behavior",
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("HERMES_MODEL", "nous/mercury-test")
    monkeypatch.setenv("HERMES_TUI_PROVIDER", "nous")
    monkeypatch.delenv("HERMES_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/mercury-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/mercury-test", None)
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with MercuryCLI / gateway: fallback_providers stays first and keeps
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "mercury_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from mercury_state_common import _RECOVERABLE_END_REASONS
...
def test_ensure_session_db_row_records_a_terminal_workspace(monkeypatch, tmp_path):
    ...
    The user cd'd there before running mercury. Leaving it null stranded the row
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mercury_state.SessionDB", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    # rather than whatever the developer's real ~/.mercury happens to hold.
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via mercury_cli.config.load_config —
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.models.resolve_fast_mode_overrides",
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("mercury_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "mercury_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_recovers_failed_profile_resume_after_build_completes(
    ...
    monkeypatch.setattr("mercury_cli.model_switch.switch_model", fake_switch_model)
    monkeypatch.setattr(
        "mercury_cli.model_selection_guards.combined_selection_warning",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr("mercury_state.SessionDB", FakeDb)
    monkeypatch.setattr(server, "_make_agent", fake_make_agent)
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("mercury_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_personality_preserves_history_and_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
    # Persistence now flows through the single owner (mercury_cli.personality),
    # never _write_config_key / agent.system_prompt.
    import mercury_cli.personality as personality_mod
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        # attachments dir.
        assert not (workspace / ".mercury" / "desktop-attachments").exists()
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Mercury TUI Status" in out
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("mercury_state.SessionDB", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("mercury_state.SessionDB", ProfileDB2)
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "mercury_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "mercury_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr("mercury_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("mercury_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr("mercury_cli.config.is_managed", lambda: False)
    save_credential = Mock()
    monkeypatch.setattr(
        "mercury_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``mercury --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"mercury_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "mercury_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("mercury_cli.browser_connect.local_port_in_use", return_value=False),
            patch("mercury_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "mercury_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            patch("mercury_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("mercury_cli.browser_connect.local_port_in_use", return_value=True),
            patch("mercury_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_hermes_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.mercury/.env
    ...
    with patch.dict(sys.modules, {"mercury_cli.config": fake}):
...
def test_session_save_writes_under_hermes_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Mercury profile
    ...
    Regression: the gateway handler wrote ``mercury_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Mercury.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("mercury_conversation_*.json"))
    ...
    assert payload["model"] == "mercury-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Mercury."
...
def test_reap_idle_sessions_calls_periodic_trim(monkeypatch):
    ...
    # `from mercury_cli.mem_trim import trim_memory` at call time.
    import mercury_cli.mem_trim as mem_trim
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import mercury_cli.nous_billing as nb
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "Mercury" / "composer-images"
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr("mercury_cli.mem_trim.trim_memory", _inspect_trim_frame)
...
# silently rewrote ~/.mercury/config.yaml top-to-bottom — top-level keys
...
def test_prompt_submit_truncation_archives_instead_of_deleting(monkeypatch):
    ...
    tests/mercury_state/test_replace_messages_archive_siblings.py.
    """
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    # to the profile home, not the root ~/.mercury.
