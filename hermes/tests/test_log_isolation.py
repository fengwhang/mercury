"""The test suite must never write into the operator's real Mercury logs.

`mercury_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(mercury_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
