...
class TestDefaultPlatformWebSearchCoverage:
    def test_hermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("mercury-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("mercury-cli")
        second = resolve_toolset("mercury-cli")
        ...
        assert (
            "mercury-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("mercury-cli", include_registry=False)
        second = resolve_toolset("mercury-cli", include_registry=False)
        assert first == second
