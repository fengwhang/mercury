"""`setup_path()` must also install a `mercury-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `mercury-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Mercury as not installed. `setup_path()` therefore
writes a `mercury-acp` launcher next to the `mercury` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/mercury-acp".*?'
    r'log_success "Installed mercury-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the mercury-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    mercury_bin = tmp_path / "venv" / "bin" / "python"
    mercury_bin.parent.mkdir(parents=True)
    mercury_bin.write_text("#!/bin/sh\n", encoding="utf-8")
    entrypoint = tmp_path / "mercury"
    entrypoint.write_text("#!/bin/sh\n", encoding="utf-8")

    script = (
        "set -e\n"
        f"HERMES_BIN={mercury_bin}\n"
    ...
    return command_link_dir / "mercury-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `mercury-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "mercury-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/mercury-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/mercury-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# mercury-agent launcher regression (#74819)
# ---------------------------------------------------------------------------

HERMES_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/mercury-agent".*?'
    r'log_success "Installed mercury-agent launcher[^\n]*\n)',
    re.S,
)
...
def _extract_hermes_agent_shim_block() -> str:
    ...
    assert match, (
        "could not locate the mercury-agent launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)
...
def _run_hermes_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted mercury-agent block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: mercury-agent is NOT installed by this block (handled
    ...
    assert result.returncode == 0, (
        f"mercury-agent shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "mercury-agent"


def test_venv_install_writes_executable_hermes_agent_launcher(tmp_path):
    """venv install must write a user-executable mercury-agent launcher."""
    ...
    assert "unset PYTHONPATH" in text
    assert "unset PYTHONHOME" in text
    assert "run_agent.py" in text, "mercury-agent must dispatch to run_agent.py"


def test_hermes_agent_launcher_cleanup_on_uninstall(tmp_path):
    """uninstall.remove_wrapper_script() must remove mercury-agent alongside
    mercury and mercury-acp."""
    from mercury_cli.uninstall import remove_wrapper_script

    # Simulate a mercury-agent wrapper in the user-local location
    local_shim = tmp_path / ".local" / "bin" / "mercury-agent"
    ...
    assert local_shim in removed, "local mercury-agent wrapper must be removed"
