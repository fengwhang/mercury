"""Tests for pip entry-point provider discovery (mercury_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``mercury_agent.plugins`` entry point, supporting both a
...
class _FakeEP:
    def __init__(self, name, loader):
        self.name = name
        self.group = "mercury_agent.plugins"
        self._loader = loader
...
def _enable(monkeypatch, *names, disabled=()):
    ...
    """
    import mercury_cli.plugins as hp
