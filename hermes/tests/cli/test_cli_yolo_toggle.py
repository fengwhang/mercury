...
Pre-fix bug (issue #33925): ``cli.MercuryCLI._toggle_yolo`` mutated only
...
prompt. Only ``mercury --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``mercury config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``MercuryCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``MercuryCLI.__init__`` imports a lot of optional
...
def _clear_approval_state(monkeypatch):
    ...
    # Mercury-driven test runs may inherit HERMES_YOLO_MODE=1 from the parent
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``MercuryCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``MercuryCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_enables_session_bypass(self):
        ...
        with patch("cli._cprint"):
            MercuryCLI._toggle_yolo(stand_in)
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            MercuryCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            MercuryCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert MercuryCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            MercuryCLI._toggle_yolo(stand_in)

        assert MercuryCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``mercury --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                MercuryCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            MercuryCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        MercuryCLI._transfer_session_yolo(stand_in, "", "new")
        MercuryCLI._transfer_session_yolo(stand_in, "old", "")
