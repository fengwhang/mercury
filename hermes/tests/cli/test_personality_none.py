...
Updated for the single-owner unification (mercury_cli.personality): built-ins
...
class TestCLIPersonalityNone:
    ...
    def test_set_persists_display_personality_not_system_prompt(self):
        ...
        with patch("mercury_cli.personality.persist_personality", side_effect=_persist):
    ...
    def test_neutral_restores_manual_system_prompt_without_wiping_config(self):
        ...
        with (
            patch("mercury_cli.personality.persist_personality", side_effect=_persist),
            patch(
    ...
    def test_builtin_personality_works_without_config_entry(self):
        # Built-ins come from mercury_cli.personality, not from config.
        cli = self._make_cli(personalities={})
        with patch("mercury_cli.personality.persist_personality", return_value=True):
