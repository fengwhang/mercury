...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.config.load_config", lambda: config)
    monkeypatch.setattr("mercury_cli.runtime_provider.load_config", lambda: config)

    shell = cli.MercuryCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "mercury_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("mercury_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("mercury_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "mercury_cli.models.union_with_portal_paid_recommendations",
    ...
    monkeypatch.setattr(
        "mercury_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    mercury_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    monkeypatch.setattr("mercury_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    saved_env = {}
    monkeypatch.setattr("mercury_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("mercury_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "mercury_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "mercury_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("mercury_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    mercury_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from mercury_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
