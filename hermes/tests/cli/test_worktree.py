...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"mercury/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"mercury pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
    ...
    def test_live_locked_survives_at_any_age(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "mercury-live", pid=os.getpid())
    ...
    def test_dead_locked_clean_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "mercury-dead", pid=999999)
    ...
    def test_dead_locked_dirty_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "mercury-deaddirty", pid=999999, dirty=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_dead_locked_unpushed_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "mercury-deadunp", pid=999999, unpushed=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_unlocked_clean_stale_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "mercury-nolock", pid=None)
        cli._prune_stale_worktrees(str(git_repo))
        assert not wt.exists(), "clean unlocked stale worktree should be reaped"

    def test_dirty_survives_over_72h(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "mercury-dirty72", pid=None, dirty=True, age_h=100)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        import cli
        p = git_repo / ".worktrees" / "mercury-x"
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "mercury/mercury-x", "HEAD"],
    ...
    def test_foreign_lock_reason_returns_dead(self, git_repo):
        import cli
        p = self._mk_locked(git_repo, "mercury-foreign", "some other tool")
...
class TestWidenedPruner:
    ...
    Previously only ``mercury-*`` directories were considered, so salvage/
    ...
    # -- named (non mercury-*) directories are now covered ------------------
    ...
    def test_squash_merged_tree_is_reaped(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "mercury-merged", commit=True, age_h=100)
    ...
    def test_merged_predicate_fails_safe_without_upstream(self, git_repo_no_remote):
        ...
        p = repo / ".worktrees" / "mercury-noremote"
        (repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
...
class TestMergeVerdictCache:
    ...
    dominates ``mercury -w`` startup (~0.2-1.0s per worktree, re-run on every
    ...
    def test_cache_hit_matches_uncached_verdict(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "mercury-cachehit", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_new_commit_invalidates_cached_verdict(self, git_repo):
        ...
        """
        import cli
        wt, sha = self._mk(git_repo, "mercury-moves", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestPruneParallelEquivalence:
    ...
    def _kinds(survivors):
        ...
        for n in survivors:
            for kind in ("merged", "unpushed", "dirty", "fresh"):
                if n.startswith(f"mercury-{kind}"):
                    out.add(kind)
        return out
    ...
    def test_pool_failure_falls_back_to_serial(self, git_repo, monkeypatch):
        ...
        wt, sha = self._mk(git_repo, "mercury-poolfail", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestShallowCloneDeepening:
    ...
    (real incident: 21 of 25 mercury-* trees stuck on a default install).
    ...
    def _stuck_worktree(self, tmp_path):
        ...
        wt = clone / ".worktrees" / "mercury-shallowstuck"
        (clone / ".worktrees").mkdir()
        self._run(
            ["git", "worktree", "add", str(wt), "-b", "mercury/mercury-shallowstuck", "HEAD"],
            clone,
        )
...
class TestPrMergedEscapeHatch:
    ...
    def test_merged_pr_tree_is_reaped(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "mercury-rebase-merged")
    ...
    def test_no_merged_pr_preserved(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "mercury-pr-open")
    ...
    def test_gh_failure_fails_safe(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "mercury-gh-down")
    ...
    def test_dirty_tree_never_reaped_even_with_merged_pr(
        ...
        import cli
        wt = self._mk_diverged(git_repo, "mercury-dirty-merged")
    ...
    def test_merged_verdict_memoized_by_branch_and_head(
        ...
        import cli
        wt = self._mk_diverged(git_repo, "mercury-memo")
        self._stub_gh(tmp_path, monkeypatch)
        cache: dict = {}
    ...
    def test_negative_verdict_not_cached(self, git_repo, tmp_path, monkeypatch):
        import cli
        wt = self._mk_diverged(git_repo, "mercury-nocache-neg")
        self._stub_gh(tmp_path, monkeypatch, stdout="[]")
        cache: dict = {}
