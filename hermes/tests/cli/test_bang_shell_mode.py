...
from mercury_cli.bang_shell import (
    USAGE_HINT,
    bang_shell_enabled,
    is_bang_command,
    parse_bang_command,
...
def _make_cli(history=None):
    """Build a MercuryCLI shell with only what handle_bang_shell touches."""
...
class TestBangHandlerDispatch:
    ...
    def test_bare_bang_prints_usage_and_runs_nothing(self):
        cli = _make_cli()
        with patch("mercury_cli.bang_shell.run_bang_command") as runner:
...
class TestBangApprovalGate:
    ...
    def test_approval_gate_is_invoked_for_a_dangerous_command(self):
        ...
        with patch("tools.terminal_tool._check_all_guards", gate), \
             patch("mercury_cli.bang_shell.run_bang_command", return_value=0):
    ...
    def test_denied_command_is_not_executed(self):
        ...
        with patch("tools.terminal_tool._check_all_guards", gate), \
             patch("mercury_cli.bang_shell.run_bang_command") as runner:
