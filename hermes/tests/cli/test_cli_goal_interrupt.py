...
minimal ``MercuryCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal MercuryCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, mercury_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, mercury_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, mercury_home):
        ...
        src = inspect.getsource(MercuryCLI.chat)
