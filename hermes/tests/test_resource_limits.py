...
from mercury_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "mercury_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import mercury_cli.plugins
    import mercury_cli.web_server
    ...
    monkeypatch.setattr(mercury_cli.plugins, "discover_plugins", lambda: None)
    monkeypatch.setattr(
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "mercury_cli.main", "-p", "default"]
