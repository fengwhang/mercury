...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("mercury.gateway.up"), _M("mercury.gateway.active_agents"),
        _M("mercury.gateway.busy"), _M("mercury.gateway.drainable"),
        _M("mercury.gateway.restart_requested"),
        _M("mercury.platform.up"), _M("mercury.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("mercury.cron.scheduler.heartbeat_age_seconds"),
        _M("mercury.cron.scheduler.last_success_age_seconds"),
        _M("mercury.cron.scheduler.catch_up_occurrences"),
        _M("mercury.cron.jobs.enabled"), _M("mercury.cron.jobs.running"),
        _M("mercury.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Mercury-owned Relay transport health" in text
