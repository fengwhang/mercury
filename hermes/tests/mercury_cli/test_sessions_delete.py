...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None, skipped_open=0):
    """Run `mercury sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(mercury_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["mercury", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `mercury sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from mercury_cli.session_filters import format_epoch
...
def test_sessions_prune_surfaces_matching_open_sessions(monkeypatch, capsys):
    ...
    assert "mercury sessions delete <id>" in out
