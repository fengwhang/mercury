"""Tests for the `mercury proxy` subcommand and its upstream adapters."""
...
from mercury_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from mercury_cli.proxy.adapters.nous_portal import NousPortalAdapter
from mercury_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(mercury_home: Path, nous_state: Dict[str, Any]) -> Path:
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "mercury_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from mercury_cli.proxy.server import create_app  # noqa: E402
