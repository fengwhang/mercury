...
sys.modules breaking the gateway auto-restart after `mercury update`.

Field failure (2026-08-20, Teknium's Linux box): `mercury update` pulled a
checkout where mercury_cli/gateway.py newly imports `line_input` from
mercury_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from mercury_cli.gateway
...
the Mercury package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Mercury modules from the cache; later
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_hermes_modules()
    assert sys.modules.get("mercury_cli.update_cmd") is update_cmd
    assert sys.modules.get("mercury_cli.main") is cli_main
    assert "mercury_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["mercury_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_hermes_modules()
    finally:
        sys.modules.pop("mercury_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from mercury_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
