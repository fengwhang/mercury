...
def _request():
    from mercury_cli.approval_transport import ApprovalRequest
...
def test_host_accepts_bound_sync_and_async_decisions():
    from mercury_cli.approval_transport import invoke_approval_transport
...
def test_host_rejects_decision_completed_after_deadline(monkeypatch):
    import mercury_cli.approval_transport as transport_module
...
def test_host_caps_hung_transport_workers():
    from mercury_cli.approval_transport import (
...
def test_live_temp_home_fixture_plugin_routes_and_hardline_stays_core_owned(
    ...
    import mercury_cli.plugins as plugins_module
    from tools import approval
...
"""
    ...
    try:
        routed = approval.check_all_command_guards(
            "rm -rf /tmp/mercury-approval-transport-fixture", "local"
        )
        manager.discover_and_load(force=True)
        reloaded = approval.check_all_command_guards(
            "rm -rf /tmp/mercury-approval-transport-fixture-reloaded", "local"
        )
        gateway_token = approval.set_hermes_interactive_context(False)
        monkeypatch.setenv("HERMES_GATEWAY_SESSION", "1")
        try:
            gateway_routed = approval.check_all_command_guards(
                "rm -rf /tmp/mercury-approval-transport-fixture-gateway", "local"
            )
        finally:
