...
`mercury` launches the classic prompt_toolkit REPL by default, but users can
flip ``display.interface: tui`` in config.yaml to make the modern Ink TUI the
default for bare ``mercury`` / ``mercury chat``. Explicit flags always win:
...
``mercury … chat -q`` on a pipe, and the TUI's no-TTY bail-out exits 0
...
These tests pin that precedence at every layer that makes the decision:
  ...
  * ``_wants_tui_early(argv)``  — the dependency-free early resolver used by
    ...
    ``mercury_cli.config`` are importable.
...
from mercury_cli import main as m
...
class TestParserFlags:
    ...
    def test_cli_and_tui_are_relaunch_inherited(self):
        from mercury_cli.relaunch import _INHERITED_FLAGS_TABLE
