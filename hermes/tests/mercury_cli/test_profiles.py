...
class TestCreateProfile:
    ...
    def test_fresh_profile_inherits_a_usable_model(self, profile_env):
        ...
        """
        default_home = profile_env / ".mercury"
...
class TestNoSkillsOptOut:
    """Tests for `mercury profile create --no-skills` and the opt-out marker."""
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["mercury-agent"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["mercury-agent"]
        assert len(called) == 1
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `mercury update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".mercury" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("mercury_cli.profiles._cleanup_gateway_service"), \
             patch("mercury_cli.profiles.time.sleep"), \
             patch("mercury_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "mercury_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "mercury_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "mercury_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "mercury_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_hermes_shim(self, profile_env, monkeypatch):
        """A `mercury` console-script shim spawned directly (e.g. Electron's
        findOnPath('mercury') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "mercury" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "mercury".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/mercury", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/mercury", "--profile", "other", "serve"]),
            # Non-mercury script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_hermes_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "mercury" (e.g.
        mercury-notes.py, mercury-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "mercury" -- only the actual known
        console-script entry points (mercury, mercury-agent, mercury-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/mercury-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/mercury-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (mercury-agent,
        mercury-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "mercury"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/mercury-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/mercury-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("mercury_cli.profiles.shutil.which", lambda name: "/opt/mercury/bin/mercury")
        from mercury_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/mercury/bin/mercury -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from mercury_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from mercury_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from mercury_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        honcho_path = tmp_path / ".mercury" / "honcho.json"
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="mercury gateway run --replace",
        ):
...
class TestResolveProfileEnvSpelling:
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        custom = tmp_path / "custom-mercury"
