...
prefix (e.g. ``mission-control.tilos.com/mercury/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /mercury`` on every request.
...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/mercury/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/mercury/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/mercury`` when behind a prefix so they
...
from mercury_cli.dashboard_auth import clear_providers, register_provider
from mercury_cli.dashboard_auth import prefix as prefix_mod
from tests.mercury_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/mercury/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/mercury/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/mercury/auth/callback`` — the
        ...
        assert parsed.path == "/mercury/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``mercury_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "mercury_cli.config.load_config", lambda: cfg
            )
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        patch_config(None)
        monkeypatch.setenv("HERMES_DASHBOARD_PUBLIC_URL", "mercury.domain.com")
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-mercury_session_pkce=")
        ]
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from mercury_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("mercury_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/mercury, so the next request under
        ...
        (``/mercury/auth/login``, ``/mercury/auth/callback``). A cookie
        set with ``Path=/mercury`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/mercury/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "mercury_session_pkce" in c
        )
        # Parse "__Secure-mercury_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-mercury_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/mercury cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/mercury",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-mercury_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/mercury" in at_cookies[0]
