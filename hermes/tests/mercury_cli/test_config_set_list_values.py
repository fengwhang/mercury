"""``mercury config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``mercury config set platform_toolsets.discord '["file","web"]'``
...
def test_list_literal_is_parsed_to_list(user_home):
    from mercury_cli.config import set_config_value, read_raw_config
...
def test_round_trip_through_load_config(user_home):
    ...
    from mercury_cli.config import set_config_value, load_config
