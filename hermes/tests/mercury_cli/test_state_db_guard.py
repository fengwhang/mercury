...
from mercury_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    def mercury_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("mercury_cli.config") or mod == "mercury_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, mercury_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, mercury_home, capsys, monkeypatch):
