"""``mercury`` must survive git operations on the checkout (launcher layout).

The Windows ``mercury`` command is a launcher derived from the venv console
...
(``mercury-agent\\bin``) was swept by ``mercury update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``mercury`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``mercury update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``mercury_constants.venv_bin_dir``), so these
...
from mercury_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: HERMES_HOME/mercury-agent/venv/Scripts + launchers."""
    home = tmp_path / "mercury"
    root = home / "mercury-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    root = tmp_path / "src" / "mercury-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``mercury -p <name>`` HERMES_HOME points inside profiles/<name>;
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `mercury update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "mercury.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "mercury.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\mercury\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/MERCURY/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``mercury update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/mercury.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/mercury.exe is not gitignored — mercury update's autostash "
