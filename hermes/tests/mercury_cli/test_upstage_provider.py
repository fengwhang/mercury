...
Regression guard for the bug where `mercury model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`mercury_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `mercury_cli/main.py` (cmd_model): read
