...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 mercury-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
...
def test_empty_provider_login_page_shows_supported_auth_paths():
    ...
    assert (
        'href="https://mercury-agent.nousresearch.com/docs/'
...
def test_loopback_public_url_fail_closed_message_is_actionable(monkeypatch):
    ...
    assert "mercury dashboard register" in msg
