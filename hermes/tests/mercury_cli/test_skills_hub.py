...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`mercury skills list` reads the active profile's config via
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `mercury skills search`
...
def test_do_search_json_flag_emits_full_identifiers(capsys):
    ...
    from mercury_cli.skills_hub import do_search
