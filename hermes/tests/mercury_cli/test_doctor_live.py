"""Tests for ``mercury doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from mercury_cli import doctor_live
from mercury_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `mercury doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("mercury_cli.doctor.HERMES_HOME", tmp_path / "home")
        monkeypatch.setattr("mercury_cli.doctor.PROJECT_ROOT", tmp_path / "root")
