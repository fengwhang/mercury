...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from mercury_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # mercury model, not via upgrade prompts.
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "mercury_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["mercury", "chat", "--provider", "gmi"])
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("mercury_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
