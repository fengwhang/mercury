...
def mercury_home(tmp_path, monkeypatch):
    ...
    # /etc/mercury on the dev/CI box can't influence the test.
...
def test_user_config_overrides_default(mercury_home, monkeypatch):
...
def test_env_expansion_in_user_config(mercury_home, monkeypatch):
    from mercury_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_hermes_dotenv(mercury_home=str(home))
