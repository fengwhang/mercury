"""Tests for mercury_cli.container_boot — the cont-init.d-time
...
from mercury_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running mercury containers, those
    ...
    """
    monkeypatch.setattr(
        "mercury_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under mercury_home/profiles/<name>/."""
    p = mercury_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `mercury profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    if state is not None:
        (mercury_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (mercury_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (mercury_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        mercury_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from mercury_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/mercury/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
