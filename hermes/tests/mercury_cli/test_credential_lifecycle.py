...
from mercury_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Mercury-Session-Token": _SESSION_TOKEN}
...
@pytest.fixture
def mercury_home(monkeypatch, tmp_path):
...
def test_delete_clears_provider_models_cache(mercury_home):
    _write_env(mercury_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(mercury_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = mercury_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(mercury_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(mercury_home, OPENAI_API_KEY=old)
    _write_config(
        mercury_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = mercury_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
# load_pool() or a separate `mercury auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(mercury_home):
    ...
    the old cached credential until a separate ``mercury auth add opencode-go``
...
def test_put_api_env_writes_auth_json_for_provider(mercury_home):
    """Sentinel for #96058: PUT /api/env must modify auth.json on disk.

    The reported symptom was ``stat -c '%y' ~/.mercury/auth.json`` returning
...
def test_update_rotates_keyed_providers_mirror(mercury_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(mercury_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(mercury_home):
    old = "sk-kp-" + "p" * 24
    _write_env(mercury_home, OPENAI_API_KEY=old)
    _write_config(
        mercury_home,
...
def test_scrub_never_touches_providers_base_url_alias(mercury_home):
