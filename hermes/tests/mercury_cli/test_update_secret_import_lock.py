...
External secret backends are useful during normal Mercury startup, but the
...
from mercury_cli import env_loader
...
""",
    ...
    dispatch = (
        "mercury_main.cmd_update = lambda _args: 0\n"
        "mercury_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import mercury_cli.main as mercury_main\n"
        f"{dispatch}"
...
def test_update_startup_does_not_import_bitwarden_or_cryptography(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["mercury", "update", "--check"])
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["mercury", "chat"])
