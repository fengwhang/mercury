...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("mercury_cli.setup.remove_env_value", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    from an old `mercury setup` run shadowed `agent.max_turns: 500` in
    ...
    monkeypatch.setattr(
        "mercury_cli.setup.get_env_value",
    ...
    monkeypatch.setattr(
        "mercury_cli.setup.remove_env_value",
