...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        mercury = "mercury_cli.main:main"
        mercury-agent = "run_agent:main"
        mercury-acp = "acp_adapter.entry:main"
    """
        )
    )
...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("mercury", "mercury-agent", "mercury-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with patch("mercury_cli.main._is_windows", return_value=True), \
             patch("mercury_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("mercury_cli.main._run_quarantined_install") as mock_install:
            from mercury_cli.main import _verify_console_scripts_installed
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        assert {"mercury.exe", "mercury-agent.exe", "mercury-acp.exe"} <= names
