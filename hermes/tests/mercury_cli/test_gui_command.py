"""Tests for ``mercury gui`` desktop launcher wiring."""
...
from mercury_cli import main as cli_main
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``mercury.desktop`` pointing
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Mercury.app" / "Contents" / "MacOS" / "Mercury"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Mercury.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "mercury"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("mercury_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("mercury_cli.main._desktop_build_needed", return_value=True), \
         patch("mercury_cli.main._write_desktop_build_stamp"), \
         patch("mercury_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("mercury_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("mercury_cli.main._register_linux_desktop_entry"), \
         patch("mercury_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Mercury-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from mercury_constants import iter_hermes_node_dirs
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["mercury"], 0)
    ...
    with patch("mercury_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("mercury_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("mercury_cli.main._desktop_build_needed", return_value=True), \
         patch("mercury_cli.main._write_desktop_build_stamp"), \
         patch("mercury_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("mercury_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("mercury_cli.main.subprocess.run", return_value=launch_ok), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Mercury.app/Contents/MacOS/Mercury in place. Re-downloading Electron can't
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("mercury_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("mercury_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("mercury_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("mercury_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("mercury_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Mercury.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Mercury.app"
    _write_info_plist(app, "com.nousresearch.mercury")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Mercury").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Mercury Helper.app"
    _write_info_plist(helper, "com.nousresearch.mercury.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Mercury.app component, silently skipping every .node/.dylib/crashpad
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(tmp_path, monkeypatch, capsys):
    ...
    identity = "Mercury Local Signing"
    calls = []
    state = {"trusted": False}
    ...
    monkeypatch.setattr("mercury_cli.config.set_config_value", lambda key, value: None)
    ...
    assert not list(tmp_path.glob("mercury-tcc-*"))
...
def test_setup_tcc_identity_fails_when_trust_step_fails(tmp_path, monkeypatch, capsys):
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("Mercury Local Signing") is False
...
def test_setup_tcc_identity_skips_generation_when_already_valid(tmp_path, monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(list(cmd))
        if cmd[:4] == ["/usr/bin/security", "find-identity", "-v", "-p"]:
            return _fake_proc(cmd, stdout='  1) ABCD "Mercury Local Signing"\n     1 valid identities found')
        return _fake_proc(cmd)
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("Mercury Local Signing") is True
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    """`mercury desktop --setup-tcc-identity` calls the setup and exits 0/1
    without building or launching the app."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    with patch("mercury_cli.main._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("mercury_cli.main._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(setup_tcc_identity=True, identity="Mercury Local Signing"))

    assert exc.value.code == 0
    mock_setup.assert_called_once_with("Mercury Local Signing")
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`mercury desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("mercury_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "mercury_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "mercury.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("mercury_cli.main._desktop_build_needed", return_value=False), \
         patch("mercury_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("mercury_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("mercury_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("mercury_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("mercury_cli.main._desktop_build_needed", return_value=False), \
         patch("mercury_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("mercury_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("mercury_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("mercury_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("mercury_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("mercury_cli.main._desktop_build_needed", return_value=True), \
         patch("mercury_cli.main._write_desktop_build_stamp"), \
         patch("mercury_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("mercury_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("mercury_cli.config.load_config", return_value=cfg), \
         patch("mercury_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("mercury_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("mercury_cli.config.load_config", return_value=cfg), \
         patch("mercury_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("mercury_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("mercury_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("mercury_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("mercury_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("mercury_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("mercury_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("mercury_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("mercury_cli.config.load_config", return_value={}), \
         patch("mercury_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("mercury_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("mercury_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("mercury_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("mercury_cli.config.load_config", return_value=cfg), \
         patch("mercury_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("mercury_cli.main._detect_linux_password_store") as mock_detect, \
         patch("mercury_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
