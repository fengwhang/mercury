...
The bug class: on Windows, when `mercury.exe`/sibling shims could not be
...
import mercury_cli._install_repair as ir
import mercury_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("mercury", "mercury-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # mercury.exe cannot be renamed; mercury-gateway.exe can
    real_rename = Path.rename

    def deny_hermes(self, target):
        if self.name == "mercury.exe":
    ...
    assert install_ran == []
    assert "mercury.exe" in exc_info.value.failed_shims
    # The successful rename (mercury-gateway.exe) was rolled back.
    assert (scripts / "mercury-gateway.exe").exists()
    assert not list(scripts.glob("mercury-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("mercury",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("mercury",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = cli_main.ShimQuarantineError(["mercury.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "mercury.exe" in out
