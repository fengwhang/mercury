...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in mercury status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.mercury/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.mercury/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: mercury auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """mercury status should surface freshest gateway last_active (#72016)."""
    from mercury_cli import status as status_mod
    import mercury_cli.auth as auth_mod
    import mercury_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(mercury_state, "SessionDB", _FakeDB)
