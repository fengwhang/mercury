...
from mercury_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
from mercury_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<mercury_home>/plugins/``; we derive
    ``<mercury_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Mercury home for this
    ...
    if auto_enable:
        ...
        if home is not None:
            mercury_home = Path(home)
        else:
            import os
            mercury_home_str = os.environ.get("HERMES_HOME")
            if mercury_home_str:
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        assert "Relay lifecycle is owned by Mercury core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed Mercury Relay plugin" in caplog.text

    def test_enabled_portable_plugin_registers_components(
        self, tmp_path, monkeypatch
    ):
        from mercury_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
        from mercury_cli import plugins as plugins_mod
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from mercury_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from mercury_cli import plugins as plugins_mod
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "mercury_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``mercury_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "mercury_cli.plugins.importlib.metadata.entry_points",
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.config.load_config", lambda: {"hooks": {}}
        )
    ...
    def test_hook_timeout_does_not_block_caller(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.plugins._resolve_hook_callback_timeout", lambda: 0.15
        )
    ...
    def test_hook_callback_within_timeout_returns_value(self, monkeypatch):
        monkeypatch.setattr(
            "mercury_cli.plugins._resolve_hook_callback_timeout", lambda: 1.0
        )
        mgr = PluginManager()
    ...
    def test_hung_callback_suppresses_repeat_fires(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.plugins._resolve_hook_callback_timeout", lambda: 0.1
        )
    ...
    def test_pre_tool_call_timeout_does_not_reach_tool_handler(self, monkeypatch):
        ...
        from mercury_cli.plugins import _PRE_TOOL_CALL_TIMEOUT_BLOCK_MESSAGE
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from mercury_cli import observability
        from mercury_cli.plugins import get_pre_tool_call_directive
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from mercury_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("mercury_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from mercury_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from mercury_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("mercury_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `mercury plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_hermes_home_override(self, tmp_path):
        ...
        ``mercury_constants.set_hermes_home_override`` — deliberately does
        ...
        # a real plugin like mercury-lcm capturing its home at registration),
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``mercury_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``mercury_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `mercury_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("mercury_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginContextProfileName:
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".mercury" / "profiles" / "coder"
