...
def _install_example_plugin(_isolate_hermes_home):
    ...
    real bundled plugins — kanban, mercury-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `mercury plugins
    # enable example`.
...
class TestWebServerEndpoints:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `mercury update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `mercury update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(mercury_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = mercury_state.SessionDB.__init__
        ...
        # the class on mercury_state covers every open the helper makes.
        monkeypatch.setattr(mercury_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(mercury_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mercury_state, "SessionDB", scripted_open)
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from mercury_constants import get_hermes_home
        from mercury_cli.config import load_config, load_env
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        assert cfg["hosts"]["mercury"]["workspace"] == "myws"
        assert cfg["hosts"]["mercury"]["peerName"] == "eri"
        assert cfg["hosts"]["mercury"]["environment"] == "local"
        assert cfg["hosts"]["mercury"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["mercury"]["apiKey"] == "hch-test-key"
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn mercury update")
        ...
        # method through mercury_cli.config directly.
        monkeypatch.setattr(
            "mercury_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_hermes_action", fail_spawn)
        web_server._ACTION_PROCS.pop("mercury-update", None)
        web_server._ACTION_RESULTS.pop("mercury-update", None)

        resp = self.client.post("/api/mercury/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "mercury-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/mercury-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/mercury-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/mercury-agent:latest" in line for line in status_data["lines"])
    ...
    def test_update_hermes_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn mercury update")

        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through mercury_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "mercury_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade mercury-agent"
        assert spawned is False

        check = self.client.get("/api/mercury/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade mercury-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "mercury-update.log").write_text(
            "=== mercury-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== mercury update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== mercury-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/mercury-update/status?lines=2000")
        ...
        assert f"=== mercury-update completed {action_id} ===" in data["lines"]
    ...
    def test_update_hermes_spawns_with_action_id(self, monkeypatch):
        ...
        assert calls == [
            (["update"], "mercury-update", {"HERMES_ACTION_ID": "a" * 32})
        ]
    ...
    def test_update_hermes_reuses_running_action(self, monkeypatch):
        ...
        web_server._ACTION_PROCS["mercury-update"] = Proc()
        web_server._ACTION_IDS["mercury-update"] = "b" * 32

        try:
            resp = self.client.post("/api/mercury/update")
        finally:
            web_server._ACTION_PROCS.pop("mercury-update", None)
            web_server._ACTION_IDS.pop("mercury-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/mercury_cli/test_setup_hidden_env.py.
        """
        from mercury_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Mercury provider — keep the user's aggregator instead of writing a
        provider that can never resolve credentials."""
        monkeypatch.setattr(
            "mercury_cli.model_cost_guard.expensive_model_warning",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "mercury_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/HermesSetupBot/mercury_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/HermesSetupBot/mercury_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "mercury_pair_restart_fails_bot",
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from mercury_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``mercury model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from mercury_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from mercury_cli import profiles as profiles_mod
        from mercury_cli.config import custom_endpoint_key_env
        from mercury_constants import get_hermes_home
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from mercury_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from mercury_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from mercury_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from mercury_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from mercury_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import mercury_cli.tools_config as tools_config
        from mercury_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "mercury_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "mercury"
        save_config(config)
        ...
        assert "mercury@devbox.example.com" in ssh["detail"]
...
class TestStatusInstallId:
    ...
    Mercury home (not the profile home), survives a fresh process-cache read,
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(mercury_home, name="ocean"):
        themes_dir = mercury_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="mercury-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        monkeypatch.setattr(
            mercury_state, "DEFAULT_DB_PATH", get_hermes_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``mercury_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in mercury_cli/web_server.py."
        )
...
class TestPluginAPIAuth:
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, mercury-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (mercury-achievements is loaded by default).
        resp = self.client.get("/api/plugins/mercury-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        plugins installed in the mercury root's plugins/ directory."""
        root = tmp_path / "mercury-root"
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"mercury-review-python{Path(sys.executable).suffix}"
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from mercury_cli import web_server as ws_mod
...
class TestHeadlessServeTokenPage:
    """Headless `mercury serve` must serve the Desktop token handshake page
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/mercury"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/mercury/ds-assets/bg.png)" in prefixed.text
        assert "url(/mercury/fonts-terminal/x.woff2)" in prefixed.text
