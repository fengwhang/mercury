...
def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "mercury_cli.main", "computer-use", *args],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def _invoke(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    cli_main = import_module("mercury_cli.main")
    monkeypatch.setattr(sys, "argv", ["mercury", "computer-use", *args])
...
def test_computer_use_status_returns_nonzero_for_incompatible_standard_driver(
    ...
    assert "Run: mercury computer-use install" in output
