...
class TestMcpTest:
    ...
    def test_probe_uses_configured_connect_timeout(self, monkeypatch):
        ...
        import asyncio
        from mercury_cli import mcp_config
...
class TestProbeEnvResolution:
    ...
    where `mercury mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from mercury_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import mercury_cli.mcp_config as mc
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from mercury_cli.mcp_config import _strip_bearer_prefix
...
class TestMcpLogin:
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.mcp_config._probe_single_server",
...
class TestMcpReauth:
    def test_reauth_all_visits_only_oauth_servers_in_order(
        ...
        visited = []
        monkeypatch.setattr(
            "mercury_cli.mcp_config._reauth_oauth_server",
            lambda name, cfg: visited.append(name) or True,
        )
        from mercury_cli.mcp_config import cmd_mcp_reauth
