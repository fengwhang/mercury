...
def _write_plugin(mercury_home: Path) -> None:
    plugin_dir = mercury_home / "plugins" / "ledger_probe"
...
def _write_profile_probe(mercury_home: Path, marker: str) -> None:
    plugin_dir = mercury_home / "plugins" / "profile_probe"
    plugin_dir.mkdir(parents=True)
    (plugin_dir / "plugin.yaml").write_text(
        yaml.safe_dump(
            {
...
def test_scoped_plugin_cannot_deregister_a_process_global_tool():
    ...
    module_name = "mercury_plugins.scoped_cleanup"
...
def test_direct_plugin_platform_registration_infers_immutable_scope(tmp_path):
    ...
    module_name = "company.mercury.direct_platform_probe"
