...
class TestMissingLogNote:
    ...
    `mercury debug share` runs on the backend, so a desktop connected to a
...
class TestRunDebugShare:
    ...
    def test_share_uploads_five_pastes(self, mercury_home, capsys):
        ...
        assert "--- mercury dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- mercury dump ---" in desktop_paste
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, mercury_home_with_secret, capsys
    ):
        ...
        with patch("mercury_cli.dump.run_dump"), \
             patch("mercury_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("mercury_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``mercury debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, mercury_home):
        ...
        with patch("mercury_cli.dump.run_dump"), patch(
            "mercury_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("mercury_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, mercury_home):
        from mercury_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, mercury_home):
        from mercury_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, mercury_home):
        from mercury_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, mercury_home):
        import gzip
        import json as _json

        from mercury_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "mercury-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, mercury_home, capsys):
        from mercury_cli.debug import run_debug_share
        ...
        with patch("mercury_cli.dump.run_dump"), patch(
            "mercury_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, mercury_home, capsys):
        from mercury_cli.debug import run_debug_share

        with patch("mercury_cli.dump.run_dump"), patch(
            "mercury_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, mercury_home):
        from mercury_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("mercury_cli.dump.run_dump"), patch(
            "mercury_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("mercury_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``MercuryCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("mercury_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`mercury debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, mercury_home, capsys, monkeypatch):
        ...
        with patch("mercury_cli.dump.run_dump"), \
             patch("mercury_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, mercury_home, capsys, monkeypatch):
