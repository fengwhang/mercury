"""Tests for issue #26670 — concurrent mercury.exe detection and improved
quarantine retry / reboot-deferred fallback during `mercury update` on Windows.
...
# helper (and need the autouse stub in tests/mercury_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "mercury.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "mercury.exe"),
    ]
    ...
    assert result == [(launcher_pid, "mercury.exe")]
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("mercury.exe.old.")
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m mercury_cli.main gateway run"
...
# when every concurrent mercury.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, mercury.exe shim, mercury-gateway.exe,
    gateway/run.py, bare `mercury gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "mercury_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\mercury.exe", "gateway", "run"],
        [r"C:\venv\Scripts\mercury-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["mercury.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["mercury.exe", "gateway"],  # bare `mercury gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["mercury.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\mercury.exe"],  # interactive REPL
        [r"C:\venv\Scripts\mercury.exe", "dashboard"],
        ["mercury.exe", "gateway", "status"],  # management, not runtime
        ["mercury.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["mercury.exe", "gateway", "run"],
                200: ["mercury.exe"],  # REPL — keep
                300: ["mercury.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "mercury.exe"), (300, "mercury.exe"), (400, "mercury.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["mercury.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\mercury-gateway.exe"],
            }
        ),
    )
    matches = [(111, "mercury.exe"), (222, "mercury-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_hermes_instances",
        return_value=[(1000, "mercury.exe"), (2000, "mercury-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another mercury.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_hermes_instances",
        return_value=[(1000, "mercury.exe"), (3000, "mercury.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "mercury.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m mercury_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
