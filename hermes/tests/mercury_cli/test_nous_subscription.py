...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "mercury_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `mercury model` overwrote it to `web.backend: nous`.
    """
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "mercury_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `mercury tools`
# ---------------------------------------------------------------------------
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("mercury_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_hermes_managed_node_path(
    ...
    still find a runnable agent-browser under the Mercury Node dir even when
    ...
    monkeypatch.setattr(
        "mercury_constants.with_hermes_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "mercury_constants.agent_browser_runnable",
