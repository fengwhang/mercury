"""Tests for mercury_cli.gateway_windows."""
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda mercury_home: "")
    monkeypatch.setattr("mercury_cli.config.get_hermes_home", lambda: str(mercury_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "mercury_cli.main"]
    assert cwd == str(mercury_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "mercury_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr("mercury_cli.config.get_hermes_home", lambda: tmp_path)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_hermes_home(self, tmp_path, monkeypatch):
        ...
        home.mkdir()
        monkeypatch.setattr("mercury_cli.config.get_hermes_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".mercury"
        project = tmp_path / "checkout"
        monkeypatch.setattr("mercury_cli.config.get_hermes_home", lambda: missing)
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Mercury\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Mercury\venv\Scripts\python.exe"
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "mercury_cli.main" in content
...
# resume_pending=True) never fires when `mercury gateway stop` kills the
