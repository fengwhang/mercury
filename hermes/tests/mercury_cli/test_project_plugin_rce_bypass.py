...
1. ``mercury_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.mercury/plugins/`` source via
...
2. ``mercury_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.mercury/plugins``
...
class TestEndToEndPocBlocked:
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "mercury_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "mercury_dashboard_plugin_evil" not in sys.modules
