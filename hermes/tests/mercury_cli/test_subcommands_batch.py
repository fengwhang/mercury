...
def test_config_get_unset_subcommands_parse():
    """`mercury config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `mercury login` fails gracefully, not with argparse error ────
#
# `mercury login` is a removed command; its handler (`login_command` in
# `mercury_cli/auth.py`) prints a deprecation notice pointing at `mercury auth` /
# `mercury model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `mercury --help`.
