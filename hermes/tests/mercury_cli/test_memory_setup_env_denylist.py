...
``mercury_cli.memory_setup._write_env_vars`` persists provider plugin
credentials to ``~/.mercury/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or Mercury-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next Mercury process would load it through the
...
from mercury_cli.config import ensure_hermes_home, get_env_path, load_env
from mercury_cli.memory_setup import _write_env_vars


def _env_file_keys() -> set[str]:
    """Parse ``~/.mercury/.env`` directly and return the set of keys present.
...
def test_explicit_hermes_home_writes_to_that_env_file(tmp_path):
    ...
    Mercury home; keep that target while still routing through
    ``save_env_value`` validation instead of writing directly."""
    home = tmp_path / "plugin-home"

    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, mercury_home=home)
