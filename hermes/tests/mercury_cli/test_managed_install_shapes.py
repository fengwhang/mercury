...
def test_env_var_names_the_managing_system(mercury_home, monkeypatch, env_value, expected):
...
def test_marker_file_names_the_managing_system(
    mercury_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the HERMES_MANAGED of the service."""
    (mercury_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("HERMES_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(mercury_home, monkeypatch):
    (mercury_home / ".managed").write_text("nixos", encoding="utf-8")
...
def test_managed_install_names_its_system_and_offers_an_update(
    mercury_home, monkeypatch, tmp_path, managed_value
):
    ...
    # `mercury update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "mercury update"
...
def test_unmanaged_install_offers_no_update_command(mercury_home, monkeypatch):
    monkeypatch.delenv("HERMES_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(mercury_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = mercury_home / ".managed"
