...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("mercury_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "mercury_cli.banner", fake_banner)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    # One driver subprocess parses ALL argvs: mercury_cli.main is a very heavy
    # import (previously 11 separate `python -m mercury_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `mercury <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `mercury -t web chat` silently dropped the
    ...
    Uses the real `mercury_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `mercury <flag> <value> chat`:\n  "
