...
Canonical owners:
  ...
  * ``mercury_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/mercury/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "mercury_cli/managed_scope.py",
