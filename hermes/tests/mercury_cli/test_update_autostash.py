...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mercury_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(mercury_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    mercury_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_mercury_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import mercury_constants

    monkeypatch.delattr(mercury_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(mercury_constants, "apply_subprocess_home_env")

    mercury_main._reload_updated_runtime_modules()

    assert callable(mercury_constants.apply_subprocess_home_env)
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        mercury_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        mercury_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "mercury_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    mercury_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    mercury_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``mercury update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(mercury_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".mercury-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by mercury update (mercury_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "mercury-update-autostash")

    assert marker.exists(), (
        ".mercury-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".mercury-bootstrap-complete" not in status
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "mercury-agent.rb").write_text("formula\n")
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    assert "made the Mercury agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert mercury_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
