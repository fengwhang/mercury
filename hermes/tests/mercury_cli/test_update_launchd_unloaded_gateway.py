"""Regression for #74973 — `mercury update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``mercury gateway restart``.
...
def launchd(monkeypatch):
    """Stub mercury_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.mercury.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.mercury.gateway"], [])
    ...
    def test_restart_failure_warns_that_gateway_is_down(self, launchd, capsys):
        ...
        assert "mercury gateway restart" in out
...
ai.mercury.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.mercury.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.mercury/bin/mercury
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.mercury.gateway")
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from mercury_cli.gateway import _parse_launchd_pid_from_print_output
