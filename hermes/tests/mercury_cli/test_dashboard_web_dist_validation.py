"""Regression tests: `mercury dashboard` validates HERMES_WEB_DIST before serving.
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "mercury_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "mercury_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    dist = project_root / "mercury_cli" / "web_dist"
