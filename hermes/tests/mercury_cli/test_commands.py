...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /mercury <command> on Slack only
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `mercury slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
class TestTelegramMenuCommands:
    ...
    def test_configured_priority_promotes_skill_into_last_menu_slot(
        ...
        with (
            patch("mercury_cli.commands.telegram_bot_commands", return_value=fake_core),
            patch("agent.skill_commands.get_skill_commands", return_value=fake_cmds),
            patch("mercury_cli.plugins.get_plugin_commands", return_value=fake_plugins),
            patch("tools.skills_tool.SKILLS_DIR", local_dir),
            patch("mercury_cli.commands._telegram_command_menu_config", return_value=menu_cfg),
        ):
    ...
    def test_scalar_configured_priority_is_accepted_as_one_command(self):
        ...
        from unittest.mock import patch
        from mercury_cli.commands import _telegram_effective_priority
        ...
        with patch("mercury_cli.config.read_raw_config", return_value=raw_config):
