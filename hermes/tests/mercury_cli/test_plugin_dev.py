...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from mercury_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``mercury plugins doctor`` with no argument defaults to ``.``.
    ...
    from mercury_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = mercury_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_hermes_home", lambda: mercury_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a mercury-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded mercury-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("mercury-plugin-doctor-*"))
