"""Tests for mercury_cli.process_identity — spawn tags, the machine spawn
...
purpose, spawner) in spawn-ledger.json, and `mercury update` reaps holders the
...
def test_install_id_stable_and_path_scoped():
    a = pi.install_id(Path("/opt/mercury"))
    assert a == pi.install_id(Path("/opt/mercury"))
...
def _holders(*pids):
    return [(p, "python.exe", f"python.exe -m mercury_cli.main --profile p{p} serve") for p in pids]
