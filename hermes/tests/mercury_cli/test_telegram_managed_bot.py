"""Tests for mercury_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Mercury & Friends",
        )
        assert "Mercury+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "mercury_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/HermesSetupBot/mercury_abcdefghijklmnop_bot?name=Mercury+Agent",
            "qr_payload": "https://t.me/newbot/HermesSetupBot/mercury_abcdefghijklmnop_bot?name=Mercury+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "mercury_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Mercury")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="mercury_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/HermesSetupBot/mercury_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/HermesSetupBot/mercury_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "mercury_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "mercury_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("mercury_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("mercury_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("mercury_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from mercury_cli.setup import _setup_telegram_auto
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        seen = {}
        profile_home = tmp_path / ".mercury" / "profiles" / "oracle"
        ...
        monkeypatch.setattr(
            "mercury_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from mercury_cli.setup import _profile_name_from_hermes_home

        assert (
            _profile_name_from_hermes_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\mercury\profiles\oracle")
            )
            == "oracle"
        )
