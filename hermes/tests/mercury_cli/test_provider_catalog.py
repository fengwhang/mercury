"""Tests for the unified provider catalog (mercury_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``mercury model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from mercury_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
