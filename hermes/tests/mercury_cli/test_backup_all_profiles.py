...
import mercury_cli.backup as backup
...
def profiles(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mercury_cli.profiles._get_default_hermes_home", lambda: default_home
    )
    monkeypatch.setattr(
        "mercury_cli.profiles._get_profiles_root", lambda: tmp_path / "home" / "profiles"
    )
    monkeypatch.setattr(
        "mercury_cli.profiles._PROFILE_ID_RE",
...
class TestSiblingEnumeration:
    ...
    def test_never_raises(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("mercury_cli.profiles._get_default_hermes_home", _boom)
...
class TestAllProfileSnapshots:
    ...
    def test_one_failing_sibling_does_not_block_others(self, profiles, monkeypatch):
        ...
        def _flaky(label=None, mercury_home=None, keep=None, max_file_size=None):
            if mercury_home == profiles["work"]:
                raise OSError("disk full")
            return real(
                label=label, mercury_home=mercury_home, keep=keep,
