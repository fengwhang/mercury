"""Venv ownership preflight for ``mercury update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo mercury`` contains root-owned files
(e.g. ``site-packages/mercury_agent-*.dist-info/INSTALLER``). A later normal
``mercury update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/mercury`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "mercury").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "mercury_agent-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "mercury_agent-1.0.0.dist-info" / "INSTALLER"
    )
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    mercury_bin = str(venv / "bin" / "mercury")
    real_uid = update_cmd._path_uid
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == mercury_bin else real_uid(p),
    )
    with pytest.raises(SystemExit) as exc:
        update_cmd._refuse_update_if_venv_foreign_owned(tmp_path)
    assert exc.value.code == 1
    out = capsys.readouterr().out
    assert mercury_bin in out
