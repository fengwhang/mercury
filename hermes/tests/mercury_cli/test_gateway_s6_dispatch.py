"""Tests for the Phase 4 s6 dispatch helper in mercury_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`mercury gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_all_handles_partial_failure(
    ...
    from mercury_cli import gateway as gw
    ...
    monkeypatch.setattr(
        "mercury_cli.service_manager.detect_service_manager", lambda: "s6",
    )
    monkeypatch.setattr(
        "mercury_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "mercury_cli.service_manager.detect_service_manager",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("mercury_cli.gateway._profile_suffix", lambda: "")

    def missing_sleep(file: str, args: list[str]) -> None:
        raise FileNotFoundError(2, "No such file or directory", file)

    monkeypatch.setattr("mercury_cli.gateway.os.execvp", missing_sleep)
    block_calls: list[bool] = []
    monkeypatch.setattr(
        "mercury_cli.gateway._block_until_terminated",
