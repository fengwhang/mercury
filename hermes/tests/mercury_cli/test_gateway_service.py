...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`mercury gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `mercury gateway restart` in the background; when a
        ...
        assert "mercury gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../mercury_test`` HERMES_HOME — silently breaking
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="HERMES_HOME=/tmp/mercury-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/mercury-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="HERMES_HOME={tmp_path}/mercury-home"\n'
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".mercury" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `mercury gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import mercury_cli.resource_limits as resource_limits
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        assert ("observe", "ai.mercury.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
...
class TestSystemUnitHermesHome:
    ...
    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        ...
        managed_bin = tmp_path / ".mercury" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".mercury")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_hermes = target_home / ".mercury"
        root_home = tmp_path / "root"
        root_hermes = root_home / ".mercury"
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "mercury_constants.iter_hermes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "mercury_constants.mercury_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HERMES_HOME=/home/alice/.mercury' in unit
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        mercury_home = str(gateway_cli.get_hermes_home().resolve())
        assert f'HERMES_HOME={mercury_home}' in unit


class TestSystemUnitRefreshSyncsHermesHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.mercury."""

    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_hermes = root_home / ".mercury"
        alice_hermes = alice_home / ".mercury"
        root_hermes.mkdir(parents=True)
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("HERMES_HOME", str(root_home / ".mercury"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".mercury")
        ...
        assert "WorkingDirectory=/home/alice/.mercury" in unit
        assert "WorkingDirectory=/home/alice/.mercury/mercury-agent" not in unit
...
class TestLegacyHermesUnitDetection:
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = "mercury.service" if i == 0 else "mercury.service"  # same name
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "mercury-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "mercury-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "mercury-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``mercury gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl start mercury-gateway" in out
        assert "sudo mercury gateway uninstall --system" in out
        assert "mercury gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``mercury gateway start --system`` etc.) must
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.mercury.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.mercury.gateway"
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        mercury = tmp_path / "home" / ".mercury"
        mercury.mkdir(parents=True)
        (mercury / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("HERMES_HOME", str(mercury))
        monkeypatch.delenv("HERMES_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("HERMES_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
