"""Regression tests for migration from the removed Mercury Relay plugin."""
...
from mercury_cli.doctor import collect_relay_plugin_cutover_findings
...
def test_enable_rejects_removed_relay_plugin_without_discovery(name, capsys):
    with (
        patch("mercury_cli.plugins_cmd._resolve_plugin_key_and_source") as resolve,
        patch("mercury_cli.plugins_cmd._save_enabled_set") as save_enabled,
    ):
...
def test_enable_rejects_alias_resolving_to_removed_relay_plugin(capsys):
    with (
        patch(
            "mercury_cli.plugins_cmd._resolve_plugin_key_and_source",
