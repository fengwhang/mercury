"""Tests for ``mercury peer`` — cross-machine bot-to-bot DMs."""
...
from mercury_cli.subcommands import peer as peer_cmd
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "mercury.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``mercury peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``mercury peer
