"""Tests for mercury_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from mercury_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://mercury-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/mercury-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import mercury_cli.model_catalog as model_catalog
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in mercury_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``mercury model`` CLI — not the live
        ...
        importlib.reload(model_catalog)
        try:
            from mercury_cli.model_switch import list_picker_providers
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("mercury_cli.models.check_nous_free_tier", return_value=False), patch(
                "mercury_cli.models.union_with_portal_free_recommendations",
