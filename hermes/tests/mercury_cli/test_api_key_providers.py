...
from mercury_cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        from mercury_cli.copilot_auth import _invalidate_gh_cli_token_cache
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config_module, "get_hermes_home", lambda: mercury_home)
    ...
    def test_config_provider_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        """
        import yaml
        mercury_home = self._setup_home(monkeypatch, tmp_path)
        ...
        monkeypatch.setattr("mercury_cli.auth.get_auth_status", _trap)
        from mercury_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert sweep_calls == [], (
    ...
    def test_auth_json_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("mercury_cli.auth.get_auth_status", _guarded_status)
        from mercury_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert calls == ["nous"], (
...
class TestZaiParallelProbe:
    ...
    def test_candidate_model_fallback_within_endpoint(self, monkeypatch):
        ...
        from mercury_cli.auth import ZAI_ENDPOINTS, detect_zai_endpoint
    ...
    def test_priority_order_wins_over_completion_order(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.auth.httpx.post", _slow_first)
        result = detect_zai_endpoint("test-key", timeout=1.0)
        assert result is not None
        assert result["id"] == first[0]

    def test_all_fail_returns_none(self, monkeypatch):
        from mercury_cli.auth import detect_zai_endpoint

        monkeypatch.setattr("mercury_cli.auth.httpx.post", self._mock_post({}))
    ...
    def test_early_exit_does_not_wait_for_slow_losers(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.auth.httpx.post", _slow_losers)
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from mercury_cli import models as models_mod
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        from mercury_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from mercury_cli.models import _fetch_deepinfra_models_by_tag
        import mercury_cli.models as _m
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from mercury_cli.auth import PROVIDER_REGISTRY, resolve_provider
