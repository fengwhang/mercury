...
import mercury_cli.auth as auth
...
def mercury_home(tmp_path, monkeypatch):
    ...
    resolve to the real user's ~/.mercury/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, mercury_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, mercury_home, windows_default_encoding):
        ...
        assert not (mercury_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, mercury_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, mercury_home):
        auth_path = mercury_home / "auth.json"
...
# --- sibling readers of the same ~/.mercury/auth.json in other modules -------
#
# _load_auth_store lives in mercury_cli/auth.py, but several other modules read
# the same ~/.mercury/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, mercury_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(mercury_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, mercury_home, windows_default_encoding, monkeypatch):
        ...
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", mercury_home / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """mercury_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, mercury_home, monkeypatch, windows_default_encoding
    ):
        """mercury_cli.main._has_any_provider_configured reads auth.json.
        ...
        # mercury_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, mercury_home, windows_default_encoding
    ):
