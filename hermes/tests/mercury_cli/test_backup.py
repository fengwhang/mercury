...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew mercury_cli.backup's datetime forward instead of sleeping.
    ...
    import mercury_cli.backup as _backup
...
def _make_hermes_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "mercury_state.db"):
...
class TestShouldExclude:
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from mercury_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        mercury_home.mkdir()
        _make_hermes_tree(mercury_home)

        monkeypatch.setenv("HERMES_HOME", str(mercury_home))
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = mercury_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, mercury_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        ...
        _symlink_file_or_skip(mercury_home / "skills" / "outside-link.txt", outside)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        mercury_home.mkdir()
        _make_hermes_tree(mercury_home)
        with sqlite3.connect(mercury_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from mercury_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup
        ...
        assert snap_id and (mercury_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Mercury configuration has been restored." in out
        assert "mercury gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        (mercury_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (mercury_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (mercury_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            mercury_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (mercury_home / "gateway.pid").write_text("4242")
        (mercury_home / "processes.json").write_text('{"live": true}')
        ...
        assert (mercury_home / "gateway.pid").read_text() == "4242"
        assert (mercury_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (mercury_home / "cron.pid").exists()
        assert not (mercury_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (mercury_home / rel).stat().st_mode & 0o777
...
class TestImportAtomicWrites:
    """`mercury import` must never leave a user's file truncated.
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (mercury_home / "config.yaml").write_text(original)
        monkeypatch.setenv("HERMES_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (mercury_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(mercury_home.glob(".config.yaml.*")) == []
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        store = mercury_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = mercury_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        target = mercury_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo mercury import`` onto a user-owned (or Docker/NAS volume-owned)
        ...
        monkeypatch.setattr(
            "mercury_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``mercury import`` is the
...
class TestSafeCopyDb:
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from mercury_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from mercury_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    ...
    def test_failed_state_db_copy_is_loud(self, mercury_home, monkeypatch, capsys):
    ...
    def test_restore_state_db_live_connection(self, mercury_home):
        ...
        live_conn = sqlite3.connect(str(mercury_home / "state.db"))
    ...
    def test_snapshot_includes_pairing_directories(self, mercury_home):
        ...
        (mercury_home / "platforms" / "pairing").mkdir(parents=True)
        (mercury_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (mercury_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (mercury_home / "pairing").mkdir()
        (mercury_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (mercury_home / "feishu_comment_pairing.json").write_text(
...
# ---------------------------------------------------------------------------
# Pre-update backup (mercury update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, mercury_home, capsys):
        ...
        first_id = create_quick_snapshot(label="complete", mercury_home=mercury_home)
        assert first_id is not None
        first_dir = mercury_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", mercury_home=mercury_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = mercury_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, mercury_home=mercury_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, mercury_home):
        ...
        board_dir = mercury_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(mercury_home=mercury_home)
        copy = (
            mercury_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, mercury_home=mercury_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, mercury_home, monkeypatch):
        ...
        import mercury_cli.backup as bk
        from mercury_cli.backup import create_quick_snapshot

        board = mercury_home / "kanban" / "boards" / "work"
        ...
        copy = mercury_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``mercury update``
    ...
    def test_backup_contents_match_full_backup(self, mercury_home):
        """Pre-update backup should include the same user data that
        ``mercury backup`` would, and should exclude the same directories."""
        from mercury_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(mercury_home=mercury_home)
        ...
        # mercury-agent repo excluded
        assert not any(n.startswith("mercury-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, mercury_home):
        """``mercury update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", mercury_home=mercury_home)
        assert snap_id and (mercury_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, mercury_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(mercury_home=mercury_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (mercury_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, mercury_home, tmp_path):
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def mercury_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".mercury"
        root.mkdir()
        _make_hermes_tree(root)
    ...
    @staticmethod
    def _set_mode(mercury_home, value):
        import yaml
        (mercury_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(mercury_home):
        d = mercury_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(mercury_home):
        d = mercury_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, mercury_home, capsys):
        ...
        self._set_mode(mercury_home, "off")
        from mercury_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(mercury_home)
        assert not self._zips(mercury_home)



    def test_config_full_mode(self, mercury_home, capsys):
        self._set_mode(mercury_home, "full")
        from mercury_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(mercury_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (mercury claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``mercury claw migrate`` runs before mutating ~/.mercury/."""
    ...
    def test_restorable_with_hermes_import(self, mercury_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Mercury
        backup — `mercury import` should accept it."""
        from mercury_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(mercury_home=mercury_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, mercury_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from mercury_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(mercury_home=mercury_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(mercury_home=mercury_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`mercury update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, mercury_home: Path, label="pre-update"):
        from mercury_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, mercury_home=mercury_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from mercury_cli.backup import restore_cron_jobs_if_emptied
        mercury_home = tmp_path / ".mercury"
        jobs_path = mercury_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(mercury_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, mercury_home=mercury_home)
        assert result is not None
...
class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, mercury_home: Path) -> None:
        mercury_home.mkdir(parents=True, exist_ok=True)
        (mercury_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (mercury_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (mercury_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        self._make_min_tree(mercury_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        ...
        assert not (mercury_home / "_external").exists()
