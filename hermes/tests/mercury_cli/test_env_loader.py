...
def test_recovered_update_retry_skips_external_secret_sources(tmp_path, monkeypatch):
    ...
    loaded = load_hermes_dotenv(mercury_home=home)
...
def test_latin1_fallback_stream_honors_override(tmp_path, monkeypatch):
    ...
    from mercury_cli.env_loader import _load_dotenv_with_fallback
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="mercury_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    """Known Mercury keys inherited from parent process are removed when absent
    ...
    load_hermes_dotenv(mercury_home=home)
...
def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Mercury key that IS explicitly set in the profile .env survives
...
def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Mercury key defined with the bash-compatible ``export KEY=value``
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from mercury_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
# the shared mercury_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def test_config_yaml_terminal_backend_overrides_stale_env(tmp_path, monkeypatch):
    """Regression for #29186: a leftover TERMINAL_ENV=docker in ~/.mercury/.env
...
def test_other_profile_home_does_not_bridge_process_config(tmp_path, monkeypatch):
    ...
    load_hermes_dotenv(mercury_home=other_home)
