...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /mercury-gateway.service"`` and runs ``systemctl --user daemon-reload``
...
def _run_native_windows_gateway_start_diag(
    ...
    from mercury_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mercury_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("mercury_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="HERMES_HOME=/home/alice/.mercury"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    fake_path = tmp_path / "mercury-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "mercury_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "mercury_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Mercury Desktop opens (``mercury serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time Mercury Desktop opens
    (``mercury serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `mercury gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "mercury-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "mercury-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "mercury-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "mercury-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "mercury_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        ...
        import mercury_cli.gateway_windows as gateway_windows
