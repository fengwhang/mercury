...
def test_windows_native_install_path_docs_match_installer() -> None:
    ...
    # the checkout (which `mercury update`'s autostash swept off disk).
    assert "%LOCALAPPDATA%\\mercury\\bin" in doc
    assert (
        "Get-Command mercury        # should print "
        "C:\\Users\\<you>\\AppData\\Local\\mercury\\bin\\mercury.exe"
    ) in doc
    # Installer exposes $HermesHome\bin, and must copy the launchers into it.
    assert '$hermesBin = "$HermesHome\\bin"' in install
    assert "mercury.exe" in install and "mercury-acp.exe" in install
