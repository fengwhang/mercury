...
``mercury*.exe`` shims aside (``mercury.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``mercury`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("mercury", "mercury-agent", "mercury-acp", "mercury-gateway"):
...
# ---------------------------------------------------------------------------
# mercury_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "mercury.exe" in names, "mercury.exe must be restored after a no-op install"
    assert "mercury-acp.exe" in names
    assert "mercury-gateway.exe" in names
    assert (scripts / "mercury.exe").read_bytes() == b"MZ-old-mercury"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "mercury.exe").read_bytes() == b"MZ-new-mercury"
...
# ---------------------------------------------------------------------------
# mercury_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "mercury.exe" in names, "mercury.exe must be restored after a no-op recovery install"
    assert (scripts / "mercury.exe").read_bytes() == b"MZ-old-mercury"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "mercury.exe").write_bytes(b"MZ-new-mercury")
