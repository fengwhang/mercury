"""Tests for mercury_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `mercury_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import mercury_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("mercury_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
# unprivileged mercury user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from mercury_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``mercury gateway run``, an agent action, the Open WebUI helper) holds
    ...
    # Root profile: bare `mercury gateway run --replace` (no -p flag).
    assert "mercury gateway run --replace" in default_text
    assert "mercury -p default" not in default_text
...
#
# `mercury gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_hermes_without_chown(
    ...
    """log/run must not root-chown/unlink volume paths; create leaf as mercury.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    mercury). Restartable log/run must not pathname-chown or pathname-rm a
    mercury-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on mercury-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid mercury mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid mercury rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_hermes_idx = log_text.index('s6-setuidgid mercury mkdir -p "$log_dir"')
    rm_as_hermes_idx = log_text.index('s6-setuidgid mercury rm -f "$log_dir/lock"')
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = mercury_home / "logs" / "gateways"
    ...
    # a real mercury uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that mercury cannot unlink a foreign
    ...
    env = os.environ.copy()
    env["HERMES_HOME"] = str(mercury_home)
