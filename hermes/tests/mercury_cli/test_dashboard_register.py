"""Tests for ``mercury dashboard register``.
...
import mercury_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "mercury_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("mercury_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Nous Portal" in out
        assert "mercury setup" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "mercury_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("mercury_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "mercury_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "mercury_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            "mercury_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("mercury_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "mercury_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "mercury_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
...
class TestPublicUrlPersistence:
    ...
    https://mercury.example.com/auth/callback). At serve time the dashboard auth
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://mercury.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "mercury_cli.config.get_env_value", return_value=None
        ), patch(
            "mercury_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://mercury.example.com/auth/callback",
                )
            )
        assert saved["HERMES_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://mercury.example.com"
