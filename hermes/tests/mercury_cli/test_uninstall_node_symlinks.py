"""Tests for mercury_cli.uninstall.remove_node_symlinks.
...
only when they still resolve into the Mercury-managed node dir.
"""
...
def _make_hermes_node(mercury_home: Path) -> Path:
    """Create a fake $HERMES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = mercury_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    mercury_home = fake_home / ".mercury"
    _make_hermes_node(mercury_home)
    ...
    removed = uninstall.remove_node_symlinks(mercury_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    mercury_home = fake_home / ".mercury"
    node_bin = _make_hermes_node(mercury_home)
