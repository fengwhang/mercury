"""Tests for mercury_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import mercury_cli.gui_uninstall as gu


def _make_agent(mercury_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = mercury_home / "mercury-agent"
    (agent_root / "mercury_cli").mkdir(parents=True)
    (agent_root / "mercury_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(mercury_home: Path) -> None:
    """Create the source-built GUI artifacts a `mercury desktop` run produces."""
    desktop = mercury_home / "mercury-agent" / "apps" / "desktop"
    ...
    (mercury_home / "mercury-agent" / "node_modules").mkdir(parents=True)
    (mercury_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(mercury_home: Path) -> None:
    (mercury_home / "config.yaml").write_text("x: 1\n")
    (mercury_home / ".env").write_text("KEY=secret\n")
    (mercury_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(mercury_home)
    _make_gui_build(mercury_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(mercury_home)
    ...
    assert summary["mercury_home"] == str(mercury_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `mercury desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    mercury_home = tmp_path / ".mercury"
    _make_agent(mercury_home)
    icon = lde.icon_path(mercury_home / "mercury-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(mercury_home)
    ...
    assert lde.icon_path(mercury_home / "mercury-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (mercury_home / "mercury-agent" / "mercury_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".mercury")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import mercury_cli.uninstall as uninstall
