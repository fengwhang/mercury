...
Mercury-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Mercury' id in the public agent-harness
registry (``mercury-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Mercury process env, and survives the
...
from mercury_cli.main import _advertise_agent_env

# Registry id — must stay in sync with huggingface.js agent-harnesses.ts.
HARNESS_ID = "mercury-agent"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._snapshot_path = "/tmp/mercury-snap-testsession0.sh"
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT MERCURY=$HERMES_AGENT"')
        ...
        assert f"AI={HARNESS_ID} MERCURY=true" in out.stdout
        ...
        assert "AI=pi MERCURY=false" in out.stdout
