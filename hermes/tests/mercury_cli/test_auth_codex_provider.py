...
def _setup_hermes_auth(mercury_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(mercury_home, access_token="")
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((mercury_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``mercury auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``mercury auth add openai-codex``.
    ...
    (mercury_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `mercury auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``mercury auth add openai-codex`` for accounts B and C
    ...
    (mercury_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``mercury auth add openai-codex`` (account B and account C).
...
def test_resolve_returns_hermes_auth_store_source(tmp_path, monkeypatch):
    mercury_home = tmp_path / "mercury"
    _setup_hermes_auth(mercury_home)
    monkeypatch.setenv("HERMES_HOME", str(mercury_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run mercury auth".
    """
    from mercury_cli.auth import (
    ...
    assert "mercury auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from mercury_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from mercury_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch mercury_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("mercury_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
