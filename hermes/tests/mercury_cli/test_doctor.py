...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/mercury-agent:latest" in hint
        assert "recreate all Mercury containers" in hint
        assert "mercury update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `mercury update`" in hint

    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade mercury-agent`" in hint
        assert "mercury update" not in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `mercury doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "HERMES_HOME", mercury_home)
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (mercury_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".mercury"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`mercury doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        ...
        leaves doctor probing the REAL ~/.mercury. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from mercury_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("mercury_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "mercury_cli.doctor", raising=False)
        from mercury_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (mercury_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "HERMES_HOME", mercury_home)
        monkeypatch.setattr(doctor_mod, "get_hermes_home", lambda: mercury_home)
        ...
        return buf.getvalue(), mercury_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, mercury_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (mercury_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (mercury_home / ".env").read_text(encoding="utf-8")
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir(parents=True)
        (mercury_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (mercury_home / ".env").write_text(env_body, encoding="utf-8")
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.mercury" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.mercury"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.nousresearch.mercury" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (mercury desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.mercury" and certificate root = H"aabbcc"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
