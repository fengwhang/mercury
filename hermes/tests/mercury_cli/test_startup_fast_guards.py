"""Guards for mercury_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from mercury_cli.config import ...`` silently makes `mercury --version`
...
def test_startup_fast_import_weight():
    ...
    probe = (
        "import sys, json\n"
        "import mercury_cli._startup_fast\n"
    ...
    assert not offenders, (
        f"mercury_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "mercury_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Mercury v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    assert "Mercury v" in result.stdout
