...
from mercury_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'mercury' instead of 'mercury-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import mercury_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["mercury"]}}

    with caplog.at_level(logging.WARNING, logger="mercury_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "mercury" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["mercury-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["mercury-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``mercury-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["mercury-telegram", "discord"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/mercury-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/mercury-agent/issues/6901
    """
    config = {
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool._find_agent_browser (PATH -> Homebrew/Mercury-managed
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("mercury_cli.tools_config._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("mercury_cli.tools_config._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "mercury_cli.tools_config._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "mercury_cli.tools_config._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Mercury-managed-Node-only setups: npx resolves via
        ...
        mercury_npx = "/home/user/.mercury/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool._chromium_installed", return_value=False
            ...
            "tools.browser_tool._resolve_npx_bin", return_value=mercury_npx
        ), patch(
            "mercury_cli.tools_config._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            mercury_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "mercury_cli.tools_config._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("mercury_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("mercury_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
            "mercury_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        with patch(
            "mercury_cli.tools_config.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from mercury_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "mercury_cli.tools_config.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("mercury_cli.tools_config._print_warning") as warn:
...
class TestImagegenModelPicker:
    ...
    def test_plugin_picker_falls_back_when_default_is_missing_from_catalog(self):
        ...
        from mercury_cli.tools_config import _configure_imagegen_model_for_plugin
        ...
        with (
            patch(
                "mercury_cli.tools_config._plugin_image_gen_catalog",
                return_value=(catalog, "also-missing"),
            ),
            patch("mercury_cli.tools_config._prompt_choice", return_value=0),
        ):
...
#    reported as added/removed by `mercury tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `mercury tools` printed ``- kanban``
...
# `mercury tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `mercury tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[mercury-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    ...
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["mercury-cli"]}},
        "cli",
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `mercury config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (mercury-acp, mercury-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"mercury-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"mercury-{platform}"))
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``mercury-cli``), it MUST be admitted as a configurable key instead of
    ...
    import mercury_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want mercury-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["mercury-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [mercury-cli, a2a]``, ``a2a``
    ...
    config = {"platform_toolsets": {"cli": ["mercury-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
...
class TestLightpandaPostSetup:
    ...
    def test_reports_binary_when_found(self, _stub_browser_use_install):
        from mercury_cli.tools_config import _run_post_setup

        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value="/opt/lightpanda"), \
             patch("mercury_cli.tools_config._print_success") as ok, \
             patch("mercury_cli.tools_config._print_warning") as warn:
    ...
    def test_prints_install_hint_when_missing(self):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value=None), \
             patch("mercury_cli.tools_config._print_warning") as warn, \
             patch("mercury_cli.tools_config._print_info") as info:
            _run_post_setup("lightpanda")
