...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        with patch("mercury_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
class TestBaseUrlValidation:
    ...
    def test_empty_base_url_keeps_default(self, config_home, monkeypatch):
        ...
        with patch("mercury_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("mercury_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestZaiEndpointPicker:
    ...
    def test_custom_proxy_rejects_invalid_url(self, config_home, monkeypatch, capsys):
        ...
        with patch("mercury_cli.main._prompt_provider_choice", return_value=custom_idx), \
             patch("mercury_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("mercury_cli.auth.deactivate_provider"), \
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from mercury_cli.auth import ZAI_ENDPOINTS
        from mercury_cli.model_setup_flows import _select_zai_endpoint
        ...
        with patch("mercury_cli.main._prompt_provider_choice", side_effect=fake_choice):
