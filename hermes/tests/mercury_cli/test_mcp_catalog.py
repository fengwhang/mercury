"""Tests for mercury_cli.mcp_catalog and mercury_cli.mcp_picker.
...
def _isolate_hermes_home(tmp_path, monkeypatch):
    ...
    hh = tmp_path / "mercury-home"
    hh.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(hh))
    monkeypatch.setattr(
        "mercury_cli.config.get_hermes_home", lambda: hh
    )
    monkeypatch.setattr(
        "mercury_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "mercury_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_hermes_home() lazily through mercury_constants
    monkeypatch.setattr(
        "mercury_constants.get_hermes_home", lambda: hh
    )
    return hh
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from mercury_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from mercury_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from mercury_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from mercury_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from mercury_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from mercury_cli.config import load_config
        from mercury_cli.mcp_catalog import CatalogError, install_entry
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from mercury_cli.mcp_catalog import install_entry, uninstall_entry
        from mercury_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from mercury_cli.mcp_catalog import uninstall_entry
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from mercury_cli import mcp_catalog
        from mercury_cli.mcp_catalog import _do_git_install
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        from mercury_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Mercury fetches/launches is pinned
        to an exact version.
