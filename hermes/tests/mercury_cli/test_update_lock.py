"""Cross-process update mutual exclusion (``mercury_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``mercury
...
dashboard-spawned ``mercury update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".mercury-update-in-progress"
...
def test_marker_path_follows_process_hermes_home(tmp_path, monkeypatch):
    ...
    assert update_marker_path() == tmp_path / ".mercury-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``mercury update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Mercury is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``mercury-setup`` under ``~/.mercury`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("mercury_cli.update_lock._pid_alive", lambda pid: True)
