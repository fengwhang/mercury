...
1. The venv-repair import probes (early recovery + `mercury update`) must
...
2. ``mercury doctor`` must fail the certificate check in that state, and
   ``mercury doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `mercury update`'s venv repair must apply
...
# =========================================================================
# 2. mercury doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "mercury doctor --fix" in message
