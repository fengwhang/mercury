...
def _home_env(tmp_path, **extra):
    """Env dict redirecting EVERY home-resolution knob into tmp_path.

    ``get_env_path()`` keys on ``MERCURY_HOME`` ($MERCURY_HOME/.env is THE
    env file and outranks HERMES_HOME), so patching HERMES_HOME alone leaves
    .env writes aimed at a launcher-exported real home — the 2026-09-07
    incident where a fixture token overwrote the operator's GITHUB_TOKEN.
    ``MERCURY_CONFIG`` is NEUTRALIZED (empty string ≡ unset for every
    ``.strip()``-based reader) instead of pointed at a file: any non-empty
    value reroutes ``get_config_path()`` onto the unified Mercury file and
    re-shapes save_config writes (``hermes:`` subtree nesting), which would
    change what these legacy-layout tests assert. Empty still guarantees an
    ambient launcher ``MERCURY_CONFIG`` cannot leak into the test.
    """
    env = {
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\mercury — see
                # mercury_constants._get_platform_default_hermes_home.
...
class TestEnsureHermesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from mercury_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Mercury, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``mercury logs`` surfaces it)
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from mercury_cli.config import _deep_merge
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_roundtrip(self, tmp_path):
        with patch.dict(os.environ, _home_env(tmp_path)):
            config = load_config()
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, _home_env(tmp_path), clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, _home_env(tmp_path, KEY_B="value_b")):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, _home_env(tmp_path, ORPHAN_KEY="orphan")):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, _home_env(tmp_path, DROP="gone")):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("mercury_cli.config.sanitize_env_file", return_value=2),
            patch(
                "mercury_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("mercury_cli.config.read_raw_config", return_value={}),
            patch("mercury_cli.config.get_missing_env_vars", return_value=[]),
            patch("mercury_cli.config.get_missing_config_fields", return_value=[]),
            patch("mercury_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `mercury setup`) lets a
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `mercury setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `mercury setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_hermes_home(self):
        from mercury_cli.config_migrations import support_floor_message
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from mercury_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Mercury runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Mercury
    process on next startup via the dotenv → ``os.environ`` chain in
    ``mercury_cli/env_loader.py``.
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from mercury_cli.config import invalidate_env_cache
    ...
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from mercury_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from mercury_cli.config import _env_line_defines_key
    ...
    def test_save_env_value_secure_inherits_denylist(self, tmp_path):
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from mercury_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, _home_env(tmp_path)):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: mercury\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "mercury"
...
class TestConfigCommandFailClosedSurface:
    """`mercury config set/unset` must exit cleanly (no traceback) when the
