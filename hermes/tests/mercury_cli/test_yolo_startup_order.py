...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    ...
    monkeypatch.setattr(
        "mercury_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("mercury_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("HERMES_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from mercury_cli.main import main as cli_main
