"""Unit tests for mercury_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/mercury_cli/test_pty_bridge.py.  Drives
...
``mercury_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from mercury_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from mercury_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from mercury_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp
    ...
    def test_non_numeric_falls_back_to_min(self):
        from mercury_cli.win_pty_bridge import _MAX_COLS, _clamp
