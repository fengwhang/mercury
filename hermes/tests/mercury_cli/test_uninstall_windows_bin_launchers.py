"""Uninstall must not leave a dangling ``mercury`` command on Windows.

Every uninstall mode deletes the code checkout, but the launchers install.ps1
staged in the managed binary dir (the default Mercury root's ``bin``, shared
with the managed uv) live outside it. A surviving launcher makes ``mercury``
...
from mercury_cli import uninstall
...
def managed_bin(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (bin_dir / "mercury.exe").write_bytes(b"MZ launcher")
    (bin_dir / "mercury-acp.cmd").write_text("@echo off\r\n", encoding="ascii")
...
def test_removes_both_launcher_forms_and_keeps_managed_uv(managed_bin: Path):
    ...
    assert sorted(p.name for p in removed) == ["mercury-acp.cmd", "mercury.exe"]
    assert not (managed_bin / "mercury.exe").exists()
    assert not (managed_bin / "mercury-acp.cmd").exists()
...
def test_noop_on_posix(managed_bin: Path):
    ...
    assert (managed_bin / "mercury.exe").exists()
...
def test_launcher_names_stay_in_lockstep_with_install_ps1():
    ...
    for name in _WINDOWS_BIN_LAUNCHERS:
        assert name.startswith("mercury")  # never a generic name it could clobber
...
class TestManagedBinPathMarker:
    ...
    HOME = r"C:\Users\me\AppData\Local\mercury"
    BIN_ENTRY = r"C:\Users\me\AppData\Local\mercury\bin"
