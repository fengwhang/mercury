...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    host install is correctly identified as 'git' and 'mercury update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("mercury_cli.config.get_managed_system", return_value=None), \
         patch("mercury_cli.config.get_hermes_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    with patch("mercury_cli.config.get_hermes_home", return_value=home):
        from mercury_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``mercury update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("mercury_cli.config.get_managed_system", return_value=None), \
         patch("mercury_cli.config.get_hermes_home", return_value=tmp_path), \
         patch("mercury_constants.is_container", return_value=True):
        from mercury_cli.config import detect_install_method
