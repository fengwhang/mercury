...
The update receipt (written by every `mercury update` run since #91283,
`latest.json` pointer) is the durable outcome record. These tests pin:

- GET /api/mercury/update/receipt returns the full receipt + summary; 404
  when none exists.
- GET /api/actions/mercury-update/status attaches the receipt summary, and
...
def client():
    ...
    from mercury_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
...
class TestUpdateReceiptEndpoint:
    ...
    def test_receipt_endpoint_returns_full_receipt_and_summary(self, client, tmp_path, monkeypatch):
        ...
        resp = client.get("/api/mercury/update/receipt")
...
class TestUpdateStatusReadsReceipt:
    ...
    def test_status_attaches_receipt_summary(self, client, tmp_path, monkeypatch):
        ...
        resp = client.get("/api/actions/mercury-update/status")
