...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.mercury/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.mercury            (has its own auth.json fixture)
    * Profile     -> tmp_path/.mercury/profiles/coder   (active, HERMES_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Mercury is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (HERMES_HOME != ~/.mercury, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    mercury_home = tmp_path / "classic"
    mercury_home.mkdir()
