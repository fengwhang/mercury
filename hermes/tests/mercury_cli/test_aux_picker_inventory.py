...
Every aux picker (``mercury model`` → Configure auxiliary models, the
``mercury tools`` vision picker, and any future one) must route through
``mercury_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from mercury_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("mercury_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    """
    import mercury_cli.main as main
    ...
    with (
        patch("mercury_cli.model_switch.list_authenticated_providers", _direct),
        patch("mercury_cli.inventory.build_aux_picker_rows", _substrate),
        patch("mercury_cli.main._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through mercury_cli.inventory.build_aux_picker_rows so custom "
