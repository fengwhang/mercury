...
#88251: ``mercury update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_constants.with_hermes_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "mercury_constants.display_hermes_home", lambda: str(tmp_path), raising=False
    )
