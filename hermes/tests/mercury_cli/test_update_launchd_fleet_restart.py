"""Regression for #41403 — ``mercury update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.mercury.gateway-<profile>`` services kept running pre-update
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through mercury_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.mercury.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.mercury.gateway", current, "ai.mercury.gateway-user-scoped"],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.mercury.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.mercury.gateway",
            f"user/{UID}/ai.mercury.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.mercury.gateway",
            f"user/{UID}/ai.mercury.gateway-user-scoped",
        ...
        assert restarted == [
            current,
            "ai.mercury.gateway",
            "ai.mercury.gateway-user-scoped",
        ]
        assert failed == []
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.mercury.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.mercury.gateway-a"],
            located={"ai.mercury.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.mercury.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mercury.gateway",
            labels=["ai.mercury.gateway", "ai.mercury.gateway-idle"],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.mercury.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mercury.gateway",
            labels=["ai.mercury.gateway", "ai.mercury.gateway-dormant"],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.mercury.gateway-dormant"]
        assert restarted == ["ai.mercury.gateway", "ai.mercury.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mercury.gateway",
            labels=["ai.mercury.gateway", "ai.mercury.gateway-a"],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.mercury.gateway-a"]
        assert restarted == ["ai.mercury.gateway", "ai.mercury.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.mercury.gateway",
                "ai.mercury.gateway-bad",
                "ai.mercury.gateway-good",
            ],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-bad": (f"gui/{UID}", 200),
                "ai.mercury.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.mercury.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.mercury.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.mercury.gateway-good"]
        assert restarted == ["ai.mercury.gateway", "ai.mercury.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.mercury.gateway",
                "ai.mercury.gateway-wedged",
                "ai.mercury.gateway-after",
            ],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.mercury.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.mercury.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.mercury.gateway-after"]
        assert restarted == ["ai.mercury.gateway", "ai.mercury.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-wedged": (f"gui/{UID}", 200),
                "ai.mercury.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.mercury.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mercury.gateway",
            labels=["ai.mercury.gateway", "ai.mercury.gateway-zombie"],
            located={
                "ai.mercury.gateway": (f"gui/{UID}", 100),
                "ai.mercury.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.mercury.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.mercury.gateway"]
        assert failed == ["ai.mercury.gateway-zombie"]
