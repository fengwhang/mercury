...
class _FakeObs:
    ...
    def detect_tailscale(self):
        return dict(self._kw.get("ts", {"available": False, "up": False,
                                        "ip": None, "dns_name": None}))
...
def _patch_common(stack, fake, *, choice=1, yes_answers=None):
    ...
    stack.enter_context(patch.object(setup_mod, "_tailscale_status",
                                     return_value={"available": False, "up": False,
                                                   "ip": None, "dns_name": None}))
    stack.enter_context(patch.object(setup_mod, "_offer_observatory_reset",
                                     return_value=False))
    stack.enter_context(patch.object(setup_mod, "_offer_bouncer_password_rotate"))
    stack.enter_context(patch.object(setup_mod, "_prompt_server_label",
                                     return_value="mercury"))
    auto = stack.enter_context(patch.object(setup_mod, "_run_observatory_auto_steps",
                                     return_value={"unit": "installed",
                                                   "gateway": "converged"}))
...
def test_bind_offer_pins_bouncer(monkeypatch):
    ...
    monkeypatch.setattr(subprocess, "run", lambda *a, **k: (_ for _ in ()).throw(
        AssertionError("should not restart in this test")) if False else None)
...
def test_bind_mismatch_action_lines():
    ...
    assert setup_mod._bind_mismatch_action_line(["100.64.0.1", "127.0.0.1"], ts_up) is None
...
def test_headless_setup_provisions():
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(
            setup_mod, "_run_observatory_auto_steps",
...
def test_setup_card_mentions_bouncer_not_password(capsys):
    ...
    setup_mod._print_observatory_setup_card(status, dict(
