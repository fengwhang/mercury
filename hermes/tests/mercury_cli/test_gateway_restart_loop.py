...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.mercury.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.mercury/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l mercury-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.mercury.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.mercury.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.mercury.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.mercury/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.mercury.gateway-apollo:/a.plist' "
            "'ai.mercury.gateway-cronus:/c.plist' 'ai.mercury.gateway-plutus:/p.plist' "
            "'ai.mercury.gateway:/Users/botuser/Library/LaunchAgents/ai.mercury.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated mercury note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.mercury.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.mercury.gateway',
        "launchctl kick'start' -k gui/501/ai.mercury.gateway",
        "launchctl kick\\start -k gui/501/ai.mercury.gateway",
        'launchctl "kickstart" -k gui/501/ai.mercury.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.mercury.gateway',
        "launchctl dis\\able gui/501/ai.mercury.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.mercury."gateway"',
        # Same class on the systemctl and mercury-CLI branches.
        'systemctl re"start" mercury-gateway',
        'mercury gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.mercury.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.mercury.update-checker',
        'systemctl re"start" mercury-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "mercury cron list",
        "mercury update",
        "mercury config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `mercury gateway start` is benign — starting a gateway from inside a
        ...
        "mercury gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway mercury
        # services must not be falsely blocked (the old `.*mercury` matched any
        # mercury token).
        "launchctl unload ai.mercury.update-checker.plist",
        "launchctl restart ai.mercury.daemon",
        ...
        "systemctl restart mercury-meta.service",
        "systemctl restart mercury-cron-helper",
        ...
        "echo after the mercury gateway restarted cleanly",
        "the mercury gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "mercury skill view gateway-notes && echo mercury gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `mercury` is a path component
        # there, not a command.
        "cat '/docs/mercury gateway restart-notes.md'",
        "less /home/user/notes/mercury gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "mercury gateway restart",
        "mercury gateway restart; echo done",
        "mercury gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;mercury gateway restart",
        "true && mercury gateway stop",
        "echo $(mercury gateway restart)",
        "echo `mercury gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.mercury.gateway"])',
        'subprocess.run(["mercury", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.mercury.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["mercury", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: mercury gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `mercury -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    @pytest.mark.parametrize("text", [
        "mercury -p zeus gateway stop",
        "mercury -p zeus gateway restart",
        "mercury --profile zeus gateway restart",
        "mercury --profile zeus gateway stop",
        "mercury --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "mercury -v -p zeus gateway restart",
        "mercury -p zeus -v gateway restart",
        "mercury --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "mercury -p venus gateway stop",
        "mercury -p venus gateway restart",
        "mercury --profile venus gateway restart",
        "mercury --profile=venus gateway stop",
        "mercury -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "mercury -p zeus gateway start",
        "mercury -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("mercury gateway restart")
        assert _contains_gateway_lifecycle_command("mercury gateway stop")
    ...
    def test_hermes_home_derived_profile(self, monkeypatch):
        ...
        monkeypatch.delenv("HERMES_PROFILE_NAME", raising=False)
        import mercury_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("mercury -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("mercury -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("mercury -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("mercury gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hermes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade mercury then run mercury gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart mercury-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart mercury-gateway",
        "systemctl --user restart mercury-gateway",
        "systemctl stop mercury-gateway.service",
        "mercury gateway restart",
        "mercury gateway uninstall",
        "launchctl kickstart gui/501/ai.mercury.gateway",
        "launchctl bootout gui/501/ai.mercury.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.mercury.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.mercury/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.mercury.gateway.restart-once.plist",
        "pkill -f mercury.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart mercury-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.mercury.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-mercury label: label-independent detection is the point
        # (#62891 second reproduction used `ai.mercury.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.mercury.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        result = json.loads(tt.terminal_tool(command="mercury gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["mercury gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.mercury.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.mercury.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-mercury systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run mercury gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("mercury gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("mercury gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/mercury\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 mercury gateway restart \x00\x00junk"
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart mercury-gateway' .",
        "grep -rn 'mercury gateway restart' ./logs",
        "rg 'mercury gateway restart' ../archive",
        "grep -c 'systemctl stop mercury-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM mercury_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell mercury gateway restart"',
        'sqlite3 ./db ".system systemctl restart mercury-gateway"',
        'psql ./x -c "\\! systemctl restart mercury-gateway"',
        "grep -r 'mercury gateway restart' . | sh",
        "grep -r 'mercury gateway restart' ./logs | bash",
        "grep -r 'mercury gateway restart' . | sudo sh",
        "grep -r 'x' . ; mercury gateway restart",
        "grep -r 'x' . && systemctl restart mercury-gateway",
        'grep -r "$(mercury gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart mercury-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell mercury gateway restart"',
        'sqlite3 db ".system systemctl restart mercury-gateway"',
        'psql -c "\\! systemctl restart mercury-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    mercury_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run mercury gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".mercury"))
        (tmp_path / ".mercury").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run mercury gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop mercury-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart mercury-gateway' /var/log/syslog",
        "rg 'mercury gateway restart' /home/user/.mercury/logs/",
        "journalctl -u mercury-gateway --grep 'systemctl restart mercury-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'mercury_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell mercury gateway restart"',
        'psql -c "\\! systemctl restart mercury-gateway"',
        "grep 'systemctl restart mercury-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart mercury-gateway",
        'grep "$(systemctl restart mercury-gateway)" f',
        "grep 'restart' log; systemctl restart mercury-gateway",
        'sqlite3 db "SELECT 1"; mercury gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "mercury gateway restart",
        "sudo systemctl stop mercury-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart mercury-gateway%'\""
        )
