"""Tests for the per-profile Projects store (mercury_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Mercury", folders=["/tmp/mercury"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "mercury-agent"
    assert proj.name == "Mercury"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/mercury"
    assert [f.path for f in proj.folders] == ["/tmp/mercury"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "mercury-agent").id == pid
