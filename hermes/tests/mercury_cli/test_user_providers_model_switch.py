...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.models.fetch_api_models", lambda *a, **k: None)
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("mercury_cli.model_switch.resolve_alias", return_value=None), \
         ...
         patch("mercury_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("mercury_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         ...
         patch("mercury_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.models.fetch_api_models", _fake_fetch)
