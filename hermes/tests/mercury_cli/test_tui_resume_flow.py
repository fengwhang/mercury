...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from mercury_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """mercury -z bypasses MercuryCLI, but recall still needs SessionDB."""
    from mercury_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "mercury_state", mod("mercury_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "mercury_cli.config",
        mod("mercury_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "mercury_cli.models",
        mod("mercury_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "mercury_cli.tools_config",
        mod("mercury_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/mercury-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["HERMES_MODEL"] == "nous/mercury-test"
    assert env["HERMES_INFERENCE_MODEL"] == "nous/mercury-test"
    assert env["HERMES_TUI_PROVIDER"] == "nous"
    assert env["HERMES_INFERENCE_PROVIDER"] == "nous"
    assert env["HERMES_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["HERMES_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("mercury-tui-active-session-")
