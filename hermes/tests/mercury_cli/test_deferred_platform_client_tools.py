...
loader so ``mercury chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``mercury tools`` checklist. The same tools worked in gateway/web processes only
...
def _write_platform_plugin(
    ...
    *,
    ...
    """
    from mercury_cli.plugins import PluginManifest
...
def clean_registry():
    ...
    ``probeplat`` name, so the imported ``mercury_plugins.*`` modules have to go
    ...
    for name in set(sys.modules) - before_modules:
        if name.startswith("mercury_plugins."):
...
class TestA2AClientToolsInCliProcess:
    ...
    def test_platform_bundle_includes_the_client_tools(self):
        ...
        assert A2A_CLIENT_TOOLS.issubset(set(resolve_toolset("mercury-a2a")))
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`mercury plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `mercury plugins list` under-report what the process is carrying, and
    ...
    def test_failed_materialization_tears_down_pre_registered_tools(
        ...
        adapter. Attribution and the registry therefore agree at zero: `mercury
