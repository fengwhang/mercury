...
from mercury_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from mercury_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(mercury_cfg, codex_home=tmp_path, expose_hermes_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Mercury' managed block, re-running
        ...
        migrate({"mcp_servers": {"mercury-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.mercury-mcp]" in final
...
class TestStripUnmanagedPluginTables:
    ...
    ``mercury codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
class TestHermesHomeLeakGuard:
    ...
    once codex spawned the mercury-tools MCP subprocess.
    """
