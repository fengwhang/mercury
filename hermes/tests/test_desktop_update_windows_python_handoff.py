...
`scripts/desktop-update/windows.ps1` drives `mercury update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\mercury.exe` console-script launcher. On Windows that launcher is
a real process that keeps `mercury.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `mercury.exe` the launcher still has
...
Start Menu shortcut points at a `Mercury.exe` that no longer exists.

Driving the update as `python.exe -m mercury_cli.main update` puts the inherited
...
`$hermesExe` shim. `mercury.exe` may still be *named* in the file for the
...
def test_invoke_hermes_step_calls_drive_python_not_the_shim() -> None:
    ...
    assert not offenders, (
        ...
        "must drive $pythonExe, not the mercury.exe shim. Driving the update "
        "through the shim keeps mercury.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "mercury_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m mercury_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "mercury_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m mercury_cli.main desktop ...` for the same reason."
    )
...
def test_update_no_longer_invokes_the_hermes_exe_shim() -> None:
    ...
    assert "Invoke-HermesStep $hermesExe" not in source, (
        ...
        "the mercury.exe shim (`Invoke-HermesStep $hermesExe`). That is the "
