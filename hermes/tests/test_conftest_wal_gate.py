"""The conftest WAL gate must agree with mercury_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``mercury_state``. That is deliberate: importing
``mercury_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.mercury`` before the per-test ``HERMES_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_hermes_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and mercury_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_hermes_state_at_collection():
    """The gate must stay import-free of mercury_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.mercury, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``mercury_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "mercury_state" or name.startswith("mercury_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported mercury_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.mercury during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("mercury_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["mercury_state"] = saved
    assert not blocked
