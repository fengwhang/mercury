...
developer's REAL ~/.mercury/state.db, and a pytest-spawned process flipped
...
# Must match the root the guard itself computes.  Hardcoding ``~/.mercury``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\mercury``: the paths under test were then *correctly*
...
REAL_ROOT = mercury_state._real_platform_state_root()
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.mercury/state.db must fail hard."""
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mercury_state, "DEFAULT_DB_PATH", mercury_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        mercury_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(mercury_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
