...
``HERMES_TEST_ISOLATION`` is Mercury's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``mercury_state`` honors it as a
...
def test_marker_alone_reports_test_context(monkeypatch):
    ...
    assert mercury_state._running_under_pytest() is True
...
def test_no_signals_reports_production(monkeypatch):
    ...
    assert mercury_state._running_under_pytest() is False
