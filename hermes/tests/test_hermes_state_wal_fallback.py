...
(SQLITE_PROTOCOL — typical on NFS/SMB), Mercury must fall back to
...
def _reset_wal_fallback_warned_paths():
    ...
    mercury_state._wal_fallback_warned_paths.clear()
    yield
    mercury_state._wal_fallback_warned_paths.clear()
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    mercury_state._wal_reset_bug_warned_paths.clear()
    yield
    mercury_state._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_falls_back_to_delete_on_locking_protocol(self, tmp_path, caplog):
        ...
        with caplog.at_level("ERROR", logger="mercury_state"):
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see mercury_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("mercury_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        mercury_state._set_last_init_error("OperationalError: locking protocol")
