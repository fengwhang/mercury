...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.mercury/mercury-agent/utils.py)
...
2. ``mercury update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``mercury_cli/main.py`` (the ``mercury update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``mercury_constants`` after the
