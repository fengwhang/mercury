"""Tests for the secret-source tracking in ``mercury_cli.env_loader``.

These cover the small public surface that lets `mercury model` / `mercury setup`
...
from mercury_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, mercury_cli/main.py, run_agent.py, ...).  The
    ...
    # cli.py, mercury_cli/main.py, run_agent.py, trajectory_compressor.py,
