...
# Use whatever RotatingFileHandler class mercury_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. mercury_logging
...
from mercury_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    mercury_logging._reset_queued_handlers()
...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, mercury_home):
        ...
        agent_handlers = [
            h for h in mercury_logging.rotating_file_handlers()
    ...
    def test_writes_to_agent_log(self, mercury_home):
        ...
        # Flush handlers
        mercury_logging.flush_log_queue()
    ...
    def test_profile_routing_follows_context_home(self, mercury_home, tmp_path):
        ...
        profile_home.mkdir()
        mercury_logging.setup_logging(mercury_home=mercury_home)
        assert mercury_logging.enable_profile_log_routing(
            [mercury_home, profile_home]
        ) is True
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, mercury_home):
        mercury_logging.setup_logging(mercury_home=mercury_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, mercury_home):
        mercury_logging.setup_logging(mercury_home=mercury_home, mode="gui")

        logging.getLogger("mercury_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        mercury_logging.flush_log_queue()

        gui_log = mercury_home / "logs" / "gui.log"
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in mercury_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = mercury_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert mercury_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not mercury_logging._is_windows_concurrent_log_lock_timeout(
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> mercury_logging._ManagedRotatingFileHandler:
        handler = mercury_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that mercury_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        mercury_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        mercury_logging.setup_logging(mercury_home=mercury_home, mode="gateway")
        gw_path = mercury_home / "logs" / "gateway.log"
        rotated = mercury_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = mercury_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, mercury_home):
        mercury_logging.setup_logging(mercury_home=mercury_home)
        root = logging.getLogger()
        ...
        assert any(
            ...
            for h in mercury_logging.rotating_file_handlers()
        )
