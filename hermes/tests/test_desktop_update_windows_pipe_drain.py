...
last of them to close it. ``mercury update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``mercury_cli/main.py``), so the
...
``.mercury-update-result.json`` is never written, ``.mercury-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Mercury" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``mercury update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``mercury update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``mercury_cli/update_cmd.py``, the update-log tee) — so the
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``mercury update`` build: output streams to
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Mercury' forever) or metered a chatty step (backpressure on the running "
