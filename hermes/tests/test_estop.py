"""Global emergency stop (`mercury pause` / `mercury resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`mercury resume`)
...
def test_engage_creates_sentinel_and_is_engaged(mercury_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (mercury_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(mercury_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (mercury_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(mercury_home):
    ...
    raw = json.loads((mercury_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(mercury_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(mercury_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (mercury_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(mercury_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(mercury_home):
    ...
    assert "mercury resume" in notice


def test_paused_reply_without_reason(mercury_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(mercury_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(mercury_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(mercury_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(mercury_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(mercury_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(mercury_home):
...
# ── CLI: mercury pause / mercury resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(mercury_home, capsys):
    from mercury_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(mercury_home, capsys):
    from mercury_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(mercury_home, capsys):
    from mercury_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(mercury_home, capsys):
    from mercury_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from mercury_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── mercury status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(mercury_home):
    from mercury_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(mercury_home, monkeypatch):
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(mercury_home):
    ...
    try:
        ...
        return
    assert reply is None or "mercury is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(mercury_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from mercury_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    ...
    A process launched with HERMES_HOME=~/.mercury/profiles/fleet-analyst must
    still treat ~/.mercury/ESTOP as engaged. Otherwise `mercury pause` is not
