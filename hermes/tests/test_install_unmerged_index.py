...
The ``mercury update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m mercury_cli.main gateway run`` — image name
    ``pythonw``, not ``mercury.exe`` — so the ``taskkill /IM mercury.exe`` guard
    ...
    # The mercury.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM mercury.exe' in text
