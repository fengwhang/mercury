...
Regression test for https://github.com/NousResearch/mercury-agent/issues/18594.

When HERMES_HOME is unset but an active_profile file indicates a non-default
profile is active, get_hermes_home() should:
  1. STILL return ~/.mercury (raising would brick 30+ module-level callers)
...
def fresh_constants(monkeypatch, tmp_path):
    """Import mercury_constants fresh and reset the one-shot warn flag."""
    import importlib
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.mercury."""
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        mercury_dir.mkdir()
        (mercury_dir / "active_profile").write_text("coder\n")
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        (tmp_path / ".mercury" / "active_profile").write_text("coder\n")
    ...
    def test_unreadable_active_profile_no_crash(
        ...
        (mercury_dir / "active_profile").write_bytes(b"\xff\xfe\x00\x00")
