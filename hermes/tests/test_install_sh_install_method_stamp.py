...
gateway as well (``~/.mercury:/opt/data``). The published image stamps 'docker'
...
``~/.mercury/mercury-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$HERMES_HOME/\.install_method"', text), (
        ...
        "clobber it and block host-side `mercury update`"
    )
