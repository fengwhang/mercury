...
IMAGE_TAG = os.environ.get("HERMES_TEST_IMAGE", "mercury-agent-harness:latest")
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged mercury user
# ---------------------------------------------------------------------------
#
# Background: every Mercury runtime path inside the container drops to UID
# 10000 (the ``mercury`` user) via ``s6-setuidgid mercury``. ``docker exec``
...
# fails (PermissionError swallowed) for mercury, so a test that ran as root
...
# helpers below run every probe under ``-u mercury`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: mercury).
