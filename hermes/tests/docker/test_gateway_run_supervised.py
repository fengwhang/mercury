...
Before the s6 migration, ``docker run nousresearch/mercury-agent gateway
...
Every ``docker exec`` runs as ``mercury`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'mercury.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'mercury.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → mercury gateway run) can take
    ...
    # (main-wrapper.sh → python → mercury gateway run) can take well over
    ...
    # top), NOT the static main-mercury service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-mercury
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``mercury gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``mercury gateway
    ...
    # (main-wrapper.sh → python → mercury gateway run) can take well
    ...
    # started either — so we'd see the CMD's `mercury gateway run`
    ...
    # `mercury gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*mercury.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `mercury gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-mercury`
