...
Phase 4 wires `mercury -p <profile> gateway start/stop` through the s6
...
Every ``docker exec`` here runs as the unprivileged ``mercury`` user
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"mercury profile create {PROFILE}")
    ...
    r = _sh(container_name, f"mercury -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"mercury -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
