...
Converted from the former ``.github/actions/mercury-smoke-test`` composite
...
def test_hermes_help(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"mercury --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"mercury --help produced a traceback: {r.stderr[-2000:]!r}"
    )
...
def test_dashboard_subcommand_present(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"mercury dashboard --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
...
def test_hermes_help_under_wrapped_init(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"mercury --help failed under `docker run --init` (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"mercury --help produced a traceback under --init: {r.stderr[-2000:]!r}"
    )
