...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <mercury-uid> (10000) is allowed (supported non-root start)
"""
...
def test_user_pinned_to_hermes_uid_works(
    ...
    """docker run --user 10000:10000 (the mercury UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (mercury UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout