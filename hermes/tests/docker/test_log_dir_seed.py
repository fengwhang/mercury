...
owned by the mercury user after container boot.
...
registered later runs its log service as the dropped mercury user and
...
def test_logs_gateways_seeded_and_hermes_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by mercury after boot."""
    ...
    assert "logs=mercury" in r.stdout, (
        f"logs/ not owned by mercury: {r.stdout}"
    )
    assert "gateways=mercury" in r.stdout, (
        f"logs/gateways/ not owned by mercury: {r.stdout}"
    )
