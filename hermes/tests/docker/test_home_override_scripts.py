...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to mercury:mercury on every boot.
    ...
    # Verify files are now owned by mercury.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "mercury" in r.stdout, (
        f"expected mercury-owned files after restart, got: {r.stdout!r} — "
