...
The shim (docker/mercury-exec-shim.sh, installed at /opt/mercury/bin/mercury)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> mercury login` would write /opt/data/auth.json as
...
failed: Mercury is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> mercury …`` (defaulting to root) gets dropped to the
   mercury user before the real binary runs.
2. ``docker exec --user mercury <c> mercury …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $HERMES_HOME from a ``docker exec`` session land
   as mercury:mercury — the actual user-visible invariant.
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-mercury-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown mercury:mercury`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to mercury:mercury,
    ...
    ``02-reconcile-profiles`` (mercury_cli.container_boot), which s6 runs
    *strictly after* ``01-mercury-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `mercury` with a Python-style `-c` shim equivalent — there's no
    pure-mercury "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `mercury` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `mercury`
    do anything that writes to disk, then check the file's owner.

    Use `mercury config set` which writes config.yaml under HERMES_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "mercury", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by mercury, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "mercury:mercury", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected mercury:mercury. "
        "The shim did not drop privileges before invoking mercury."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/mercury/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid mercury mercury <args>` which resolves `mercury`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> mercury login`` (root) wrote
    ...
    "Mercury is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `mercury config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is mercury-readable.

    Specifically: pretend the operator ran `mercury login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the mercury UID. We use `mercury auth list` since that touches the
    ...
    # `mercury auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `mercury config set` first to
    # provoke a write into HERMES_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "mercury", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "mercury", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the mercury user "
