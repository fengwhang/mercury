...
Build the real image and verify at runtime:
  ...
  1. /opt/mercury is not writable by the mercury user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and HERMES_DISABLE_LAZY_INSTALLS are set
  3. /opt/mercury/.install_method contains "docker" (code-scoped stamp)
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The mercury user must not be able to modify /opt/mercury.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/mercury as the mercury user
        "touch /opt/mercury/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"mercury user can write to /opt/mercury (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/mercury/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"mercury user can write to /opt/mercury/.venv: {r.stdout}"
    )
