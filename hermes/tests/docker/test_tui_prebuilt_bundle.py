...
The fix is ``ENV HERMES_TUI_DIR=/opt/mercury/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the mercury user, return stdout."""
    inner = (
        "source /opt/mercury/.venv/bin/activate && "
        "cd /opt/mercury && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the mercury user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "mercury", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    ...
    assert r.stdout.strip() == "/opt/mercury/ui-tui", (
        f"HERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/mercury/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from mercury_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/mercury/ui-tui')\n"
