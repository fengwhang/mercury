...
def restart_container(request, built_image: str):
    ...
    name = f"mercury-restart-{safe}"
    volume = f"mercury-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "mercury", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "mercury", "profile", "create", "ghost").check_returncode()
