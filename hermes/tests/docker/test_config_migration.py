...
is migrated by docker_config_migrate.py on boot, running as the mercury
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    docker_config_migrate.py, running as the mercury user."""
    # Start container
    ...
    r = docker_exec_sh(
        container_name,
        "test -f /opt/mercury/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by mercury (migration ran as mercury)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "mercury", (
        f"config.yaml not owned by mercury (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
