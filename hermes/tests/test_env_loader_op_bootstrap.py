...
1. ``load_hermes_dotenv()`` auto-loads ``~/.mercury/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_hermes_dotenv(mercury_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "mercury_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
