...
- Job store (cron/jobs.py): the field is validated at the storage choke
  point against the canonical Mercury effort grammar (parse_reasoning_effort
  in mercury_constants — the SAME parser every other effort surface uses).
...
class TestCronjobToolReasoningEffort:
    ...
    via `mercury cron create/edit --reasoning-effort` only."""
    ...
    def test_schema_does_not_expose_reasoning_effort(self):
        ...
        keeps the parameter for the CLI lane (mercury_cli/cron.py), but the
