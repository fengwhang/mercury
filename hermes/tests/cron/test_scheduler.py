...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("mercury_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._hermes_home", tmp_path), \
             ...
             patch("mercury_cli.env_loader.load_hermes_dotenv"), \
             patch("mercury_cli.env_loader.reset_secret_source_cache"), \
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._hermes_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("mercury_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("mercury_cli.env_loader.load_hermes_dotenv", _record_load), \
             patch("mercury_state.SessionDB", return_value=fake_db), \
             patch(
                 "mercury_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
...
class TestRunJobModelResolution:
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``mercury_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``mercury cron create --deliver=<platform>`` silently
...
class TestFailureStreakNudge:
    ...
    def test_nudges_at_threshold(self):
        ...
        assert "mercury cron pause scout" in out
