    ...
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        try:
            with patch("cron.scheduler._hermes_home", None), \
                 ...
                 patch("mercury_state.SessionDB", side_effect=make_session_db), \
                 patch(
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._hermes_home", tmp_path), \
                 ...
                 patch("mercury_cli.env_loader.load_hermes_dotenv"), \
                 patch("mercury_cli.env_loader.reset_secret_source_cache"), \
                 patch("mercury_state.SessionDB", side_effect=_hanging_then_capture), \
                 patch(
                     "mercury_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._hermes_home", tmp_path), \
             ...
             patch("mercury_cli.env_loader.load_hermes_dotenv"), \
             patch("mercury_cli.env_loader.reset_secret_source_cache"), \
             patch("mercury_state.SessionDB") as mock_db_cls, \
             patch(
