...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mercury_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "mercury_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setattr(
        "mercury_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )
