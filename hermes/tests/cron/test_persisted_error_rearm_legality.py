...
def cron_store(tmp_path, monkeypatch):
    ...
    (mercury_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("HERMES_HOME", str(mercury_home))
    monkeypatch.setattr(J, "HERMES_DIR", mercury_home)
    monkeypatch.setattr(J, "CRON_DIR", mercury_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", mercury_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", mercury_home / "cron" / "output")
