...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        assert "mercury cron edit pin-test --provider <provider> --model <model>" in blob
        ...
        assert "host running mercury" in delivered
        assert "mercury cron edit pin-test --provider <provider> --model <model>" in delivered
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert agent_constructed is False
        assert error is not None
        assert "mercury cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("mercury_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
