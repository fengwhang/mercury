...
def test_required_frontmatter_fields(p):
    ...
    mercury = (fm.get("metadata") or {}).get("mercury") or {}
    if not (mercury.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.mercury.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    mercury = (fm.get("metadata") or {}).get("mercury") or {}
    dangling = [
        rs for rs in (mercury.get("related_skills") or []) if rs not in _all_names()
    ]
