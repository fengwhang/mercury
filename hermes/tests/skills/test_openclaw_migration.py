...
"""
    ...
    assert "Tyler Williams > Active Projects: Mercury" in entries
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.mercury/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: mercury-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "mercury-4-405b" in real.read_text(encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # user at ~/.mercury/config.yaml in its post-migration summary — it never
    # writes to either. Under skills-guard-v2 (#92021) these score as
    # informational _ref findings (the old critical agent_config_mod /
    # mercury_config_mod findings no longer fire for bare mentions), so the
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_ref", "mercury_config_ref"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Mercury``.
    ...
    # All-lowercase matches → lowercase ``mercury``; this preserves the
    # real filesystem path ``~/.mercury`` (Mercury home) when rebranding
