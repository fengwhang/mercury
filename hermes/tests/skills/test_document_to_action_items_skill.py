...
def test_frontmatter_required_fields():
    ...
    mercury = fm["metadata"]["mercury"]
    assert mercury["tags"]
    assert "related_skills" in mercury
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Mercury"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["mercury"]["related_skills"]:
