"""The `mercury-agent` skill is what a running Mercury knows about itself.

`website/` is never packaged, so an installed Mercury has no local copy of the
...
shipped feature is invisible and the agent answers "Mercury can't do that."
"""
