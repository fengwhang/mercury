...
Mercury *processes* via the cross-process ``_auth_store_lock`` flock,
...
Mercury's own PKCE login (``manual:mercury_pkce`` / ``mercury_pkce``) got no
...
def _fake_pool_store(monkeypatch):
    ...
    This stands in for ``~/.mercury/auth.json`` so the two "process-local"
...
def test_anthropic_refresh_is_protected_by_cross_process_lock(monkeypatch):
    ...
    pool = CredentialPool(
        "anthropic",
        [_entry(id="a1", access_token="stale-at", refresh_token="stale-rt", source="manual:mercury_pkce")],
    )
...
def test_concurrent_hermes_pkce_refresh_loses_credential_despite_valid_token_on_disk(monkeypatch):
    """Two 'Mercury processes' race to refresh the same stale mercury_pkce
    ...
    assert loser_result is not None, (
        ...
        "disk. mercury_pkce-sourced entries must adopt the winner's token "
    ...
    assert loser_entry_after.last_status != STATUS_EXHAUSTED, (
        "regression: the losing process marked its Anthropic mercury_pkce "
        "credential STATUS_EXHAUSTED after a lost refresh race, even "
        "though the account is not actually exhausted -- a sibling Mercury "
        "process holds a perfectly valid rotated token. This causes "
        "spurious 're-authenticate with Anthropic' failures under "
        "ordinary concurrent Mercury usage (fleet workers, cron jobs, "
...
def test_concurrent_claude_code_refresh_recovers_via_credentials_file(monkeypatch):
    ...
    evidence that mercury_pkce/dashboard-sourced credentials were simply
