...
def wire(profile, monkeypatch):
    """Replace only HTTP transports; use Mercury routing and the real SDK."""
...
def _assert_identity(request, account_id="acct-attribution-test"):
    assert request.headers["originator"] == "mercury-agent"
...
def test_required_identity_preserves_account_id(profile, legacy_enabled):
    ...
    assert headers["originator"] == "mercury-agent"
...
def test_new_identity_is_limited_to_the_official_endpoint(base_url, attributed):
    ...
    assert headers["originator"] == ("mercury-agent" if attributed else "codex_cli_rs")
    assert headers["User-Agent"] == (
        f"HermesAgent/{__version__}"
        if attributed else "codex_cli_rs/0.0.0 (Mercury)"
    )
...
def test_credential_pool_custom_endpoint_keeps_existing_identity(
    ...
    try:
        ...
        assert wire[-1].headers["user-agent"] == "codex_cli_rs/0.0.0 (Mercury)"
