...
@pytest.fixture
def mercury_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.mercury/`` with a config.yaml referencing one external dir."""
...
def test_cache_invalidates_on_mtime_change(mercury_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = mercury_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    ...
    home_a = tmp_path / "home_a" / ".mercury"
    ...
    home_b = tmp_path / "home_b" / ".mercury"
