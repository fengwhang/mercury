...
def test_foreign_active_plugin_configuration_is_left_unchanged(
    ...
    try:
        ...
        assert relay.events == []
        assert "already active outside Mercury native ownership" in caplog.text
...
def test_initialization_failure_is_fail_open(explicit_static_config, caplog):
    ...
    try:
        ...
        assert "Mercury Relay plugin initialization failed" in caplog.text
    finally:
        host.shutdown()
...
def test_session_close_does_not_flush_during_concurrent_managed_publication(
    ...
    event_loop_thread = threading.Thread(
        ...
        name="mercury-relay-session-close-regression",
        daemon=True,
    )
...
""".strip(),
    ...
    try:
        ...
        assert relay.events == []
        assert "Mercury [[dynamic_plugins]] records are unsupported" in caplog.text
...
agent_name = "Mercury Native Test"
agent_version = "test"
""".strip(),
        encoding="utf-8",
    )
