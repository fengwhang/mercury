...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("mercury_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
def wire_env():
    ...
    test_home = tempfile.mkdtemp(prefix="mercury_api_content_")
    os.makedirs(os.path.join(test_home, ".mercury"))
    prev_home = os.environ.get("HERMES_HOME")
    os.environ["HERMES_HOME"] = os.path.join(test_home, ".mercury")
