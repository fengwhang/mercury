...
class TestHermesAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        ...
        assert "skill_view(name='mercury-agent')" in HERMES_AGENT_HELP_GUIDANCE
    ...
    def test_no_skills_variant_has_no_skill_view_reference(self):
        ...
        assert "mercury-agent.nousresearch.com/docs" in HERMES_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "mercury-agent" not in disabled
    ...
    def test_skill_manage_delete_refused(self):
        ...
        msg = _pinned_guard("mercury-agent")
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the mercury-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "mercury-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        assert result["copied"] == ["mercury-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "mercury-agent" / "SKILL.md").exists()
