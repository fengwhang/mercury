...
class TestNamedProfileHintIntegration:
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        ...
        assert "Active Mercury profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "get_hermes_home", lambda: Path("/mercury"))

    # Production renders this as str(get_hermes_home()) + "/profiles/<name>/",
    # and str(Path("/mercury")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/mercury".
    _home_str = str(Path("/mercury"))
    expected_profile = (
    ...
    with (
        ...
        patch("mercury_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("mercury_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        from mercury_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("mercury_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from mercury_time import now as mercury_now
        sid = mercury_now().strftime("%Y%m%d_%H%M%S_fresh")
