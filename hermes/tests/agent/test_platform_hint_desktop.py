...
class TestEmbeddedTuiPaneClarifier:
    """When ``HERMES_DESKTOP_TERMINAL=1``, a standalone ``mercury --tui`` is
...
class TestContradictionGone:
    ...
    contained both ``You are running in the Mercury terminal UI (TUI).`` and
    ``Runtime surface: you're running inside the Mercury desktop GUI app.``.
