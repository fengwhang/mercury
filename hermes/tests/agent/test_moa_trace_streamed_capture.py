...
path via ``mercury chat --query``) couldn't see what the aggregator actually
...
def _enable_traces(tmp_path, monkeypatch):
    ...
    # save_moa_turn reads config via mercury_cli.config.load_config; stub it to
    ...
    return mercury_home / "moa-traces"
