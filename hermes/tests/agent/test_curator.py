...
def curator_env(tmp_path, monkeypatch):
    ...
    # the *next* test has configured (or the default ~/.mercury once monkeypatch
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from mercury_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `mercury model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from mercury_cli.main import _AUX_TASKS
    from mercury_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. mercury_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. mercury_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``mercury curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``mercury curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.mercury/skills/<umbrella>/references/ && mv ...`` its
