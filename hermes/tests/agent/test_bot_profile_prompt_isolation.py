...
so an unbound build thread fell back to ``~/.mercury`` (default) and leaked
default's full skills index + "Active Mercury profile: default" into a bot's
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    mercury ROOT (get_default_hermes_root)."""
    from agent import system_prompt
