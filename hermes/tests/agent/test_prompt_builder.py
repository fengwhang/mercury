...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.config.load_config", default_load_config)
        monkeypatch.setattr("mercury_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("mercury_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.config.load_config_readonly", lambda: {})
...
class TestBuildContextFilesPrompt:
    ...
    def test_hermes_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".mercury.md").write_text("Mercury-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Mercury-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "mercury_home"))
...
class TestFindHermesMd:
    def test_finds_in_cwd(self, tmp_path):
        ...
        assert _find_hermes_md(tmp_path) == tmp_path / ".mercury.md"
    ...
    def test_walks_to_git_root(self, tmp_path):
        ...
        assert _find_hermes_md(sub) == tmp_path / ".mercury.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .mercury.md planted in /tmp, /home, or /
        ...
        parent.mkdir()
        (parent / ".mercury.md").write_text("planted by another user")
        cwd = parent / "work"
        cwd.mkdir()
