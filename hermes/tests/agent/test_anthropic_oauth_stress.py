...
def _process_claude_code_refresh_worker(
    ...
    def fake_refresh(refresh_token, *, use_json=False):
        ...
        # what must ensure that the second Mercury process never calls this
...
def test_high_concurrency_anthropic_refresh_no_lost_updates_no_deadlock(
    ...
    """CONCURRENCY 'Mercury processes' race the same stale refresh token
    ...
    shared_stale_entry = _entry(
        id="pool-entry", refresh_token="stale-rt", source="manual:mercury_pkce"
    )
    pools = [
...
def test_distinct_profiles_share_one_claude_refresh_without_duplicate_post(
    ...
    """
    shared_credentials_path = mercury_home / "shared-claude-credentials.json"
    ...
    server_state_path = mercury_home / "fake-token-server.json"
    server_state_path.write_text(
        json.dumps({"calls": [], "spent": [], "rotation": 0}),
        encoding="utf-8",
    )

    profile_homes = [mercury_home / "profile-a", mercury_home / "profile-b"]
