...
``mercury_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
SCAN_DIRS = ("agent", "mercury_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "mercury_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / mercury_subprocess_env /
