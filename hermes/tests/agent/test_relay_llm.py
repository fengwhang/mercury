...
def test_relay_metadata_preserves_provider_name():
    metadata = {"api_mode": "chat_completions", "mercury.provider": "explicit"}
...
def test_stream_uses_rewritten_request_and_post_intercept_chunks(relay_turn):
    ...
    try:
        ...
        relay.intercepts.deregister_llm_stream_execution("mercury-test-stream")
        relay.intercepts.deregister_llm_request("mercury-test-request")
...
def test_stream_current_primes_lazy_completed_response(relay_turn, monkeypatch):
    """A lazy Relay stream must run once before Mercury decides its shape."""
    _relay, _turn = relay_turn
