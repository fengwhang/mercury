...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"mercury-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "mercury-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "mercury-agent-dev"
...
# `mercury curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `mercury curator pin <umbrella>` in the rename block so
