"""Tests for the cross-Mercury-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Mercury profile's
...
Reference: May 2026 incident — a mercury-security profile session
accidentally edited skills under both ~/.mercury/profiles/mercury-security/skills/
AND ~/.mercury/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Mercury root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    ...
    sec_home = root / "profiles" / "mercury-security"
    ...
    monkeypatch.setattr(mercury_constants, "get_default_hermes_root", lambda: root)
...
def _set_active_home(monkeypatch, mercury_home: Path):
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "mercury-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "mercury-security"
