...
def test_persisted_claude_code_row_carries_no_token_material(
    mercury_home, claude_credentials
):
    ...
    rows = [r for r in _persisted_rows(mercury_home) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(mercury_home, claude_credentials):
...
def test_empty_oauth_entry_is_never_leased(mercury_home, claude_credentials):
...
def test_hermes_pkce_row_still_syncs_from_the_pool_store(monkeypatch):
    ...
    ``mercury_pkce`` *is* pool-owned, so its persisted row keeps its tokens and
