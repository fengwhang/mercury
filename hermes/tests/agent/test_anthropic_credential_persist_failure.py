...
``~/.mercury/.anthropic_oauth.json`` for ``mercury_pkce`` ones.  Those singletons
...
def mercury_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_hermes_oauth_writer_raises_instead_of_swallowing(mercury_home, monkeypatch):
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    mercury_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
def test_pool_hermes_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "mercury_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
