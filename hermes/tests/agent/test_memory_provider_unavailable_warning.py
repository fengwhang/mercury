"""Regression tests for NousResearch/mercury-agent#2765.
...
service that didn't inherit ``~/.mercury/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "mercury memory status" in msg
