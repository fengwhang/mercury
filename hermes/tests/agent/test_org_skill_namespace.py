...
class TestOrgPullIsWiredIn:
    ...
    def test_sync_pull_command_refreshes_org_mirror(self):
        ...
        assert "maybe_pull_org_skills" in main_src, (
            "`mercury sync pull` must also refresh the org mirror."
        )
    ...
    def test_no_internal_jargon_in_user_facing_strings(self):
        ...
        targets = [
            root / "mercury_cli" / "subcommands" / "sync.py",
            root / "mercury_cli" / "subcommands" / "skills.py",
        ]
        banned = re.compile(
...
class TestSkillSyncIsOneCommand:
    """Every Skill Sync verb lives under `mercury sync` for launch.
    ...
    belong elsewhere. `propose` in particular used to sit under `mercury
    ...
    def test_propose_is_a_sync_subcommand(self):
        sync_src = self._src("mercury_cli", "subcommands", "sync.py")
        assert '"propose"' in sync_src, (
            "`propose` must be a `mercury sync` subcommand."
        )

    def test_propose_is_not_under_skills(self):
        skills_src = self._src("mercury_cli", "subcommands", "skills.py")
        assert '"propose"' not in skills_src, (
            "`propose` must NOT remain under `mercury skills` — Skill Sync is "
            "one command for launch."
        )

    def test_sync_usage_lists_propose(self):
        main_src = self._src("mercury_cli", "main.py")
        usage_start = main_src.index("usage: mercury sync ")
        usage_block = main_src[usage_start : usage_start + 1400]
        assert "propose" in usage_block, (
            "`mercury sync` usage must list the propose verb."
        )
