...
# `mercury_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
# real `~/.mercury/logs/agent.log` and `errors.log`.
#
...
# the REAL Mercury root — capture it BEFORE the sandbox rewires HERMES_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.mercury (#69385).
...
def _hermes_home_points_at_production(value: str) -> bool:
    ...
    HERMES_HOME=~/.mercury``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``mercury_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.mercury`` — the escape vector that landed pytest
    ...
    try:
        ...
        real_root = (Path.home() / ".mercury").resolve()
    except Exception:
        return True
...
if _hermes_home_points_at_production(os.environ.get("HERMES_HOME", "")):
    _SESSION_HERMES_HOME = tempfile.mkdtemp(prefix="mercury-test-home-")
...
# like a real CLI") — which used to disarm mercury_state's live-DB guard in
...
# mercury_state._running_under_pytest() as a test-context signal. A child
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``mercury_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Mercury agent session inherits them and
    # mercury_constants home-resolution helpers prefer them over monkeypatched
    ...
    # the real ~/.mercury/kanban.db instead of the per-test HERMES_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
    ...
    # otherwise-unrelated config tests away from the default "mercury" host.
    "HERMES_HONCHO_HOST",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HONCHO_HOST", "mercury")
    ...
    #    ``Path.home() / ".mercury"`` instead of ``get_hermes_home()``
    #    is a bug to fix at the callsite.
    fake_hermes_home = tmp_path / "mercury_test"
    ...
    # mercury_state's live-DB guard stays armed in them even when the test
    ...
    # 3b. mercury_state computes ``DEFAULT_DB_PATH = get_hermes_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from mercury_state import ...``) that
    ...
    mercury_state_mod = sys.modules.get("mercury_state")
    if mercury_state_mod is not None and hasattr(mercury_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            mercury_state_mod, "DEFAULT_DB_PATH", fake_hermes_home / "state.db"
        )
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from mercury_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.mercury. This autouse
...
def _capture_real_kanban_root() -> Path:
    ...
    2. the real (pre-sandbox) Mercury root otherwise
    """
    ...
    return (Path.home() / ".mercury").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.mercury`` captured at import time. Hermetic tests that legitimately
    move HERMES_HOME to sibling tempdirs are unaffected.

    Only patches when ``mercury_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("mercury_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import mercury_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.mercury. See #69283."
        )
...
# ``mercury_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Mercury root. This fixture wires the
# test-side knobs:
...
#     installs where HERMES_HOME is not ~/.mercury) into the guard's
...
# so subprocess children that import mercury_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("mercury_state")
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Mercury home override redirects every later
    ...
    # to its default.
    try:
        from mercury_constants import get_hermes_home_override, set_hermes_home_override
...
# environment and finds the developer's live ``mercury-gateway`` process
...
# correlated with a ``tests/mercury_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> mercury-gateway``
...
def _wal_is_usable() -> bool:
    """True when Mercury will actually put a database into WAL mode here.

    Mercury refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Mercury can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Mercury managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``mercury_state``. That module computes
    ...
    ``~/.mercury/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``mercury_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      ``mercury_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``mercury_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``mercury_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``mercury_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Mercury runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.mercury``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Mercury falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Mercury uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart mercury-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting mercury/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch mercury-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m mercury_cli.main".
                if (
                    "mercury" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting mercury/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `mercury update` (or the
        # equivalent `python -m mercury_cli.main update`).  These commands
        ...
        if "update" in low and (
            # mercury update / mercury update --gateway / setsid bash -c ... mercury update
            ("mercury" in low and "update" in low.split())
            or
            # python -m mercury_cli.main update --gateway
            ("mercury_cli" in low and "update" in low.split())
            or
            # venv/bin/mercury update  (absolute path variant used in tests)
            (".venv/bin/mercury" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`mercury update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    try:
        import mercury_cli.voice as _voice
    except Exception:
