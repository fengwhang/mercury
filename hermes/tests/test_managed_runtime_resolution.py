"""Guard: Mercury-owned subprocesses must not resolve managed runtimes by bare PATH.

Mercury installs runtimes for itself — ``uv`` at ``$HERMES_HOME/bin/uv``, Node at
``$HERMES_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Mercury's own
...
* when a system copy also exists, the one Mercury does not own wins — which is
...
# Runtimes Mercury provisions into HERMES_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Mercury-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # mercury_agent-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Mercury-managed runtime.\n\n"
        ...
        "  node/npm -> mercury_constants.find_node_executable()\n"
        "  PATH env -> mercury_constants.iter_hermes_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(mercury_constants, helper))


def test_managed_uv_helpers_exist():
    from mercury_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
