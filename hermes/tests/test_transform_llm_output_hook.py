...
from mercury_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(mercury_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <mercury_home>/plugins/<name> and opt it in."""
    plugin_dir = mercury_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "mercury_empty"))
