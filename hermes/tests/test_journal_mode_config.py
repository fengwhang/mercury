...
def _disable_vulnerable_gate(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "mercury_state.is_sqlite_wal_reset_vulnerable",
        lambda **kwargs: False,
    )
...
def _reset_configured_delete_override_warned_paths():
    ...
    mercury_state._delete_overridden_warned_paths.clear()
    yield
    mercury_state._delete_overridden_warned_paths.clear()
...
def test_resolve_journal_mode_uses_real_database_config(monkeypatch, tmp_path):
    from mercury_state import resolve_journal_mode
...
def test_real_db_openers_honor_configured_delete(monkeypatch, tmp_path):
    ...
    from mercury_cli import kanban_db, projects_db
    ...
    discord = DiscordRecoveryStore(mercury_home=tmp_path)
