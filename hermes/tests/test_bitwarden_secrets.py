...
binary download are exercised manually by `mercury secrets bitwarden
...
def mercury_home(tmp_path, monkeypatch):
    """Point Mercury at an isolated home directory."""
    ...
    if hasattr(mercury_constants, "_HERMES_HOME_CACHE"):
        mercury_constants._HERMES_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(mercury_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from mercury_cli.env_loader import _apply_external_secret_sources
