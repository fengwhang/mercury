"""POSIX advisory locks must survive Mercury' own database inspection.
...
Mercury used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `mercury sessions
...
out across Mercury' inspection calls.
"""
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import mercury_cli.sqlite_safe_read as ssr
