...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m mercury_cli.main serve --port 0"),
    ...
    def test_foreign_holder_detection_includes_deleted_wal(
        ...
        monkeypatch.setattr(mercury_state, "psutil", FakePsutil)
        monkeypatch.setattr(mercury_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(mercury_state.os, "getpid", lambda: 111)
        # Force the macOS/psutil path even on Linux test runners
        monkeypatch.setattr(mercury_state.sys, "platform", "darwin")
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(mercury_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(mercury_state.os, "getpid", lambda: 111)
        monkeypatch.setattr(mercury_state.sys, "platform", "linux")
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(mercury_state.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(mercury_state.os, "readlink", _readlink)
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Mercury process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Mercury
        ...
        def _fake_cmdline(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(mercury_state, "_read_proc_cmdline", _fake_cmdline)

        holders = db._foreign_state_db_holders()
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "mercury_cli.main" in holders[0][1]
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(mercury_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="mercury_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
