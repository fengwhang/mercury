...
Under a gateway (Telegram, Discord, Slack, ...) Mercury passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Mercury via Telegram AND Discord
...
class TestPinUserPeerAlias:
    ...
    def test_pinPeerName_still_works_unchanged(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "peerName": "eri",
            "hosts": {"mercury": {"pinPeerName": True}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.pin_peer_name is True
...
class TestProfilePeerUniqueness:
    """Each Mercury profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `mercury -p partner` pinned to a different person's peer).
