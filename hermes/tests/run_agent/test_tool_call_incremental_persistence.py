...
def _make_agent():
    mercury_home = Path(tempfile.mkdtemp(prefix="mercury-test-home-"))
    (mercury_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._hermes_home", mercury_home),
