...
def test_verify_on_stop_preserves_composed_report_at_budget_limit(agent, monkeypatch):
    ...
    with (
        ...
        patch("mercury_cli.plugins.invoke_hook", return_value=[]),
    ):
...
def test_pre_verify_preserves_composed_report_at_budget_limit(agent, monkeypatch):
    ...
    with (
        patch("mercury_cli.plugins.has_hook", side_effect=lambda name: name == "pre_verify"),
        patch(
            "mercury_cli.plugins.get_pre_verify_continue_message",
