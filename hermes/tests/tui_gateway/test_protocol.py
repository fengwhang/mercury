...
def server():
    ...
    # e.g. mercury_cli.active_sessions would bind the mocked get_hermes_home
    ...
    with patch.dict("sys.modules", {
        "mercury_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/mercury_test")),
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `mercury config set` does)
    ...
    import mercury_cli.skin_engine as skin_engine
    ...
    # Activate midnight, as `mercury config set display.skin midnight` would.
