...
def mercury_root(tmp_path, monkeypatch):
    ...
    Pointing HERMES_HOME at a dir outside ~/.mercury makes it the profile ROOT
...
def test_add_lands_in_named_profile_only(mercury_root):
    root = mercury_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(mercury_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(mercury_root):
    root = mercury_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(mercury_root):
    root = mercury_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(mercury_root):
    root = mercury_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(mercury_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(mercury_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(mercury_root):
    root = mercury_root
    _result(
        _call(
            "mcp.servers.add",
