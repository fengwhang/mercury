...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="mercury/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    ...
    resolve = _resolver(
        {
            "/home/me/.mercury": ("/home/me/.mercury", "/home/me/.mercury"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.mercury", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.mercury"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.mercury")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.mercury"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/mercury-agent": ("/www/mercury-agent", "/www/mercury-agent")})
    sessions = [
        _session("/www/mercury-agent", branch="main"),
        _session("/www/mercury-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/mercury-agent"]
    assert _lane_ids(project) == ["/www/mercury-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/mercury-agent"
