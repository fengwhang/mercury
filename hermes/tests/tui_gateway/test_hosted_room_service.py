...
def test_create_send_drive_publish_and_replay_without_client_transport(tmp_path: Path):
    ...
    room = service.create_room(
        ...
        name="Release room",
        members=[
            {"member_id": "default", "profile": "default", "handle": "mercury"},
...
def test_same_thread_followup_migrates_and_delivers_committed_peer_reply(
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@mercury continue", "thread_id": "thread-1"},
    )
    ...
    assert "User (user): @mercury continue" in prompt
...
def test_active_same_thread_followup_waits_for_current_task(tmp_path: Path):
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@mercury follow up", "thread_id": "thread-1"},
    )
    ...
    assert service.stop(timeout=1.0)
    assert "User (user): @mercury follow up" in service.rpc.prompts[1][1]
