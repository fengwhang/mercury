...
def test_slash_worker_accepts_profile_home():
    ...
    # mercury_state evaluates get_hermes_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "mercury_constants": MagicMock(
            get_hermes_home=MagicMock(return_value=Path("/tmp/mercury_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            worker = _SlashWorker(
                ...
                model="test-model",
                profile_home="/home/luke/.mercury/profiles/work"
            )
            ...
            assert call_kwargs["env"]["HERMES_HOME"] == "/home/luke/.mercury/profiles/work"
