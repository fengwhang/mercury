...
def live_home(monkeypatch):
    ...
    tmp = Path(tempfile.mkdtemp(prefix="mercury-live-staleprov-"))
    home = tmp / ".mercury"
    ...
    # mercury_constants caches the resolved home at first read — the env var
    ...
    import mercury_cli.config as hconfig
    ...
    # conftest also re-pins mercury_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(mercury_state, "DEFAULT_DB_PATH", home / "state.db")
