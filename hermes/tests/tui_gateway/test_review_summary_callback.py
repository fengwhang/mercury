    ...
    with patch.dict(
        "sys.modules",
        {
            "mercury_constants": MagicMock(
                get_hermes_home=MagicMock(return_value="/tmp/mercury_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'mercury-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'mercury-release' patched"
    }
