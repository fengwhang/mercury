"""Tests for mercury_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Mercury entry point
(mercury, mercury-agent, mercury-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Mercury entry point imports mercury_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported mercury_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("mercury_bootstrap", None)
    import mercury_bootstrap  # noqa: WPS433
    return mercury_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Mercury CLI
...
class TestEntryPointsImportBootstrap:
    """Every Mercury entry point must import mercury_bootstrap as its
    ...
    # Entry points that invoke Mercury as a process.  Each one must
    # import mercury_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "mercury_cli/main.py",   # mercury CLI (console_script)
        "run_agent.py",          # mercury-agent (console_script)
        "acp_adapter/entry.py",  # mercury-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import mercury_bootstrap' and that
        ...
        half-finished ``mercury update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``mercury_bootstrap`` as a top-level module) leaves mercury
        ...
        """
        # Resolve relative to the mercury-agent repo root.  Tests live
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # mercury start even when mercury_bootstrap hasn't been
        ...
        assert first_import_name == "mercury_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'mercury_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import mercury_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Mercury's own top-level modules — covering both
    ...
    def _run(self, hb, path_seed, env=None):
        ...
        try:
            ...
            hb.harden_import_path(src_root="/opt/mercury")
            return sys.path[:]
        finally:
            sys.path[:] = original
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/mercury", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Mercury root.  The guard must relocate Mercury to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/mercury"])
        assert result[0] == "/opt/mercury"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Mercury root.
        assert result.index("/opt/mercury") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HERMES_PYTHON_SRC_ROOT"] = "/env/mercury"
            hb.harden_import_path()
            assert sys.path[0] == "/env/mercury"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HERMES_PYTHON_SRC_ROOT", None)
