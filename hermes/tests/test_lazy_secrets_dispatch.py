...
1. `mercury secrets bitwarden setup --help` works (dispatch path)
2. `mercury update --check` works (update path)
3. `mercury secrets bitwarden disable` works (handler execution)
4. `mercury secrets onepassword status` works (lazy backend loads on demand)
...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run mercury CLI as a subprocess from repo root.
    ...
    connectivity — but ``mercury update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-mercury-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-mercury-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "mercury_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_setup_help(self) -> None:
        """`mercury secrets bitwarden setup --help` must exit 0 and print usage.
    ...
    def test_bitwarden_status(self) -> None:
        """`mercury secrets bitwarden status` must exit 0 (runs lazy backend)."""
    ...
    def test_bitwarden_disable(self) -> None:
        """`mercury secrets bitwarden disable` must exit 0."""
    ...
    def test_onepassword_status(self) -> None:
        """`mercury secrets onepassword status` must exit 0 (1Password lazy backend)."""
    ...
    def test_onepassword_setup_help(self) -> None:
        """`mercury secrets onepassword setup --help` must exit 0."""
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `mercury update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`mercury update --check` must not load cryptography._rust."""
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['mercury','update','--check']
...
# resolves the mercury_cli.main module's lazily re-exported attribute —
# so the patch must land on mercury_cli.main._cmd_update_check.
with patch('mercury_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
