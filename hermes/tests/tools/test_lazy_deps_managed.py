...
A package-manager install (NixOS, and anything else shipping Mercury from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.mercury/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("mercury_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.config.get_managed_system", _raise)
