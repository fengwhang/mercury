...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, mercury_subprocess_env
...
# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (mercury_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_hermes_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """mercury_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
