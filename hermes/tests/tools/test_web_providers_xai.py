...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``mercury tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `mercury tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``mercury tools``,
...
class TestXAIProviderOAuthPath:
    ...
    credential pool (``mercury auth`` sign-in) rather than an env-var key.
    ...
    def test_pool_only_direct_refresh_updates_main_runtime(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "mercury_cli.auth.refresh_xai_oauth_pure",
            fake_refresh,
        )
