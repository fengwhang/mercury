...
code paths Mercury takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``mercury_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("mercury_cli.stdio", None)
        # Fresh import now; tests import from mercury_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import mercury_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("mercury_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from mercury_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("mercury_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mercury_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, mercury_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "mercury_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call mercury_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """mercury_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from mercury_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from mercury_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self):
        ...
        Without it, the gateway-respawn watcher spawned by ``mercury update``
        (which runs under mercury-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# mercury_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mercury_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / mercury_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "mercury_cli/tools_config.py",
            "mercury_cli/doctor.py",
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_hermes_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mercury_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "mercury_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        mercury_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "mercury_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from mercury_cli._subprocess_compat import" in block
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import mercury_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "mercury_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        it up — a pre-import so the lazy ``mercury_cli.gateway`` import didn't
        ...
        with mock.patch.object(
            gw, "_stable_gateway_working_dir", return_value="C:/mercury"
        ), mock.patch(
            "mercury_cli.config.get_hermes_home", return_value="C:/mercury"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr(gr, "_resolve_hermes_bin", lambda: ["mercury"])
