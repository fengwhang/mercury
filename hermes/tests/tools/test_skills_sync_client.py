...
# ---------------------------------------------------------------------------
# Env-var configuration (Mercury Cloud "on by default" via environment)
# ---------------------------------------------------------------------------
...
class TestEnvConfig:
    ...
    def test_base_url_defaults_to_production(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.config.load_config", lambda: {}, raising=False)
...
class TestDeviceName:
    ...
    def test_env_seeds_first_use(self, tmp_path, monkeypatch):
        # Mercury Cloud path: HERMES_SYNC_DEVICE_NAME seeds the first-use label.
        monkeypatch.setattr(ssc, "_skills_dir", lambda: tmp_path)
        monkeypatch.setenv("HERMES_SYNC_DEVICE_NAME", "mercury-cloud-ben-1")
        assert ssc.stable_device_id() == "mercury-cloud-ben-1"
        # persisted so it stays stable even if the env later changes
        assert (tmp_path / ".sync_device_id").read_text() == "mercury-cloud-ben-1"
