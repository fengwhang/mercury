...
Mercury exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_hermes_cache_not_tmp(tmp_path, monkeypatch):
    ...
    import mercury_constants  # noqa: F401 — resolves HERMES_HOME per call
    ...
    assert result == str(tmp_path / ".mercury" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "mercury_bg_dead1.log").exists()
