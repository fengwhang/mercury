...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Mercury layout and point HERMES_HOME at
    the mercury-security profile (matching the original-incident shape).
    """
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'mercury-security'" in err
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        """When active profile is 'mercury-security', the prompt warns
        ...
        assert "Active Mercury profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.mercury/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Mercury profile: default" in src
        assert "Active Mercury profile: {active_profile}" in src
