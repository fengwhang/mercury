...
def test_e2e_child_sees_hermes_home_and_no_planted_secret(tmp_path, monkeypatch):
    ...
    assert result["home"] == str(mercury_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `mercury` on PATH
# ---------------------------------------------------------------------------
...
def test_e2e_scrubbed_env_resolves_bare_hermes_under_minimal_parent_path(monkeypatch):
    ...
    minimal PATH (no mercury console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``mercury``.
    ...
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "mercury.exe" if os.name == "nt" else "mercury")
    ):
        pytest.skip("no real mercury console-script install available")

    # Simulate the service-manager minimal PATH: mercury dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("mercury", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("mercury", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'mercury' must resolve from the child PATH {env.get('PATH')!r}"
    )
