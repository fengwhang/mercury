...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from mercury_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.mercury/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.mercury/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "mercury-agent"
