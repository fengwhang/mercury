...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(mercury_home)}):
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        (mercury_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(mercury_home))
        ...
        assert "/root/.mercury/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        legacy_doc = mercury_home / "document_cache"
        legacy_img = mercury_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.mercury/attachments" in container_paths
        assert "/root/.mercury/images" in container_paths
        assert "/root/.mercury/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        """
        mercury_home = tmp_path / ".mercury"
        (mercury_home / "images").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(mercury_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.mercury/images" in by_container
        assert by_container["/root/.mercury/images"] == str(mercury_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        (mercury_home / "images").mkdir(parents=True)
        upload = mercury_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("HERMES_HOME", str(mercury_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.mercury/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        mercury_home = tmp_path / ".mercury"
        img_dir = mercury_home / "cache" / "images"
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(mercury_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.mercury/cache/images/x.png"
        assert (mercury_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Mercury cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        mercury_home = tmp_path / ".mercury"
        (mercury_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(mercury_home))
        return str(mercury_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_hermes(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.mercury/attachments/drop.zip"

    def test_ssh_maps_to_tilde_hermes(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.mercury/attachments/drop.zip"
...
class TestMasterCredentialStoresAreNeverMountable:
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.mercury/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.mercury/google_token.json" in paths
        assert "/root/.mercury/.env" not in paths
