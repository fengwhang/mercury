...
class TestFileSync:
    def test_initial_sync_uploads_managed_files_under_remote_home(
        ...
        monkeypatch.setattr(
            ...
            lambda: [
                {
                    "host_path": str(src),
                    "container_path": "/root/.mercury/credentials/token.txt",
                }
            ],
        )
        monkeypatch.setattr("tools.credential_files.iter_skills_files", lambda **kwargs: [])
        ...
        assert uploaded == [
            {
                "path": "/home/vercel/.mercury/credentials/token.txt",
    ...
    def test_cleanup_syncs_back_snapshots_closes_and_is_idempotent(
        ...
        vercel_sdk.current.download_file_content = _tar_bytes(
            {
                "home/vercel/.mercury/credentials/token.txt": b"remote-token",
                "home/vercel/.mercury/credentials/new.txt": b"new-remote",
                "home/vercel/.mercury/unmapped/skip.txt": b"skip",
            }
        )
