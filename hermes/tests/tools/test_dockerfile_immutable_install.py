"""Contract tests for the Docker image's immutable /opt/mercury install tree."""
...
def test_dockerfile_makes_opt_hermes_readonly_for_hermes_user() -> None:
    ...
    assert "chown -R root:root /opt/mercury" not in text
    assert "chmod -R a+rX /opt/mercury" not in text
    assert "chmod -R a-w /opt/mercury" not in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+mercury:mercury\s+/opt/mercury/\.venv",
        r"chown\s+-R\s+mercury:mercury\s+/opt/mercury/ui-tui",
        r"chown\s+-R\s+mercury:mercury\s+/opt/mercury/gateway",
        r"chown\s+-R\s+mercury:mercury\s+/opt/mercury/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/mercury must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/mercury/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $HERMES_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the mercury
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/mercury/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/mercury/bin && \\\n"
        r"(?:.*\\\n)+?"
        r"\s+printf 'docker\\n' > /opt/mercury/\.install_method",
        text,
    )
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/mercury tree.
    ...
    assert "ENV HERMES_LAZY_INSTALL_TARGET=/opt/mercury" not in text
    ...
    # succeed as the unprivileged mercury runtime user.
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+mercury:mercury\s+/opt/mercury/plugins", text
    )
