...
def test_output_path_rejects_hermes_oauth_store(tmp_path, monkeypatch):
    ...
    mercury_home.mkdir()
    monkeypatch.setattr(file_safety, "_hermes_home_path", lambda: mercury_home)
    monkeypatch.setattr(file_safety, "_hermes_root_path", lambda: mercury_home)

    target = mercury_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    mercury_home = tmp_path / "mercury-home"
    token_dir = mercury_home / "mcp-tokens"
