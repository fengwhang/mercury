...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.mercury/)``
    ...
    def test_refuses_anything_that_is_not_a_strict_child_of_skills(self, tmp_path):
        """``/``, ``~/.mercury`` itself, a sibling dir, and the skills root
        ...
        mercury = tmp_path / "home"
        mercury.mkdir()
        skills = mercury / "skills"
        (skills / "keep").mkdir(parents=True)
        sibling = mercury / "kanban.db"  # any non-skills path
        sibling.mkdir()

        with patch("tools.skills_sync.SKILLS_DIR", skills):
            for target in (Path("/"), mercury, sibling, skills):
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `mercury profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.mercury) rely on so bundled
    skills are never seeded at install time NOR re-injected by `mercury update`.
    """
    ...
    def test_marker_skips_sync_and_removal_seeds_normally(self, tmp_path):
        ...
        mercury_home.mkdir()
        marker = mercury_home / ".no-bundled-skills"
        ...
        def _patches():
            ...
            stack.enter_context(patch("tools.skills_sync.SKILLS_DIR", skills_dir))
            stack.enter_context(patch("tools.skills_sync.MANIFEST_FILE", manifest_file))
            stack.enter_context(patch("tools.skills_sync.HERMES_HOME", mercury_home))
            return stack
...
class TestOptOutToggleAndRemove:
    """`mercury skills opt-out/opt-in` core: marker toggle + safe removal."""
