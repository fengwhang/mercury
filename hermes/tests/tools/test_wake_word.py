"""Tests for tools.wake_word — the "Hey Mercury" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey mercury"}) == "hey mercury"
    assert ww.wake_phrase({}) == "hey mercury"
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "mercury_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey mercury"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey mercury"
...
def test_bundled_hey_hermes_model_ships_on_disk():
    # The "hey mercury" wake word works out of the box only if the model is
