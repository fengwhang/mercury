    ...
    return [
        {
            ...
            "connection_label": "Mercury Cloud",
            "title": "Moxie",
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "mercury", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@mercury", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("mercury@cloud-1", roster)["profile"] == "default"
    assert bot_relay.resolve_remote_target("mercury@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "mercury" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "mercury", "connection_id": "cloud-1",
         "connection_label": "Mercury Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="mercury", message="ping", agent=agent))
    assert out.get("status") == "sent"
    assert "Mercury Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 mercury (@mercury): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@mercury` — on Mercury Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="mercury",
    )
...
def test_ttl_config_read_is_lazy_and_defensive(monkeypatch):
    ...
    def _boom(name, *a, **k):
        if name.startswith("mercury_cli"):
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "mercury", "connection_id": "cloud-1",
         "connection_label": "Mercury Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
