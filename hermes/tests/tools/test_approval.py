...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("mercury_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("mercury-verify-", "mercury-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "mercury-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/mercury-verify-example.py",
            "rm -f /tmp/mercury-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../mercury-verify-example.py",
            "rm -f /tmp/a/../../tmp/mercury-verify-example.py",
            "rm -f /var/tmp/mercury-verify-example.py",
            "rm -f /tmp/mercury-verify-*",
            "rm -f /tmp/mercury-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/mercury-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/mercury-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\mercury-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\mercury-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\mercury-victim",
...
class TestHermesConfigWriteProtection:
    ...
    ~/.mercury/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.mercury/config.yaml",
            "echo '  mode: off' >> ~/.mercury/config.yaml",
            "echo x | tee ~/.mercury/config.yaml",
            "echo x | tee $HERMES_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.mercury/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Mercury absolute config.yaml is
        # handled by the project patterns, not the Mercury-home rule.
        for cmd in (
            "cat ~/.mercury/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.mercury/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Mercury-home prefixes must fold to ~/ and
    ~/.mercury/ in dangerous-command detection.
    ...
    SSH, and Mercury config/env files returned "safe" without an approval prompt.
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        ...
        for variant in (
            ...
            "nohup python -m mercury_cli.main gateway run --replace",
        ):
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Mercury security config via IFS.
            "sed${IFS}-i ~/.mercury/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "mercury.*gateway")',
            "kill -9 `pgrep mercury`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof mercury)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof mercury_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof mercury`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Mercury service
    label achieves the same effect as `mercury gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_hermes_label_detected(self):
        for cmd in (
            "launchctl stop ai.mercury.gateway",
            "launchctl kickstart -k system/ai.mercury.gateway",
            "launchctl bootout system/ai.mercury.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.mercury.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Mercury labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a mercury-gateway identifier, so quoted prose and non-gateway mercury
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "mercury"/"ai.mercury" to appear AFTER the verb and missed this
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("mercury_cli.config.load_config_readonly", return_value=cfg):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.mercury.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.mercury.gateway.plist",
    "launchctl stop ai.mercury.gateway",
    "launchctl restart ai.mercury.gateway",
    "launchctl bootout gui/501/ai.mercury.gateway",
    "launchctl remove ai.mercury.gateway",
    "launchctl disable gui/501/ai.mercury.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Mercury ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.mercury.update-checker",
