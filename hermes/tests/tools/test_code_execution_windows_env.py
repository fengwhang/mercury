...
``mercury_tools.py`` and ``script.py`` into a temp dir, and those files
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        "HERMES_HOME": "/home/alice/.mercury",        # allowlisted → kept
...
# ``mercury_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``mercury_tools.py`` and ``script.py`` writes in
...
# let the child import mercury_tools, a user script that printed non-ASCII
