...
def _run_helper(
    ...
    *,
    ...
    mercury_home = target if mercury_home is None else mercury_home
    script = (
        "set -eu\n"
        f'HERMES_HOME="{mercury_home}"\n'
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "mercury-home"
    try:
...
def test_stage2_uses_symlink_safe_helper_for_hermes_home_trees(stage2_text: str) -> None:
    ...
    assert 'chown -R mercury:mercury "$HERMES_HOME/$sub"' not in stage2_text
    assert 'chown -R mercury:mercury "$HERMES_HOME/profiles"' not in stage2_text
    assert 'chown -R mercury:mercury "$HERMES_HOME/cron"' not in stage2_text
