...
class TestOrphanedPipeReconciliation:
    ...
    `mercury update` in Feishu spawned a background subprocess that restarted
...
class TestCheckpoint:
    ...
    def test_recover_dead_wrapper_retries_unreaped_systemd_scope(
        ...
        entry = {
            ...
            "systemd_unit": "mercury-worker-proc_dead_scope.scope",
        }
...
class TestSystemdCgroupIsolation:
    ...
    def test_wraps_in_systemd_scope_when_supervisor_and_available(
        ...
        wrapped in ``systemd-run --user --scope --unit=mercury-worker-<id>``."""
        ...
        assert argv[unit_idx + 1].startswith("mercury-worker-"), argv
        assert argv[unit_idx + 1] == f"mercury-worker-{session.id}", (
            argv
        ...
        assert session.systemd_unit == f"mercury-worker-{session.id}.scope"
    ...
    def test_systemd_post_spawn_failure_never_kills_gateway_process_group(
        ...
        assert stop_unit.call_args.args[0].startswith("mercury-worker-proc_")
    ...
    def test_pty_spawn_failure_reaps_scope_before_distinct_pipe_fallback(
        ...
        assert stopped_unit == f"mercury-worker-{session.id}.scope"
        unit_idx = fallback_argv.index("--unit")
        assert fallback_argv[unit_idx + 1] == (
            f"mercury-worker-{session.id}-pipe-fallback"
        )
        assert session.systemd_unit == (
            f"mercury-worker-{session.id}-pipe-fallback.scope"
        )
    ...
    def test_kill_recovered_detached_already_exited_stops_persisted_scope(
        ...
        session.systemd_unit = "mercury-worker-proc_recovered_scope.scope"
        registry._running[session.id] = session
        ...
        assert result["status"] == "already_exited"
        assert stopped == ["mercury-worker-proc_recovered_scope.scope"]
        assert terminated == []
        assert session.exited is True
    ...
    def test_stop_systemd_unit_treats_absent_unit_as_clean(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: subprocess.CompletedProcess(
                args=args[0],
                returncode=5,
                stderr=b"Unit mercury-worker-gone.scope not loaded.\n",
            ),
        )

        assert pr._stop_systemd_unit("mercury-worker-gone.scope") is True
