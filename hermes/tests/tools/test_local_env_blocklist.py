...
See: https://github.com/NousResearch/mercury-agent/issues/1002
See: https://github.com/NousResearch/mercury-agent/issues/1264
"""
...
class TestProviderEnvBlocklist:
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Mercury inference secret
        ...
        models`` run inside a Mercury terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        Mercury-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_bare_hermes_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Mercury even when the gateway PATH cannot."""
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    children then get the BUZZ_* vars; execute_code, mercury_subprocess_env,
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        mercury_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in mercury-snap-<id>.sh
...
class TestActiveVenvMarkerStripping:
    ...
    into the Mercury venv path instead of the project's own ``.venv`` —
    silently clobbering the Mercury environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Mercury venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.mercury/mercury-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/mercury",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "mercury-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Mercury-owned entry stripping (#74817).

    The Desktop Electron app injects the Mercury repo root and the Mercury
    ...
    entries Mercury itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Mercury-owned entries are removed; everything else survives
        ...
        cannot silently pass), duplicate Mercury entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Mercury ordering with an
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Mercury-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Mercury venv
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Mercury-owned entries.
        ...
        Mercury-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Mercury venv site-packages entries are stripped
        by the same Mercury-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_hermes_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        mercury_win = f"C:\\\\Users\\\\u\\\\.mercury\\\\mercury-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([mercury_win, user_win]),
        }
        ...
        # the Mercury-owned check cannot match a backslash path here.
        assert mercury_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_hermes_owned_paths_stripped(self):
        """On Windows, a Mercury venv site-packages entry written with
        backslashes is stripped by the same Mercury-owned check, while a
        ...
        _strip_hermes_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert mercury_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Mercury venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Mercury runtime producer contract.
        """
        import tools.environments.local as local

        repo_root = tmp_path / "mercury-agent"
    ...
    def test_builders_strip_hermes_venv_pythonpath(self, builder):
        ...
        Mercury venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.mercury_subprocess_env()
    ...
    def test_scrub_child_env_strips_hermes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Mercury venv
    ...
    def test_execute_code_composition_strips_inherited_hermes_entries(self, same_env):
        ...
        Seeds a contaminated inherited PYTHONPATH (Mercury repo root + Mercury
        ...
        * inherited Mercury venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        mercury_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [mercury_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(mercury_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Mercury venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from mercury_cli.gateway_windows import _preserve_hermes_home_path
        ...
        assert launcher_entry == configured_home / "mercury-agent"
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "mercury-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("HERMES_HOME", str(configured_home))
        lexical_root = configured_home / "mercury-agent"
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\mercury\\mercury-agent -> C:\\...\\mercury-agent) while the
        ...
        # repo-level link: <configured-home>/mercury-agent -> physical repo
        try:
            _make_directory_link(configured_home / "mercury-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "mercury-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "mercury-agent"
        ...
        for lookalike in (configured_home / "mercury-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/mercury-agent.
        ...
        try:
            _make_directory_link(configured_root / "mercury-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "mercury-agent"
        aliases = local._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local._same_path(a, lexical_root) for a in aliases)
        assert not any(local._same_path(a, configured_home / "mercury-agent") for a in aliases)
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Mercury runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Mercury venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "mercury_subprocess_env":
                result = local_mod.mercury_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/mercury-venv",
            "VIRTUAL_ENV": "/opt/mercury-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/mercury-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/mercury-venv"
...
class TestBlocklistCoverage:
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Mercury (#55878).
        """
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Mercury-managed
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Mercury, and blocklisting
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        (subscription OAuth), not a Mercury inference credential. Stripping it
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        mercury-install-dir prepend (a separate concern, covered by
        TestHermesBinDirOnPath) so a real ``mercury`` on the test runner's PATH
...
class TestHermesBinDirOnPath:
    """The mercury install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``mercury`` via the terminal tool must work
    even when the gateway was launched without the mercury install dir on
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/mercury/bin/mercury" if name == "mercury" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/mercury/bin")
        assert local_mod._resolve_hermes_bin_dir() == "/opt/mercury/bin"
    ...
    def test_make_run_env_injects_hermes_bin_dir(self):
        """A gateway env missing the mercury dir gets it back in the subshell PATH.
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._HERMES_BIN_DIR = "/opt/mercury/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/mercury/bin"
        assert "/usr/bin" in entries


class TestHermesInternalDynamicSecrets:
    """Dynamically-named Mercury secrets injected at gateway/CLI startup must
    ...
    ``mercury_subprocess_env``, Docker forward filter, ``env_passthrough``)
