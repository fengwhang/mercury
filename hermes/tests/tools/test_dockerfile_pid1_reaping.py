...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, mercury accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and mercury would run without zombie reaping.
    """
    ...
    assert "/opt/mercury/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/mercury/docker/main-wrapper.sh" in dispatcher, (
