...
class TestAtomicWrite:
    ...
    def test_no_temp_file_leaked_on_success(self, ops, tmp_path: Path):
        ...
        assert [p for p in os.listdir(tmp_path) if ".mercury-tmp" in p] == []
...
class TestProtectedInstructionFiles:
    ...
    AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project-local .mercury
    ...
    def test_checkout_nested_under_hermes_dir_not_gated(self, tmp_path, approvals):
        """A repo living UNDER a .mercury dir (e.g. ~/.mercury/mercury-agent)
        must not have every write gated — only files directly inside a
        .mercury dir count as project config."""
        repo = tmp_path / ".mercury" / "some-repo" / "src"
    ...
    def test_real_hermes_home_not_gated_by_this_check(
        ...
        """~/.mercury itself is governed by existing guards, not this gate."""
        import tools.file_tools as ft
        fake_home = tmp_path / ".mercury"
