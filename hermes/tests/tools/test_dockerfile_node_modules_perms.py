...
Older images made /opt/mercury/ui-tui and /opt/mercury/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/mercury/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R mercury:mercury /opt/mercury/ui-tui" not in text
    assert "chown -R mercury:mercury /opt/mercury/node_modules" not in text
