...
PROMPT = "Mercury, Teknium, Nous Research, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("mercury_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("mercury_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.mercury_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    mercury_home = Path(os.environ["HERMES_HOME"])
    plugin_dir = mercury_home / "plugins" / "stt_vocab"
