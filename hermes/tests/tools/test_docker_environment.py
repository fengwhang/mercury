...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across mercury processes.
...
def test_init_env_args_uses_hermes_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.mercury/.env.
    """
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Mercury image, or ``gosu``/``su``
...
def test_run_command_tags_hermes_agent_label(monkeypatch):
    """Every container mercury-agent starts must carry the mercury-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=mercury-agent=1`` call. Regression test
    ...
    assert "mercury-agent=1" in labels, (
        f"mercury-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "mercury-task-id=task_with_weird_chars" in labels, (
...
def test_shared_container_key_replaces_profile_identity(monkeypatch):
    ...
    assert a._labels["mercury-profile"] == b._labels["mercury-profile"]
    assert a._labels["mercury-profile"] != "research"
    assert a._labels["mercury-profile"].startswith("team_workspace-")
...
def test_distinct_shared_keys_never_collide(monkeypatch):
    ...
    assert c._labels["mercury-profile"] != d._labels["mercury-profile"]
    # Both stay within Docker's 63-char label-value bound.
    assert len(c._labels["mercury-profile"]) <= 63
    assert len(d._labels["mercury-profile"]) <= 63
...
def test_empty_shared_container_key_preserves_profile_isolation(monkeypatch):
    ...
    assert env._labels["mercury-profile"] == "research"
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Mercury process spawning a new container
...
def test_egress_enabled_does_not_reuse_pre_egress_container(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ["-v", "/tmp/ca:/etc/ssl/certs/mercury-egress-ca.crt:ro"],
    ...
    def _run(cmd, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                assert any(str(part).startswith("label=mercury-egress=") for part in cmd)
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
...
def test_extra_args_proxy_override_refuses_under_egress(monkeypatch):
    """docker_extra_args are appended after Mercury args, so egress enforcement
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Mercury process uses it. Without this
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("mercury-"), "should remove the container by its generated name"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Mercury startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.mercury/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.mercury/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="mercury-agent:latest")
