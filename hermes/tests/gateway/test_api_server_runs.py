    ...
    async def test_start_returns_202(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                ...
                assert status["object"] == "mercury.run"
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as create:
                ...
                first_headers = {
                    ...
                    "X-Mercury-Session-Key": "memory-a",
                }
                second_headers = {
                    "Authorization": "Bearer sk-secret",
                    "Idempotency-Key": "memory-scope",
                    "X-Mercury-Session-Key": "memory-b",
                }
                first = await cli.post(
    ...
    async def test_replay_bypasses_concurrency_limit_and_preserves_session_header(
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as create:
                ...
                assert replay.headers["X-Mercury-Session-Key"] == "memory-a"
...
class TestHostedRoomRuns:
    ...
    async def test_invitation_uses_validated_app_managed_local_catalog(
        ...
        monkeypatch.setenv(
            "HERMES_ROOM_LINK_URL", "https://peer.example.test/mercury"
        )
        app = _create_runs_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
        ...
        assert body["catalog"]["endpoint"] == {
            "available": True,
            "url": "https://peer.example.test/mercury",
