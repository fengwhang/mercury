...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    mercury_home.mkdir()
    monkeypatch.setattr(base, "_HERMES_HOME", mercury_home)
    monkeypatch.setattr(base, "_HERMES_ROOT", mercury_home)
    path = mercury_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.mercury/media/'
...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("HERMES_KANBAN_HOME", str(tmp_path / "mercury"))
        monkeypatch.setenv("HERMES_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "mercury" / "kanban" / "boards" / "research"
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.mercury/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        mercury_dir = fake_home / ".mercury"
        (mercury_dir / "mcp-tokens").mkdir(parents=True)
        secret = mercury_dir / rel
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        mercury_dir = fake_home / ".mercury"
        mercury_dir.mkdir(parents=True)
        token = mercury_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._HERMES_HOME", mercury_dir)
        monkeypatch.setattr("gateway.platforms.base._HERMES_ROOT", mercury_dir)
    ...
    def test_denylist_blocks_non_cache_file_under_hermes_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.mercury
        ...
        mercury_dir = fake_home / ".mercury"
        mercury_dir.mkdir(parents=True)
        artifact = mercury_dir / "adhoc_report.pdf"
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        ...
        under a denied prefix (e.g. /opt/data -> /root/.mercury) emits
        ...
        mercury_root = denied_root / ".mercury"
        prof_cache = mercury_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._HERMES_ROOT", mercury_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.mercury/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        mercury_home = tmp_path / ".mercury"
        cache = mercury_home / "cache" / "images"
        ...
        assert BasePlatformAdapter.validate_media_delivery_path(
            "/root/.mercury/cache/images/generated.png"
        ) == str(media.resolve())

    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.mercury/* outside a cache mount (the sandbox's credential
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Mercury
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../mercury/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.mercury/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.mercury exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".mercury"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.mercury/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
