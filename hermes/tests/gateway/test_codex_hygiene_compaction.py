...
* ``compression.codex_app_server_auto`` semantics hold: only ``mercury``
  lets Mercury' threshold start a compaction; ``native``/``off`` skip
...
class LiveCodexAgent:
    ...
    def __init__(self, mode="mercury", session=None):
...
# ---------------------------------------------------------------------------
# Core no-op regression: mercury mode + live thread => thread/compact runs
# ---------------------------------------------------------------------------

def test_hermes_mode_compacts_live_thread_at_rpc_boundary(tmp_path):
    agent = LiveCodexAgent(mode="mercury")
    key = "tg:123"
...
@pytest.mark.parametrize("mode", ["native", "mercury", "off"])
...
def test_force_without_live_thread_does_not_run_local_compressor(mode):
    # The #73715 branch let force=True fall through to the local Mercury
...
def test_timeout_records_persistent_cooldown(tmp_path):
    ...
    agent = LiveCodexAgent(mode="mercury", session=HangingSession())
    key = "tg:123"
    gw, db = _gateway(tmp_path, key, agent)
