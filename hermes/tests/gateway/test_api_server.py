...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_runtime_config(self, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "mercury.api_server.capabilities"
            assert data["platform"] == "mercury-agent"
            ...
            assert data["features"]["session_continuity_header"] == "X-Mercury-Session-Id"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "mercury_cli.tools_config._get_effective_configurable_toolsets",
            ...
            "mercury_cli.tools_config._toolset_has_keys",
            return_value=True,
        ) as has_keys, patch(
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``mercury.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: mercury.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: mercury.tool.progress":
                    continue
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "mercury-agent", "messages": [{"role": "user", "content": "hello"}]},
                )
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-Mercury-Error", "")
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["mercury"]["failed"] is True
...
class TestSessionIdHeader:
    ...
    async def test_traversal_session_id_header_rejected(self, auth_adapter):
        """Security (#5958): a path-traversal X-Mercury-Session-Id must be
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={"X-Mercury-Session-Id": bad, "Authorization": "Bearer sk-secret"},
...
# ---------------------------------------------------------------------------
# X-Mercury-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Mercury-Session-Key": "agent:main:webui:dm:user-7",
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Mercury-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Mercury-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "mercury-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Mercury-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Mercury-Session-Key"
...
class TestStoredSessionModelFilter:
    ...
    "no stored model" — replaying "mercury-agent" upstream 400s. Found live
    (Aug 2026): the first cross-gateway `mercury peer dm` against a fresh
    api_server failed every turn with "mercury-agent is not a valid model ID".
    """
...
class TestSessionDbOffEventLoop:
    ...
    async def test_create_session_without_model_does_not_persist_virtual_alias(self, auth_adapter):
        ...
        virtual model alias (self._model_name, e.g. "mercury-agent") as if it
        ...
        "mercury-agent" as an invalid model identifier on every turn.
        """
        app = _create_app(auth_adapter)
    ...
    async def test_create_session_with_explicit_virtual_alias_does_not_persist_it(self, auth_adapter):
        """Sending ``model: "mercury-agent"`` explicitly (the virtual alias
    ...
    async def test_create_session_with_provider_prefixed_virtual_alias_does_not_persist_it(self, auth_adapter):
        ...
        "openrouter::mercury-agent" never matched self._model_name and leaked
...
class TestApiKeyStartupGuardFailsClosed:
    ...
    def _blocking_auth_import():
        ...
        def _blocked(name, *args, **kwargs):
            if name == "mercury_cli.auth":
                raise ImportError("simulated: mercury_cli.auth unavailable")
...
class TestCreateAgentModelRecovery:
    ...
    def test_create_agent_does_not_cache_virtual_alias(self, monkeypatch):
        """Write-side guard: the advertised virtual model (``mercury-agent``)
