...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(mercury_home):
    ...
    with patch("mercury_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None, False)):
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(mercury_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from mercury_cli.goals import GoalManager, save_goal
    ...
    with patch("mercury_cli.goals.judge_goal", return_value=("continue", "keep going", False, None, False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
