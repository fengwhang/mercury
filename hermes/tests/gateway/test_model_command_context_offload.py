...
def _runner_with_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mercury_constants.get_hermes_home", lambda: mercury_home)
    monkeypatch.setattr("mercury_cli.config.get_hermes_home", lambda: mercury_home)
...
async def test_warning_enrichment_is_offloaded(tmp_path, monkeypatch):
    ...
    from mercury_cli import context_switch_guard
