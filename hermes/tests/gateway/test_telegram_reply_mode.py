...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/NousResearch/mercury-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Mercury-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/NousResearch/mercury-agent/issues/87051:
    ...
    response in a different chat lane than the Mercury topic the session runs
    in. The no-anchor branch must prefer the Mercury topic's
