...
def test_room_link_endpoint_reads_supported_config_with_env_override(
    ...
    (home / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://configured.example.test/mercury\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("HERMES_HOME", str(home))
    ...
    assert local_room_link_endpoint() == {
        "available": True,
        "url": "https://configured.example.test/mercury",
        "transport_security": "tls",
    }

    monkeypatch.setenv(
        "HERMES_ROOM_LINK_URL", "https://override.example.test/mercury"
    )
    assert local_room_link_endpoint()["url"] == (
        "https://override.example.test/mercury"
    )
...
def test_named_profile_inherits_gateway_room_link_endpoint(tmp_path, monkeypatch):
    ...
    (root / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://gateway.example.test/mercury\n",
        encoding="utf-8",
    )
    ...
    try:
        assert local_room_link_endpoint() == {
            "available": True,
            "url": "https://gateway.example.test/mercury",
            "transport_security": "tls",
        }
    finally:
        reset_hermes_home_override(token)
...
def test_named_profile_room_link_override_wins_over_gateway_root(
    ...
    (profile / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://profile.example.test/mercury\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("HERMES_HOME", str(root))
    monkeypatch.delenv("HERMES_ROOM_LINK_URL", raising=False)
    ...
    try:
        assert local_room_link_endpoint()["url"] == (
            "https://profile.example.test/mercury"
        )
    finally:
        reset_hermes_home_override(token)
