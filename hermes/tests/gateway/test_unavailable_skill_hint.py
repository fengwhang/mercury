...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`mercury_cli.skills_config.save_disabled_skills` stores.
"""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `mercury skills config` writes).
    monkeypatch.setattr(
    ...
    assert "mercury skills config" in msg
