...
async def test_windows_detached_restart_scrubs_gateway_marker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run, "_resolve_hermes_bin", lambda: ["mercury"])
    ...
    assert cmd[-3:] == ["mercury", "gateway", "restart"]
...
async def test_request_restart_skips_wait_when_only_wedged_turns(monkeypatch):
    ...
    Regression: a WhatsApp turn wedged for 30+ min pinned `mercury update`
