...
Telegram topics act as independent Mercury session lanes.
"""
...
def _make_runner(session_db=None):
    ...
    if session_db is not None:
        from mercury_state import AsyncSessionDB
...
async def test_group_new_keeps_existing_reset_semantics_when_dm_topic_mode_enabled(
    ...
    # /new appends a random tip from mercury_cli.tips; one tip's text contains
    # the phrase "parallel work", which collides with the negative assertion
    # below (observed as a 1-in-N CI flake). Pin the tip.
    monkeypatch.setattr(
        "mercury_cli.tips.get_random_tip", lambda: "pinned tip for test"
    )

    result = await runner._handle_message(_make_group_event("/new", thread_id="555"))

    assert "Started a new Mercury session in this topic" not in result
