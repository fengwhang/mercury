...
def isolated_home(tmp_path, monkeypatch):
    ...
    These tests deliberately re-import ``mercury_cli`` / ``gateway`` so each
    ...
    _saved = {
        ...
        if k.startswith(("mercury_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (mercury_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("mercury_cli", "gateway")):
                del sys.modules[mod]
