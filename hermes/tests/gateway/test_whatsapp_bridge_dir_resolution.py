...
(/opt/mercury/scripts/whatsapp-bridge) is read-only, so `npm install` fails
...
def test_readonly_install_mirrors_to_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_constants.get_hermes_home", lambda: mercury_home
    )
    ...
    expected = mercury_home / "scripts" / "whatsapp-bridge"
