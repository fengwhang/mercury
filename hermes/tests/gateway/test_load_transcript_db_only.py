...
def test_load_transcript_returns_db_messages_when_no_jsonl(tmp_path, monkeypatch):
    ...
    ~/.mercury/state.db. (DEFAULT_DB_PATH is a module-level constant computed
    at mercury_state import time, before pytest's HERMES_HOME monkeypatch
    fires — the autouse fixture's HERMES_HOME override doesn't help here.)
    """
    import mercury_state
    monkeypatch.setattr(mercury_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
