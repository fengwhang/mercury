...
def _run_gateway_import(mercury_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    # otherwise intentionally clean so stale Mercury settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def mercury_home(tmp_path: Path) -> Path:
    home = tmp_path / ".mercury"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(mercury_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(mercury_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(mercury_home, {
        "HERMES_AGENT_TIMEOUT": "60",
        "HERMES_AGENT_TIMEOUT_WARNING": "30",
        "HERMES_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(mercury_home, initial_env={})
...
def test_config_turn_lease_timeout_wins_over_stale_env(mercury_home: Path) -> None:
...
def test_config_platform_connect_timeout_supplies_env_when_unset(mercury_home: Path) -> None:
    ...
    _write_config(mercury_home, gateway_cfg={"platform_connect_timeout": 90})
...
def test_env_platform_connect_timeout_wins_over_config(mercury_home: Path) -> None:
