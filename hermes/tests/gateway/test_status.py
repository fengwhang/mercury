...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``mercury gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "mercury gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "mercury -p coder gateway run --replace",
            "/opt/mercury/.venv/bin/mercury --profile coder gateway run --replace",
            "mercury_home=/opt/data/profiles/coder mercury gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        ...
        cmdline = r"mercury_home=c:\opt\data\profiles\coder mercury gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "mercury-gateway",
            "argv": ["/Users/user/.mercury/mercury-agent/mercury_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_self_reacquires_when_disk_start_time_null(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "mercury-gateway",
            "argv": ["mercury_cli/main.py", "--profile", "milena", "gateway", "run", "--replace"],
    ...
    def test_acquire_scoped_lock_self_reacquires_when_start_times_differ(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 111,
            "kind": "mercury-gateway",
            "argv": ["mercury_cli/main.py", "gateway", "run", "--replace"],
            "scope": "discord-bot-token",
        }))
...
class TestScopedLockOwnerLabel:
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "mercury_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # mercury_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "mercury_home": "/opt/data/profiles/zerocool",
        }
    ...
    def test_owner_label_falls_back_to_hermes_home(self):
        ...
        record = {"pid": 559, "mercury_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
class TestTakeoverMarker:
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        match — otherwise two profile services sharing a default ~/.mercury flap
    ...
    def test_consume_accepts_legacy_marker_without_hermes_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Mercury versions have no
...
class TestScopedLockTakeover:
    ...
    def _owner_record(target_home: Path, *, pid: int = 4242, start_time: int = 123):
        ...
        record = {
            "pid": pid,
            "kind": "mercury-gateway",
            "argv": ["python", "-m", "mercury_cli.main", "gateway", "run"],
            "start_time": start_time,
            "mercury_home": str(target_home),
        }
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        monkeypatch.setattr(
            ...
            lambda _pid: "python -m mercury_cli.main gateway run",
        )
        calls = []
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``mercury gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        assert "mercury_cli/main.py" in result
        assert calls == ["proc"]
...
def test_strict_gateway_identity_rejects_reused_pid(tmp_path, monkeypatch):
    pid_path = tmp_path / "gateway.pid"
    lock_path = tmp_path / "gateway.lock"
    record = {"pid": 123, "start_time": 10.0, "kind": "mercury-gateway"}
