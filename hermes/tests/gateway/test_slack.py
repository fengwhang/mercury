...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /mercury, /btw, /stop,
        ...
        for expected in ("/mercury", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``mercury gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "mercury_feedback" in created_apps[0].registered_actions
        assert "mercury_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("mercury_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("mercury_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestStandaloneSendUserDmResolution:
    ...
    async def test_channel_delivery_honors_unfurl_config(self):
        ...
        assert payload["text"] == "<https://example.com/mercury|Mercury>"
...
class TestSlashCommands:
    ...
    # instead of as /mercury subcommands. This is the Discord/Telegram parity
    ...
    async def test_legacy_hermes_prefix_still_works(self, adapter):
        """Backward compat: /mercury btw foo must still route to /btw foo.

        Old workspace manifests only declared /mercury as the single slash.
...
class TestMessageSplitting:
    ...
    async def test_send_coerces_string_unfurl_options(self, adapter):
        """`mercury config set` / Railway persist YAML booleans as strings.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "mercury gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/mercury-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/mercury-cached.png"]
