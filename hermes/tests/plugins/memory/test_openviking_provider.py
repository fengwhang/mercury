...
_EXPECTED_USER_AGENT = f"openviking-memory-mercury/{_HERMES_VERSION}"
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(
    ...
    from mercury_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(mercury_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Mercury's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Mercury
    venv instead of its own, and on Windows locks Mercury venv DLLs so the
    venv cannot be rebuilt during `mercury update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/mercury/.venv/Lib/site-packages")
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Mercury will retry on a later access "
...
def test_tool_add_resource_rejects_hermes_credential_file_upload(tmp_path, monkeypatch):
    ...
    mercury_home.mkdir()
    auth_json = mercury_home / "auth.json"
    auth_json.write_text('{"OPENROUTER_API_KEY":"sk-test-secret"}', encoding="utf-8")
    monkeypatch.setattr(fs, "_hermes_home_path", lambda: mercury_home)
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "mercury"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (mercury-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "mercury"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for mercury-agent#28296 review (M3): once a session is marked
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Mercury and
