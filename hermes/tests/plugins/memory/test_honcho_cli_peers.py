"""Regression tests for #76414: `mercury honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("mercury.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("mercury_work"). The lookup
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "mercury": {"peerName": "alice", "aiPeer": "mercury"},
            "mercury_work": {"peerName": "alice", "aiPeer": "mercury"},
            "mercury_my_profile": {"peerName": "bob", "aiPeer": "mercury"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "mercury_cli.profiles.list_profiles",
        ...
        assert host == "mercury_work"  # not "mercury.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'mercury_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("mercury.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["mercury_work"]
        cfg["hosts"]["mercury.work"] = {"peerName": "carol", "aiPeer": "mercury"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "mercury_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'mercury.work' (or '(not set)')."""
        monkeypatch.setattr(
            "mercury_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "mercury.work" not in out
        ...
        assert "alice" in work_line and "mercury" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "mercury.new" not in out  # well-formed key, no dot-form leak
