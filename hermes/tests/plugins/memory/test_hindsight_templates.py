...
_CATALOG = {
    "templates": [
        {"id": "conversation", "name": "Conversation", "integrations": ["litellm", "mercury"],
        ...
        {"id": "mercury-gateway-bot", "name": "Gateway Bot", "integrations": ["mercury"],
         "manifest_file": "templates/mercury-gateway-bot.json"},
    ]
}
...
def test_fetch_hermes_templates_filters_to_hermes(monkeypatch):
    ...
    assert ids == ["conversation", "mercury-gateway-bot"]  # coding-agent excluded
...
def test_fetch_manifest_resolves_relative_url(monkeypatch):
    ...
    assert seen["url"] == "https://raw.example/data/templates/mercury-gateway-bot.json"
...
def test_apply_template_posts_to_import_endpoint(monkeypatch):
    ...
    tpl.apply_template("https://api.hindsight.vectorize.io/", "mercury", "hsk_abc", {"version": "1"})

    assert captured["url"] == "https://api.hindsight.vectorize.io/v1/default/banks/mercury/import"
...
def test_apply_template_omits_auth_when_no_key(monkeypatch):
    ...
    monkeypatch.setattr(tpl, "open_credentialed_url", _fake_open)
    tpl.apply_template("http://localhost:8888", "mercury", None, {"version": "1"})
...
def test_run_template_step_applies_selected(monkeypatch):
    monkeypatch.setattr(tpl, "fetch_hermes_templates", lambda url=None: [
        {"id": "mercury-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"},
    ])
    ...
    result = tpl.run_template_step(
        api_url="https://api", bank_id="mercury", api_key="k",
        select=_select_returning(0), cancelled=-1, log=lambda *_: None,
    )
    assert result == "mercury-gateway-bot"
    assert applied["bank"] == "mercury"


def test_run_template_step_blank_selection_skips(monkeypatch):
    entries = [{"id": "mercury-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"}]
...
def test_run_template_step_swallows_apply_errors(monkeypatch):
    ...
    logs = []
    result = tpl.run_template_step(
        api_url="https://api", bank_id="mercury", api_key=None,
...
def test_probe_existing_customization_true_when_bank_has_config(monkeypatch):
    ...
    assert tpl.probe_existing_customization("https://api", "mercury", "k") is True


def test_probe_existing_customization_false_when_empty(monkeypatch):
    monkeypatch.setattr(tpl, "open_credentialed_url",
                        _fake_open({"version": "1"}))
    assert tpl.probe_existing_customization("https://api", "mercury", "k") is False
