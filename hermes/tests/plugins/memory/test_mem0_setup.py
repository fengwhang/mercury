...
def _inject_fake_hermes_cli(monkeypatch):
    """Inject fake mercury_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("mercury_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("mercury_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_hermes_cli = types.ModuleType("mercury_cli")
    fake_hermes_cli.config = fake_config_mod
    fake_hermes_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "mercury_cli", fake_hermes_cli)
    monkeypatch.setitem(sys.modules, "mercury_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "mercury_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["mercury", "--mode", "platform", "--api-key", "sk-test"])
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "mercury-user"
