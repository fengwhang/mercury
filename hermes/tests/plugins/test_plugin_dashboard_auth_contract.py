...
loopback (``X-Mercury-Session-Token`` header) and gated OAuth
(``mercury_session_at`` cookie / single-use ``?ticket=``).
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + mercury-achievements are bundled today; assert at least one is
