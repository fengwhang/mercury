...
def test_cli_register_includes_node_subcommand():
    """`mercury meet` argparse tree includes the node subtree."""
    import argparse
...
def test_looks_like_human_speaker():
    ...
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "Mercury", "mercury agent"):
        assert not _looks_like_human_speaker(s, "Mercury"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "Mercury"), f"{s!r} SHOULD be human"
