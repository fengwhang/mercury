...
def progress(tmp_path):
    ...
    status = tmp_path / "mercury-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Mercury to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `mercury update`, and reports the stage that was on screen
...
for f in "$TMPDIR"/mercury-update-status.[0-9]*; do
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "mercury-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    mercury = install_root / "venv" / "bin" / "mercury"
    mercury.write_text(FAKE_HERMES)
    mercury.chmod(0o755)
    ...
    result = tmp_path / ".mercury-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`mercury update` is the longest wait in the hand-off and the one the
