...
``mercury_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import mercury_state (cycle) — shared
module-level constants live in mercury_state_common.
"""
...
# Moved methods logged under the "mercury_state" logger before the split;
...
def schema_read_probe_statements() -> tuple:
    ...
    ``mercury_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `mercury update` until the user's
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`mercury sessions optimize-storage` with the gateway stopped. "
                        "`mercury doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `mercury sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `mercury db optimize`) must keep its EXISTING
