...
409), and is driven by:
  ...
  * the ``mercury sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``mercury_cli/``) so the
...
is toggled via ``mercury sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.mercury/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``mercury sync enable`` — it is
...
# namespace from mercury-agent's local ``content_hash`` (skills_guard.py:846),
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Mercury Cloud instance can be set
...
def sync_feature_enabled() -> bool:
    ...
    switch a Mercury Cloud deployment sets to opt its instances into sync by
...
def sync_org_auto_propose() -> bool:
    ...
    runs ``mercury sync propose <skill>``. The skill keeps working with the
...
def sync_default_opt_in() -> bool:
    ...
    ``mercury sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Mercury Cloud deployment wants. Per the design notes, this default is
...
#
# Only agent-created + user-authored skills under ~/.mercury/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.mercury/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Mercury Cloud "on by default"):** every *eligible* skill syncs
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.mercury/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``mercury sync device --name`` to set an explicit
    label."""
    ...
    # Mercury Cloud (and any templated deployment) can seed the label
    ...
    # is then persisted, so a later `mercury sync device --name` (or editing the
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.mercury/skills/.sync_device_id, overwriting
...
# ~/.mercury/skills/.sync_state as JSON.
#
...
# path under ~/.mercury/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.mercury/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Mercury speaks "
            f"{WIRE_VERSION} — update Mercury to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "mercury skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (mercury sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.mercury/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "mercury skill sync") -> Optional[Dict[str, Any]]:
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``mercury sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.mercury/skills/_org/
...
#
# `mercury sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.mercury/skills/_org/<org_id>/``.
