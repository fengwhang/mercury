...
_HERMES_CONFIG_FILES = r'\.mercury/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    #     For Mercury/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_HERMES_CONFIG_FILES),
     "mercury_config_mod", "high", "persistence",
     "modification language aimed at Mercury configuration files (verify intent)"),
    (_shell_write_re(_HERMES_CONFIG_FILES),
     "mercury_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting Mercury configuration files"),
    (r'\.mercury/config\.yaml|\.mercury/SOUL\.md',
     "mercury_config_ref", "low", "persistence",
     "references Mercury configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the Mercury-native name; `.clawhubignore` is honored
