...
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `mercury tools` selection.
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.mercury"
    ...
    # Mercury cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `mercury tools` → Image Generation."
            )
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `mercury tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`mercury setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `mercury tools` "
        "→ Image Generation (run `mercury plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `mercury tools` → Image Generation."
                ),
...
# model/provider via `mercury tools` or `/skills`.
