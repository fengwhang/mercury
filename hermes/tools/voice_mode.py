...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.mercury/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``MercuryCLI._voice_start_recording``). A value <= 0
...
# playback-phase barge transcript is treated as a self-capture of Mercury'
...
def is_tts_echo(
    ...
    Mercury just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match Mercury' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Mercury TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"mercury-tts-{_unique}.wav")
