...
Closes the gap tracked in mercury-agent#96873: local execute_code holds a
...
"""Auto-generated Mercury REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    kernel_dir = f"{_env_temp_dir(env)}/mercury_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/mercury_tools.py", tools_src)
