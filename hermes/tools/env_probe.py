...
machine as Mercury itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Mercury ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Mercury's own uv call sites: this
    ...
    # local.py puts the Mercury-managed $HERMES_HOME/bin there, so a managed-only
