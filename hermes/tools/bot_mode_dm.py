...
teammate agent (another Mercury profile on this install, or an agent on a
...
DM each other via a prompt-injected ``mercury -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``mercury -p`` / ``mercury peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``mercury -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``mercury peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'mercury' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('mercury' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "mercury":
...
def message_agent_tool(
    ...
    if peer_match or bare_peer:
        ...
        # Pin the registry-owning profile (#93935): `mercury peer` resolves
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "mercury-dm-*.txt"),
        (temp_root, "mercury-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``mercury -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative mercury next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("mercury", "mercury.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("HERMES_HOME") or os.path.expanduser("~/.mercury"))
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("HERMES_HOME") or os.path.expanduser("~/.mercury")
