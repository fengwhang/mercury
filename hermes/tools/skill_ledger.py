...
``~/.mercury/skills/.curator_ledger.jsonl`` describing who changed what, with
before/after file manifests whose contents are stored content-addressed
(sha256-deduped) under ``~/.mercury/.curator_backups/blobs/``.
...
Design decisions (Teknium-approved):
  ...
  - The ledger covers ALL actors, tagged ``curator`` / ``agent`` / ``user``.
    ...
    ``mercury curator rollback <entry-id>``.
...
# the tree first) makes `mercury curator rollback` restore a shell of a
