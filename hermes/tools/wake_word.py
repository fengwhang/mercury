"""Wake-word ("Hey Mercury") detection — hands-free session trigger.
...
wake word, Mercury opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey mercury" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey mercury" can't
...
# Bundled "hey mercury" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_hermes"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_hermes", "hey mercury", "mercury"})
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey mercury"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from mercury_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Mercury backend "
...
class _SherpaKwsEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        phrase = str(_get(cfg, "phrase") or "hey mercury").strip()
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `mercury tools` "
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors mercury_cli.voice's continuous API)
# ---------------------------------------------------------------------------
