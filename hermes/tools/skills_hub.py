...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            mercury_meta = metadata.get("mercury", {})
            if isinstance(mercury_meta, dict):
                raw_tags = mercury_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/mercury-agent"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Mercury-native bundle
        # under pbakaus/impeccable:.mercury/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``mercury update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.mercury.upstream`` in its SKILL.md
        frontmatter:
            ...
            metadata:
              mercury:
                upstream:
                  ...
                  path: .mercury/skills/impeccable
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.mercury.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(mercury_meta, dict):
            return None
        upstream = mercury_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``mercury_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _hermes_index_cache_file() -> Path:
    return _index_cache_dir() / "mercury-index.json"
...
def _load_hermes_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    mercury_index_cache_file = _hermes_index_cache_file()
    if mercury_index_cache_file.exists():
        try:
            age = time.time() - mercury_index_cache_file.stat().st_mtime
            if age < HERMES_INDEX_TTL:
                return json.loads(mercury_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Mercury index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    try:
        ...
        mercury_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
