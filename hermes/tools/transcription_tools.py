...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``mercury_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so STT does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from mercury_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``mercury auth add <provider_id>``).
...
# DeepInfra STT base URL now resolved via mercury_cli.models.deepinfra_base_url (shared).
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Mercury's remote upload cap."""
...
def _command_stt_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Mercury secrets by
...
def _run_command_stt(
    ...
    Child env is scrubbed of Mercury secrets (salvage of #56332) while still
    ...
    scrubbed = mercury_subprocess_env(inherit_credentials=False)
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"mercury-cmd-stt-{provider_name}-") as tmpdir:
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `mercury plugins list` to see "
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from mercury_cli.plugins import has_hook, invoke_hook
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="mercury-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="mercury-stt-idle-unload", daemon=True
        )
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="mercury-local-stt-") as output_dir:
            ...
            # Scrub Mercury secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import mercury_subprocess_env

            child_env = mercury_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="mercury-stt-") as work_dir:
                try:
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `mercury model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``mercury_cli.models`` helpers so every DeepInfra surface resolves the
    ...
    from mercury_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="mercury-stt-trim-")
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `mercury tools` remediation —
