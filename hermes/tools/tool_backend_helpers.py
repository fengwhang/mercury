...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `mercury model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``mercury auth add
    ...
    2. The environment / ``~/.mercury/.env``. Under a multiplexed gateway turn
       ...
       ``mercury_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``mercury auth
    ...
    when omitted, ``mercury_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `mercury auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``mercury auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `mercury tools` provider string for a config section.
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via mercury "
        f"tools), but {failure}. Run 'mercury tools' to change it."
    )
