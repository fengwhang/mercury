...
# Mercury session id (observability identity, distinct from the gateway
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Mercury session id so observer plugins parent approval
...
def _is_single_query_approval_context() -> bool:
    ...
    ``mercury chat -q "..."`` runs one turn and exits with no user waiting to
...
# active profile home path such as /home/mercury/.mercury/config.yaml. The
# resolved-absolute form is folded into the ~/.mercury/ patterns at detection
...
# ~/.mercury/config.yaml IS the security policy: approvals.mode, yolo, and the
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Mercury: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Mercury
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]mercury[^\n]*\.env\b', "access to Mercury secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `mercury` and `gateway` (e.g. `mercury -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart mercury gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "mercury update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `mercury gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `mercury ...
    ...
    # the `mercury gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.mercury.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "mercury"/"ai.mercury" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.mercury.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "mercury" appear
    ...
    # an unrelated "mercury" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:mercury|ai\.mercury)\b)', "stop/restart mercury launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Mercury file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.mercury/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Mercury-managed security file (~/.mercury/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Mercury config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Mercury config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Mercury config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.mercury/ forms so static user-sensitive patterns catch
    ...
    #
    # Fold the (more specific) Mercury home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\mercury), so folding the user home
    # first would eat the prefix the Mercury-home fold needs.
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.mercury``); the matched
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Mercury home prefix to ``~/.mercury/``.
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.mercury")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``mercury send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Mercury ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"mercury-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart mercury gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.mercury.gateway`` auto-approved
    ...
    anchored on a mercury-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Mercury YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Mercury approval prompts.
...
def get_plugin_manager():
    ...
    from mercury_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from mercury_cli.approval_transport import ApprovalRequest, invoke_approval_transport
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from mercury_cli.config import load_config_readonly as _load_cfg
