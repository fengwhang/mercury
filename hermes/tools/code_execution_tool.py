...
# available on every platform Mercury itself runs on.
...
# operational marker: without it, a sandbox script that spawns/imports Mercury
...
# are non-secret runtime-location flags (the same set mercury_cli treats as the
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    mercury_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'mercury_tools'", window
        )
        if m:
            missing = m.group(1)
...
_RPC_DIR = os.environ.get("HERMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "mercury_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        # --- Session-kernel path (mercury-agent#96873) -------------------
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # HERMES_TIMEZONE is an internal Mercury setting and must not leak
    ...
    # ``mercury_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Mercury's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Mercury's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Mercury-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Mercury-
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        # per interpreter path) so "import mercury_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Mercury "
            "environment; mercury root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.mercury.gateway`, but the
    ...
    try:
        ...
        with open(os.path.join(tmpdir, "mercury_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
...
def _load_config() -> dict:
    ...
    ``mercury --tui`` where it is never used.  Read the lightweight raw config
...
# Interpreter paths already reported as outside the Mercury environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the mercury root from every
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Mercury's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "Mercury's own python (the common case — stdlib plus Mercury's "
    ...
    description = (
        "Run Python that calls Mercury tools programmatically. Use when you "
