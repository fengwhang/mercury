...
URL of Mercury' published Client ID Metadata Document (CIMD) as the
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Mercury"
...
from mercury_constants import secure_parent_dir
...
def _get_token_dir(mercury_home: str | Path | None = None) -> Path:
    ...
    base = Path(mercury_home) if mercury_home is not None else Path(get_hermes_home())
...
def _cached_redirect_port(storage: "HermesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Mercury restarts and chooses a
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``mercury mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `mercury mcp login <server>` interactively to (re)authorize, "
...
class HermesTokenStorage:
    ...
    def __init__(self, server_name: str, *, mercury_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._hermes_home = Path(mercury_home) if mercury_home is not None else None
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``mercury mcp login`` /
        ``mercury mcp remove``, so a fixed document gets another chance.
        """
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://mercury-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Mercury' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `mercury mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = HermesTokenStorage(server_name, mercury_home=mercury_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Mercury only decides whether a
...
# mercury-agent.nousresearch.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://nousresearch.github.io/mercury-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Mercury picks
...
def _server_declined_cimd(storage: "HermesTokenStorage | None") -> bool:
    ...
    Mercury has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    ...
    Every early return below is a case where the redirect URI Mercury would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Mercury" / "Mercury" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Mercury")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Mercury is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: mercury mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Mercury auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Mercury is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`mercury mcp login {server_name}` interactively first to complete "
