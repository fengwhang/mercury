...
`mercury skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.mercury.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.mercury.blueprint`` is a mapping containing
    ...
    mercury = meta.get("mercury") if isinstance(meta, dict) else None
    blueprint = mercury.get("blueprint") if isinstance(mercury, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.mercury.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.mercury.blueprint`` block) they can hand
    to ``mercury skills publish`` to share. ``body`` is the plain-language
