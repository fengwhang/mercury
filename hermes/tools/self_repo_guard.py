...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Mercury's live source checkout "
        ...
        "checkout, stop Mercury, run the command externally, then restart "
        "Mercury."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(mercury_home).expanduser() if mercury_home else Path.home() / ".mercury"
