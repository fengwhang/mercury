...
# set), not Mercury' per-turn session identity.
#
...
class BaseEnvironment(ABC):
    ...
    # True only when commands execute on the SAME host as the Mercury process
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # under Mercury via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Mercury-specific
        # ``HERMES_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``mercury-agent`` — see huggingface.js
        ...
        # inherited from the Mercury process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Mercury running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-mercury-agent}" '
        ...
        # Restrict Mercury metadata files without changing the user's command
