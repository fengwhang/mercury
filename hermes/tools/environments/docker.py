...
_EGRESS_LABEL_KEY = "mercury-egress"
...
def _get_active_profile_name() -> str:
    """Return the active Mercury profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale mercury-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=mercury-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Mercury process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=mercury-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a mercury process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Mercury
    process leaves its container behind permanently because there's no
    subsequent Mercury process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=mercury-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=mercury-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'mercury'
    ...
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `mercury egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `mercury egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `mercury egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `mercury egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/mercury-egress-ca.crt"
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``mercury-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if egress_env_overrides:
            try:
                from mercury_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from mercury_cli.config import load_config as _load_cfg_for_precedence
        ...
        # s6-overlay images (e.g. mercury-agent:latest) already use /init as PID 1
        ...
        # Labels make mercury-created containers identifiable to:
        #   * the orphan reaper (`mercury-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`mercury-task-id`, `mercury-profile`)
        #   * operators running `docker ps --filter label=mercury-agent=1`
        ...
        label_args = [
            "--label", "mercury-agent=1",
            "--label", f"mercury-task-id={task_label}",
            "--label", f"mercury-profile={profile_name}",
        ...
        self._labels = {
            "mercury-agent": "1",
            "mercury-task-id": task_label,
            "mercury-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Mercury process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # keys are filtered. Also strip Mercury-internal dynamic secrets
        ...
        mercury_env = _load_hermes_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or mercury_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("mercury-task-id", "")
        profile_label = self._labels.get("mercury-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"mercury-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``mercury-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=mercury-agent=1",
                "--filter", f"label=mercury-task-id={task_label}",
                "--filter", f"label=mercury-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # mercury-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "mercury egress disable",
        ...
        # container) but can happen if a previous Mercury process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Mercury exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Mercury process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Mercury startup. That covers the
        ...
        # outlives Mercury processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"mercury-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``mercury /quit``
