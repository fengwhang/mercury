...
def iter_sync_files(container_base: str = "/root/.mercury") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.mercury to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, mercury_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.mercury/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(mercury_home=mercury_home)

    def _sync_back_transaction(self, mercury_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="mercury-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.mercury/skills/a.md`` →
        ``~/.mercury/skills/a.md``, a new remote file at
        ``/root/.mercury/skills/b.md`` maps to ``~/.mercury/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
