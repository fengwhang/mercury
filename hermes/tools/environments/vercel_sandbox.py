...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Mercury'
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Mercury
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.mercury"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.mercury"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.mercury"
        )
        archive_member = remote_hermes.lstrip("/")
        remote_tar = f"/tmp/.mercury_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
