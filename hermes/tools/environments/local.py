...
from mercury_constants import get_process_hermes_home
...
#
# Background-process artifacts come in triplets (mercury_bg_<id>.log/.pid/
...
_BG_GROUP_RE = re.compile(r"^(mercury_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")
...
def cleanup_terminal_temp_cache(
    ...
    # Newest mtime per mercury_bg_<id> group, so a live server's fresh .log
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Mercury was already launched from (for example ``mercury-agent``
    while the process cwd is ``~/.mercury/mercury-agent``), passing it through
    unchanged makes the wrapper run ``cd mercury-agent`` *inside* the project
    ...
    # Common recovery for config values like ``mercury-agent`` when Mercury was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./mercury-agent``; use the current directory instead.
...
# Mercury-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Mercury inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Mercury-managed inference credential — Claude subscription auth is not a
    # working Mercury provider path.  Stripping it broke agent-spawned
    ...
    # the :func:`mercury_subprocess_env` Tier-2 strip (browser / TUI host /
...
# :func:`mercury_subprocess_env` (browser / TUI host / copilot-executor
...
# from the blocklist entirely because it is NOT a Mercury credential; these ARE
# Mercury-managed first-party platform credentials, so they stay IN the
...
def _buzz_terminal_context_active() -> bool:
    ...
    * ``BUZZ_MANAGED_AGENT`` in the process env — set exclusively by Buzz
      Desktop's buzz-acp harness when it spawns ``mercury acp`` (#76243).
...
# project's dependencies into the Mercury venv path instead of the project's own
# ``.venv`` — silently clobbering the Mercury environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Mercury venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Mercury venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Mercury itself treats PYTHONHOME as contamination in its own
...
# _strip_hermes_owned_pythonpath() which removes only Mercury-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for Mercury-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Mercury-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`mercury_subprocess_env` helper all call it, so the
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Mercury home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    from mercury_constants import apply_subprocess_home_env
    ...
    # the separate Mercury runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_hermes_owned_pythonpath_and_runtime_markers(sanitized)

    # Keep bare ``mercury`` invocations available to child jobs even when the
...
# legitimate child Mercury spawns needs them, and they are the highest-value
...
# the conditional Tier-2 strip in mercury_subprocess_env().
...
def mercury_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Mercury spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and Mercury-internal dynamic secrets
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`mercury_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``HERMES_HOME`` / subprocess ``HOME`` contract) and the
    Mercury secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Mercury home override into ``HERMES_HOME`` and apply the
      subprocess HOME contract (``mercury_constants.apply_subprocess_home_env``).
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\mercury\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\mercury\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hermes_portable_git = os.path.join(_local_appdata, "mercury", "git") if _local_appdata else ""
    ...
    # healthy portable Git under %LOCALAPPDATA%\\mercury\\git.
    ...
    raise RuntimeError(
        "Git Bash not found. Mercury requires Git for Windows on Windows.\n"
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Mercury. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``mercury`` console-script.
...
def _resolve_hermes_bin_dir() -> str | None:
    """Return the directory holding the ``mercury`` console-script, or None.
    ...
    mercury install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``mercury``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``mercury`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``mercury`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("mercury")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``mercury`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "mercury" or base.startswith("mercury."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "mercury.exe" if _IS_WINDOWS else "mercury"
...
def _prepend_hermes_bin_dir(existing_path: str) -> str:
    """Prepend the mercury install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Mercury-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Mercury installs for
    itself, so on a machine where Mercury provisioned its own toolchain a
    ...
    """
    try:
        from mercury_constants import get_hermes_home, iter_hermes_node_dirs
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Mercury-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Mercury
...
def _make_run_env(env: dict) -> dict:
    ...
    if path_key is not None:
        ...
        # Ensure the mercury install dir is reachable so plugins can shell out
        # to bare ``mercury`` via the terminal tool even when the gateway was
...
def _build_hermes_repo_root_aliases(
    ...
    """Return exact repo-root spellings emitted by Mercury launchers.
    ...
    without treating arbitrary descendants of HERMES_HOME as Mercury-owned.
    ...
    # junction/symlink under the configured root (e.g. D:\mercury\mercury-agent
    # -> C:\...\mercury-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Mercury-owned.  Fail-closed: a
...
# --- Mercury venv / repo-root detection (module-level, computed once) ---

#: The Mercury repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import mercury_cli``,
#: etc.  Subprocesses that are NOT the Mercury backend don't need it and it
#: can shadow local packages.
_hermes_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Mercury launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Mercury-owned paths under
#: the configured HERMES_HOME spelling (which may be a junction to another
#: drive — see ``mercury_cli/gateway_windows.py::_preserve_hermes_home_path``).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Mercury repo>/venv`` layout and a real venv marker; require both before
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Mercury runtime.
...
def _strip_hermes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Mercury-owned PYTHONPATH entries, then the runtime marker vars.
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    """Remove Mercury-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Mercury repo root and the Mercury venv's
    ...
    user entries, so only entries proven Mercury-owned are removed:
    ...
    mercury_site_packages = _get_hermes_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Mercury-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Mercury venv site-packages ---
        ...
        for sp in mercury_site_packages:
    ...
    if stripped:
        logger.debug(
            "Stripped Mercury-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Mercury host itself — controller-side platform
    ...
    def _additional_profile_scoped_passthrough_names(self) -> tuple[str, ...]:
        ...
        ``mercury-snap-<id>.sh`` and a later command from profile B sharing
    ...
    def get_temp_dir(self) -> str:
        ...
        **Default (no override set):** a dedicated cache dir under
        ``HERMES_HOME`` (``~/.mercury/cache/terminal``) rather than ``/tmp``.
        On several distros (Arch and friends) ``/tmp`` is a small RAM-backed
        tmpfs, and Mercury session artifacts — background-process logs,
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = Path(tempfile.gettempdir()) / "mercury_terminal"
        ...
        # Honored ahead of the generic TMPDIR so users can redirect Mercury' temp
        ...
        # because RAM-backed tmpfs /tmp fills up under Mercury load.
        try:
            from mercury_constants import get_hermes_home
            cache_dir = get_hermes_home() / "cache" / "terminal"
            cache_dir.mkdir(parents=True, exist_ok=True)
