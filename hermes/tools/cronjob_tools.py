"""
Cron job management tools for Mercury.
...
#      saying "cat ~/.mercury/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `mercury-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.mercury/.env`, which tripped `read_secrets` and silently
    ...
    for part in str(deliver).split(","):
        ...
        if not profile_exists(canon):
            return (
                ...
                "profile that exists here (mercury profile list), or omit the "
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.mercury/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``mercury -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    # 'running' by a dead owner process -- e.g. a PRIOR one-shot `mercury
    ...
    if not session_key:
        # Direct Python callers (`mercury cron run`, tests) have no agent
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``mercury_cli.cron._builtin_gateway_liveness``
    ...
    """
    try:
        from mercury_cli.cron import _builtin_gateway_liveness
    ...
    if _gw is False:
        return {
            ...
            "warning": (
                f"The Mercury gateway is not running — {subject} "
                "but will NOT fire until the gateway is started "
                "(mercury gateway install / mercury gateway start). "
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            ...
            elif bool(omp_direct):
                # MERCURY-OMP PATCH: omp_direct jobs fire the omp engine
                # directly at tick time — no mercury agent turn, no LLM in
                # the scheduling layer. The prompt IS the omp prompt.
                if not prompt:
                    return tool_error(
                        "create with omp_direct=True requires a prompt — "
                        "the prompt is passed verbatim to omp one-shot "
                        "(`omp --model <delegate_model> -p <prompt>`). "
                        "Skills/script are ignored; omp output (stdout) is "
                        "delivered verbatim; non-zero exit or timeout sends "
                        "an error alert.",
                        success=False,
                    )
                if script:
                    return tool_error(
                        "omp_direct=True ignores script — the prompt is the "
                        "omp invocation payload. Drop script or drop "
                        "omp_direct.",
                        success=False,
                    )
            ...
            try:
                job = create_job_with_scheduler_registration(
                    ...
                    # (mercury cron create --reasoning-effort) ONLY — it is
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "omp_direct": {
                "type": "boolean",
                "default": False,
                "description": "True = fire the omp engine DIRECTLY at tick time, bypassing the mercury agent entirely: the prompt is passed verbatim to `omp --model <delegate_model> -p <prompt>` and omp's output is delivered verbatim. A separate job type from no_agent (omp runs an LLM — the delegate model — while no_agent runs a bare script). Requires prompt; script/skills are rejected. Use for recurring coding/repo tasks that need omp's fan-out but no mercury orchestration."
            },
            "context_from": {
                "type": "array",
                "items": {"type": "string"},
...
def _cronjob_handler(args, **kw):
    ...
    return cronjob(
        ...
        # `mercury cron create/edit --model`, or hand-edited jobs). The agent
        ...
        omp_direct=args.get("omp_direct"),
