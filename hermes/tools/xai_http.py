...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``mercury tools`` repaint, tool-registration scans,
    ...
    2. ``~/.mercury/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``mercury auth add xai-oauth``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.mercury/.env`` first, then ``os.environ``.

    Wraps :func:`mercury_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from mercury_cli.config import get_env_value as _hermes_get_env_value
    except ImportError:
...
def mercury_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Mercury,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": mercury_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Mercury config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Mercury home, then mark it seen."""
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    :func:`mercury_cli.auth._xai_validate_inference_base_url` so a tampered
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Mercury-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``mercury_cli.config.get_env_value`` so keys
    stored in ``~/.mercury/.env`` (the standard Mercury location) are honored —
