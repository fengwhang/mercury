...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``mercury-agent``) are treated as permanently pinned: the system prompt
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to Mercury (the agent's own "
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`mercury curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`mercury curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `mercury curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `mercury curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `mercury sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `mercury sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `mercury sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `mercury sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Mercury profiles.
    ...
    # Default profile (~/.mercury/skills) — only consider when active is non-default.
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"profiles (`mercury -p {other_profile}`) or edit the file "
            ...
            base += (
                ...
                f"Switch profiles (`mercury -p <name>`) to edit there, or "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "to match Mercury skill standards."
    )
...
def _patch_skill(
    ...
    if not old_string:
        ...
        # how to recover. Upstream: NousResearch/mercury-agent#33064.
        return {
            "success": False,
            "error": (
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.mercury/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `mercury curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `mercury curator
