...
_OWN_TAB_PREAMBLE = """\
# mercury: pin this named session to its own tab (once per daemon process)
def _hermes_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "mercury-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Mercury's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Mercury's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Run `mercury tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Mercury' own bin dir ($HERMES_HOME/bin) — where install.sh puts uv/uvx
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Mercury' own ``$HERMES_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Mercury-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Mercury' managed uv (bootstrapped on demand via
    ``mercury_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    ...
    # prevent provisioning the canonical Mercury-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through mercury tools).
...
def _resolve_lightpanda_cdp(
    ...
    """Point the harness at a Mercury-spawned ``lightpanda serve``.
    ...
    try:
        ...
        return (
            ...
            "to use local Chrome, or switch backends via `mercury tools` → "
            "Browser Automation."
        )
...
def _resolve_backend_cdp(
    ...
    4. ``browser.engine: lightpanda``: a Mercury-spawned ``lightpanda serve``
    ...
    try:
        ...
        return (
            ...
            "switch backends via `mercury tools` → Browser Automation."
        )
    cdp = str((session_info or {}).get("cdp_url") or "")
    if not cdp:
        return (
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Mercury launches on a
    SNAPSHOT of their real profile (see mercury_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Mercury-managed copy of "
