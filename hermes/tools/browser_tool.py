...
from mercury_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Mercury secret
...
def _build_browser_env() -> dict:
    ...
    Strips Mercury-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``mercury_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = mercury_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    mercury_node_root = str(mercury_home / "node")
    mercury_nm_bin = str(mercury_home / "node_modules" / ".bin")
    return [mercury_node_bin, mercury_node_root, mercury_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``mercury doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.mercury/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Mercury profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.mercury/plugins/browser/<vendor>/``) participate
...
# comparison sites (four here, plus mercury_cli/tools_config.py and
# mercury_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Mercury-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Mercury CLI where sys.stdout and sys.stderr both report
...
_REAL_PROFILE_SESSION = "mercury-real-profile"
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from mercury_cli.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a mercury-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from mercury_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous mercury process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from mercury_cli.browser_connect import _PROFILE_LOCKED_PREFIX
            ...
            if err and err.startswith(_PROFILE_LOCKED_PREFIX):
                ...
                return None, (
                    ...
                    "`mercury browser close-profile`, then retry."
                )
        ...
        from mercury_cli.browser_connect import chromium_executable
...
# run exactly once, before the cleanup loop started, which meant a mercury
...
# Hard ceiling for a daemon whose owning mercury process is still alive but
...
def _reap_orphaned_browser_sessions():
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live mercury process and must not be touched — but "owner alive"
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a mercury-owned SNAPSHOT of their real profile —
    ...
    # ``browser.engine: lightpanda`` that endpoint is a Mercury-spawned
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Mercury-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Mercury-managed one with no recovery — every candidate
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``mercury update`` and
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``mercury update`` (or ``mercury doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Mercury holds, while running registry-fetched
    npm code on every ``mercury update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `mercury update`/`doctor
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Mercury
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if session_info:
        ...
        # A Lightpanda session is a process Mercury spawned itself (Browser
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/mercury-agent:latest")
                else:
