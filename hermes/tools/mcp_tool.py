...
transport, discovers their tools, and registers them into the mercury-agent
...
def sdk_httpx():
    ...
    side by side with Mercury' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Mercury' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Mercury explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Mercury and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            mercury_home = os.path.expanduser(
                os.getenv(
                    "HERMES_HOME", os.path.join(os.path.expanduser("~"), ".mercury")
                )
            )
            candidates = [
                ...
                # image (which the Mercury Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Mercury MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Mercury gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Mercury document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`mercury_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Mercury profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Mercury' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Mercury' logging (agent.log via mercury_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `mercury setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Mercury process (kill -9, crash, force-quit)
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from mercury_cli.process_identity import register_child
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Mercury' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Mercury is fully
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``mercury mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Mercury' pinned ``httpx``, and the two define unrelated
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Mercury processes (e.g.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Mercury' MEDIA: tag convention and out to messaging
            ...
            # Mercury' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
# Native MCP tool-name prefix. Mercury uses the ``mcp__<server>__<tool>``
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple mercury
