...
left behind by a crashed Mercury.
"""
...
def _home_candidates() -> list:
    ...
    try:
        ...
        logger.debug("mercury home unavailable for lightpanda lookup: %s", e)
    return candidates
    ...
    if not binary:
        if os.name == "nt":
            return None, (
                ...
                "build. Set browser.engine to auto (or run Mercury under WSL2)."
            )
        return None, (
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Mercury is gone.

    Records are written by :func:`launch_lightpanda`; a live owner (another
    Mercury process, or this one still tracking the session) is never
