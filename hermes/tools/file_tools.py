...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``mercury_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where mercury previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Mercury.
...
def _get_hermes_config_resolved() -> str | None:
    """Return the resolved absolute path of the Mercury config file (cached)."""
    ...
    try:
        ...
        try:
            _hermes_config_resolved = str(Path(_expand_tilde("~/.mercury/config.yaml")).resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Mercury config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    mercury_config = _get_hermes_config_resolved()
    if mercury_config and (resolved == mercury_config or normalized == mercury_config):
        return (
            f"Refusing to write to Mercury config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.mercury/config.yaml directly or use 'mercury config' instead."
        )
    return None
...
# .cursorrules (or a project-local .mercury config tree) outlives the current
...
def _get_real_hermes_home() -> str | None:
    """Return the realpath of the authoritative Mercury home (cached)."""
    ...
    try:
        ...
        try:
            _real_hermes_home_cached = os.path.realpath(_expand_tilde("~/.mercury"))
        except Exception:
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from mercury_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.mercury home is governed by its own guards
    ...
    # ``.mercury`` component rule below, which would otherwise match the
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .mercury config dirs (e.g. <repo>/.mercury/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.mercury`` — matching
        # any ancestor named .mercury would gate every write inside a
        # checkout that happens to live under ~/.mercury (e.g. the
        # mercury-agent repo itself at ~/.mercury/mercury-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".mercury":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Mercury mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Mercury state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Mercury process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside Mercury scope.
    """
    try:
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with mercury_tools.write_file() for very "
            "large files."
        )
