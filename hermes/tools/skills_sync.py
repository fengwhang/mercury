...
# Marker file written by `mercury profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.mercury profile).
# When present in HERMES_HOME, sync_skills() is a no-op so neither the
# installer, `mercury update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# mercury_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _essential_names() -> frozenset:
    ...
    try:
        ...
        return frozenset({"mercury-agent"})
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.mercury/skills/.curator_suppressed`` directly if that import
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``mercury skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `mercury skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    """
    # Opt-out: a profile (named or the default ~/.mercury) that wrote the
    ...
    # ``mercury-agent`` skill is the agent's own operating manual and the
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.mercury/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `mercury update` would resurrect a skill the user
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``~/.mercury/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``shutil.rmtree(~/.mercury)`` into a loud, recoverable ``ValueError``
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``mercury update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``mercury update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``mercury skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `mercury skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    installer, ``mercury update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``mercury skills opt-out`` /
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `mercury update` (or `mercury skills "
