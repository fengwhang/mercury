...
-----------------------------------------------------------------------------
    ...
    ~/.mercury/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/mercury/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Mercury convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Mercury wrote
...
def _git_env(
    ...
    The shared store is internal Mercury infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``mercury checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `mercury checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "mercury@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Mercury Checkpoint"], store, cfg_wd)
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Mercury just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Mercury last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Mercury last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Mercury'
        last write or that Mercury never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Mercury has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Mercury never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Mercury wrote it: restoring it back is
                # safe — its last content was Mercury-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Mercury changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Mercury changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Mercury' last write — per the agent-write ledger —
        are left untouched, and only Mercury-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Mercury-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Mercury created it. Deleting it would not restore a prior
...
# ---------------------------------------------------------------------------
# Public helpers for `mercury checkpoints` CLI
# ---------------------------------------------------------------------------
