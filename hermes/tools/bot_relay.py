...
Mercury Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own MERCURY root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in mercury_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "mercury" if profile == "default" else profile
    if (
...
def _hermes_cli() -> str:
    """Resolve the mercury CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``mercury``
    entrypoint. A bare ``"mercury"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("mercury.exe" if sys.platform == "win32" else "mercury")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("mercury")
    if found:
        return found
    return "mercury"
...
# turns concurrently: deliveries spawn separate ``mercury`` subprocesses, so
