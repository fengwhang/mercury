...
# display_hermes_home imported lazily at call site (stale-module safety during mercury update)
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive Mercury password prompt or the sudo -S password-pipe path.
    ...
    # silently without Mercury prompting.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``mercury-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Mercury processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Mercury processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Mercury processes a generous grace
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per Mercury
...
def _current_session_profile() -> str:
    """Return the active session's Mercury profile name, or "" when unset.
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``mercury
    ...
    be stale from ``mercury setup``). Environment values for omitted terminal
    ...
    try:
        from mercury_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from mercury_cli.config import _is_ssh_remote_tilde_cwd
    ...
    return {
        ...
        # Startup orphan reaper for mercury-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Mercury processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Mercury tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        effective_task_id = _resolve_container_task_id(task_id)
        if _host_local:
            # Mercury-owned control-plane children must run beside the current
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/mercury
        # restart|stop|uninstall targeting mercury-gateway) must never run inside the
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Mercury cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `mercury gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # mercury-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in mercury-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `mercury -z`, a "
