...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (mercury_constants itself is stdlib-only, so no cycle).
...
# timescales. Cache results for ~30 s so env-var flips via ``mercury tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Mercury-home override for every
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``mercury tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("mercury_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``mercury_plugins.{name}``), not the exact module string.
                # A handler defined in ``mercury_plugins.pkg.handlers`` is
                # still owned by the ``mercury_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``mercury tools enable foo``)
