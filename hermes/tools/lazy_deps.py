"""
Lazy dependency installer for opt-in Mercury backends.

Many Mercury features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``mercury-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``mercury tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  and makes ``/opt/mercury`` read-only), setting
  ...
  installed package cannot brick Mercury, which is what made it safe to seal
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``mercury update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Mercury under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``mercury update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="mercury-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``mercury_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = mercury_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $HERMES_HOME/bin is never on PATH, so a bare
        # which() misses the uv Mercury installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Mercury
    ...
    if _lazy_install_target() is None:
        try:
            from mercury_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Mercury cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Mercury instead)."
            )
...
def active_features() -> list[str]:
    ...
    Used by ``mercury update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``mercury update``. Never raises; lazy-install failures
