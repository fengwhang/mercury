...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.mercury/config.yaml``. Mercury
...
# Base URL now resolved via mercury_cli.models.deepinfra_base_url (shared).
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``mercury tools``); otherwise the historical Edge backend remains active.
...
# custom voice-cloning scripts, etc.) into Mercury without any Python code
...
#           output_format: wav
#
# Mercury writes the input text to a temp UTF-8 file, runs the command with
...
# file, so retired models disappear from mercury the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``mercury_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `mercury setup` to configure, "
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `mercury model` or set XAI_API_KEY.")
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import mercury_cli as _hermes_cli

            _hermes_version = str(_hermes_cli.__version__)
        except Exception:
            _hermes_version = "0.0.0"
        # Include Mercury client context following Gemini's partner
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `mercury setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'mercury tools' and select Piper under TTS, or install manually: "
