"""
`mercury computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Mercury side — the only
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Mercury-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    if json_output:
        ...
        # attach Mercury identity under mercury_identity so existing parsers
