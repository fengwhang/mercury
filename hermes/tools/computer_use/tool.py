...
`health_report` (surfaced by `mercury computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Mercury runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Mercury-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/mercury-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Mercury's explicit approval bypass onto Cua's immutable mode.

    Mercury has TWO session-identity namespaces: the tool-dispatch path passes
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Mercury YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Mercury sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Mercury process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `mercury computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/mercury-agent#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/mercury-agent#47072 — trycua/cua#1961
...
def check_computer_use_requirements() -> bool:
    ...
    `mercury computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
