...
etc.) surface as specific blocked checks via `mercury computer-use doctor`
...
def _cua_no_overlay() -> bool:
    """True when Mercury should pass ``--no-overlay`` to cua-driver.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session Mercury YOLO toggle so a stale config line can never
...
def _empty_discovery_reason() -> str:
    ...
    if sys.platform == "darwin":
        ...
        return (
            ...
            "`mercury computer-use doctor`"
        )
    return (
        "window discovery returned no windows; run `mercury computer-use "
    ...
    *,
    ...
    if not resolved_app:
        raise RuntimeError(
            ...
            "Run `mercury computer-use install` to restore it."
        )
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Mercury session's YOLO choice
    ...
    * ``unrestricted`` — explicit Mercury YOLO; launch-time risk
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\mercury-cua-{token}"
        else:
    ...
    def start(self) -> None:
        ...
        self._stderr_thread = threading.Thread(
            ...
            args=(self._process,),
            name="mercury-cua-daemon-stderr",
            daemon=True,
        )
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/mercury-agent#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Mercury an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    Mercury Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `mercury computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Mercury process is restarted.
            os.path.join(
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Mercury' 0.20 integration."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `mercury computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  mercury computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `mercury tools` and enable the Computer Use toolset to install it automatically."
    )
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/mercury-agent#47072: per-tool
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `mercury computer-use doctor` and check "
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # Mercury talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so Mercury did not replay it. Take fresh state before "
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/mercury-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Mercury run, with
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a Mercury
    ...
    def capture(
        ...
        Maps mercury `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/mercury-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/mercury-agent#47072: prefer the
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Mercury adapter, and uses cua-driver's
