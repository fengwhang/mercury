...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Mercury' — mcp 2.0 builds on httpx2,
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``mercury mcp reauth``.
            ...
            try:
                ...
                # walking back into the same refusal. `mercury mcp login`
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._hermes_server_name,
                    mercury_home=self._hermes_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `mercury mcp login "
    ...
    def remove(
        ...
        *,
        ...
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, mercury_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, mercury_home=mercury_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, mercury_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, mercury_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, mercury_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(mercury_home) / f"{_safe_filename(server_name)}.json"
            try:
