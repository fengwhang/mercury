"""Progressive tool disclosure ("tool search") for Mercury.
...
``tool_call`` — and surfaced on demand. Core Mercury tools never defer.
...
def load_config_readonly() -> ToolSearchConfig:
    ...
    try:
        from mercury_cli.config import load_config_readonly as _load
        cfg = _load() or {}
        tools_cfg = cfg.get("tools") if isinstance(cfg.get("tools"), dict) else {}
        if not isinstance(tools_cfg, dict):
            tools_cfg = {}
