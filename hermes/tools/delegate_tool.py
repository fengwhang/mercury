...
# Must match mercury_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _get_max_spawn_depth() -> int:
    ...
    HERMES-OMP PATCH: in Mercury, delegate_task children run as omp
    one-shots, so RECURSION IS omp-side — omp subagents spawn their own
    subagents natively and this hermes-side guard never applies to them.
    Raise to 2+ only for nested HERMES-style orchestration.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``mercury-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``mercury-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. mercury-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (mercury-cli,
    # mercury-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Mercury/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from mercury_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # request-scoped chat_id binding (the raw X-Mercury-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Mercury-Session-Id once the batch completes.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `mercury -z`, a cron job, a Kanban "
