...
"""Auto-generated Mercury session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
# mercury-agent#88637 by @z80dev (stable owner id, owner-teardown disposal,
...
def shutdown_kernels_for_owner(owner: str) -> None:
    ...
    (the /new + session-close disposal shape from mercury-agent#88637).
    """
    if not owner:
        return
    with _KERNELS_LOCK:
...
def _spawn(kernel: SessionKernel, *, task_id: str, child_python: str,
    ...
    kernel.tmpdir = tempfile.mkdtemp(prefix="mercury_kernel_")
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()

    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@MERCURY-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    ...
    if _IS_WINDOWS:
        ...
        kernel.sock_path = os.path.join(_sock_tmpdir, f"mercury_rpc_{uuid.uuid4().hex}.sock")
    ...
    with open(os.path.join(kernel.tmpdir, "mercury_tools.py"), "w", encoding="utf-8") as f:
        f.write(tools_src)
    runner_path = os.path.join(kernel.tmpdir, "mercury_kernel_runner.py")
