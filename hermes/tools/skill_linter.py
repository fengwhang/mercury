...
Design contract (matches the Mercury "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    mercury_meta = meta.get("mercury") if isinstance(meta, dict) else None
    if not isinstance(mercury_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.mercury.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in mercury_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.mercury.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("mercury", "agent", "mercury agent") and (
        author != "Mercury"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Mercury' (proper caps) "
