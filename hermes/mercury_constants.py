...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Mercury home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Mercury home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Mercury home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Mercury home path."""
...
def get_hermes_home() -> Path:
    """Return the Mercury home directory (default: platform-native path).
    ...
    template in ``mercury_cli/gateway.py`` and the kanban dispatcher in
    ``mercury_cli/kanban_db.py``).  See https://github.com/NousResearch/mercury-agent/issues/18594.
    """
...
def mercury_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Mercury home/profile directory.
...
def get_process_hermes_home() -> Path:
    """Return the Mercury home for the running process, ignoring task overrides.
...
def get_default_hermes_root() -> Path:
    ...
    In standard deployments this is the platform-native Mercury home
    (``~/.mercury`` on POSIX, ``%LOCALAPPDATA%\\mercury`` on native Windows).

    In Docker or custom deployments where ``HERMES_HOME`` points outside
    ``~/.mercury`` (e.g. ``/opt/data``), returns ``HERMES_HOME`` directly
    — that IS the root.
    ...
    if not env_home:
        ...
        try:
            ...
            # HERMES_HOME is under ~/.mercury (normal or profile mode)
            result = native_home
        except ValueError:
...
# Files whose presence marks a directory as a real Mercury home. A fresh home
...
def _is_hermes_profiles_root(profiles_dir: Path) -> bool:
    """Return True when *profiles_dir* is a canonical ``<mercury-home>/profiles``.

    Anchors named-profile recognition so it only fires for directories that
    provably live under a Mercury home: the classic ``~/.mercury`` layout, a
    root carrying Mercury-home marker files (Docker/custom ``HERMES_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``mercury profile delete``), or the process's resolved
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Mercury home (see :func:`_is_hermes_profiles_root`). A default Mercury home
    whose path merely contains a ``profiles`` segment
    (e.g. ``/tmp/foo/profiles/notahome/.mercury``) is not a named profile,
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            return candidate
        # Stop at a default Mercury home so a coincidental ``profiles/``
        # ancestor is not treated as a named-profile root.
        if candidate.name == ".mercury":
            return None
    return None
...
def get_hermes_dir(
    ...
    *,
    ...
    Args:
        ...
        home: Optional explicit Mercury home. Profile-aware callers that manage
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Mercury-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\mercury\\node``. POSIX installs use
...
# Install tree root (this file lives at <install_root>/mercury_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks mercury-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Mercury-managed Node trees live under ``$HERMES_HOME/node`` (or a profile's
    ...
    used to trust file presence alone, so ``mercury update`` would pick that
...
def mercury_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Mercury-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Mercury-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `mercury update` afterwards).",
        flush=True,
    )
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the mercury.exe quarantine);
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a Mercury-managed Node tree and return its npm path.
    ...
    Mercury never modifies a toolchain it does not own, so instead it provisions
...
def heal_hermes_managed_node() -> bool:
    """Redownload Mercury-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not mercury_managed_node_tree_present():
        return False
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Mercury-managed Node/npm executable path, healing broken trees.
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Mercury-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Mercury-managed installs.

    This is for Mercury-owned subprocesses that should not be broken by a bad,
    ...
    if managed:
        return managed
    if mercury_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Mercury-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``mercury update`` — leaving a **dangling symlink** that ``which`` still
...
def display_hermes_home() -> str:
    ...
    try:
        ...
        # ``~/AppData\Local\mercury/skills/`` once callers append
...
def secure_parent_dir(path: Path) -> None:
    ...
    Also refuses to chmod the mercury-agent install tree (the directory this
    ...
    See https://github.com/NousResearch/mercury-agent/issues/25821 and
    https://github.com/NousResearch/mercury-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks mercury-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "mercury-agent install directory (%s). Credential files are "
            "normally stored under the mercury home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    mercury_home = get_hermes_home_override() or (env or {}).get("HERMES_HOME") or os.getenv("HERMES_HOME")
    if not mercury_home:
        return None
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Mercury profile HOME.

    ``HERMES_HOME`` scopes Mercury state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Mercury' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Mercury itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/mercury-agent#47111
    """
...
def get_config_path() -> Path:
    ...
    MERCURY-OMP PATCH (unified config): when MERCURY_CONFIG is set, mercury'
    config lives in the ONE Mercury file (~/.mercury/config.yaml) under the
    ``mercury:`` subtree — same file as omp's ``omp:`` subtree and the shared
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``mercury_cli`` modules using three different Windows
    ...
    A few sites outside ``mercury_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``mercury_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Mercury itself. An ImportError
...
# `mercury_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Mercury.
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `mercury update` is exactly
...
# MERCURY-OMP PATCH (rename completeness): dynamic CLI invocation name. Under
# the mercury launcher (MERCURY_HOME set) every user-facing "run mercury …"
# hint must say `mercury`; stock installs keep `mercury`. One helper, used
# at every call site — no hardcoded stragglers (drive finding #7).
def cli_invocation_name() -> str:
    """Return the command name users should type: 'mercury' (launcher) or 'hermes' (stock)."""
    if os.environ.get("MERCURY_HOME", "").strip():
        return "mercury"
    return "hermes"
