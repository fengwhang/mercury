...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return Path(os.environ.get("HERMES_HOME", Path.home() / ".mercury")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("HERMES_HOME", Path.home() / ".mercury")
...
class EventBridge:
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            ...
            db_file = Path(os.environ.get("HERMES_HOME", Path.home() / ".mercury")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    ...
    mcp = MCPServer(
        "mercury",
        instructions=(
            "Mercury messaging bridge. Use these tools to interact with "
