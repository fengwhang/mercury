...
# IMPORTANT: mercury_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See mercury_bootstrap.py for full rationale.
try:
    import mercury_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when mercury_bootstrap isn't registered in the venv
    # yet — happens during partial ``mercury update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``mercury -c`` / ``--resume`` (relaunch
...
_loaded_env_paths = load_hermes_dotenv(mercury_home=_hermes_home, project_env=_project_env)
...
# (mercury_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Mercury-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.mercury/sessions/``.  Without
...
class AIAgent:
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `mercury --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from mercury_cli.models import ensure_lmstudio_model_loaded
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``mercury chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        mercury-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See mercury-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Mercury against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Mercury can't reach the model provider. You may be offline. "
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from mercury_cli import lifecycle as _lifecycle
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.mercury/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Mercury-Session-Id header) and must not escape
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Mercury model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Mercury-authored content from later user hand-edits.
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.mercury/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Mercury process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Mercury process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `mercury doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.mercury/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.mercury/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`mercury doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        # MERCURY-OMP PATCH (B2 phase 2): kanban worker heartbeat bridge removed
        # (kanban engine deleted; delegation = omp).
        if force_persist:
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        if _dev:
            ...
            # `mercury logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Mercury YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``mercury auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                ...
                from mercury_cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.mercury/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``mercury setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from mercury_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from mercury_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from tools.xai_http import mercury_xai_default_headers

            self._client_kwargs["default_headers"] = mercury_xai_default_headers()
        else:
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from mercury_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from mercury_cli.models import ollama_model_supports_thinking
    ...
    def _github_models_reasoning_extra_body(self) -> dict | None:
        ...
        if self.reasoning_config and isinstance(self.reasoning_config, dict):
            ...
            requested_effort = str(
                self.reasoning_config.get("effort", "xhigh")
            ).strip().lower()
        else:
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated mercury_logging._session_context (a
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        MERCURY-OMP PATCH (B1): delegation engine = omp one-shot children.
        The model-facing dispatch routes to tools/omp_delegation.py (patched
        omp build, single delegate model, roles structurally inert). Mercury
        child agents are no longer spawned from this path; top-level
        delegations still run in the background through the same async
        delegation registry (consolidated summaries re-enter the
        conversation), orchestrator-subagent delegations stay synchronous.
        Hidden ACP transport args are stripped HERE (same invariant as the
        old engine; the omp engine strips again — idempotent) so they can
        never reach the verbatim omp prompt.
        """
        from tools.delegate_tool import _strip_model_hidden_task_fields
        from tools.omp_delegation import dispatch_omp_delegation
        cleaned = dict(function_args)
        cleaned["tasks"] = _strip_model_hidden_task_fields(cleaned.get("tasks"))
        # Top-level hidden transport args are dropped, not forwarded (the old
        # engine forwarded a kwargs whitelist; same effect, explicit).
        for hidden in ("acp_command", "acp_args"):
            cleaned.pop(hidden, None)
        return dispatch_omp_delegation(self, cleaned)
    ...
    def run_conversation(
        ...
        from mercury_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            # Serialize the full load -> run -> flush region across Mercury
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Mercury process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Mercury process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Mercury process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Mercury process kept this session busy too "
