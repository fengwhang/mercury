...
Mercury (see website/docs/getting-started/platform-support.md). The
...
Mercury package derivation, so only that build may create an artifact.
...
_BLOCK_MESSAGE = (
    "Building wheels or sdists for mercury-agent is not supported.\n"
    "Mercury is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://mercury-agent.nousresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Mercury Nix derivation sets HERMES_NIX_BUILD=1. If it does, file a bug."
)
