"""Mercury-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
Mercury' richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those Mercury tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.mercury-tools]`) and
the user gets full Mercury capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - skill_view, skills_list              — Mercury' skill library
  - text_to_speech                       — TTS
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    heartbeat/show/list/create/            handoff (stateless: read env var,
    unblock/link)                          write ~/.mercury/kanban.db)
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in Mercury
...
Run with: python -m agent.transports.mercury_tools_mcp_server
...
# Tools we expose. Each name MUST match a registered Mercury tool that
...
#     `_AGENT_LOOP_TOOLS` in Mercury (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. Mercury' default runtime
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # the env var and write to ~/.mercury/kanban.db.
    "kanban_complete",
    "kanban_block",
...
def _build_server() -> Any:
    """Create the MCP server with Mercury tools attached. Lazy imports
    ...
    try:
        ...
        raise ImportError(
            f"mercury-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc

    # Discover Mercury tools so dispatch works.
    ...
    mcp = MCPServer(
        "mercury-tools",
        instructions=(
            "Mercury's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative Mercury tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs Mercury gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this Mercury process", name
            )
            continue

        description = spec.get("description") or f"Mercury {name} tool"
        ...
        # string, and carries a __signature__ synthesized from the Mercury
    ...
    logger.info(
        "mercury-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.mercury_tools_mcp_server`."""
    ...
    # Quiet mode: keep Mercury' own banners off stdout (which is the MCP wire).
    ...
    try:
        ...
        sys.stderr.write(f"mercury-tools MCP server cannot start: {exc}\n")
        return 2
    ...
    try:
        ...
        logger.exception("mercury-tools MCP server crashed")
        sys.stderr.write(f"mercury-tools MCP server error: {exc}\n")
        return 1
    return 0
