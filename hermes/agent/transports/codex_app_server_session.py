...
Owns one Codex thread per Mercury session. Drives `turn/start`, consumes
...
# Mercury' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Mercury/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Mercury session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Mercury' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Mercury' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own mercury-tools callback we
            # auto-accept — the user already approved Mercury' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Mercury
        approval-mode/timeout logic. The Mercury-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Mercury approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Mercury approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Mercury returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Mercury approval mode/timeout resolution
    ...
    # "prompt expired"; the Mercury-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort Mercury version string for codex's userAgent line."""
    try:
