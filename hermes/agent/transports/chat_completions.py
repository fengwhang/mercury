...
_XAI_TOOL_SEARCH_ALIAS = "mercury_tool_search"
...
def _rename_tool_search_bridge_for_xai(
    ...
    occupies ``mercury_tool_search``, the bridge takes a ``_2``/``_3``
...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    ...
    Mercury' internal effort set extends the wire vocabulary with ``ultra``
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Mercury/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from Mercury'
    ...
    # is stricter (low/high). Clamp Mercury' wider effort set to what each
...
def _raise_gemini_thinking_max_tokens(
    ...
    global Mercury cap of 4096 is enough for visible text, but Ultra/high
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        - Mercury-internal scaffolding markers — any top-level message key
        ...
        for msg_idx, msg in enumerate(messages):
            ...
            # Drop all Mercury-internal scaffolding markers (``_``-prefixed).
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if message_tool_calls:
            ...
            for tc in message_tool_calls:
                ...
                # preserve an explicit blank name for Mercury's recovery path.
                ...
                # ``mercury_tool_search`` dispatches as itself when no alias
