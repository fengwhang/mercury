...
# Wire-name used when Mercury keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Mercury's configured provider
# (Firecrawl / Exa / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "mercury_web_search"
...
# Mercury dispatch is unaffected.
...
# Mercury's progressive-disclosure bridge registers exactly that literal
...
# named ``mercury_tool_search`` is never silently rewritten to ``tool_search``
# unless THIS request actually emitted that alias (#95003 review contract).
_LEGACY_ALIAS_FALLBACK = {
    **_RESERVED_ALIAS_TO_NAME,
    "mercury_web_search": "web_search",
}
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Mercury web providers.
        #
        ...
        #    keep Mercury dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``mercury_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo THIS request's wire aliases before Mercury dispatch.
                ...
                # ``mercury_tool_search`` etc. is dispatched as itself when
