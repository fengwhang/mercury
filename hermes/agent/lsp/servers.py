...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_path = os.path.join(
        mercury_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(mercury_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Mercury -HostProfileId mercury -HostVersion 1.0.0 "
...
def mercury_lsp_session_dir() -> str:
