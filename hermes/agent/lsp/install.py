...
appropriate.  All installs go to a Mercury-owned bin staging dir,
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``mercury lsp
  status``.
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `mercury lsp status` reports the host's presence.
...
def mercury_lsp_bin_dir() -> Path:
    """Return the Mercury-owned bin staging dir for LSP servers."""
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(mercury_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    # PATH, so a bare which() misses the Node that Mercury installed and
    ...
    if npm is None:
        ...
        return None
    staging = mercury_lsp_bin_dir().parent  # <HERMES_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = mercury_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a mercury-owned target dir.
    ...
    console script.
    """
    pip_target = mercury_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = mercury_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``mercury lsp status`` CLI to give users a quick
