...
# Minimum context length required to run Mercury.  Models with fewer
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Mercury' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Mercury minimum.
...
# who never asked for it. The variant suffix is a Mercury-side alias: it is
...
# Mercury-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a Mercury picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are Mercury picker aliases — the Codex catalog only
...
def get_model_context_length(
    ...
    # ``Mercury-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
