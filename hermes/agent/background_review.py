...
See the ``mercury-agent-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Mercury through a client
    shim, and a shim that cannot carry Mercury tool calls back turns the fork into
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Mercury, e.g. 'mercury-agent').\n"
    "  • Hub-installed skills (installed via 'mercury skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'mercury curator pin'). You are an "
    ...
    "'mercury curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'mercury skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'mercury curator pin'). Pin blocks "
    ...
    "reply and recommend 'mercury curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Mercury tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit Mercury tool calls, "
