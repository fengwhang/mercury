"""Upload a Mercury session transcript to Hugging Face as an agent trace.

Mercury stores sessions in its own SQLite store (``mercury_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``mercury trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Mercury' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "mercury-traces"
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.mercury/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `mercury trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/mercury-traces`` dataset. Returns a
