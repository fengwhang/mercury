...
_HERMES_MD_NAMES = (".hermes.md", ".mercury.md", "HERMES.md", "MERCURY.md")
...
def _find_hermes_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .mercury.md planted in /tmp, /home, etc.
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Mercury. Mercury is a hybrid distribution of Hermes by Nous Research and omp by can1357. Be direct: match the "
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    # precedence. Injected only when skill_view exists AND the mercury-agent
...
#
# The mercury-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to mercury-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Mercury appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Mercury floating over another application, so the
    user is rarely asking about Mercury — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Mercury "
        ...
        "rather than anything inside Mercury. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Mercury terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the Mercury desktop app, a graphical chat "
        ...
        "data-mercury-send=\"prompt\" on any clickable element (or "
        "window.mercury.send(\"prompt\")) sends that prompt as a hidden user "
...
# container / remote host rather than on the machine where Mercury itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Mercury runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Mercury itself is running. The host OS, home, and cwd "
                f"of the Mercury process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Mercury "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Mercury
...
def build_skills_system_prompt(
    ...
    try:
        ...
        # Trusted project-local dirs (./.mercury/skills, ./.agents/skills at
...
def load_soul_md(
    ...
    MERCURY-OMP PATCH (F0): under MERCURY_HOME, SOUL.md is the SHARED persona
    file at the mercury home top level (both engines read it; omp via its
    discovery patch). The home_override profile-scoping still wins when
    provided. Without MERCURY_HOME, stock behavior.
    """
    try:
...
def load_project_memory_hint(context_length=None):
    """MERCURY-OMP PATCH (memory consolidation): shared project memory.

    Under MERCURY_HOME, per-project memory lives in the ENGINE-NEUTRAL
    shared tree at $MERCURY_HOME/memories/projects/--<encoded-cwd>--/
    (both engines read and write it; no engine name in the shared path).
    memory_summary.md rides inline (<=4k) and learned lessons (<=2k,
    newest kept); oversize stores degrade to pointer form.
    """
    mercury_home = os.environ.get("MERCURY_HOME", "").strip()
    if not mercury_home:
        return None
    cwd = os.getcwd()
    encoded = "--" + cwd.strip("/").replace("/", "-").replace(":", "-").replace("\\", "-") + "--"
    root = Path(mercury_home) / "memories" / "projects" / encoded
    if not root.is_dir():
        return None
    # CONSOLIDATED (user decision): inline the actual content, bounded.
    # memory_summary.md (omp's consolidated project memory) rides inline
    # up to 4k chars; learned.md lessons up to 2k. Larger stores degrade
    # to the pointer form. hermes therefore has omp's memories in-context,
    # not just knowledge that they exist.
    MAX_SUMMARY = 4000
    MAX_LESSONS = 2000
    lines = ["## omp project memory (shared, consolidated)"]
    summary = root / "memory_summary.md"
    lessons = root / "learned.md"
    inlined = False
    try:
        if summary.is_file():
            ...
            if text:
                if len(text) > MAX_SUMMARY:
                    text = text[:MAX_SUMMARY] + f"\n…[truncated — full store at {summary}]"
                lines.append(text)
                inlined = True
    except Exception:
        pass
    try:
        if lessons.is_file():
            text = lessons.read_text(encoding="utf-8").strip()
            if text:
                if len(text) > MAX_LESSONS:
                    text = "\n".join(text.splitlines()[-30:]) + f"\n…[truncated — full lessons at {lessons}]"
                lines.append("Lessons (newest kept):\n" + text)
                inlined = True
    except Exception:
        pass
    if not inlined:
        have = [f.name for f in root.iterdir() if f.is_file() and f.suffix == ".md"]
        if not have:
            return None
        lines.append(f"omp memory store for this project at {root}: " + ", ".join(sorted(have)) + ".")
    return "\n".join(lines)


def load_agents_md_home(
    context_length: Optional[int] = None,
    home_override: "Path | None" = None,
) -> Optional[str]:
    """MERCURY-OMP PATCH (config/ reorg): shared AGENTS.md from the Mercury home.

    Cross-project WORKING INSTRUCTIONS for BOTH engines (distinct from
    SOUL.md persona and the per-side HERMES.md/OMP.md supplements).
    hermes injects it here; omp reads the same file via the
    mercury-memory discovery provider. Project-level AGENTS.md (cwd walk)
    still loads additively through the normal context-file scan.
    """
    mercury_home = os.environ.get("MERCURY_HOME", "").strip()
    if not mercury_home:
        return None
    ...
    try:
        ...
        content = _truncate_content(
            content, "AGENTS.md", context_length=context_length,
            read_path=str(path),
        )
        return content or None
    except Exception:
        logger.debug("AGENTS.md (mercury home) unreadable: %s", path, exc_info=True)
        return None


def load_hermes_md_home(
    context_length: Optional[int] = None,
    home_override: "Path | None" = None,
) -> Optional[str]:
    """MERCURY-OMP PATCH: load MERCURY.md from the Mercury home (MERCURY_HOME).

    Mercury's harness-scoped prompt file: content is injected ONLY into the
    mercury system prompt (stable tier, after SOUL.md). Never seen by omp —
    omp's counterpart is OMP.md (discovery-layer patch on the omp side).
    Optional: absent file → None, no prompt change. Distinct from the
    PROJECT-level .mercury.md/MERCURY.md walk (cwd git root) which is
    workspace instructions — this is home-level, harness-scoped persona
    supplement for the mercury half of the distribution.
    """
    mercury_home = os.environ.get("MERCURY_HOME", "").strip()
    if not mercury_home:
        return None
    ...
    try:
        ...
        content = _truncate_content(
            content, "HERMES.md", context_length=context_length,
            read_path=str(path),
        )
        return content
    except Exception as e:
        logger.debug("Could not read MERCURY.md from %s: %s", path, e)
        return None


def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.mercury.md / MERCURY.md — walk to git root."""
    mercury_md_path = _find_hermes_md(cwd_path)
    ...
    try:
        ...
        return _truncate_content(
            result, ".mercury.md", context_length=context_length,
            read_path=str(mercury_md_path),
        )
    except Exception as e:
...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Mercury install/source
    ...
    # explicitly configured cwd is honored verbatim — the Mercury tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Mercury in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Mercury install tree (%s) — set terminal.cwd to "
