...
3. ``~/.mercury/.anthropic_oauth.json`` (Mercury PKCE login)
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in mercury_cli/runtime_provider.py.
    """
...
class CredentialPersistError(RuntimeError):
    ...
    ``~/.mercury/.anthropic_oauth.json`` for ``mercury_pkce``).
...
#     authority boundary of ``claude_code``/``mercury_pkce`` is the shared
#     singleton file, which other Mercury processes/profiles read with fresh
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    ...
    return source_path.with_name(source_path.name + ".mercury-spent-rotations.json")
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by Mercury so sibling "
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every Mercury profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Mercury notices an expired token, Claude Code may have already rotated it.
    ...
    try:
        from mercury_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``mercury models``) never mutates ``~/.mercury/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      5. Anthropic credential_pool OAuth entry (~/.mercury/auth.json)
    ...
    # 5. Mercury credential_pool OAuth entry.
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from mercury_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def _write_hermes_oauth_credentials(
    ...
    """Write refreshed mercury_pkce tokens back to ~/.mercury/.anthropic_oauth.json.

    Without this, a successful pool-level refresh of a ``mercury_pkce``-sourced
