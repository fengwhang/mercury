...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    3. A peek into the in-process pricing cache in ``mercury_cli.models``
    ...
    if not base_url:
        return False
    try:
        from mercury_cli.models import _is_model_free, _pricing_cache
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Mercury",
                    version_val,
                )
            return None
