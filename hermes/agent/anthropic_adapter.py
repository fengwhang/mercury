"""Anthropic Messages API adapter for Mercury.
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to mercury's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to mercury's outer conversation
    ...
    # from ``ANTHROPIC_API_KEY`` (Mercury loads that into the process env from
    # ``~/.mercury/.env``). The result is dual auth —
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Mercury", "Claude Code")
        ...
        #      a) bare Mercury-native tools (``read_file``)  -> ``mcp__read_file``
