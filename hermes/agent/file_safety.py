...
def _hermes_root_path() -> Path:
    """Resolve the Mercury root dir (always the parent of any profile, never per-profile)."""
    try:
        from mercury_constants import get_default_hermes_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    mercury_home = _hermes_home_path()
    mercury_root = _hermes_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(mercury_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(mercury_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(mercury_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(mercury_home / "cache" / "bws_cache.enc.json"),
            str(mercury_root / "cache" / "bws_cache.enc.json"),
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in (_hermes_home_path(), _hermes_root_path()):
        try:
            ...
            if real not in mercury_dirs:
                mercury_dirs.append(real)
        except Exception:
            continue

    for base_real in mercury_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Mercury path.

    Three categories are blocked:

      * Internal Mercury cache files under ``HERMES_HOME/skills/.hub`` —
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.mercury/.env`` and exfiltrate the file. The read-deny exists
    ...
    # HERMES_HOME or <root> (profile-root widening: mercury_dirs covers
    # both, so observatory secrets are blocked profile-wide too).
    ...
    for hd in mercury_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Mercury credential store "
    ...
    for hd in mercury_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Mercury MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Mercury MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in mercury_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Mercury real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Mercury real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Mercury read (see
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Mercury scope, or is inside
...
#
#   <HERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.mercury/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.mercury/…`` segment and warns
# regardless of which Mercury profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.mercury/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.mercury`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.mercury/…`` and returns the
    index where the inner Mercury-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .mercury / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".mercury":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Mercury state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.mercury``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.mercury`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Mercury resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Mercury process never "
...
# full ``…/sandboxes/<backend>/<task>/home/.mercury/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.mercury/…``.  The root:root
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Mercury state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Mercury "
