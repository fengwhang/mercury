...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Mercury starts and finds ``~/.openclaw/``.

    Points users at ``mercury claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``mercury claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Mercury, run "
        "`mercury claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `mercury claw cleanup` (renames it to ~/.openclaw.pre-migration — "
