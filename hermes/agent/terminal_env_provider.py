...
Providers live in ``~/.mercury/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.mercury/cache`` files land
  inside the backend (e.g. ``"~/.mercury"`` for home-synced backends,
  ``"/root/.mercury"`` for root-homed containers), or ``None`` when host
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Mercury cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``mercury setup`` after this backend is selected.
