...
Plugins built on Mercury often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Mercury
...
:class:`~mercury_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Mercury' main config
...
handles every provider, fallback chain, and per-task override Mercury
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`mercury_cli.plugins.PluginContext`
