...
Mercury pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.mercury/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<mercury_home>/bin/bws`` on
  first use.  Mercury pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.mercury/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``mercury`` invocations don't
  hammer the API.
* Failures NEVER block Mercury startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `mercury chat -q ...`
...
# <mercury_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.mercury/.env (which
...
_ENCRYPTED_CACHE_INFO = b"mercury-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under mercury_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$HERMES_HOME` / `~/.mercury` when home is None.
    """
    return _DISK_CACHE.path(home_path)


def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the encrypted disk cache path under mercury_home/cache/."""
...
def _hermes_bin_dir() -> Path:
    """Where Mercury stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<mercury_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``mercury secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="mercury-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Mercury
...
# ---------------------------------------------------------------------------
# Public entry point — called from mercury_cli.env_loader
# ---------------------------------------------------------------------------
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `mercury secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `mercury secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `mercury secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `mercury secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`mercury secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`mercury secrets bitwarden token`) so the
