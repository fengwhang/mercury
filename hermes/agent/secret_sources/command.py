...
``CommandSecretsProvider`` (mercury-desktop ``src/main/secrets/commandProvider.ts``)
...
# mercury_cli.env_loader can consume both providers identically.
...
class CommandSource(SecretSource):
    ...
    Bulk shape: the helper enumerates a KEY=VALUE blob in one run.  Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/mercury-secrets.env"
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "Mercury discards helper stderr so diagnostics can't leak "
                "secret material."
            )
