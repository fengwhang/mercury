...
Every Mercury request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Mercury and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=mercury-agent",
        "client=mercury-client-v<__version__>",
    ]

The version is sourced live from ``mercury_cli.__version__`` so it auto-aligns
...
``mercury_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current Mercury release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``mercury_cli`` cannot be imported (should
...
def mercury_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=mercury-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=mercury-client-v{_hermes_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Mercury session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Mercury conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.mercury/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Mercury
    ...
    """
    tags = ["product=mercury-agent", mercury_client_tag()]
