...
# un-throttled log fires on *every* selection. On Windows several Mercury
...
# readiness handshake ("Timed out connecting to Mercury backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``mercury_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.mercury/auth.json`` / profile equivalent), so it works for
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``mercury model`` / ``mercury auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Mercury process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``mercury`` invocation that defaults to the active provider
        (e.g. setup wizard, ``mercury auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Mercury
        ...
        # Previously "anthropic" was excluded from this tuple: two Mercury
        ...
        # "claude_code" (mercury_pkce, dashboard-issued manual entries) —
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Mercury profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Mercury process) that might
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        ``mercury_pkce`` sources the singleton file — not ``auth.json`` — is the
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # Same rationale for the singleton source mercury_pkce:
                # _seed_from_singletons() reads ~/.mercury/.anthropic_oauth.json
                ...
                # a second authority for the same refresh-token family.
                    ...
                    try:
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.mercury/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token — single-use tokens consumed by another Mercury
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                # Backstop for non-claude_code sources (mercury_pkce,
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Mercury process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # re-authed via `mercury model` / `mercury auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `mercury model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``mercury auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `mercury auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `mercury setup` ("Claude
        ...
        # Claude Code CLI, mercury_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.mercury/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `mercury setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"mercury_pkce", "claude_code"}
            ]
        ...
        for source_name, creds in (
            ("mercury_pkce", read_hermes_oauth_credentials()),
        ...
        try:
            ...
            # (mercury auth remove copilot gh_cli) must not pay either on
        ...
        # MiniMax OAuth tokens live in ~/.mercury/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `mercury auth remove minimax-oauth <N>` flow works.
        ...
        # When the user logs in via ``mercury model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``mercury auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from mercury_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.mercury/.env over os.environ — the user's config file is the
# authoritative source for Mercury credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from mercury_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.mercury/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `mercury auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Mercury
