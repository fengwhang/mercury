...
``kind: backend``) or ``~/.mercury/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`mercury_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.mercury/.env``) so credentials set
    through Mercury' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``mercury tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``mercury tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``mercury_cli/tools_config.py`` to inject this provider as a
