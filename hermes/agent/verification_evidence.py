...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("mercury-verify-", "mercury-ad-hoc-")
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "mercury verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``mercury verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``mercury verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="mercury verify",
        kind="verify",
