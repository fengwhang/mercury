...
decomposition campaign (``~/.mercury/plans/god-file-decomposition.md``, Phase 1
...
# MERCURY-OMP PATCH (B2 phase 2): ``import os`` dropped — its only consumer
# was the kanban budget-exhausted recording block (removed above).
...
def finalize_turn(
    ...
    *,
    ...
    # MERCURY-OMP PATCH (B2 phase 2): kanban budget-exhausted recording
    # removed (kanban engine deleted; delegation = omp). Its only consumer
    # was the ``iteration_limit_fallback`` kanban hook, removed with it.
    ...
    result = {
        ...
        # billing audits by callers like `mercury -z --usage-file`.
        "service_tier": (
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        result["error"] = final_response or (
            ...
            "health (`mercury doctor`), then send your message again"
        )
