"""OpenAI-shape bridge shared by Mercury' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Mercury as
...
the agent's *own* tool notifications. Mercury' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Mercury' perspective).
...
of its own (Copilot) forwards everything Mercury offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Mercury'
agent-level tools, because re-offering the overlapping ones makes Mercury re-run
...
class StreamChunks(list):
    ...
    Mercury reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``mercury_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Mercury'
