...
from mercury_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.mercury/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    result = True
    if env == "kanban":
        # MERCURY-OMP PATCH (B2 phase 2): kanban environment detection removed
        # (kanban engine deleted; delegation = omp). The tag stays recognized
        # so skills declaring ``environments: [kanban]`` keep parsing — it
        ...
        # The Mercury Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``mercury_cli.config`` on the
...
# `mercury-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Mercury itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Mercury, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"mercury-agent"})
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``mercury config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``mercury`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    parsing a non-trivial config dominates ``mercury`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    ...
    NOTE: trusted project-local dirs (``./.mercury/skills`` at the git root) are
...
#   <root>/.mercury/skills/   — Mercury-native location
...
# a one-line "run `mercury skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    ``<root>/.mercury/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``mercury skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`mercury skills trust`) is a REPO-level decision made once; the repo's
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.mercury/skills/`` (including via symlinks) or configured
    ...
    # ~/.mercury/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Mercury can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    return {
        ...
        "session_platforms": mercury.get("session_platforms", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Mercury metadata, VCS, virtualenv/dependency, cache, and skill
