...
from mercury_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from mercury_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _report_stale_nonstream_kill(
    ...
    *,
    ...
    """Emit the three user/operator-facing messages for a stale non-streaming kill.
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "mercury",
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        # Strict machine-readable mode (mercury chat -Q, oneshot, background
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Mercury-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Mercury bookkeeping,
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Mercury. Preserve only
        ...
        if agent.provider == "moa":
            # Mercury interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Mercury preserves
