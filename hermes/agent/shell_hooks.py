...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    # Modify tool input for pre_tool_call (Mercury-canonical):
...
from mercury_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# mercury_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from mercury_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _evaluate_result(
    ...
    * non-JSON / unparseable stdout on a ``fail_closed`` blocking hook —
    ...
    test helper (:func:`run_once`) so ``mercury hooks test`` reflects
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    if event == "pre_verify":
        # "continue" (Mercury) / "block" (Claude-Code Stop: block the stop) both
...
def run_once(
    ...
    holding the canonical Mercury-wire-shape response — including exit-code-2
    blocking and ``fail_closed`` semantics, so what ``mercury hooks test``
