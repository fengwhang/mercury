...
* The ``iron-proxy`` binary is auto-installed into ``<mercury_home>/bin/iron-proxy``
  on first use.  Mercury pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<mercury_home>/proxy/ca.{crt,key}`` is generated on
  first ``mercury egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<mercury_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``mercury egress start``), pidfile
  at ``<mercury_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<mercury_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``mercury egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<mercury_home>/proxy/management.token`` (0600), and injected into the
...
# credential (Mercury' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.mercury/proxy/`` just to
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<mercury_home>/bin/iron-proxy``  (our managed copy — preferred)
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``mercury proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="mercury-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "mercury-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``mercury egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="mercury-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "mercury-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="mercury-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `mercury egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `mercury egress setup` and use "
            "`mercury egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `mercury egress setup`, "
            "then `mercury egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``mercury egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `mercury egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<mercury_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Mercury dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``mercury egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``mercury egress setup`` should not invalidate the tokens
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `mercury egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `mercury egress setup` first."
        )
    ...
    # close-on-success path, every restart leaked one fd in the Mercury
    # process.
    #
    ...
    # ``mercury egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``mercury egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `mercury egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`mercury egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `mercury secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
