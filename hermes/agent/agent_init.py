...
from mercury_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.mercury/.env``. See NousResearch/mercury-agent#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'mercury memory status'. Note: "
        "systemd/gateway services do not inherit ~/.mercury/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from mercury_cli.providers import HERMES_OVERLAYS, get_provider
    ...
    try:
        ...
        from mercury_cli.models import normalize_provider as normalize_model_provider
        from mercury_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: mercury config set compression.codex_gpt55_autoraise false"
    )
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Mercury never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .mercury.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HERMES_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.mercury/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
        ...
        try:
            from mercury_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent.reasoning_config = reasoning_config  # None = use default (xhigh for OpenRouter)
    ...
    agent._cache_ttl = "5m"
    try:
        from mercury_cli.config import load_config_readonly as _load_pc_cfg
    ...
    setup_logging(mercury_home=_ra()._hermes_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that mercury_logging.setup_logging() does not
        ...
        # handler level inside mercury_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from mercury_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from mercury_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = mercury_xai_default_headers()
    ...
    # MERCURY-OMP PATCH (B2 p2): kanban worker guidance removed — the
    # kanban_show tool no longer exists in Mercury (delegation is omp).
    ...
    agent.logs_dir = mercury_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.mercury/sessions/session_{sid}.json)
    ...
    try:
        from mercury_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `mercury --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from mercury_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Mercury behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Mercury' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "mercury", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, mercury, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from mercury_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from mercury_cli.config import get_custom_provider_context_length
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Mercury.  Choose a model with at least "
    ...
    # Nous Mercury 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from mercury_cli.model_switch import _check_hermes_model_warning

            _hermes_warn = _check_hermes_model_warning(agent.model or "")
            if _hermes_warn:
                _user_msg = (
                    "⚠ Nous Research Mercury 3 & 4 models are NOT agentic — they "
