...
upstream prefix cache warm.  See ``mercury-agent-dev``'s
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- mercury-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from mercury_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from mercury_cli.plugins import format_system_prompt_sections
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Mercury never re-renders parts of this string mid-
    ...
    # MERCURY-OMP PATCH (config/ reorg): shared AGENTS.md — cross-project
    # working instructions BOTH engines read (omp via mercury-memory
    # provider). Injected before the per-side HERMES.md.
    if agent.load_soul_identity or not agent.skip_context_files:
        _agents_md = _r.load_agents_md_home(_ctx_len, home_override=_agent_home(agent))
        if _agents_md:
            stable_parts.append(_agents_md)
        # MERCURY-OMP PATCH (memory consolidation): the engine-neutral
        # project memory store for the current CWD, inlined bounded.
        try:
            _omp_mem_hint = _r.load_project_memory_hint(_ctx_len)
            if _omp_mem_hint:
                stable_parts.append(_omp_mem_hint)
        except Exception:
            pass

    # MERCURY-OMP PATCH: harness-scoped HERMES.md from the Mercury home —
    # injected ONLY into mercury' system prompt (omp never sees it; omp's
    # counterpart is OMP.md). Optional: absent file contributes nothing.
    if agent.load_soul_identity or not agent.skip_context_files:
        _hermes_md = _r.load_hermes_md_home(_ctx_len, home_override=_agent_home(agent))
        if _hermes_md:
            stable_parts.append(_hermes_md)

    if not _soul_loaded:
        # Fallback to hardcoded identity
        stable_parts.append(DEFAULT_AGENT_IDENTITY)

    # Pointer to the docs (and, when it exists, the mercury-agent skill) for
    # user questions about Mercury itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the mercury-agent skill not installed — so the
    ...
    # MERCURY-OMP PATCH (B2 p2): kanban worker guidance unreachable — removed.
    if tool_guidance:
    ...
    # the mercury-agent skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- mercury-agent:" in skills_prompt:
    ...
    # Coding posture (base Mercury, any interactive coding surface in a code
    ...
    # `mercury -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['mercury-bots']).
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Mercury profile: default. Other profiles (if any) live "
        ...
        post_workspace_parts.append(
            f"Active Mercury profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Mercury). Every other surface (desktop chat panel,
        ...
        if getattr(agent, "_context_cwd_is_launch_artifact", False):
            ...
            # then reject Mercury's bundled contributor AGENTS.md (#97448).
            context_cwd = None
    ...
    from mercury_time import get_timezone as _hermes_tz, now as _hermes_now
    now = _hermes_now()
