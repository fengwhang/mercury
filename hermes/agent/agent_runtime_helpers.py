...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Mercury-Session-Id header), and an
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from mercury_cli.config import load_config as _load_moa_cfg
    ...
    # that are canonicalized only after Mercury sends the request. Honor its
    ...
    if not _supports_anthropic_cache_markers:
        ...
        from mercury_cli.providers import custom_provider_aliases
    ...
    if _supports_anthropic_cache_markers and (
        ...
        try:
            from mercury_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to mercury's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from mercury_cli.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify Mercury to the
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from mercury_cli.config import load_config, get_compatible_custom_providers
...
def switch_model(
    ...
    try:
        ...
        from mercury_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from mercury_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from mercury_cli.plugins import _dispatch_pre_tool_call_hooks
    ...
    from mercury_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: mercury' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Mercury' keepalive client (#10324 / ``_build_keepalive_http_client``) and
