...
# (bridged to this env var in mercury_cli/main.py, gateway/run.py, and
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.mercury/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Mercury — used by
    ``mercury config``, ``mercury status``, ``mercury dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical mercury log line
