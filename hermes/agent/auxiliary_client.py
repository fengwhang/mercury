...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.mercury/auth.json active provider)
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `mercury update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Mercury owns auxiliary retry + provider/model fallback policy (the
    ...
    # Mercury sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Mercury control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="mercury-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Mercury triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from mercury_cli.models import fetch_models_with_pricing
...
# in lockstep with mercury_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``mercury_cli.__version__`` is
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Mercury-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Mercury plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``mercury_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from mercury_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from mercury_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``mercury auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``mercury_cli.auth``'s singleton runtime resolver for older
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from mercury_cli.config import load_config
        from mercury_cli.moa_config import resolve_moa_preset
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=mercury_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`mercury_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from mercury_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second mercury
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``mercury config aux reset``)."""
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``mercury_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Mercury' built-in discovery
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = mercury_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Mercury owns
...
def resolve_provider_client(
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: mercury model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: mercury doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from mercury_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(mercury_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`mercury_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Mercury will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Mercury reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Mercury reasoning config for direct model calls
