"""Coding-context awareness — base Mercury, every interactive surface.

When the user runs Mercury inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Mercury shifts into a **coding posture**. This module is the
...
from mercury_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("mercury.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "mercury", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
def is_coding_context(
    *,
    ...
    """Whether Mercury should operate in its coding posture right now."""
