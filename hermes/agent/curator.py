...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.mercury/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `mercury update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``mercury curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``mercury update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``mercury curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `mercury curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Mercury' background skill CURATOR. This is an "
    ...
    "into ~/.mercury/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: mercury-config-*, mercury-dashboard-*, gateway-*, codex-*, "
    ...
    "`mercury curator rollback` restores a hollow skill (issue #96962).\n\n"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.mercury/logs/curator/``)
    ...
    authored skill data in ``~/.mercury/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every mercury install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: mercury curator status
        keep an umbrella stable: mercury curator pin document-tools
    ...
    lines.append("full report: mercury curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: mercury curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.mercury/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.mercury/skills/.archive/` for "
            "safety and can be restored via `mercury curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.mercury/skills/.archive/`. "
            "Restore any via `mercury curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `mercury curator restore <name>`")
    lines.append("- All archives live under `~/.mercury/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``mercury curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `mercury curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `mercury update` only if "
        ...
        # recorded in state so `mercury curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``mercury model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `mercury model` → auxiliary
    ...
    try:
        ...
        review_agent = AIAgent(
            ...
            **_agent_kwargs,
            ...
            # followed snapshotted an already-stripped package and ``mercury
