...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``mercury logs``.
    ...
    if (
        ...
        # Mercury only initiates compaction in "mercury" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Mercury preflight compression for codex app-server "
                "(mode=%s); Mercury will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Mercury (via _emit_status), so it always
