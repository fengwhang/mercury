...
``mercury pause`` writes a sentinel file at ``$HERMES_HOME/ESTOP``;
``mercury resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Mercury is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.mercury/ESTOP``.
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `mercury pause`."""
...
def is_engaged() -> bool:
    ...
    (profile-local) or the fleet canonical root (~/.mercury). Fail SAFE on
...
def disengage() -> bool:
    ...
    ``mercury resume`` from a profile gateway still clears an operator pause
    written at ~/.mercury/ESTOP.
    """
    lifted = False
    for path in _candidate_sentinel_paths():
        try:
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Mercury is paused ({reason}). New work is on hold; "
            "run `mercury resume` to pick things back up."
        )
    return (
        "⏸️ Mercury is paused. New work is on hold; "
        "run `mercury resume` to pick things back up."
    )
