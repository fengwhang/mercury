...
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Mercury' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Mercury ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Mercury provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for mercury_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(mercury_id)
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Mercury provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Mercury id.
    for mercury_id in _models_dev_to_hermes_ids(provider_key):
        if mercury_id != provider_key:
            candidates.append(mercury_id)
