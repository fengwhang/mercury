...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``mercury verify`` can run.

    True when a saved ``.mercury/environment.json`` manifest exists, or when
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `mercury verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `mercury verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `mercury-verify-` filename prefix, run it "
