"""Fold an agent-as-provider's own activity back into Mercury' turn state.

Most providers are models: they ask Mercury to run a tool and Mercury runs it, so
...
time Mercury sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Mercury would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Mercury tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``mercury_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``mercury_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "mercury_projected_messages", None)
    ...
    raw_iters = getattr(response, "mercury_provider_tool_iterations", 0)
    try:
