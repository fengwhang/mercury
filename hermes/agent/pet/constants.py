...
row taxonomy is inferred from the atlas shape so Mercury can render both legacy
...
# rows (1536x1872), while older Mercury/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Mercury' activity state names. They are not always identical to the
...
# Legacy Mercury/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Mercury activity names -> accepted row-name aliases in descending
