...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in mercury_cli/config.py. read_text() with no
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `mercury auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `mercury auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``mercury auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `mercury model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
