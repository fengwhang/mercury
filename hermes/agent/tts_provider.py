...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Mercury with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.mercury/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Mercury
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``mercury tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``mercury setup`` for
