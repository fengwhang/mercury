...
``auth.json``.  They intentionally have no dependency on ``mercury_cli.auth`` so
...
# Sources Mercury owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Mercury-owned OAuth/device-code state)
