...
from mercury_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    mercury_home = get_hermes_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(mercury_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(mercury_home / rel for rel in _SENSITIVE_HERMES_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.mercury/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Mercury path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
