...
(``gateway/platforms/webhook.py``): inbound wakes Mercury when the world
changes; outbound tells the world when Mercury does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Mercury-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
Config schema (``~/.mercury/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/mercury-events
          ...
          name: ci-notify   # optional label for logs / `mercury hooks list`
...
Headers::
    ...
    User-Agent:              Mercury-Agent-Outbound-Webhook
    X-Mercury-Event:          <hook event name>
    X-Mercury-Delivery:       <delivery_id>
    X-Mercury-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Mercury-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Mercury-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
