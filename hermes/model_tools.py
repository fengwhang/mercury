...
#   - cli.py, mercury_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        # MERCURY-OMP PATCH (B2): the kanban-worker toolset auto-append is
        # removed from the Mercury distribution — the dispatcher no longer
        # spawns workers (delegation is omp; see tools/omp_delegation.py).
        # A legacy HERMES_KANBAN_TASK env can no longer re-add kanban tools.
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("mercury-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (mercury-*) include _HERMES_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("mercury-")
    ...
    # window). Core Mercury tools (toolsets._HERMES_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from mercury_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to mercury-agent's coercion layer.
