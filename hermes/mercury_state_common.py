...
Extracted verbatim from mercury_state.py so the SessionDB mixin modules
(mercury_state_search / mercury_state_schema / mercury_state_portability) can
reference them without importing mercury_state (which would be a cycle).
mercury_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``mercury sessions optimize-storage``. A legacy DB sits at
...
# `mercury sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `mercury db optimize`. This is
...
#
# Several independent Mercury processes routinely share one state.db (gateway
# service, the Desktop app's `mercury serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `mercury_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not mercury_state)
# because the search/schema mixins cannot import mercury_state (cycle).
#
...
logger = logging.getLogger("mercury_state")
