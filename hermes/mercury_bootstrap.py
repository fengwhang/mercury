"""Windows UTF-8 bootstrap for Mercury entry points.
...
should be imported at the very top of every Mercury entry point
(``mercury``, ``mercury-agent``, ``mercury-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``mercury_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``mercury_bootstrap``, never ``mercury_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Mercury modules.

    Mercury ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Mercury source root
