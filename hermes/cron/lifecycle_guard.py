...
``mercury gateway restart`` (or ``launchctl kickstart ai.mercury.gateway``
or ``systemctl restart mercury-gateway``).  When the cron fires, the
...
``mercury cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``mercury gateway``, ``launchctl ... mercury-gateway``,
``systemctl ... mercury-gateway``, ``pkill`` against the gateway) so it
...
primitive as a persistent KeepAlive job, not a one-shot task. ``mercury gateway
...
The profile-flag form (``mercury -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `mercury gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `mercury` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/mercury gateway restart-notes.md`) from
    # matching via the `/mercury` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])mercury\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops on a mercury-gateway label. macOS launchd
    # labels look like `ai.mercury.gateway` / `mercury-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated mercury services (e.g.
    # `launchctl unload ai.mercury.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.mercury.gateway-<suffix> --
...
# lines (e.g. `launchctl submit \` / `  -l ai.mercury.gateway-... \` / `  -- ...`,
...
# selector — `mercury -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `mercury` and `gateway`
...
# `mercury -p venus gateway restart` operates on a sibling profile's gateway
...
def _current_profile_name() -> Optional[str]:
    ...
    ``mercury_cli.profiles.get_active_profile_name`` (derived from
...
# Branch B only catches `launchctl <verb> ... mercury[.-]?gateway` when the
...
# command — `for item in 'ai.mercury.gateway-apollo:...' 'ai.mercury.gateway:...';
...
# is the unexpanded variable `$label`, not the string "mercury.gateway". This
...
# EXISTING job, so anchoring to the mercury-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.mercury.update-checker.plist`) to stay unblocked. The fix is checking
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``mercury``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: mercury gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `mercury -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `mercury` and
    ...
    if profile_match:
        ...
        if named:
            # Profile ids cannot contain quotes (mercury_cli.profiles
...
# for `systemctl restart mercury-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart mercury-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart mercury-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.mercury.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart mercury-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart mercury-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".mercury"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    (``~/.mercury/scripts/restart.sh``) but is passed as the bare name
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "mercury gateway restart"
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.mercury/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `mercury gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `mercury gateway restart` from a shell outside "
