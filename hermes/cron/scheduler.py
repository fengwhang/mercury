...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `mercury fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `mercury cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    (``mercury cron status`` must not show green while jobs only fire from the
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Mercury, pin it explicitly: "
                f"`mercury cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `mercury update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`mercury gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy mercury_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from mercury_cli.tools_config import enabled_mcp_server_names
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from mercury_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.mercury/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``mercury update`` / ``mercury gateway stop`` / systemd restart, or an
...
def _get_hermes_home() -> Path:
    """Resolve Mercury home dynamically while preserving test monkeypatch hooks.
...
def _get_lock_paths() -> tuple[Path, Path]:
    ...
    mercury_home = _get_hermes_home()
    lock_dir = mercury_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Mercury — {task_name}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from mercury_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``mercury [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    if mercury_bin:
        ...
        try:
            ...
            if _ilu.find_spec("mercury_cli") is not None:
                argv = [sys.executable, "-m", "mercury_cli.main"]
            else:
                return "bot-chat delivery failed: mercury CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: mercury CLI not resolvable"
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="mercury-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    try:
        from mercury_cli.profiles import normalize_profile_name, profile_exists
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `mercury cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`mercury cron run`) has no live relay adapter, so surface the
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Mercury-managed secrets are not inherited
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `mercury setup`), or pin a "
            "working provider via `mercury cron edit "
...
def _delivery_platform_routed_from_primary_gateway(platform_name: str) -> bool:
    ...
    """
    try:
        from mercury_constants import get_default_hermes_root, get_hermes_home
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            ...
            return (
                ...
                "`mercury setup` or change the job's `deliver` target."
            )
    return None
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        try:
            ...
            load_hermes_dotenv(mercury_home=_get_hermes_home())
        except Exception:
            logger.debug(
    ...
    # ---------------------------------------------------------------
    # MERCURY-OMP PATCH: omp_direct short-circuit — fire the omp engine
    # directly, skipping the mercury agent entirely (no AIAgent, no mercury
    # prompt, no tool loop). A SEPARATE job type from no_agent: omp runs an
    # LLM (the delegate model from the unified config); no_agent runs a
    # bare script with no LLM. The job prompt is passed VERBATIM as one
    # argv element to `omp --model <delegate_model> -p <prompt>` via the
    # mercury bridge (fail-hard on invalid model slots). omp stdout is
    # delivered verbatim; non-zero exit or timeout → error alert.
    # ---------------------------------------------------------------
    if job.get("omp_direct"):
        ...
        try:
            ...
            logger.debug("Job '%s': omp_direct .env reload failed", job_id, exc_info=True)

        _repo = os.environ.get("MERCURY_REPO", "").strip()
        if not _repo:
            _home_h = os.path.expanduser("~")
            for _cand in (
                os.path.join(_home_h, "Documents", "mercury-omp"),
                os.path.join(_home_h, "mercury-omp"),
            ):
                if os.path.isfile(os.path.join(_cand, "bridge", "bridge.py")):
                    _repo = _cand
                    break
        if not _repo or not os.path.isfile(os.path.join(_repo, "bridge", "bridge.py")):
            return _block_and_pause_job(
                job_id,
                job_name,
                "omp_direct: mercury repo not found (set MERCURY_REPO) — job paused.",
            )

        _omp_prompt = str(job.get("prompt") or "")
        if not _omp_prompt.strip():
            return _block_and_pause_job(
                job_id,
                job_name,
                "omp_direct: blank prompt — job paused.",
            )

        # Render delegate model via the bridge (fail-hard semantics).
        _bridge = os.path.join(_repo, "bridge", "bridge.py")
        # MERCURY-OMP PATCH (C2): refresh privilege inheritance (mercury
        # approvals.deny → omp bash.patterns deny) before every omp spawn.
        _rr = _sp.run(
            [sys.executable, _bridge, "--render-omp"],
            capture_output=True, text=True, timeout=60,
        )
        if _rr.returncode != 0:
            return False, f"# Cron Job: {job_name}\n\n**Mode:** omp_direct\n\n**FATAL (bridge render):** {_rr.stderr.strip()}\n", _rr.stderr.strip()[:400], "bridge_failed"
        _br = _sp.run(
            [sys.executable, _bridge, "--delegate"],
            capture_output=True, text=True, timeout=60,
        )
        if _br.returncode != 0:
            return False, f"# Cron Job: {job_name}\n\n**Mode:** omp_direct\n\n**FATAL (bridge):** {_br.stderr.strip()}\n", _br.stderr.strip()[:400], "bridge_failed"
        _env_model = ""
        for _line in _br.stdout.splitlines():
            if _line.startswith("OMP_MODEL="):
                _env_model = _line.split("=", 1)[1].strip()
        if not _env_model:
            return False, f"# Cron Job: {job_name}\n\n**Mode:** omp_direct\n\n**FATAL:** bridge returned no OMP_MODEL\n", "no model from bridge", "bridge_failed"

        _omp_bin = os.environ.get("HERMES_OMP_BIN", "").strip() or os.path.join(
            _repo, "omp", "packages", "coding-agent", "dist", "omp"
        )
        _omp_env = dict(os.environ)
        _omp_env.setdefault("MERCURY_CONFIG", os.path.expanduser("~/.mercury/config.yaml"))
        _omp_env.setdefault("MERCURY_HOME", os.path.expanduser("~/.mercury"))
        for _kv in _br.stdout.splitlines():
            if "=" in _kv:
        ...
        try:
            ...
            _tdoc = f"# Cron Job: {job_name}\n\n**Mode:** omp_direct (timeout)\n\nJob exceeded OMP_DIRECT_TIMEOUT and was killed.\n"
            return False, _tdoc, "omp_direct timeout", "omp_timeout"
        if _proc.returncode != 0:
            _err = (_proc.stderr or _proc.stdout or "").strip()[:1500]
            _edoc = f"# Cron Job: {job_name}\n\n**Mode:** omp_direct (exit {_proc.returncode})\n\n```\n{_err}\n```\n"
            return False, _edoc, f"omp exited {_proc.returncode}", f"omp_exit_{_proc.returncode}"
        _out = (_proc.stdout or "").strip()
        if not _out:
            return True, f"# Cron Job: {job_name}\n\n**Mode:** omp_direct\n\n(omp produced no output)\n", SILENT_MARKER, None
        _now_iso = _hermes_now().strftime("%Y-%m-%d %H:%M:%S")
        _odoc = (
            f"# Cron Job: {job_name}\n\n"
            f"**Job ID:** {job_id}\n"
            f"**Run Time:** {_now_iso}\n"
            f"**Mode:** omp_direct (model {_env_model})\n\n"
            f"---\n\n"
            f"{_out}\n"
        )
        return True, _odoc, _out, None
    ...
    try:
        ...
        #
        # A kanban worker is a normal `mercury chat -q` CLI agent whose default
        ...
        from mercury_cli.env_loader import (
            load_hermes_dotenv,
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                "default with `mercury model <name>`."
            )
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Mercury "
                        "pin it explicitly: "
                        f"`mercury cron edit {job_id} --provider <provider> "
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from mercury_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
"""
        ...
        if _session_db:
            ...
            # mercury_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`mercury cron run --prompt` /
    ...
    *,
    ...
    try:
        ...
        if delivery_error:
            ...
            # exact signature via `mercury cron incidents ack`.
...
# `mercury -w` launches, so on gateway-driven boxes (where sessions arrive via
...
def _worktree_maintenance_repos() -> List[str]:
    ...
    Candidates: the mercury install checkout itself (where ``mercury -w``
    ...
    # The mercury source checkout (editable/git installs). Wheel installs have
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    ``mercury -w`` startup path uses — dirty/unpushed/live-locked trees are
...
def tick(
    ...
    *,
    ...
    # ``git pull``, interrupted ``mercury update``) serves MIXED sys.modules:
    ...
    try:
        # Global emergency stop (`mercury pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `mercury resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `mercury cron run` that claimed a job
        ...
        # boxes never hit the `mercury -w` startup pruner, so this is the only
