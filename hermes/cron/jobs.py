...
# gateway process and the (separate) ``mercury cron status`` process share it
...
# threshold in `mercury cron status` (mercury_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``mercury`` CLI invocations, which previously shared no lock
...
_PAYLOAD_FIELDS = frozenset({"prompt", "script", "skill", "skills", "no_agent", "omp_direct"})
...
def job_payload_is_empty(job: Dict[str, Any]) -> bool:
    ...
    ``omp_direct`` jobs (MERCURY-OMP PATCH) carry their payload in the
    prompt, so a non-blank prompt already clears them above.
    """
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec mercury mercury cron create ...`` — ``docker exec``
...
def _is_named_profile_path(path: Path) -> bool:
    ...
    Named profiles live under ``<mercury_home>/profiles/<name>/``.  The
    default profile lives at ``<mercury_home>`` directly (no ``profiles``
    parent), as do custom ``HERMES_HOME`` paths outside ``~/.mercury``.
...
def _ensure_cron_dir(cron_dir: Path) -> None:
    ...
    Default and custom Mercury homes keep ``parents=True`` so first-run
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Mercury timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `mercury_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                mercury_tz = _hermes_now().tzinfo
                dt = dt.replace(tzinfo=mercury_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Mercury/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall mercury-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `mercury cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``mercury cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `mercury cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``mercury cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``mercury cron
...
def _normalize_reasoning_effort(value: Any) -> Optional[str]:
    ...
    every other effort surface uses (``mercury_constants.parse_reasoning_effort``)
...
def _validate_job_mode_invariants(
    ...
    # MERCURY-OMP PATCH: omp_direct is a SEPARATE job type from no_agent —
    # omp jobs run an LLM (the omp engine), just not the mercury agent.
    # Naming them no_agent would be misleading; they are their own type.
    if omp_direct:
        if no_agent:
            raise ValueError(
                "omp_direct=True cannot be combined with no_agent=True — "
                "omp_direct jobs run the omp engine (an LLM), no_agent jobs "
                "run a bare script with no LLM at all. Pick one."
            )
        if not str(prompt or "").strip():
            raise ValueError(
                "omp_direct=True requires a prompt — the prompt is passed "
                "verbatim to omp one-shot at tick time."
            )
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.mercury/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.mercury/scripts/, .sh/.bash via bash, else Python). Each
        ...
        reasoning_effort: Optional per-job reasoning effort pin. One of the
                canonical Mercury levels (none|minimal|low|medium|high|xhigh|
    ...
    _validate_job_mode_invariants(
        ...
        normalized_no_agent,
        normalized_script,
        omp_direct=bool(omp_direct),
        prompt=prompt_text if omp_direct else None,
    )
    ...
    # covered, not just `mercury cron create`.
...
def trigger_job(
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'mercury cron resume {name} "
                        ...
                        if times is not None and times > 0 and completed >= times:
                            ...
                            if job.get("last_run_at") is not None:
                                ...
                                logger.warning(
                                    ...
                                    "'mercury cron resume <job> --run-now' "
                                    "(#93524).",
                                    job.get("name", job.get("id", "?")),
                                    completed,
                                    times,
...
# execution. Unlike the quick-snapshot store (`mercury_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``mercury_cli.backup._prune_quick_snapshots``:
