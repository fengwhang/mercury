...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``mercury://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``mercury://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"mercury://blueprint/{quote(blueprint.key)}{qs}"
