...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.mercury/.env`` into the gateway process via
    ``mercury_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        r = subprocess.run(
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m mercury_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=mercury_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "plan":
        ...
        # .mercury/plans/ via write_file. Works on any backend.
    ...
    if name == "moa":
        ...
        try:
            from mercury_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    if profile:
        try:
            ...
            from mercury_constants import set_hermes_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``mercury journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `mercury mcp` offers, so
...
# (mercury_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
...
# All persistence reuses mercury_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from mercury_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    the SAME interactive MCP OAuth machinery ``mercury mcp login`` uses
    ...
    try:
        ...
        mercury_home = str(get_hermes_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(
            mercury_home, name, cfg, client_redirect_uri=client_redirect_uri
        )
        return _ok(
            rid,
            {
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``mercury plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.mercury/plugins/`` (non-interactive).
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Mercury plugin.
