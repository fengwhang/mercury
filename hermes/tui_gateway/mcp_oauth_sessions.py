...
The underlying token machinery is the *same* one the CLI ``mercury mcp login``
uses — ``mercury_cli.mcp_config._probe_single_server`` under
...
def _worker(session_id: str, mercury_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        home_token = set_hermes_home_override(mercury_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(mercury_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(server_name, mercury_home=mercury_home)
                    ...
                    manager.restore_entry(server_name, previous_entry, mercury_home=mercury_home)
                    raise
        finally:
...
def start_flow(
    ...
    *,
    ...
    OAuth-capable). ``mercury_home`` is the already-resolved profile home dir
    ...
    with _sessions_lock:
        ...
        if any(
            ...
            and r["mercury_home"] == mercury_home
    ...
    threading.Thread(
        target=_worker,
        args=(session_id, mercury_home, server_name, dict(cfg), reconnect_live),
        daemon=True,
