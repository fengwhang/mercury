...
# from shadowing Mercury's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``mercury_bootstrap``
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before MercuryCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``mercury serve``
...
def main():
    ...
    # Start before the (hundreds-of-ms) MercuryCLI build — that window is itself
