...
handlers delegate to the same `mercury_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Mercury profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(db, profile_path):
        ...
        describe the same session (mercury-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(db):
        ...
        sessions never surface in conversation lists (mercury-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's mercury-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``mercury setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (mercury_cli.auth: profile reads fall back to the global
    ...
    if model and provider:
        try:
            from mercury_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from mercury_cli.config import load_config_readonly, read_user_config_raw
            from mercury_cli.web_routers.profiles import _write_profile_model
            from mercury_constants import (
                reset_hermes_home_override,
                set_hermes_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `mercury tools`
            ...
            # composites (mercury-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from mercury_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `mercury tools` / Settings is where
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from mercury_cli.profiles import write_profile_meta
