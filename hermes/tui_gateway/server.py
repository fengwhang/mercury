...
from tools.environments.local import mercury_subprocess_env
...
load_hermes_dotenv(
    mercury_home=_hermes_home, project_env=Path(__file__).parent.parent / ".env"
)
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("mercury_gateway_runtime_session_record", default=None)
)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar(
    "mercury_gateway_rpc_method", default=""
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Mercury' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Mercury-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Mercury agent → needs provider credentials.
        ...
        env = build_subprocess_env(
            mercury_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
        ...
        # Prepend the Mercury venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Mercury-managed CLIs
...
def _notify_session_boundary(
    ...
    try:
        from mercury_cli.lifecycle import finalize_session, invoke_hook
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "Mercury could not safely reserve this session. Try again."
)
...
def _claim_active_session_slot(
    ...
    *,
    ...
    try:
        from mercury_cli.active_sessions import try_acquire_active_session
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    lease = session.get("active_session_lease")
    try:
        from mercury_cli.active_sessions import (
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from mercury_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from mercury_cli.lifecycle import invoke_hook
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if current.get("close_on_disconnect"):
                    ...
                    # `mercury --tui` keeps real _stdio. UNLESS another window
...
# A `mercury serve` killed mid-update used to lose every un-flushed in-memory
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    worker = threading.Thread(target=_run, daemon=True, name="mercury-exit-flush")
    worker.start()
    worker.join(budget)
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``mercury serve`` / dashboard startup before uvicorn takes over
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `mercury --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from mercury_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from mercury_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from mercury_cli.active_sessions import coerce_max_concurrent_sessions
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    thread = threading.Thread(
        ...
        args=(stop_event,),
        name="mercury-gateway-heartbeat",
        daemon=True,
    )
    thread.start()
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from mercury_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``mercury dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `mercury` shows the prompt before constructing AIAgent; the TUI used
...
def _ensure_session_db_row(session: dict) -> bool:
    ...
    * A terminal session (``mercury`` / ``mercury --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from mercury_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from mercury_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from mercury_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``mercury_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``mercury_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    mercury_cli.config.load_config, so without this a managed skin / reasoning_effort
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Mercury Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Mercury Desktop app and start a new "
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``mercury config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Mercury activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="mercury-change-watcher", daemon=True).start()
...
def _config_model_target() -> tuple[str, str]:
    ...
    (`mercury --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # HERMES_INFERENCE_MODEL, which `mercury --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from mercury_cli.models import detect_static_provider_for_model
...
from mercury_state import _BARE_BILLING_PROVIDERS
...
def _overrides_have_routable_provider(overrides: dict) -> bool:
    ...
    if not provider:
        return False
    try:
        from mercury_cli.runtime_provider import is_routable_provider
...
def _load_approval_mode() -> str:
    ...
    canonical ``mercury_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`mercury_constants.resolve_reasoning_config` (per-model override >
    global ``agent.reasoning_effort``; YAML boolean False = disabled).
    Closes #21256.
    """
    from mercury_constants import resolve_reasoning_config
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    the same backend told the model it was "chatting inside the Mercury desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base Mercury): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `mercury --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from mercury_cli.tools_config import _parse_enabled_flag
    ...
    try:
        ...
        # recovery above — which keys off mercury-cli's tool universe — can't
...
def _apply_model_switch(
    ...
    *,
    ...
    if agent:
        try:
            from mercury_cli.context_switch_guard import merge_preflight_compression_warning
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `mercury --tui -m`
...
def _compress_session_history(
    ...
    from mercury_cli.partial_compress import (
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from mercury_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from mercury_cli import projects_db as pdb
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Mercury window, and answers
...
def _render_personality_prompt(value) -> str:
    """Delegates to mercury_cli.personality (single owner of rendering)."""
    from mercury_cli.personality import render_personality_prompt

    return render_personality_prompt(value)


def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via mercury_cli.personality (single owner)."""
    from mercury_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as mercury_cli.personality.resolve_personality — (name,
    ...
    """
    from mercury_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to mercury_cli.personality (single owner).
    """
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    from mercury_cli.config import resolve_turn_limit as _resolve_turn_limit
...
def _load_fallback_model():
    ...
    stays in parity with ``MercuryCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses mercury_cli.mcp_startup; TUI stdio keeps
    # its existing tui_gateway.entry-owned thread.
    try:
        from mercury_cli.mcp_startup import wait_for_mcp_discovery
    ...
    cfg = _load_cfg()
    from mercury_cli.config import resolve_ephemeral_system_prompt_from_config
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `mercury skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
...
def _finalize_superseded_runtimes(stale: list[tuple[str, dict]]) -> None:
    ...
    mercury_state_common._RECOVERABLE_END_REASONS so canonical Bot Chat
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Mercury has to support both the legacy 8-row petdex atlas and the current
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from mercury_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    if not session_key or session.get("_finalized"):
        return []
    try:
        from mercury_cli import kanban_db as _kb
    except Exception:
        return []
    texts: list = []
    try:
...
def _plan_goal_compression_recovery(
    ...
    *,
    ...
    from mercury_cli.goals import GoalManager
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from mercury_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from mercury_cli.voice import is_continuous_active
                ...
                try:
                    ...
                    if sid_key:
                        ...
                        if goal_mgr.is_active():
                            try:
                                from mercury_cli.goals import gather_background_processes as _gather_bg
            ...
            if status == "complete":
                try:
                    from mercury_cli.loops import LoopManager
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from mercury_cli.model_switch import parse_model_switch_args
    ...
    if key == "focus":
        ...
        from mercury_cli.focus_view import (
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through mercury_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from mercury_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under HERMES_HOME (~/.mercury by
    ...
    mercury_home = os.path.realpath(str(get_hermes_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == mercury_home
        or real.startswith(mercury_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    mercury_home = os.path.normcase(os.path.realpath(str(get_hermes_home())))
    return real in _non_workspace_dirs() or real == mercury_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Mercury sessions still appear in the sidebar (#81723). Mirrors the
        ...
        try:
            for dirpath, dirnames, _filenames in os.walk(root):
                ...
                if ".git" in dirnames:
                    ...
                    # Not a repo: skip hidden dirs (e.g. .mercury) and node_modules.
...
def _discover_repos_payload(
    ...
    repos even with zero mercury sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (mercury home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Mercury sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Mercury
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return f"`{cli_invocation_name()} sessions browse` is interactive — use /resume here, or run browse in another terminal"
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Mercury" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``mercury_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via mercury_cli.voice.speak_text with spoken barge-in.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            # are tolerated.
            try:
                from mercury_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import mercury_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from mercury_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from mercury_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
