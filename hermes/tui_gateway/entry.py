...
# from shadowing Mercury's own top-level modules.  ``mercury_bootstrap`` lives at
...
# shared owner in mercury_cli.mcp_startup — the wait/in-flight/join helpers
...
# mercury_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
# ``mercury --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``mercury_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from mercury_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → mercury_cli.mcp_startup); wait on it so the first agent build still
    ...
    try:
        from mercury_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``mercury --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``mercury dashboard`` spawn theirs via
    ``mercury_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``mercury_cli.mcp_startup`` thread used by the
    ...
    try:
        from mercury_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from mercury_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    ``mercury_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Mercury activates mid-conversation.
    ...
    try:
        from mercury_cli.model_switch import prewarm_picker_cache_async
