...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/mercury-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Mercury"',
        "---",
        "",
        ...
        "",
        "Mercury also syncs bundled skills on `mercury update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.mercury/skills/` tree, it is still shipped with Mercury; restore it with `mercury skills reset <name> --restore`.",
        "",
