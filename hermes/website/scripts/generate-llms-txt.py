#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Mercury docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Mercury as a Python Library", None),
        ("guides/use-mcp-with-mercury", "Use MCP with Mercury", None),
        ("guides/use-voice-mode-with-mercury", "Use Voice Mode with Mercury", None),
        ("guides/use-soul-with-mercury", "Use SOUL.md with Mercury", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Mercury"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "mercury-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/mercury-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Mercury — Full Documentation\n",
        (
            "This file is the entire Mercury documentation concatenated for LLM "
