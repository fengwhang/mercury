...
into Mercury, archives selected unmapped docs for manual review, and
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`mercury config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.mercury/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
...
#
# Case-preserving: ``OpenClaw`` → ``Mercury`` (prose), but lowercase matches
# like ``openclaw`` → ``mercury`` (so filesystem paths like ``~/.openclaw``
# become ``~/.mercury`` — the real Mercury home — not the broken ``~/.Mercury``).
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Mercury`` but maps ``openclaw`` → ``mercury`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.mercury/config.yaml`` (the real Mercury home) instead of the broken
    ``~/.Mercury/config.yaml``.
    """
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Mercury memory store (memories/MEMORY.md, USER.md).
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-mercury/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Mercury config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-mercury/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Mercury session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            mercury_cfg_path = self.target_root / "config.yaml"
            mercury_cfg = load_yaml_file(mercury_cfg_path)
            discord_hermes = mercury_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                mercury_cfg["discord"] = discord_hermes
                dump_yaml_file(mercury_cfg_path, mercury_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        mercury_cfg_path = self.target_root / "config.yaml"
        mercury_cfg = load_yaml_file(mercury_cfg_path)
        browser_hermes = mercury_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            mercury_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(mercury_cfg_path)
                dump_yaml_file(mercury_cfg_path, mercury_cfg)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Mercury.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("HERMES_HOME") or str(Path.home() / ".mercury"), help="Mercury home directory")
    parser.add_argument(
        "--workspace-target",
