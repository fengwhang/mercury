#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `mercury kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed mercury CLI."""
    try:
        out = subprocess.run(
            ["mercury", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `mercury kanban list`
    out = subprocess.run(
        ["mercury", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["mercury", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not mercury_available():
        print("ERROR: 'mercury' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
