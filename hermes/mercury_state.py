#!/usr/bin/env python3
"""
SQLite State Store for Mercury.
...
# transitive dependency via mercury_state_common). run_agent keeps its own
# predating copy — mercury_state cannot import run_agent (circular) — guarded
...
from mercury_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, cast

from mercury_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from mercury_state_portability import SessionPortabilityMixin
from mercury_state_schema import SessionSchemaMixin
from mercury_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``mercury -c``/``--resume`` to continue the most recent session in
...
#
# The ceilings above bound Mercury's SQLite descriptors, which is only ever part
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.mercury`` even when a test fixture later
...
# ~/.mercury/state.db accumulated pytest fixture rows — sessions with
...
#: ``~/.mercury``).  The test conftest injects the pre-sandbox production
...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Mercury root for the guard.

    Deliberately avoids ``Path.home()`` / ``mercury_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``mercury_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "mercury"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "mercury"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".mercury"
        return root.resolve()
    except Exception:
        return None
...
#: is OURS: it declares "this process tree is running under Mercury test
...
def _in_test_context() -> bool:
    ...
    so a real ``mercury`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Mercury home *root*.
    ...
    under ``~/.mercury/mercury-agent/...``) so hermetic tests using unusual
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Mercury root {root}). "
...
# mercury_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``mercury sessions optimize`` (FTS merge + VACUUM)
    ...
    ``mercury_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``mercury_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``mercury update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Mercury-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Mercury"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Mercury-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `mercury doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``mercury_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see mercury_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with mercury_cli.config.
        from mercury_cli.config import cfg_get, load_config_readonly
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``mercury doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Mercury host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``mercury serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Mercury's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``mercury_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship mercury_state without mercury_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``mercury_cli.sqlite_safe_read``).
    ...
    try:
        from mercury_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Mercury home tree (``get_hermes_home()``) — the safe repair scope:
      Mercury owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``mercury_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Mercury needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (mercury_state.py:645-723). The supported degraded-runtime
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Mercury deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (mercury gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.mercury/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``mercury sessions optimize-storage`` on a capable host).
#
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship mercury_state without mercury_cli). A
    ...
    """
    try:
        from mercury_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "mercury_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch mercury_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``mercury_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    if size < 0:
        return False
    from mercury_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `mercury snapshot list` / "
                "`mercury snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (mercury doctor, dashboards) ──────────
...
_HERMES_CMDLINE_MARKERS = ("mercury_cli.main", "mercury_cli/main", "mercury serve",
                           "mercury-agent", "mercury gateway", "mercury chat")


def _looks_like_hermes(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Mercury process?
    ...
    We only flag processes that look like Mercury, not every system daemon.
    """
    lower = cmdline.lower()
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Mercury
    ...
    # `mercury update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `mercury snapshot list` / "
                        f"`mercury snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`mercury db optimize`),
            ...
            # ``mercury_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `mercury update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `mercury sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Mercury process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Mercury process —
                        # only flag uninspectable holders that look like
                        # another Mercury instance, not every system daemon.
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from mercury_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `mercury sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`mercury sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``mercury sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``mercury --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``mercury serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: mercury_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: mercury_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``mercury -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``mercury sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Mercury, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Mercury session.

        A Mercury session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Mercury session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `mercury sessions vacuum` runs in a transient
