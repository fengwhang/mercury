...
TIPS = [
    ...
    "mercury chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    ...
    "/plan writes a markdown implementation plan to .mercury/plans/ without executing anything.",
    ...
    # --- Gateway & Messaging ---
    "Mercury runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    # --- MCP ---
    "mercury mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "mercury mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "mercury mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "mercury auth add nous or mercury auth add openai-codex sets up OAuth-based providers.",
    ...
    "Profiles scope Mercury state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Mercury auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Mercury config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Mercury is working.',
    ...
    '/reload reloads ~/.mercury/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.mercury/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.mercury/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'mercury curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "mercury curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'mercury curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'mercury auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'mercury dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'mercury dashboard embeds the full Mercury TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.mercury/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.mercury/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.mercury/cache/piper-voices/.',
    ...
    'mercury chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'mercury chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'mercury chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "mercury chat --source tool tags programmatic chats so they don't clutter mercury sessions list.",
    'mercury dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'mercury sessions rename <ID> "new title" renames any past session; mercury sessions delete <ID> removes one.',
    'mercury import restores a full Mercury backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'mercury fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'mercury pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'mercury setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'mercury status --deep runs the full health sweep across every component; plain mercury status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.mercury/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Mercury block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from mercury sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.mercury/dashboard-plugins/.',
]
...
# any project or none — Mercury is not a coding-only agent.
# ---------------------------------------------------------------------------
