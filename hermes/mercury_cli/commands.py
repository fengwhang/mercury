...
from mercury_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``mercury_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``mercury_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("goal", "Set a standing goal Mercury works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .mercury/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Mercury' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Mercury is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.mercury/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Mercury to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    ...
    # mercury-omp hybrid — native verbatim routing to omp one-shot.
    # MERCURY-OMP PATCH: exists only in the mercury-omp distribution.
    CommandDef("omp", "Route task text verbatim to omp one-shot (single-model fan-out, no role routing)",
               "Delegation", args_hint="<task>", busy_policy="dispatch",
               execute="omp"),
]
...
# Telegram allows up to 100 BotCommands. Mercury ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Mercury
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``mercury skills list`` and the agent's ``/skill-name`` dispatch but
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``mercury skills list`` and
...
# unrelated command landed. These claim their slots right after /mercury,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /mercury <command>).
...
# native slot, the alias spelling stays reachable via /mercury reset).
...
# ``/mercury <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/mercury" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /mercury topup on Slack.
...
#   - version: low-frequency info command; reachable as /mercury version on
...
#   - diff: git working-tree diff; reached via /mercury diff on Slack so it
...
#     /mercury update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /mercury heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /mercury refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /mercury pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /mercury whoami on Slack.
...
#     /mercury platform on Slack. Demoted when /save became gateway-available
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/mercury <verb>`` subcommand.
    ...
    can still reach them via ``/mercury <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/mercury`` is always reserved as the first entry so the
    legacy ``/mercury <subcommand>`` form keeps working for anything that
    ...
    # Reserve /mercury as the catch-all top-level command.
    entries.append(("mercury", "Talk to Mercury or run a subcommand", "[subcommand] [args]"))
    seen.add("mercury")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_VIA_HERMES_ONLY:
            # Intentionally Slack-via-/mercury only (see _SLACK_VIA_HERMES_ONLY).
            return
    ...
    # everything except /mercury, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://mercury-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    ...
    the same as /mercury bg do stuff.
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via mercury_cli.personality."""
        try:
