"""``mercury sessions`` command — extracted from ``mercury_cli/main.py``.
...
Helpers that stay in ``mercury_cli.main`` (``get_hermes_home``,
...
``mercury_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `mercury sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`mercury sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from mercury_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    mercury sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from mercury_cli.session_recovery import (
    ...
    if action == "import":
        from mercury_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from mercury_state import workspace_key as _ws_key
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from mercury_cli.session_export_html import (
        ...
        # Markdown / QMD export
        from mercury_cli.session_export_md import (
        ...
        # `mercury sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `mercury sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `mercury sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `mercury sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: mercury sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`mercury sessions optimize` later to reclaim freed space.)")
