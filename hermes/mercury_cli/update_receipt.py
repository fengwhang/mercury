...
1. **Update receipt** — a machine-readable JSON record of what one
   ``mercury update`` run discovered, did, skipped (and why), written to
...
``mercury_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``mercury update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``mercury update`` run."""
...
def finalize_pending_update_receipt(
    ...
    ``mercury update`` has many early-termination paths (Windows
