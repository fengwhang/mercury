...
# Used by setup / `mercury model` flows to label detected credentials so
...
# several hot modules (cli.py, mercury_cli/main.py, run_agent.py,
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Mercury env-var keys.
    ...
    """
    from mercury_cli.config import _EXTRA_ENV_KEYS
    from mercury_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Mercury process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Mercury keys absent from ``.env``.
    ...
    parent Mercury process injects and that silently change *which provider
    ...
    ``tests/mercury_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source_values(
    mercury_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``mercury_home``."""
    home_key = str(Path(mercury_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(mercury_home))
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``mercury_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_hermes_dotenv(
    *,
    ...
    #   EnvironmentFile=-/path/to/.mercury/.op.env
    ...
    # 2. A fresh ``mercury update`` retry just completed a deferred dependency
    ...
    from mercury_cli import _early_recovery
    ...
    # TERMINAL_ENV=docker left in ~/.mercury/.env (e.g. written by an older
    # `mercury setup` before the user switched terminal.backend in config.yaml)
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``mercury_cli.config.apply_terminal_config_to_env`` — the
    ...
    ``load_hermes_dotenv(mercury_home=...)`` call would bridge the wrong
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Mercury reads
    ...
    time from several hot modules (cli.py, mercury_cli/main.py,
    ...
    # a generic dict entry must not force crypto load on every mercury launch.
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `mercury model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `mercury` startup, so populating the
    # shared cache here lets main.py's early bridge and mercury_logging reuse
