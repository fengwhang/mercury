...
logger = logging.getLogger("mercury.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Mercury as an unprivileged user (or in a "
...
def _container_no_volume_mount(mercury_home: Optional[Path]) -> Optional[str]:
...
def run_security_audit(
    *, mercury_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(mercury_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(mercury_home=mercury_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
