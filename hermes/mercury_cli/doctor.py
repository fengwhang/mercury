...
Diagnoses issues with Mercury setup.
"""
...
from mercury_constants import agent_browser_runnable
...
load_hermes_dotenv(mercury_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from mercury_cli.colors import Colors, color
from mercury_cli.models import _HERMES_USER_AGENT
from mercury_cli.vercel_auth import describe_vercel_auth
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Mercury containers"
...
def _hermes_database_paths(mercury_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Mercury-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from mercury_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, mercury_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((mercury_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(mercury_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``mercury_cli.sqlite_safe_read``).
    ...
    """
    from mercury_cli.sqlite_safe_read import (
        has_live_connection,
...
def _report_database_journal_modes(
    ...
    from mercury_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = mercury_home if mercury_home is not None else HERMES_HOME
    try:
        databases = _hermes_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Mercury databases: {exc}")
        return
    exposed = []
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from mercury_cli.auth import get_minimax_oauth_auth_status
...
from mercury_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'mercury sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'mercury sessions optimize-storage' offline "
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Mercury Relay plugin."""
    from mercury_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches mercury_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``mercury --version`` reporting a stale version while
    ...
    """
    try:
        from mercury_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != mercury_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'mercury update', or set "
            "mercury_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-mercury and dashboard static services are up
    ...
    for static in ("main-mercury", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `mercury profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `mercury doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    """
    _static = [
        ("Z.AI / GLM",      ("ZAI_API_KEY", "GLM_API_KEY", "Z_AI_API_KEY"), "https://api.z.ai/api/paas/v4/models", "GLM_BASE_URL", True),
    ...
    try:
        ...
        try:
            from mercury_cli.providers import normalize_provider as _normalize_provider
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.nousresearch.mercury"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `mercury update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (mercury desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `mercury desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.nousresearch.mercury` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Mercury once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Mercury.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Mercury.app``
    is deliberately not probed: it is the separately-signed Mercury-Setup
    launcher (``com.nousresearch.mercury.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Mercury.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from mercury_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Mercury will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Mercury.app if you use Desktop), "
        "and restart them once. With Mercury' stable signing identities the "
...
def run_doctor(args):
    ...
    # Handle `mercury doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from mercury_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.mercury/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`mercury doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from mercury_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Mercury already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and mercury_cli/__init__.py versions
    ...
    if env_path.exists():
        ...
        # matches mercury_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    if config_path.exists():
        ...
        try:
            ...
            if isinstance(user_providers, dict):
                from mercury_cli.config import is_provider_enabled
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from mercury_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        try:
            from mercury_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `mercury setup` run)",
                )
                if should_fix:
        ...
        try:
            from mercury_cli.config import load_env as _load_env_depr
            from mercury_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Mercury' own device-code flow — the Codex CLI is
    ...
    if state_db_path.exists():
        try:
            ...
            from mercury_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'mercury doctor --fix' "
                        "(or 'mercury sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from mercury_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'mercury doctor --fix' "
                        "(or 'mercury sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Mercury
        ...
        try:
            from mercury_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'mercury sessions optimize-storage' "
    ...
    try:
        from mercury_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'mercury-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'mercury-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `mercury update` does, so a session's first browser call
            ...
            # `mercury update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `mercury doctor` invocation.
    ...
    try:
        ...
        if _using_lightpanda_engine():
            ...
            if not _lp_used:
                ...
                check_info(
                    "Fix: pick Lightpanda in `mercury tools` → Browser Automation, "
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Mercury one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'mercury config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``mercury doctor``.
    ...
    # never the legitimate source for `mercury doctor`.
    ...
    try:
        from mercury_cli.config import read_user_config_raw as _read_raw_mem
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                _fail_and_issue(
                    ...
                    "run: mercury memory setup",
                    "No Honcho API key — run 'mercury memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run mercury memory setup)",
    ...
    # `mercury doctor --live` (real network calls; bounded + read-only).
    try:
        from mercury_cli.doctor_live import maybe_run_live_checks
