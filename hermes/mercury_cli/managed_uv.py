"""Mercury-managed uv and Python runtime repair.

Mercury owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
The Python backing the install is different: it is shared by every Mercury
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.mercury-runtime/python``. A
...
from mercury_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".mercury-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nousresearch.mercury.managed-python"
...
def managed_uv_path() -> Path:
    """Return the path where Mercury keeps *its* uv binary.
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Mercury-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Mercury provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Mercury keeps databases "
            "out of WAL mode on this SQLite build. The next `mercury update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``mercury update`` runs
    the call site from the *old*, already-imported ``mercury_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``mercury update`` daily;
...
def update_managed_uv(
    *,
    ...
    Call this during ``mercury update`` so the managed copy stays current.
...
def _reload_mercury_constants():
    """Re-execute ``mercury_constants`` from disk and return the fresh module.

    ``mercury update`` imports ``mercury_constants`` from the OLD checkout,
    ...
    return importlib.reload(mercury_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Mercury generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `mercury update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("mercury_cli.main")
    ...
    if holders:
        ...
        return True, f"other Mercury processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Mercury-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``mercury update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``mercury update`` repairs the runtime (issue class:
