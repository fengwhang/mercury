"""MCP picker — interactive `mercury mcp picker` (also the default `mercury mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``mercury mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `mercury plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from mercury_cli.cli_output import prompt_yes_no
from mercury_cli.curses_ui import curses_single_select
from mercury_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Mercury session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from mercury_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `mercury mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: mercury mcp install <name>    Picker: mercury mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Mercury is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Mercury — run `mercury update` "
...
def show_catalog() -> None:
    """`mercury mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`mercury mcp picker` (and default `mercury mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`mercury mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `mercury mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
