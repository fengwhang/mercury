...
Usage:
    ...
    mercury claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    mercury claw migrate --no-backup  # Skip pre-migration snapshot
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'mercury gateway stop'.")
    print()
...
def _cmd_migrate(args):
    ...
    # migrate-mercury posture (two-phase: run once without secrets, rerun with
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.mercury/. "
    ...
    # ── Phase 2b: Pre-apply backup of the Mercury home ─────────
    # Delegates to mercury_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `mercury import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from mercury_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(mercury_home=mercury_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: mercury import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Mercury home."
            )
