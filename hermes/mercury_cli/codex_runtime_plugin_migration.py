"""Migrate Mercury' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Mercury' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Mercury mcp_servers.<n>.command/args/env  → codex stdio transport
  Mercury mcp_servers.<n>.url/headers       → codex streamable_http transport
  Mercury mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Mercury mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Mercury-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Mercury' own
...
MIGRATION_MARKER = (
    "# managed by mercury-agent — `mercury codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end mercury-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Mercury config.")
...
# Mercury keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Mercury' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, mercury_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Mercury MCP server config to the codex inline-table dict
    ...
    if not isinstance(mercury_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(mercury_cfg.get("command"))
    has_url = bool(mercury_cfg.get("url"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Mercury)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Mercury' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Mercury' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Mercury
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="mercury-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    codex-routed mercury-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.mercury`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Mercury' own
    ...
    so a mercury installed under /opt/, /usr/local/, or a venv all work.
    ...
    mercury_home = os.environ.get("HERMES_HOME") or ""
    if mercury_home and _looks_like_test_tempdir(mercury_home):
        mercury_home = ""
    if mercury_home:
        env["HERMES_HOME"] = mercury_home
    # PYTHONPATH passes through so a worktree-launched mercury finds the
...
def migrate(
    ...
    *,
    ...
    """Translate Mercury mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        mercury_config: full ~/.mercury/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Mercury' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Mercury for tools
    ...
    mercury_servers = (mercury_config or {}).get("mcp_servers") or {}
    if not isinstance(mercury_servers, dict):
        report.errors.append(
            "mcp_servers in Mercury config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in mercury_servers.items():
    ...
    # Inject Mercury' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Mercury for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/mercury_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_hermes_tools:
        translated["mercury-tools"] = _build_hermes_tools_mcp_entry()
        if "mercury-tools" not in report.migrated:
            report.migrated.append("mercury-tools")
