"""``mercury project`` CLI — manage first-class, multi-folder Projects.
...
(see :mod:`mercury_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Mercury'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``mercury project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: mercury project <action> [options]\n"
                "Run 'mercury project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `mercury project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `mercury project add-folder`.",
            file=sys.stderr,
        )
        return 1
...
# MERCURY-OMP PATCH (B2 phase 2): _sync_board_default_workdir() removed — it
# anchored a bound kanban board's default_workdir at the project repo; the
# kanban engine is deleted (delegation = omp) and no writer remains.
