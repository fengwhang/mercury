...
from mercury_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
from mercury_cli.install_identity import get_install_id as _shared_get_install_id
...
try:
    ...
    # running `mercury dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``mercury gateway run`` — but the
    desktop app spawns a ``mercury dashboard`` backend, not a gateway, so a cron
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from mercury_cli.profiles import profiles_to_serve

            profile_homes = list(profiles_to_serve(multiplex=True))
            if len(profile_homes) > 1:
                start_kwargs["profile_homes"] = profile_homes
                from mercury_logging import enable_profile_log_routing
...
def _warm_gateway_module() -> None:
    ...
    ``mercury_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "mercury_cli.auth",
        "mercury_cli.copilot_auth",
        "mercury_cli.runtime_provider",
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `mercury update` current before the
    ...
    """
    try:
        from mercury_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `mercury update` kept 500ing every
    ...
    # Import mercury_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `mercury update` replaced the code
    ...
    # since the app has no gateway running the scheduler. Server `mercury
    ...
    if os.getenv("HERMES_DESKTOP") == "1":
        ...
        # a healthy standalone gateway (launched via `mercury gateway run`,
        ...
        try:
            from mercury_cli.gateway import _reap_unsupervised_gateway_orphans
...
app = FastAPI(title="Mercury", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from mercury_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".mercury-ssh-runtime-{nonce}")
...
#
# This list is defined in ``mercury_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from mercury_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Mercury-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Mercury' canonical browser-facing URL behind a
    ...
    """
    from mercury_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``mercury-0day``
...
def _desktop_loopback_auth_exempt(
    ...
    for EVERY ``mercury serve`` on the machine — including the private loopback
    ...
    ``mercury serve`` with ``HERMES_DESKTOP=1`` exported but no credential is
    NOT exempt.
    """
...
async def _dashboard_auth_gate(request: Request, call_next):
    from mercury_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `mercury egress setup` "
            "and `mercury egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Mercury (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during mercury update. "
...
from mercury_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Mercury provider slug — fine.
    ...
    1. Vendor-name → Mercury-provider mapping: when the provider string is
       not a known Mercury provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    from mercury_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from mercury_cli.model_normalize import normalize_model_for_provider
    from mercury_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Mercury
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from mercury_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    Mercury process is using it.
    """
    from mercury_state import _default_db_path
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Mercury credential-store basenames (see
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``mercury update``.
    ...
    a container — e.g. the mercury-webui image sharing the Mercury source tree),
    the dashboard's ``mercury update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Mercury root is actually /opt/data
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Mercury inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``mercury:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.mercury-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``mercury_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from mercury_cli import web_git as _web_git  # noqa: E402
...
from mercury_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from mercury_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from mercury_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Mercury home
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors mercury_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # Windows install the first import of mercury_cli.gateway blocks the
        ...
        # and which providers are registered so ``mercury status`` and the
        ...
        auth_flows: list[str] = []
        try:
            from mercury_cli.dashboard_auth import (
        ...
        try:
            from mercury_cli.auth import get_nous_session_validity
        ...
        try:
            from mercury_state import SessionDB as _SDB
            from mercury_constants import get_hermes_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Mercury Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the mercury home root).
    """
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from mercury_cli.auth import get_nous_auth_status_local
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(
    r"^=== mercury-update completed ([0-9a-f]{32}) ===$"
)
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('mercury_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_hermes_action(
    ...
    *,
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "mercury_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _HERMES_GATEWAY=1. Inheriting it makes a spawned `mercury gateway restart`
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== mercury update started "):
            last_start = index
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["mercury", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``mercury gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``mercury gateway restart`` children race each other on the
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Mercury updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("mercury-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "mercury-update",
    ...
    if refusal is not None:
        _record_completed_action("mercury-update", refusal.message, exit_code=1)
    ...
    existing = _ACTION_PROCS.get("mercury-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("mercury-update")
        if action_id:
...
@app.get("/api/mercury/update/check")
async def check_hermes_update(force: bool = False):
    """Report whether a Mercury update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/mercury/update`` actually runs ``mercury update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Mercury version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "Mercury is managed by Termux APT; run `pkg upgrade mercury-agent`."
        )
        return payload
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.mercury/config.yaml`` under ``tts.``.
    """
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "mercury-update":
        ...
        if durable_update_action_id:
            marker = f"=== mercury-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``mercury
    ...
    """
    try:
        from mercury_cli.update_receipt import read_latest_receipt
...
@app.get("/api/mercury/update/receipt")
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `mercury update` run recorded).",
        )
...
from mercury_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from mercury_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from mercury_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from mercury_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/mercury is sealed read-only, and installs must be redirected
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        mercury_home / f"{name}.json",
        mercury_home / name / "config.json",
    ):
...
async def get_egress_status():
    ...
    from mercury_cli.proxy_cli import format_status_text
...
# in mercury_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    boot.  When ``mercury update`` (or a manual ``git pull``) replaces the
    ...
    return (
        ...
        f"(systemctl --user restart mercury-dashboard, or mercury dashboard --port <port>)"
    )
...
async def get_model_options(
    ...
    try:
        ...
        from mercury_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `mercury model` does so GUI onboarding lands on a
    ...
    try:
        from mercury_cli.inventory import build_models_payload, load_picker_context
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from mercury_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from mercury_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.mercury/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from mercury_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (mercury_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from mercury_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``mercury model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from mercury_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``mercury model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from mercury_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from mercury_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `mercury model` provider — would be
        ...
        # Vertex — a `mercury model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `mercury model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from mercury_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from mercury_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Mercury from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Mercury to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Mercury from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Mercury to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Mercury in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Mercury through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Mercury via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Mercury through an IMAP/SMTP mailbox.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Mercury to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Mercury inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Mercury to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Mercury through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Mercury to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Mercury to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Mercury.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Mercury from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Mercury over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Mercury through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Mercury over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Mercury to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Mercury as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Mercury via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Mercury Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in mercury gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in mercury gateway setup",
...
# mercury_cli/setup_hidden_env.py. Shared with the `mercury setup gateway`
# wizard so the surfaces ask for the same things.
from mercury_cli.setup_hidden_env import (  # noqa: E402
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.mercury-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Mercury").strip() or "Mercury"
...
# the canonical ``mercury auth add <provider>`` command so the dashboard can
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.mercury/.anthropic_oauth.json`` — Mercury-managed terminal PKCE
    ...
    if read_hermes_oauth_credentials:
        try:
            mercury_creds = read_hermes_oauth_credentials()
        except Exception:
            mercury_creds = None
    if mercury_creds and mercury_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "mercury_pkce",
            "source_label": f"Mercury PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Mercury never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "mercury auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "mercury auth add xai-oauth",
        "docs_url": "https://mercury-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # server). Mercury previously reimplemented that subscription-OAuth PKCE
    ...
    # credentials. Login still works via the terminal (`mercury auth add
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Mercury, so the disconnect
    ...
    user sees exactly what executes, and Mercury then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, Mercury still OWNS
    # the credential it can show here: the Mercury-managed PKCE file
    # (~/.mercury/.anthropic_oauth.json) and its credential-pool entry, both
    # written by `mercury auth add anthropic` in the terminal. Those are ours
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Mercury — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``mercury model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `mercury model` universe (zero-edit for new plugins).
    try:
        from mercury_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"mercury auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `mercury model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Mercury-managed PKCE file and auth-store entry.
            ...
            try:
                from mercury_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
# points at `mercury auth add anthropic` (terminal PKCE, unaffected) instead.
#
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from mercury_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``mercury auth add minimax-oauth``.
    """
    from mercury_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `mercury auth add xai-oauth`: first credential may become
            ...
            # prior ``mercury auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``mercury model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Mercury "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from mercury_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `mercury update` until the first message forced a writable open.
    """
    from mercury_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from mercury_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``mercury serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `mercury sessions prune`)."""
    has_window = (
...
def _cron_default_profile() -> str:
    ...
    fallback would write a non-default profile's job into ``~/.mercury``.
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["mercury_home"] = str(home)
...
from mercury_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from mercury_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/mercury_cli/
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``mercury gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (mercury_cli.mcp_config), so
# servers managed here show up under `mercury mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    from mercury_cli.mcp_security import validate_mcp_server_entry
...
from mercury_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from mercury_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from mercury_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.mercury_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_hermes_home_override(flow.mercury_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.mercury_home)))
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        mercury_home=flow.mercury_home,
                    )
...
#
# Wraps the same JSON store the CLI uses (mercury_cli.webhook); the webhook
...
async def list_webhooks():
    import mercury_cli.webhook as wh
...
# spawn the real `mercury gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from mercury_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `mercury auth remove`) can seed again.
        # Mirrors the `mercury auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``mercury auth remove`` uses: each source cleans up its
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f"mercury-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def list_hooks():
    ...
    """
    def _run():
        from mercury_cli.config import load_config as _load_config
...
async def list_checkpoints():
    ...
    # Checkpoints live under <mercury_home>/checkpoints/.  Surface a count +
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``mercury`` subprocess, and
...
from mercury_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from mercury_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _profile_setup_command(name: str) -> str:
    ...
    return "mercury setup" if name == "default" else f"{name} setup"
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from mercury_constants import set_hermes_home_override, reset_hermes_home_override
    from mercury_cli.mcp_config import _save_bearer_auth_token
...
from mercury_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from mercury_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Mercury,
...
if sys.platform.startswith("win"):
    try:
        from mercury_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
...
from mercury_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
_GATEWAY_WS_PROTOCOL = "mercury-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "mercury-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from mercury_cli.dashboard_auth.audit import AuditEvent, audit_log
        from mercury_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    — the same propagation ``mercury -p <name>`` performs. The in-process
    ...
    """
    from mercury_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Mercury TUI child: build via the single spawn-env factory (profile-home
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from mercury_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="mercury-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Mercury Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Mercury CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="mercury-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from mercury_cli.console_engine import HermesConsoleEngine
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Mercury Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Mercury inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`mercury_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from mercury_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Mercury Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="mercury-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/mercury/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /mercury`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HERMES_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `mercury serve` is the headless backend: it must NEVER serve the browser
    ...
    if _headless:
        _msg = (
            "Headless backend (mercury serve): web UI disabled — use "
            "`mercury dashboard` for the browser UI."
        )
        ...
        async def no_frontend(full_path: str):
            ...
            # `mercury update` replaced the backend) had no way to adopt the
    ...
    # (#82614): a long-lived `mercury dashboard --skip-build` process that
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/mercury``)
        ...
        # Without this the SPA paints the default Mercury Teal canvas, then
    ...
    # under ``/mercury`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Mercury backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "default-large", "label": "Mercury Teal (Large)", "description": "Mercury Teal with bigger fonts and roomier spacing"},
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.mercury/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from mercury_cli.plugins import get_bundled_plugins_dir
    ...
    # installed in the mercury root (``~/.mercury/plugins``). Scan the default
    ...
    # ``mercury_cli.plugins`` resolves plugin install locations. The
    ...
    # ``mercury_cli/plugins.py`` and the documented user contract.
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from mercury_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"mercury auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from mercury_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from mercury_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from mercury_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.mercury/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.mercury/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"mercury_dashboard_plugin_{plugin['name']}"
...
from mercury_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _is_serve_orphaned(
    ...
    *,
    ...
    Python process's immediate PPID. On Windows the venv ``mercury.exe`` launcher
...
# spawn (and any script wrapping ``mercury serve``) cannot tell "port occupied"
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'mercury serve' / 'mercury dashboard' backend or the Mercury gateway. "
...
def start_server(
    ...
    try:
        from mercury_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the mercury-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Mercury exposes a
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from mercury_cli.dashboard_auth.prefix import (
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `mercury dashboard register` (Nous Portal) or "
            ...
            # (#54489).
            try:
                from mercury_cli.config import load_config as _load_cfg
                from mercury_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`mercury plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("HERMES_DESKTOP") == "1":
                try:
                    from mercury_cli.dashboard_procs import (
            ...
            try:
                from mercury_cli.process_identity import reap_orphaned_mcp_helpers
            ...
            # for standalone `mercury serve` (no HERMES_PARENT_PID env).
            ...
            # is what lets `mercury update` relaunch a manually-started serve
            # on its real endpoint instead of dropping it (#63206).
            try:
                from mercury_cli.process_identity import (
            ...
            if headless:
                ...
                print(f"  Mercury backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Mercury Web UI → http://{host}:{actual_port}")
