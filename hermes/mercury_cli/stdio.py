...
on Japanese locales, etc.).  Mercury's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``mercury_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Mercury is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Mercury-managed Git install directories so
    ...
    # already-running shells don't see the change — which means mercury
    ...
    # install without Mercury-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Mercury's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Mercury's
    ``mercury config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Mercury (User env var in Windows Settings, or
    export in a PowerShell profile) and Mercury picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Mercury-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\mercury\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``mercury`` from right after install) retain their old PATH.

    Any subprocess Mercury spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Mercury-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "mercury", "git", "cmd"),
        os.path.join(local_appdata, "mercury", "git", "bin"),
        os.path.join(local_appdata, "mercury", "git", "usr", "bin"),
        # Mercury venv Scripts directory — host of the mercury.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes mercury, but harmless to include.
        os.path.join(local_appdata, "mercury", "mercury-agent", "venv", "Scripts"),
