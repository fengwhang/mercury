...
_GOLD = "\033[1;38;2;255;99;71m"  # True-color tomato bold (orangered family)
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `mercury ... > file`) raises
...
HERMES_AGENT_LOGO = ""  # HERMES-OMP PATCH: ASCII art removed

HERMES_CADUCEUS = ""  # HERMES-OMP PATCH: ASCII art removed
...
# (e.g. nix-built mercury — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/mercury-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/mercury-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/nousresearch/mercury-agent/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "mercury-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `mercury update`,
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$HERMES_HOME/mercury-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``mercury_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from mercury_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/mercury-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/mercury-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Mercury v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from mercury_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built mercury; we know an update
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # HERMES-OMP PATCH: ASCII art removed — hero only when a skin ships one
    try:
        from mercury_cli.skin_engine import get_active_skin
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or mercury setup[/]"
            )
        else:
    ...
    if not _has_native_mcp:
        try:
            from mercury_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from mercury_cli.codex_runtime_switch import get_current_runtime
        from mercury_cli.config import load_config as _load_cfg
