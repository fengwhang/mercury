"""On-demand supply-chain audit for Mercury installs.

Scans three surfaces a Mercury user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Mercury venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.mercury/plugins``
...
def _discover_plugins(mercury_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.mercury/plugins``.
...
def _discover_components(
    *,
    ...
    home = mercury_home or Path(get_hermes_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `mercury security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, mercury_home=home
    )
