"""
Interactive setup wizard for Mercury.
...
from mercury_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
...
# display_hermes_home imported lazily at call sites (stale-module safety during mercury update)
...
from mercury_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🌡️ Mercury Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "mercury_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``mercury_cli/web_server.py``). In that
...
def _print_setup_summary(config: dict, mercury_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Mercury cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    mercury model            (pick any provider/model)")
        print_info("    mercury setup --portal   (Nous Portal OAuth, no API key)")
    ...
    # Video generation — opt-in via `mercury tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from mercury_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'mercury tools' → Speech-to-Text")
            )
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # on demand via `mercury auth add`, `mercury setup` vision, and
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``mercury model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Mercury auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via mercury setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_terminal_backend(config: dict):
    ...
    # under ~/.mercury/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `mercury setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `mercury setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `mercury egress setup` then `mercury egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `mercury egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'mercury-agent[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Mercury installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'mercury-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.mercury/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from mercury_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def setup_gateway(config: dict):
    ...
    # via `mercury import` or `mercury setup gateway`). Gating this on
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Mercury profile and are not uploaded.")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        from mercury_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``mercury setup --portal`` and ``mercury portal``. This is the
    ...
    Mercury session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``mercury setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``mercury model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``mercury portal`` always offers a model picker,
    ...
    print(color("│    🌡️ Mercury Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `mercury portal` == quick setup's Nous step.
    try:
        from mercury_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `mercury portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `mercury portal`: %s", exc)
        print()
    ...
    print_info("  Run `mercury portal info` to inspect routing.")
    print_info("  Run `mercury` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from mercury_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``mercury model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│    🌡️ Mercury Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│               🌡️ Mercury Setup Wizard                   │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Mercury installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, mercury_home))]
            )
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'mercury setup model|terminal|")
                ...
                [
                    (
                        ...
                        lambda: _run_first_time_quick_setup(
                            config, mercury_home, is_existing
                        ),
                    )
                ]
            )
    ...
    # Tune later with `mercury setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from mercury_cli.gateway import ensure_gateway_service
...
def _run_first_time_quick_setup(config: dict, mercury_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``mercury setup <section>``
    or switch providers with ``mercury model``.
    """
    from mercury_cli.config import load_config
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `mercury import` from another machine).
        from mercury_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Mercury.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Mercury update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``mercury-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Mercury) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``mercury tools`` (which
    ...
    try:
        ...
        from mercury_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``mercury setup agent`` /
    ``mercury config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, mercury_home, is_existing: bool):
    ...
    print_info("essential `mercury-agent` skill is always kept so the agent")
    print_info("can help you drive and configure Mercury itself.")
    print()
    ...
    if path == 0:
        ...
        # `mercury update` runs don't re-inject them. Essential skills (the
        # `mercury-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        mercury tools")
        print_info("  Seed skills:         mercury skills opt-in --sync")
        print_info("  Add MCP servers:     mercury mcp add")
        print_info("  Enable plugins:      mercury plugins")
        print_info("  Tune agent settings: mercury setup agent")
        print()
        _print_setup_summary(config, mercury_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, mercury_home)


def _blank_slate_walkthrough(config: dict, mercury_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `mercury-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `mercury-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`mercury update` runs from re-injecting them. Opt back in any")
            print_info("time with `mercury skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `mercury tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `mercury plugins list` / `mercury plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `mercury plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `mercury mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `mercury mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   mercury tools")
    print_info("  Seed skills:         mercury skills opt-in --sync")
    print_info("  Add MCP servers:     mercury mcp add")
    print_info("  Tune agent settings: mercury setup agent")
    print()
