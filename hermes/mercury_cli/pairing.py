...
Usage:
    ...
    mercury pairing approve <platform> <request-id|code>  # Approve a pairing request
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: mercury pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.mercury/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'mercury pairing list' to see pending requests.\n")
