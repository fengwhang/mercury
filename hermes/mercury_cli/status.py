...
Shows the status of all Mercury components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``mercury config``'s
...
def _estop_status_line():
    """One-line pause banner for `mercury status`, or None when not paused.
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `mercury resume` to lift)"


def show_status(args):
    """Show status of all Mercury components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                   🌡️ Mercury Status                     │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `mercury status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: mercury portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: mercury auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: mercury auth add xai-oauth)'}"
    )
    ...
    if _effective_provider_label() == "LM Studio":
        from mercury_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'mercury-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `mercury status`).
        try:
