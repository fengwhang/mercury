...
``CommandDef.execute`` (mercury_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/mercury_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``mercury_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: mercury bundles create <name> --skill <s1> --skill <s2>\n"
...
# MERCURY-OMP PATCH: native /omp — verbatim routing to omp one-shot.
# Late registration (bottom of module): omp_exec imports CommandContext/
# CommandReply from THIS module, so it cannot be imported at the top; at this
# point the module namespace is fully initialized and the cycle is safe.
def _register_omp_executor() -> None:
    from mercury_cli.omp_exec import _exec_omp

    EXECUTORS["omp"] = _exec_omp


_register_omp_executor()
