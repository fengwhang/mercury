...
Three layers that make every long-lived Mercury process positively
identifiable, so reapers (``mercury update``, Desktop startup sweeps) never
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Mercury root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Mercury install on the
    same machine without path comparisons at scan time.
    """
    if project_root is None:
        try:
            from mercury_constants import PROJECT_ROOT as _root
...
def register_self(
    ...
    *,
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `mercury serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import Mercury code). The entry records the
