    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Mercury" comment and following line
                if '# Mercury' in line or '# mercury-agent' in line:
                    skip_next = True
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Mercury-owned file, and
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        ...
        Path.home() / ".local" / "bin" / "mercury-acp",
        Path.home() / ".local" / "bin" / "mercury-agent",
        Path("/usr/local/bin/mercury"),
        Path("/usr/local/bin/mercury-acp"),
        Path("/usr/local/bin/mercury-agent"),
    ]
...
def remove_node_symlinks(mercury_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``mercury`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Mercury home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (mercury_home / "node").resolve()
    removed = []
...
#      like ``%LOCALAPPDATA%\mercury\git\cmd``, ``%LOCALAPPDATA%\mercury\git\bin``,
#      ``%LOCALAPPDATA%\mercury\git\usr\bin``, ``%LOCALAPPDATA%\mercury\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\mercury\git\`` and Node to
#      ``%LOCALAPPDATA%\mercury\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``mercury dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\mercury\gateway-service\`` and sometimes
...
def _hermes_path_markers(mercury_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Mercury-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the mercury launchers and the managed uv) — only wanted when
    ...
    """
    root = str(mercury_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare mercury-agent install dir.
    markers = [root + "\\mercury-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if HERMES_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "mercury".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-mercury as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(mercury_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Mercury-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<mercury_home>\\bin`` (the managed binary
    dir holding the mercury launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(mercury_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``mercury`` launchers install.ps1 staged in the managed
    binary dir (the default Mercury root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``mercury`` in a new terminal resolves to a launcher whose target is
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from mercury_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
...
def _is_default_hermes_home(mercury_home: Path) -> bool:
    """Return True when ``mercury_home`` points at the default (non-profile) root."""
    try:
        from mercury_constants import get_default_hermes_root
        return mercury_home.resolve() == get_default_hermes_root().resolve()
    except Exception:
        return False
...
def run_gui_uninstall(args):
    ...
    Mirrors ``mercury uninstall --gui``. Removes the desktop app's built
    ...
    """
    from mercury_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(mercury_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│        🌡️ Mercury Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Mercury Chat GUI installation was found.")
        print(f"  Checked: {mercury_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Mercury agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(mercury_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Mercury agent at {mercury_home / 'mercury-agent'}")
        print(f"  • Your config, sessions, and secrets under {mercury_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Mercury agent is still installed. Run 'mercury' to use the CLI,")
    print("or 'mercury uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    is_default_profile = _is_default_hermes_home(mercury_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.mercury data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│              🌡️ Mercury Uninstaller                     │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
...
def _print_uninstall_dry_run(*, project_root: Path, mercury_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        print(f"  • Mercury config/data: {mercury_home}")
        if _is_default_hermes_home(mercury_home):
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in mercury_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\mercury\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (mercury\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_hermes_home(mercury_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(mercury_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Mercury-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Mercury-set User env vars to remove")
    ...
    #     would dangle — `mercury` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows mercury launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows mercury launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Mercury home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Mercury-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(mercury_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Mercury-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(mercury_home) would sweep the in-tree artifacts anyway,
    ...
    try:
        from mercury_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(mercury_home)
        if not gui_removed:
    ...
    #     the step-5 rmtree(mercury_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(mercury_home)
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://mercury-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://mercury-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Mercury! 🌡️")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m mercury_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import mercury_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``mercury_constants`` + ``mercury_cli.colors``
    (and lazily ``mercury_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m mercury_cli.uninstall")
    parser.add_argument(
        "--mode",
