...
Usage:
    ...
    mercury --version           Show version and update status
...
# IMPORTANT: mercury_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``mercury_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``mercury update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``mercury_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, mercury crashes on import and the user can't run
# ``mercury update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import mercury_bootstrap  # noqa: F401
...
from mercury_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python mercury_cli/main.py`` (script
# mode — sys.path[0] is mercury_cli/, not the repo root) can import the
# canonical helpers; everything else lives in mercury_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from mercury_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``mercury update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from mercury_cli.env_loader import ...`` / ``from mercury_cli.config import
...
# package directory, so if IT can't import, nothing else in mercury_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from mercury_cli import _early_recovery as _early_recovery_mod
...
def _set_process_title() -> None:
    """Set the process title to 'mercury' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``mercury tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``mercury.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("mercury")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"mercury", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"mercury")
        # Windows: the .exe name is already ``mercury.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* mercury_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".mercury", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    cron jobs, pipes run ``mercury … chat -q`` with stdio on a pipe. This
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`mercury --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(mercury_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(mercury_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``mercury --version`` before config/logging imports."""
...
from mercury_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from mercury_cli.subcommands.cron import build_cron_parser
from mercury_cli.subcommands.sync import build_sync_parser
from mercury_cli.subcommands.gateway import build_gateway_parser
from mercury_cli.subcommands.profile import build_profile_parser
from mercury_cli.subcommands.model import build_model_parser
from mercury_cli.subcommands.setup import build_setup_parser

from mercury_cli.subcommands.whatsapp import build_whatsapp_parser
from mercury_cli.subcommands.slack import build_slack_parser
from mercury_cli.subcommands.login import build_login_parser
from mercury_cli.subcommands.logout import build_logout_parser
from mercury_cli.subcommands.auth import build_auth_parser
from mercury_cli.subcommands.status import build_status_parser
from mercury_cli.subcommands.pause import build_pause_parser
from mercury_cli.subcommands.webhook import build_webhook_parser
from mercury_cli.subcommands.hooks import build_hooks_parser
from mercury_cli.subcommands.doctor import build_doctor_parser
from mercury_cli.subcommands.verify import build_verify_parser
from mercury_cli.subcommands.security import build_security_parser
from mercury_cli.subcommands.approvals import build_approvals_parser
from mercury_cli.subcommands.dump import build_dump_parser
from mercury_cli.subcommands.debug import build_debug_parser
from mercury_cli.subcommands.backup import build_backup_parser
from mercury_cli.subcommands.import_cmd import build_import_cmd_parser
from mercury_cli.subcommands.import_agent import build_import_agent_parser
from mercury_cli.subcommands.config import build_config_parser
from mercury_cli.subcommands.skin import build_skin_parser
from mercury_cli.subcommands.console import build_console_parser
from mercury_cli.subcommands.update import build_update_parser
from mercury_cli.subcommands.uninstall import build_uninstall_parser
from mercury_cli.subcommands.dashboard import build_dashboard_parser
from mercury_cli.subcommands.gui import build_gui_parser
from mercury_cli.subcommands.logs import build_logs_parser
from mercury_cli.subcommands.prompt_size import build_prompt_size_parser
from mercury_cli.subcommands.memory import build_memory_parser
from mercury_cli.subcommands.acp import build_acp_parser
from mercury_cli.subcommands.tools import build_tools_parser
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Mercury' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo mercury -p <name>` against the invoking user's home.
    ...
    # after the subcommand (`mercury chat -p coder`), so keep scanning broadly.
    # The exception is command-argv passthrough regions such as `mcp add --args`.
    from mercury_cli._parser import top_level_value_flag_sets
    ...
    # Mirrors mercury_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # `mercury profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # profile (e.g. via the dashboard or ``mercury profile use``) would
    ...
    if profile_name is not None:
        try:
            ...
            mercury_home = _resolve_sudo_user_profile_env(profile_name)
            if not mercury_home:
...
# Windows launcher self-heal — the ``mercury`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Mercury root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``mercury update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``mercury``
...
# ``python -m mercury_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no mercury module may be imported before
...
if sys.platform == "win32":
    try:
        from mercury_cli import _install_repair as _install_repair_mod
...
try:
    ...
    # yaml.load — the SAME parse then serves mercury_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from mercury_cli.config import read_raw_config as _read_raw_early
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort — don't crash if mercury_constants not importable yet
...
# Provider model-selection wizard flows extracted to mercury_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (mercury_cli.main._model_flow_*) keep resolving unchanged.
from mercury_cli.model_setup_flows import (
...
def _relative_time(ts) -> str:
    ...
    in :mod:`mercury_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from mercury_cli.timefmt import relative_time
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from mercury_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``mercury -c`` from repo A continues repo A's last session rather than the
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from mercury_cli.terminal_breadcrumbs import resolve_breadcrumb_session
    ...
    *starts* is the set of workspace keys the launch install explicitly scopes
    ...
    while stack:
        ...
        if not isinstance(entry, dict):
            continue
        # Workspace symlink (e.g. node_modules/@mercury/ink → ui-tui/packages/…):
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @mercury/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@mercury" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/mercury-ink/package.json",
    "packages/mercury-ink/index.js",
    "packages/mercury-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": mercury_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _find_bundled_tui(mercury_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if mercury_cli_dir is None:
        mercury_cli_dir = Path(__file__).parent
    bundled = mercury_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``mercury update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Mercury checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `mercury update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Mercury checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `mercury --tui`\n"
        "If the checkout is still inconsistent, run `mercury update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        # and exit on an install whose only Node is the one Mercury installed,
        # and would pick a system Node over the managed one when both exist.
        from mercury_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from mercury_cli.dep_ensure import ensure_dependency
    ...
    # mercury_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from mercury_constants import with_hermes_node_path
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Mercury-managed
            ...
            from mercury_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @mercury/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @mercury/ink resolves through
        # packages/mercury-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "mercury-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from mercury_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    # TUI child is a mercury process: propagate the profile-home contract via
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="mercury-tui-active-session-", suffix=".json"
    )
    ...
    # in the user's shell would otherwise make a plain `mercury --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `mercury update` so
    ...
    if code == 42:
        from mercury_cli.relaunch import relaunch
...
# MERCURY-OMP PATCH (B2 phase 2): _pin_kanban_board_env() deleted — kanban
# board env-pinning served the removed kanban engine; delegation = omp.


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.mercury/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Mercury — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Mercury still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``mercury … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: {cli_invocation_name()} --resume {_imported_id})")
    ...
    try:
        from mercury_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from mercury_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'mercury doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if _termux_should_prefetch_update_check():
        try:
            from mercury_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.mercury/skills the banner
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import mercury_cli.banner as _banner_mod
    ...
    # MERCURY-OMP PATCH (B2 phase 2): _pin_kanban_board_env() removed —
    # the kanban board env-pin (and the engine it served) is deleted;
    # delegation = omp.
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older mercury_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: MercuryCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `mercury` / `mercury chat`.
        from mercury_constants import emit_partial_update_hint
...
def cmd_proxy(args):
    ...
    from mercury_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from mercury_constants import find_node_executable, with_hermes_node_path
    ...
    # has no creds.json.  Every subsequent `mercury gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `mercury gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '🌡️ Mercury'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  mercury gateway")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``mercury whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``mercury_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from mercury_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from mercury_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from mercury_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    # PROVIDER_GROUPS in mercury_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``mercury model``
    ...
    # matching against mercury_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
_AUX_TASKS: list[tuple[str, str, str]] = [
    ...
    # MERCURY-OMP PATCH (B2 phase 2): triage_specifier + kanban_decomposer
    # aux-model entries removed — their consumers (kanban_specify.py /
    # kanban_decompose.py) are dead code slated for deletion with the
    # kanban engine.
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`mercury_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from mercury_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``mercury model`` flow, then route aux tasks
    to them here.
    """
    from mercury_cli.config import load_config
    from mercury_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Mercury URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `mercury_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `mercury` invocation — including
# fast paths like `mercury --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on mercury_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `mercury` invocation, including fast paths like `mercury --version`
...
_LAZY_COMMAND_EXPORTS = {
    "mercury_cli.sessions_cmd": (
        "cmd_sessions",
    ),
    "mercury_cli.dashboard_procs": (
        "_detect_concurrent_hermes_instances",
        "_filter_dashboard_respawn_candidates",
        "_kill_stale_dashboard_processes",
        "_scan_dashboard_processes",
    ),
    "mercury_cli.update_cmd": (
...
def _self():
    ...
    keeps monkeypatches on mercury_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``mercury setup`` / ``mercury model``.
    ...
    recover from a malformed paste without editing ``~/.mercury/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from mercury_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from mercury_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `mercury setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.mercury/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `mercury sync propose <skill>`. "
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="mercury sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="mercury sync now")
...
def cmd_slack(args):
    ...
    Dispatches ``mercury slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: mercury slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `mercury slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from mercury_cli.slack_cli import slack_manifest_command
...
# MERCURY-OMP PATCH (B2): cmd_kanban removed from the Mercury distribution —
# the `mercury kanban` subparser is no longer registered (see main()).


def cmd_project(args):
    """Manage projects (named, multi-folder workspaces)."""
    from mercury_cli.projects_cmd import projects_command
...
def cmd_verify(args):
    ...
    from mercury_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `mercury security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from mercury_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `mercury approvals <subcmd>`."""
    from mercury_cli.approvals_suggest import approvals_command
...
def cmd_skin(args):
    ...
    from mercury_cli.skin_cmd import skin_command
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `mercury --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    """Uninstall Mercury (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from mercury_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from mercury_cli.uninstall import run_gui_uninstall
...
# Update pipeline lives in mercury_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on mercury_cli.main.<name> keep resolving unchanged without
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``mercury update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``mercury update``
    ...
    ``mercury`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``mercury update``
    ...
    ``mercury_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "mercury_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``mercury`` launcher present but ``mercury_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the mercury venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — mercury venv python3, no nix-shell overhead
    ...
    # a mercury venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``mercury update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Mercury-managed install, or a freshly provisioned managed npm when
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "mercury_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m mercury_cli.main web` invocation.
    ...
    # as `mercury update`'s _update_node_dependencies() (ui-tui + web +
...
#   - ``mercury update`` can unconditionally call ``mercury desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``mercury desktop`` (interactive launch) skips the build when the
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Mercury.app/Contents/MacOS/Mercury → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``mercury desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``mercury update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Mercury.app/Contents/MacOS/Mercury"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Mercury.exe",
            release_dir / "win-ia32-unpacked" / "Mercury.exe",
            release_dir / "win-arm64-unpacked" / "Mercury.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "mercury",
            release_dir / "linux-unpacked" / "Mercury",
            release_dir / "linux-arm64-unpacked" / "mercury",
            release_dir / "linux-arm64-unpacked" / "Mercury",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Mercury.exe
...
#
# The desktop self-update chain (Desktop → mercury-setup --update →
# `mercury update` → `mercury desktop --build-only` → relaunch) rebuilds
# Mercury.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Mercury.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Mercury.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 mercury-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Mercury.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Mercury.exe from backup.")
        print("    Your existing version was kept and still works. Run `mercury desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `mercury desktop --force-build` to rebuild, or re-run the Mercury")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Mercury'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Mercury.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Mercury.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Mercury.exe: Access is
    ...
    an unrelated "Mercury" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Mercury.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Mercury is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Mercury.app/Contents/MacOS/Mercury  ->  app bundle = .../Mercury.app
    app = exe.parents[2]
...
def _desktop_macos_setup_tcc_identity(identity: str = "Mercury Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Mercury to use it.

    One-shot setup for ``mercury desktop --setup-tcc-identity``. Creates a
    ...
    if not already_imported:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="mercury-tcc-"))
        try:
    ...
    # Point Mercury at the identity (config.yaml, not .env — it's not a secret).
    try:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.mercury"
    )
    return True
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``mercury desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Mercury Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Mercury Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `mercury desktop` because the launcher environment doesn't look
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Mercury Desktop (Linux only, best-effort).
    ...
    shell. ``mercury uninstall --gui`` removes it.
    """
    try:
        from mercury_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from mercury_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "mercury_root", None):
        env["HERMES_DESKTOP_HERMES_ROOT"] = str(Path(args.mercury_root).expanduser().resolve())
    ...
    # `HERMES_DESKTOP_DISABLE_GPU=... mercury desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... mercury desktop` keep working.
    ...
    # `HERMES_DESKTOP_PASSWORD_STORE=... mercury desktop` keeps working.
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Mercury Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  mercury gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Mercury-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → mercury-setup →
            # mercury update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `mercury update` path).
            ...
            if not source_mode:
                ...
                # holds Mercury.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Mercury.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Mercury.exe, close any")
                    print("  running Mercury desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> mercury desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Mercury is
                ...
                # success on a Mercury.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Mercury shows up
    ...
    if source_mode:
        print("→ Launching Mercury Desktop from source build...")
    ...
    print(f"→ Launching packaged Mercury Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in mercury_cli/dashboard_procs.py
...
# mercury_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "mercury dashboard",
            "mercury_cli.main dashboard",
            "mercury_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "mercury serve",
            "mercury_cli.main serve",
            "mercury_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "mercury-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Mercury installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``mercury-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/mercury-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but mercury launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``mercury update``.
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``mercury update``.
    ...
    protocol stream (``mercury acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `mercury update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``mercury.exe`` launch always has the launcher as an
    ...
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from mercury.exe; applying package-only first aid, "
    ...
    try:
        from mercury_cli import _install_repair as _ir
        ...
        from mercury_cli.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Mercury is still running from the launcher that needs "
                "replacing. Close other Mercury windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\mercury.exe`` is a launcher that runs the interpreter with
    ...
    ``mercury ...`` command holds its own shim, and an editable install run
    ...
    ``mercury.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
...
def _windows_running_hermes_launcher_locked() -> bool:
    """True when a venv ``mercury*.exe`` shim is this process or an ancestor.
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``mercury update``, so the whole remaining flow — the
    ...
    from mercury_constants import venv_python_path

    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "mercury_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from mercury_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``mercury update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from mercury_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``mercury.exe``.
    ...
    when ``mercury update`` runs, ``mercury.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``mercury.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next mercury invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Mercury Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Mercury Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``mercury.exe`` re-runs itself under the venv Python before
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Mercury Desktop, exit other `mercury` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `mercury update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Mercury versions queued for our shims.

    Mercury used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    update; a failed *restore* leaves the install with no ``mercury`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``mercury*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``mercury.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``mercury.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``mercury`` drops off PATH. This wraps
    ...
    try:
        ...
        # quarantined aside and `mercury` missing from PATH after a green
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``mercury.exe.old.*`` from updates.

    Called early on every mercury invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``mercury`` gone from PATH:

    1. **Orphan rescue.** If ``mercury.exe`` is missing while
       ``mercury.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``mercury update`` does exactly this). Leave anything inside the grace
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``mercury.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``mercury.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `mercury update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from mercury_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``mercury.exe`` / ``mercury-gateway.exe`` shims
    ...
    if (
        ...
        # Quarantining the wrong dir means the running mercury.exe stays locked
    ...
    # hours later inside ``mercury update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``mercury.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``mercury.exe`` shim is locked during ``mercury update``, or when uv/distlib
    skips a launcher write. The symptom is ``mercury-agent.exe`` and
    ``mercury-acp.exe`` present but ``mercury.exe`` missing, so ``mercury`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `mercury update --force` after "
            "closing other mercury processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m mercury_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``mercury update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``mercury.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``mercury`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `mercury update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `mercury update --force` after closing other mercury processes."
        )
        return
...
def cmd_update(args):
    """Update Mercury to the latest version.
    ...
    if getattr(args, "plan", False):
        ...
        # Mercury runtime across profiles, its supervisor, and its running
        # code version — without mutating anything. Safe on a live fleet.
        from mercury_cli.update_inventory import (
    ...
    # exit 1 errors).
    from mercury_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `mercury update --branch=<x>` would actually pull.
    ...
    from mercury_cli.update_lock import (
    ...
    try:
        ...
        try:
            from mercury_cli.update_receipt import finalize_pending_update_receipt
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        from mercury_cli.profiles import format_profile_label
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → mercury -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.mercury)")
            else:
        ...
        from mercury_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from mercury_constants import get_hermes_home as _hh
        ...
        from mercury_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `mercury profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → mercury -p {name}  ({wrapper})")
        print()
        ...
        from mercury_cli.profiles import profile_exists, validate_alias_name
        ...
        from mercury_cli.profiles import normalize_profile_name, rename_profile
        ...
        import tempfile
        from mercury_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="mercury_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  mercury -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      mercury -p {plan.manifest.name} chat")
        ...
        from mercury_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `mercury profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"mercury -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from mercury_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("mercury_requires"):
            print(f"Requires:     Mercury {data['mercury_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from mercury_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.mercury_requires:
        print(f"  Requires: Mercury {mf.mercury_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No mercury dashboard or serve processes running.")
        return 0

    print(f"{len(live)} mercury dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``mercury dashboard register`` for
    OAuth.
    ...
    try:
        from mercury_cli.web_server import should_require_dashboard_auth
    ...
    try:
        from mercury_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `mercury dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from mercury_cli.config import load_config, save_config
        from mercury_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from mercury_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.mercury/desktop-ssh: a
    # literal "~/.mercury/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    ...
    token_root = _Path.home() / ".mercury" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    ``.../app.asar.unpacked/dist``. A standalone ``mercury dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No mercury dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `mercury update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with mercury serve")
    ...
    # those vars then runs `mercury dashboard` would otherwise:
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "mercury_cli.main",
            "-p", "default",
        ...
        # HERMES_HOME falls back to $HOME/.mercury = /opt/data/.mercury — an
        ...
        # returns the root for both layouts: ~/.mercury for a standard install
    ...
    from mercury_cli.resource_limits import apply_nofile_soft_limit
    ...
    # the same logs directory as every other Mercury surface.
    try:
        from mercury_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "mercury_cli" / "web_dist"
        )
    ...
    try:
        from mercury_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard_register(args):
    ...
    from mercury_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from mercury_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_prompt_size(args):
    ...
    from mercury_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_console(args):
    """Open the safe Mercury command console."""
    from mercury_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from mercury_cli.models import CANONICAL_PROVIDERS as _cp
...
def _first_positional_argv() -> str | None:
    ...
    ``mercury -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``mercury`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``mercury <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``mercury photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from mercury_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``mercury`` / ``mercury -w -s foo --yolo`` / ``mercury chat`` don't need the
    ...
    from mercury_cli._parser import build_top_level_parser
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `mercury --tui` is the hot path on phones. The full parser setup imports
...
def cmd_acp(args):
    """Launch Mercury as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from mercury_cli.tools_config import run_post_setup_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: mercury-agent[otlp])")
...
def _advertise_agent_env() -> None:
    ...
    (``mercury-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``HERMES_AGENT`` is the Mercury-specific marker. setdefault: never
    clobber an outer harness (e.g. Mercury running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "mercury-agent")
    os.environ.setdefault("HERMES_AGENT", "true")


def main():
    """Main entry point for mercury CLI."""
    # Cosmetic: make the process show up as 'mercury' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # MERCURY-OMP PATCH (NB-2): pre-argparse `omp` intercept. omp's argv must
    # pass through to the omp binary BYTE-FOR-BYTE (its flag surface is huge
    # and collisions with mercury' own flags — e.g. top-level `-p <profile>` —
    # would corrupt it). When the first positional token is `omp`, everything
    ...
    # there: everything before is mercury', everything after is omp's, verbatim.
    _omp_idx = None
    _i = 0
    _VAL_FLAGS = {"-p", "--profile", "--model", "--provider", "--reasoning",
                  "-m", "-z", "-t", "--resume", "--in", "--skills", "-c", "--continue"}
    while _i < len(_argv):
        _tok = _argv[_i]
        if _tok == "omp":
            _omp_idx = _i
            break
        if _tok in _VAL_FLAGS:
            _i += 2  # flag + value — skip both
            continue
        if _tok.startswith("-") and "=" not in _tok and _tok not in ("--yolo", "--version"):
            _i += 2  # assume value-taking unknown flag (conservative)
            continue
        _i += 1
    if _omp_idx is not None:
        from mercury_cli.omp_command import cmd_omp as _cmd_omp

        _rest = _argv[_omp_idx + 1:]
        _print_cmd = False
        if _rest[:1] == ["--print-cmd"]:
            _print_cmd = True
            _rest = _rest[1:]
        raise SystemExit(
            _cmd_omp(
                type("OmpArgs", (), {"omp_args": _rest, "print_cmd": _print_cmd})()
            )
        )
    ...
    try:
        from mercury_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``mercury.exe.old.*`` quarantine files left by previous
    # ``mercury update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # If the checkout changed since the last launch (mercury update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``mercury update``
    ...
    # ``mercury skills install update``) merely defers recovery one launch;
    # under-matching (missing ``mercury -p work update``) would race a recovery
    ...
    # never restarted the fleet. Do NOT restart here — that is ``mercury
    ...
    # =========================================================================
    # MERCURY-OMP PATCH (NB-2): omp command — launch the omp half of Mercury.
    # Execs the patched omp build with the bridge-rendered delegate model;
    # argv passes through untouched after that. The omp TUI is the interface.
    # =========================================================================
    from mercury_cli.omp_command import cmd_omp

    omp_parser = subparsers.add_parser(
        "omp",
        help="Launch the omp coding agent (Mercury delegate engine)",
        description=(
    ...
    omp_parser.add_argument(
        "--print-cmd",
        action="store_true",
        help="Print the resolved omp invocation instead of running it",
    )
    omp_parser.add_argument(
        "omp_args",
    ...
    from mercury_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from mercury_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `mercury model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory mercury -w sessions "
    ...
    def _dispatch_worktree(_args):
        from mercury_cli.worktree_cmd import cmd_worktree
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Mercury can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    def _dispatch_browser(_args):
        from mercury_cli.browser_connect import (
            UNSUPPORTED_CHANNEL,
    ...
    from mercury_cli import secrets_cli as _secrets_cli
    from mercury_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `mercury proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from mercury_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``mercury proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from mercury_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in mercury_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in mercury_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in mercury_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `mercury whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in mercury_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from mercury_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in mercury_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in mercury_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in mercury_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in mercury_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in mercury_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in mercury_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in mercury_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Mercury gateways)
    # =========================================================================
    ...
    # =========================================================================
    from mercury_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # MERCURY-OMP PATCH (B2): kanban subcommand removed from the Mercury
    # distribution — `mercury kanban …` now falls to argparse's invalid-choice
    # error. (Delegation is omp; see tools/omp_delegation.py.)
    # =========================================================================

    # =========================================================================
    # project command — named, multi-folder workspaces
    # =========================================================================
    from mercury_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in mercury_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in mercury_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in mercury_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in mercury_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in mercury_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in mercury_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in mercury_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in mercury_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.mercury/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo mercury uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from mercury_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in mercury_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: mercury_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from mercury_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in mercury_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in mercury_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in mercury_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in mercury_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in mercury_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from mercury_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in mercury_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``mercury --help``, ``mercury logs``,
    ...
    try:
        from mercury_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Mercury shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from mercury_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from mercury_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in mercury_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in mercury_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `mercury computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `mercury tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `mercury computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Mercury app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from mercury_cli.tools_config import _cua_driver_contract_status
            ...
            if path:
                ...
                try:
                    ...
                    pass
                from mercury_cli.tools_config import _cua_version_summary
                version = _cua_version_summary(version)
                # Name the override here too. Without it the operator is told
                # to repair an install that `mercury computer-use install` will
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            ...
                            "the override and run: mercury computer-use install --upgrade"
                        )
                    else:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: mercury computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: mercury computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: mercury computer-use permissions grant")
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <mercury home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Mercury",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Mercury session store "
            "so it can be resumed with 'mercury --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in mercury_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on mercury_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in mercury_cli/subcommands/insights.py)
    # =========================================================================
    ...
    # =========================================================================
    # update command  (parser built in mercury_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in mercury_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in mercury_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in mercury_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in mercury_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Mercury-Setup.exe success screen tells users to
    # run `mercury desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in mercury_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in mercury_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in mercury_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # ``mercury egress start`` refusing when credential_source=bitwarden
