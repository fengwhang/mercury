...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.mercury/.env.
    ...
    exports. ``mercury debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.mercury_build_sha`` by the Dockerfile's
    ``HERMES_GIT_SHA`` build-arg (see ``mercury_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``mercury dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    lines.append(f"mercury_home:      {display_hermes_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.mercury/.env: a managed
        ...
        # A credential added via `mercury auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `mercury auth list` shows it (#42130).
