...
late-binding seam in :mod:`mercury_cli.web_deps` (``late`` for callables,
...
from mercury_cli.web_deps import late, LateState
from mercury_cli.web_models import (
...
_log = logging.getLogger("mercury_cli.web_server")
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `mercury tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `mercury computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``mercury tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `mercury tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``mercury tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.mercury/.env`` via ``save_env_value`` —
    the same store ``mercury tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``mercury tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``mercury -p <profile> tools post-setup``.
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``mercury computer-use permissions grant`` as a background action.
