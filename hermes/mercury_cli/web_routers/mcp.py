...
web_server - reached via the late-binding seam in :mod:`mercury_cli.web_deps`
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from mercury_cli.mcp_config import _replace_mcp_servers
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.mercury_home == flow_home
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Mercury and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Mercury for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Mercury.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `mercury mcp catalog` / `mercury mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from mercury_cli.config import validate_env_var_name_for_write
