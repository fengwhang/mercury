...
late-binding seam in :mod:`mercury_cli.web_deps` so tests that
...
from mercury_cli.web_deps import late
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``mercury profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``mercury profile use``).
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (mercury_cli.profiles
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``mercury profile describe
    <name> --auto``.
    ...
    try:
        from mercury_cli import profile_describer
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps mercury_cli.profiles.export_profile /
# import_profile (the same machinery behind `mercury profile export|import`).
