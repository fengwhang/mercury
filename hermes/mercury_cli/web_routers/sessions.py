...
the late-binding seam in :mod:`mercury_cli.web_deps` so tests that
...
from mercury_state import is_malformed_db_error
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another Mercury surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Mercury backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `mercury sessions stats`).
