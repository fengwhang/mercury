...
_TASK_DESCRIPTION = "Mercury Gateway - Messaging Platform Integration"
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Mercury-owned paths under the configured HERMES_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\mercury`` as a symlink/junction to
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Mercury profile."""
    from mercury_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "mercury_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during mercury_cli boot.
    from mercury_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\mercury\\gateway-service\\<task_name>.cmd``
    (or ``<HERMES_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Mercury installs stay self-contained).
    """
    _assert_windows()
    from mercury_cli.config import get_hermes_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m mercury_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``mercury gateway run``.
    ...
    lines.append(f'set "HERMES_HOME={mercury_home}"')
    ...
    # if someone imports mercury_constants-based logic during startup.
    ...
    prog_args = [python_exe_path, "-m", "mercury_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('HERMES_HOME')}) = {_quote_vbs_string(mercury_home)}",
...
def _write_task_script() -> Path:
    ...
    mercury_home = str(Path(get_hermes_home()))
    profile_arg = _profile_arg(mercury_home)

    content = _build_gateway_cmd_script(python_path, working_dir, mercury_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, mercury_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Mercury builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "mercury_cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "HERMES_HOME": mercury_home,
...
def windowless_gateway_restart_spec(
    ...
    preserves every argument after the interpreter (``-m mercury_cli.main
    ...
    from mercury_cli.config import get_hermes_home
    from mercury_cli.gateway import PROJECT_ROOT
    ...
    try:
        mercury_home = str(Path(get_hermes_home()).resolve())
    except Exception:
        mercury_home = ""
    ...
    if mercury_home:
        env_overlay["HERMES_HOME"] = mercury_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m mercury_cli.main gateway run`` directly — NOT
...
def _prompt_install_choices(
    ...
    from mercury_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `mercury -p <profile> gateway install` must be safe.
    ...
    from mercury_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"mercury {profile_arg} gateway start" if profile_arg else "mercury gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: mercury gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Mercury gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: mercury gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: mercury gateway start")
        _print_next_steps()
        return
...
def _print_next_steps() -> None:
    ...
    mercury_home = Path(get_hermes_home())
    print()
    print("Next steps:")
    print("  mercury gateway status                      # Check status")
    print(f"  type {mercury_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Mercury gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: mercury gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: mercury gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: mercury gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `mercury gateway status`."
        )
