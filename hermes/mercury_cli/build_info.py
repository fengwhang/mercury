"""
Baked-in build metadata for Mercury.

Source installs report their git revision live via ``git rev-parse`` (see
``mercury_cli/dump.py`` and ``mercury_cli/banner.py``).  That doesn't work inside
...
To make ``mercury dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$HERMES_GIT_SHA``
arg into ``<project_root>/.mercury_build_sha``.  This module is the single
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".mercury_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    rev-parse`` for source installs, the baked ``.mercury_build_sha`` for
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.mercury_build_sha`` if present.  The file is
