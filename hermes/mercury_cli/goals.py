"""Persistent session goals — the Ralph loop for Mercury.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Mercury feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.MercuryCLI`` or the gateway
...
# creation, and the first mercury_cli.config import (journal-mode
...
def _get_session_db() -> Optional[Any]:
    ...
    ``mercury_home`` path so profile switches still pick up the right DB.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.mercury/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
# MERCURY-OMP PATCH (B2 phase 2): the kanban worker goal loop
# (KANBAN_GOAL_* templates + run_kanban_goal_loop) deleted — it drove
# dispatcher-spawned goal-mode kanban workers; the kanban engine is
# deleted and delegation = omp. The interactive /goal contract machinery
# above is untouched.
