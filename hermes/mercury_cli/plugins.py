...
from mercury_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from mercury_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from mercury_cli.plugin_capabilities import (
...
# stderr in addition to ~/.mercury/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Mercury loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Mercury' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # MERCURY-OMP PATCH (B2 p2): kanban lifecycle hooks removed (kanban is dead).
...
ENTRY_POINT_CAPABILITIES_GROUP = "mercury_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``mercury_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- mercury-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- mercury-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- mercury-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``mercury:<event>``.
HERMES_EVENT_NAMESPACE = "mercury"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Mercury home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "mercury", "depends",
}

# Highest manifest schema version this Mercury understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Mercury "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `mercury plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    # ``mercury_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Mercury never
    ...
    # (``mercury plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Mercury paths, traversal syntax, and the security-
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from mercury_cli import config as config_mod
        ...
        # threads. Other Mercury config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from mercury_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Mercury profile name (e.g. ``"default"``).

        Derived from ``HERMES_HOME`` via
        :func:`mercury_cli.profiles.get_active_profile_name`, so it works in
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Mercury core) are trusted by default —
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`mercury_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from mercury_cli.dashboard_auth import DashboardAuthProvider
        from mercury_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.mercury/.env`` loads, before Mercury reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Mercury' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Mercury' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``mercury model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: mercury_cli.main imports plugins indirectly
        from mercury_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``mercury:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``mercury:``
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``mercury:<event>``
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or mercury_home_key()
        ...
        # by resolved mercury home). The ledger is therefore keyed per manager
        # — i.e. per (mercury_home, plugin_id) — and every release/restore
        ...
        # contributed, so `mercury plugins list` still attributes them once the
        # full plugin loads.
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from mercury_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Mercury plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Mercury core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Mercury core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Mercury Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # all ~20 of them added several seconds to every `mercury`
            # invocation — including plain `mercury chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Mercury ships
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from mercury_cli.approval_transport import RegisteredApprovalTransport
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 3. Project plugins (./.mercury/plugins/), only when explicitly opted
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from mercury_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from mercury_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Mercury process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``mercury_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``mercury plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`mercury plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``mercury tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `mercury plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Mercury validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Mercury does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `mercury plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from mercury_cli.agent_plugins import load_agent_plugin
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Mercury home (profile switch reusing a slug
        # like "mercury-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"mercury-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved Mercury home -> PluginManager. Mercury supports
...
# mercury_constants.set_hermes_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    plugins (notably context engines such as mercury-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Mercury home (env var *or* the context-local
    ``set_hermes_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Mercury home instead of
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``mercury_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``mercury_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Mercury profile/home.
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=mercury_home_key(current_home))
...
def _get_pre_tool_call_directive_details(
    ...
    from mercury_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="mercury-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``mercury:<event>``) to subscriber callbacks in
