"""CLI handlers for the ``mercury proxy`` subcommand."""
...
from mercury_cli.proxy.adapters import ADAPTERS, get_adapter
from mercury_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "mercury proxy requires aiohttp. Run `mercury setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"mercury auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Mercury proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Mercury proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: mercury proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``mercury proxy <subcommand>``."""
    ...
    print(
        "mercury proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  mercury proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  mercury proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  mercury proxy providers\n"
