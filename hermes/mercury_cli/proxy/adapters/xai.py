...
# xAI's public API is OpenAI-compatible for the endpoints Mercury commonly
# uses. The Responses endpoint is included because Mercury' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Mercury-managed OAuth credentials."""

    auth_hint = "mercury auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`mercury auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`mercury auth reset xai-oauth` or re-authenticate with "
                    "`mercury auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `mercury auth add xai-oauth --type oauth`."
            )
