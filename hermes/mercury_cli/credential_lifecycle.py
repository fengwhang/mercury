"""Unified provider-credential lifecycle across every store Mercury reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.mercury/.env``                     — the canonical secret store
    2. ``~/.mercury/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.mercury/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `mercury auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``mercury auth add``.
    ...
    ran ``mercury auth add <provider> --type api-key`` separately. This makes
    the Desktop save's effect on disk match what ``mercury auth add`` does.
    """
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from mercury_cli.config import load_env, remove_env_value
