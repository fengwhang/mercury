...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # skill: `mercury skills install impeccable` should resolve to the
        ...
        # so users can copy it for `mercury skills install`.
...
def _resolve_source_meta_and_bundle(identifier: str, sources):
    ...
    skill is how ``mercury skills inspect owner/repo/skills/foo`` showed
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.mercury/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.mercury/skills/<name>/)[/]"
        )
    from mercury_cli.cli_output import line_input
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `mercury skills install` fail.
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "mercury-index".
    ...
    # The identifier is what you pass to `mercury skills install`. Browse used
    ...
    c.print("[dim]Tip: 'mercury skills inspect <identifier>' to preview, "
            "'mercury skills install <identifier>' to install, "
            "'mercury skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]mercury skills install {url} --name <your-name>[/]\n\n"
    ...
    # metadata.mercury.blueprint block, it is a runnable automation. Register it as
        ...
        if spec is not None:
            ...
            if registered is not None:
                ...
                c.print(
                    ...
                    "or via[/] [bold]mercury cron add[/][dim].[/]\n"
                )
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "mercury-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"mercury-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying mercury-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``mercury -p <profile> skills list`` reads that profile's
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``mercury update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: mercury skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `mercury update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `mercury update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   mercury skills diff <name>[/]")
    c.print("[dim]Resume updates: mercury skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: mercury skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: mercury skills reset {name} --restore[/]\n")
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print("Usage: mercury skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
