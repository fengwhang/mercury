...
def process_state(pid: int, creation_time_ns: int, mercury_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(mercury_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the mercury exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs mercury_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "mercury_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "mercury_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, mercury_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, mercury_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves mercury_cli's own location and prepends its parent, because the
    ...
    # the mercury_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the mercury_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not os.path.isabs(configured_path):
        raise ValueError("Mercury path must be absolute")
    mercury_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(mercury_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Mercury Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('mercury_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_hermes(mercury_path: str) -> dict[str, Any]:
    path = os.path.abspath(mercury_path)
    if not os.path.isabs(mercury_path) or not os.path.isfile(path):
        raise ValueError("Mercury path is not an executable file")
