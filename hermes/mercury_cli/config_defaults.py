"""Default configuration data for Mercury.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from mercury_cli/config.py. Must not import from mercury_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    # SQLite journal mode used by every Mercury database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Mercury server processes.
    ...
    "session": {
        # Per-terminal `mercury -c`: each CLI session drops a breadcrumb file
    ...
    "agent": {
        ...
        # all normalized by mercury_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `mercury chat --run-budget N`.
        ...
        # an interactive `mercury gateway restart` must never block for hours
        ...
        # prompt's environment-hints block. Lets a host that wraps Mercury
        ...
        # app, ACP) in a code workspace, Mercury adds a coding operating brief
        ...
        # `mercury status` / fleet monitoring). Retries never stop — this is a
    ...
    "terminal": {
        ...
        # Root directory for Mercury' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Mercury uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        ...
        # bot_relay from a short-lived `mercury -p <bot> chat -Q` recipient
        ...
        # bundled Mercury image, which drops to the `mercury` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `mercury tools` picker's
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes Mercury spawn
        ...
        # `mercury doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a Mercury-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Mercury' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.mercury/browser-profile/) so copied
        ...
        # approval does it run `mercury browser close-profile` (terminates the
        ...
        "camofox": {
            ...
            # app owns the visible browser and Mercury should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``mercury chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.mercury/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: mercury sweeps the checkpoint base at startup
        ...
        # ``mercury checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .mercury.md, or .cursorrules before Mercury applies
    ...
    # Single-query (``mercury -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Mercury' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); mercury =
                                      # Mercury' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Mercury' local
    ...
    "auxiliary": {
        ...
        # ``mercury profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `mercury model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `mercury` (and `mercury chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `mercury --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `mercury -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `mercury --tui` drops a one-time hint
        ...
        # model — see mercury_cli/focus_view.py.
        "focus_view": False,
        ...
        # e.g. `model · 68% · ~/projects/mercury`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `mercury pets`. Disabled until
    ...
    "dashboard": {
        ...
        # protocol ping (see mercury_cli/web_server.py — an event-loop stall
        ...
        # optional path prefix (e.g. ``https://example.com/mercury``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Mercury appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.mercury/cache/piper-voices/
    ...
    "wake_word": {
        ...
        "phrase": "hey mercury",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.mercury/cache/delegation/
    ...
    # response. If not, Mercury feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Mercury auto-pauses the goal and
    ...
    "moa": {
        ...
        # <mercury_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "skills": {
        ...
        # checkout, ``<root>/.mercury/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``mercury skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``mercury skills trust`` / ``mercury skills untrust``.
        ...
        # (`mercury skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.mercury/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.mercury/.curator_backups/blobs/. Enables `mercury curator ledger`
        # and single-mutation `mercury curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `mercury curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `mercury curator run --consolidate` overrides
        ...
        # normally restored on every `mercury update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `mercury curator purge` deletes
        ...
        # skipped), snapshot ~/.mercury/skills/ into
        # ~/.mercury/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `mercury curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "🌡️ *Mercury*" header.
    ...
    # replace Mercury' built-in platform hint for a single messaging platform
    ...
    # ``mercury plugins enable|disable`` and intentionally omitted here so an
    ...
    "security": {
        ...
        # .cursorrules, project-local .mercury config) always require human
        ...
        # longer trigger the startup banner. Add via `mercury doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``mercury_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Mercury to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core Mercury tools (terminal, read_file, write_file, patch,
    ...
    # update model picker lists without shipping a mercury-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://mercury-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `mercury model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Mercury provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # producing ~/.mercury/sessions/sessions.json entirely.
        ...
        # `launchctl kickstart ai.mercury.gateway` that defenses 1-2 don't
        ...
        # `mercury update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.mercury/.env,
        ...
        # behavior: files must live under the Mercury cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Mercury
        # cache (~/.mercury/cache/{documents,images,audio,video,screenshots})
    ...
    "sessions": {
        ...
        # the default of ``mercury sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.mercury/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `mercury sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `mercury update` prints a one-line notice with
        ...
        # ~/.mercury/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # (non-streaming) export such as `mercury sessions export`. Checked
    ...
    # ``mercury doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `mercury doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``mercury update`` behaviour.
    "updates": {
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``mercury backup``-style zip
        #     of HERMES_HOME into <HERMES_HOME>/backups/, restorable with
        #     ``mercury import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `mercury update` does with uncommitted local changes to the
        ...
        # When `mercury update` finds the source checkout parked on a feature
        ...
        #                         merge. `mercury update --switch-branch`
    ...
    # `mercury tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.mercury/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.mercury/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.mercury/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Mercury may use this encrypted
            ...
            # ~/.mercury/bin/ on first use.  When False you must install
            ...
            # `mercury secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.mercury/.env (or the shell)
    ...
    "computer_use": {
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Mercury also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Mercury computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Mercury passes the
    ...
    #
    # Configure with `mercury egress setup`.  Disabled by default — the rest of
    # Mercury works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.mercury/bin/ on
    ...
    # Mercury Desktop (Electron app) launch options. These only affect
    # `mercury desktop`; they do not touch the CLI/gateway.
    "desktop": {
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Mercury mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'mercury model' for endpoint-specific config)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Mercury' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Mercury, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #mercury)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: mercury-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
