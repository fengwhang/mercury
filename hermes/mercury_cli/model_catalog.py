...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/mercury-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Mercury silently lands on when
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``mercury update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.mercury/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``mercury model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
