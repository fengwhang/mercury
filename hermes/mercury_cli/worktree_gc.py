"""On-demand worktree + branch reclaim (``mercury worktree`` / ``/worktree prune``).

The startup pruner in ``cli._prune_stale_worktrees`` is deliberately
conservative and silent: it runs before the banner on every ``mercury -w``
...
- **Orphaned local branches** beyond the two auto-generated prefixes the
  startup pass deletes (``mercury/mercury-*``, ``pr-*``): salvage lanes, port
...
- untracked-only dirt is ARCHIVED to ``~/.mercury/archive/worktree-prune/``
...
def _archive_untracked(tree: Path, untracked: List[str]) -> Optional[Path]:
    ...
    dest = (
        Path.home() / ".mercury" / "archive" / "worktree-prune"
        / f"{tree.name}-{stamp}"
    )
    try:
...
def audit_worktrees(repo_root: str, *, with_sizes: bool = True) -> List[TreeRecord]:
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        ...
        if lock_state == "live":
            rec("keep", "in use by a running mercury session")
            continue
...
def audit_branches(repo_root: str) -> List[BranchRecord]:
    ...
    Generalizes the startup pass's prefix list (``mercury/mercury-*``/``pr-*``)
    ...
    if workers > 1:
        try:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="mercury-branch-gc"
            ) as pool:
