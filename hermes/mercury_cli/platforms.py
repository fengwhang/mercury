"""
Shared platform registry for Mercury.
...
PLATFORMS: OrderedDict[str, PlatformInfo] = OrderedDict([
    ...
    ("whatsapp_cloud", PlatformInfo(label="📱 WhatsApp Business (Cloud)", default_toolset="mercury-whatsapp")),
    ...
    ("yuanbao",        PlatformInfo(label="🤖 Yuanbao",         default_toolset="mercury-yuanbao")),
        ...
        for entry in platform_registry.plugin_entries():
            if entry.name not in merged:
                merged[entry.name] = PlatformInfo(
                    ...
                    default_toolset=f"mercury-{entry.name}",
                )
    except Exception:
        pass
    return merged
