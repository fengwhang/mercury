...
- ``mercury_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``mercury_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``mercury_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``mercury_cli.main`` imports
``managed_uv``, ``mercury_constants``, and friends only in its late path; the
...
:func:`mercury_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from mercury_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (mercury_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``mercury acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # mercury_constants is stdlib-only, so the canonical layout helpers are safe
...
#: managed binary dir (the default Mercury root's ``bin``, next to uv.exe)
#: on the user PATH. Keep in lockstep with the launcher list in
#: scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("mercury", "mercury-acp")


def _venv_is_relocatable(venv_dir: Path) -> bool:
    """True when the venv's pyvenv.cfg declares ``relocatable = true``.

    uv writes the flag; ``mercury_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``mercury`` launchers when they vanish.

    On Windows, ``mercury`` resolves through launchers derived from the venv
    ...
    the managed binary dir — the default Mercury root's ``bin``
    (``%LOCALAPPDATA%\\mercury\\bin``, next to the managed uv) — which lives
    ...
    point inside ``profiles\\<name>`` under ``mercury -p``, so the anchor
    here is :func:`mercury_constants.get_default_hermes_root`.

    Earlier installer versions staged them at ``<checkout>\\bin`` instead —
    inside the git working tree — where ``mercury update``'s pre-update
    ...
    nothing restored them and ``mercury`` stopped resolving in every new
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    as ``mercury_constants.venv_bin_dir``.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    # Per-machine anchor: the DEFAULT Mercury root, not get_hermes_home() —
    # under ``mercury -p <name>`` that returns ``profiles\\<name>``, which
    ...
    # every mercury_cli.main process start (right after the profile
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                "  ✓ Restored mercury launcher(s): " + ", ".join(restored),
                file=sys.stderr,
            )
    return restored
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``mercury_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *,
    ...
    Runs from the ``mercury update`` tail (and mirrors what install.ps1's
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from mercury_constants import get_default_hermes_root, venv_bin_dir
    ...
    if kept != entries:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                f"  ✓ mercury launchers now resolve from {home_bin} "
...
def _quarantine_running_hermes_exe(
    ...
    """Rename live mercury*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``mercury acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `mercury` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``mercury*.exe`` shims on Windows so they can be replaced
