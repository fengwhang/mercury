...
``cli.py``'s ``MercuryCLI`` class. ``MercuryCLI`` inherits ``CLICommandsMixin`` so
...
from mercury_constants import display_hermes_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Mercury did)
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Mercury changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Mercury hasn't edited any files here yet.")
            return
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from mercury_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `mercury profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: mercury -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `mercury journey`).
        ...
        from cli import _cprint
        from mercury_cli.journey import register_cli
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from cli import _cprint
        from mercury_state import format_session_db_unavailable
        ...
        # "Is `mercury gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `mercury gateway` running?" while the gateway was
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `mercury gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `mercury sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `mercury -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``mercury -w``).

        ``prune`` is the same attended reclaim as ``mercury worktree prune``
        (mercury_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"prune", "gc", "clean"}:
            ...
            from mercury_cli import worktree_gc
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `mercury -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `mercury -w`.
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        All resolution/persistence goes through mercury_cli.personality —
        the single owner of personality state on every surface.
        """
        from mercury_cli.personality import (
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from mercury_cli.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from mercury_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from mercury_cli.pets import _set_active
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from mercury_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from mercury_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to mercury_cli.curator so the CLI and the `mercury curator`
        ...
        try:
            from mercury_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from mercury_cli.write_approval_commands import handle_pending_subcommand
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        ``.mercury/plans/`` via ``write_file``. No engine, no model-tool
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from mercury_cli.init_command import build_init_prompt_for_cwd
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        label = _skin.get_branding("response_label", "🌡️ Mercury")
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``mercury bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: mercury bundles create "
        ...
        _cprint(
            ...
            f"Manage with `mercury bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Mercury-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `mercury cron`.
        """
        from cli import _DIM, _RST, _cprint
        from mercury_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from mercury_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Mercury process runs — use `mercury cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from mercury_cli.goals import parse_contract
        ...
        _cprint(
            ...
            f"Mercury keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from mercury_cli.goals import draft_contract
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from mercury_cli.loops import dispatch_loop_command
        ...
        if result.get("created"):
            try:
                from mercury_cli.loops import goal_blocks_loop_tick
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="mercury_prompt_")
        try:
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from mercury_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from mercury_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from mercury_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from mercury_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Mercury is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Mercury is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from mercury_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Mercury to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``mercury update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Mercury')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Mercury"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="🌡️  Update Mercury",
            detail="This will exit the current session and run `mercury update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Mercury' hotword listener.
