...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-mercury-setup
...
def reconcile_profile_gateways(
    *,
    ...
    not under ``profiles/``). The dispatcher in ``mercury_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``mercury gateway start`` (no ``-p``) targets. Without it,
    bare ``mercury gateway start`` inside the container would land on
    ...
    Args:
        mercury_home: The container's HERMES_HOME (typically /opt/data).
            Profiles live under ``<mercury_home>/profiles/<name>/``;
            the default profile lives at ``<mercury_home>`` itself.
    ...
    # ``mercury gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(mercury_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(mercury_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(mercury_home),
    ))

    profiles_root = mercury_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `mercury profile create` (config.yaml
            ...
            # via ``mercury -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(mercury_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = mercury_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the mercury args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/mercury/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/mercury/docker/main-wrapper.sh
    ...
    ``mercury`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``mercury <subcommand>``, so an
    explicit leading ``mercury`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `mercury <subcommand>`; peel an explicit mercury.
    if args and Path(args[0]).name == "mercury":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``mercury dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `mercury -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit mercury
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the mercury user) won't hit EACCES. See
...
def _write_reconcile_log(
    mercury_home: Path, actions: list[ReconcileAction],
) -> None:
...
def main() -> int:
    ...
    actions = reconcile_profile_gateways(
        mercury_home=mercury_home, scandir=scandir,
    )
    for a in actions:
        print(
