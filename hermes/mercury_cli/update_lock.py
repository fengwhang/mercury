"""Cross-process mutual exclusion for in-flight Mercury updates.

Three different surfaces can start an update of the same install tree:

* ``mercury update`` from a terminal,
* the dashboard's Update button (``POST /api/mercury/update`` →
  ``_spawn_hermes_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``mercury-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``mercury update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <HERMES_HOME>/.mercury-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``mercury update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Mercury is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``mercury-setup`` binary under ``~/.mercury`` is only refreshed by a full
  ...
  ancestry check those users get exit 2 ("Mercury is still running") on every
  GUI update forever, with no Mercury process actually running.
"""
...
MARKER_NAME = ".mercury-update-in-progress"

# Set by an orchestrating updater (the Tauri `mercury-setup --update` flow) to
# its own pid before spawning `mercury update` as a child stage. The parent
...
# to show "Mercury is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Mercury home (never the context-local profile override):
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``mercury update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``mercury-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Mercury update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `mercury update` as a stage): we run under ITS claim
