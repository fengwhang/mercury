"""Dependency-light venv recovery that runs BEFORE mercury_cli.main's imports.

The ``mercury`` console entry point is ``mercury_cli.main:main``.  Importing
``mercury_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``mercury_cli.env_loader``, ``yaml`` via ``mercury_cli.config``, ...).  In
...
a corrupted venv.  ``mercury_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``mercury_cli.main`` to become
...
#
# ``mercury update`` renames the live ``mercury*.exe`` shims aside
# (``mercury.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``mercury`` on PATH, and the command that would repair it IS ``mercury
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY mercury
    invocation, and ``mercury acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Mercury already vendors
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``mercury acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``mercury_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``mercury_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``mercury update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `mercury update`: CLI and Desktop retries preserve
...
# every `mercury` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``mercury update`` (or the self-lock preflight, #83569) left the
    ...
    ``mercury_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``mercury acp`` included).
    ...
    """
    try:
        from mercury_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; mercury will keep working from "
