...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from mercury_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``mercury -z "..."`` / ``-q``), the larger
    ...
    try:
        from mercury_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``mercury dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``mercury -z``) can construct
