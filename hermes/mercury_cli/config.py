"""
Configuration management for Mercury.
...
- mercury config get      - Print a resolved configuration value
- mercury config set      - Set a specific value
- mercury config unset    - Remove a user configuration value
...
from mercury_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``mercury config set`` (which rewrites
    ...
    ``config.yaml`` in place: mercury never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.mercury/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``mercury logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (mercury config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  mercury config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Mercury makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Mercury itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``mercury update``, the TUI build.
...
#   on every plugin install / ``mercury update``.
...
#   ``HERMES_ENV`` — Mercury runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Mercury security policy / approval-routing context. These remain available
...
from mercury_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
_NIX_UPDATE_MSG = (
    "Update Mercury through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``mercury_cli/`` — i.e. the git checkout for source
    installs, ``/opt/mercury`` inside the published image. It is a property of
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Mercury was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.mercury``)
    --------------------------------------------------------------------------
    ...
    bind-mount it (``~/.mercury:/opt/data``) so config/sessions/memory persist
    ...
    ``docker`` and ``mercury update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/mercury-agent`` image bakes a ``docker``
        stamp into ``/opt/mercury`` at build time.
    ...
    #    container, and honouring it wrongly blocks ``mercury update``.
    try:
        method = (
...
def _running_in_container() -> bool:
    """Thin wrapper around ``mercury_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    ``/opt/mercury`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``mercury update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# mercury_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Mercury, it doesn't update
...
✗ ``mercury update`` doesn't apply inside the Docker container.

Mercury runs as a published image (nousresearch/mercury-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/mercury-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate mercury-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/mercury-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/mercury-agent/tags
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``mercury update`` inside Docker.
...
from mercury_constants import get_hermes_home, get_process_hermes_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path.

    MERCURY-OMP PATCH (unified config): under MERCURY_CONFIG the ONE Mercury
    file (~/.mercury/config.yaml) is mercury' config; the loader extracts the
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Mercury commonly set these to map the in-container
...
def ensure_hermes_home():
    ...
    # Named profiles must be created explicitly (e.g. ``mercury profile create``).
...
from mercury_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``mercury config set`` uses ``.`` as the nesting separator, so a key that
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `mercury config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `mercury update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Mercury' own config writer has historically emitted it
    ...
    # Mercury (``_save_discovered_models_to_config``), not hand-curated.
    # Older Mercury versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Mercury versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``mercury_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (mercury_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (mercury_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (mercury_cli/setup.py)
    "known_plugin_toolsets", # written/read by mercury_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (mercury_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, mercury send) so
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "mercury update / mercury -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from mercury_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ mercury config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in mercury_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from mercury_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from mercury_cli.toolset_validation import validate_platform_toolsets
        from mercury_cli.toolset_scope import toolset_allowed_for_platform
    ...
    # `mercury update`, but only the version bump is persisted.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.mercury/.env); "
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``MercuryCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``mercury config set``
    ...
    backends) — while display paths (``mercury status``/``dump``) read ``name``
...
# Back-compat alias — canonical set lives in mercury_cli.personality.
from mercury_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`mercury_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from mercury_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from mercury_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    Delegates to :mod:`mercury_cli.personality` (single owner).
    """
    from mercury_cli.personality import resolve_ephemeral_system_prompt
...
def read_raw_config() -> Dict[str, Any]:
    ...
    with _CONFIG_LOCK:
        ...
        # MERCURY-OMP PATCH (unified config): raw reads of the Mercury file
        # return the ``mercury:`` subtree so save-path logic (explicit paths,
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Mercury host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/mercury/config.yaml invalidates the
...
def save_config(
    ...
    *,
    ...
    with _CONFIG_LOCK:
        ...
        # MERCURY-OMP PATCH (unified config): under MERCURY_CONFIG, mercury'
        # normalized config nests under ``mercury:`` in the ONE Mercury file,
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Mercury writes itself."""
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`mercury tools`, `mercury setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `mercury tools`
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.mercury/.env`` over ``os.environ``.

    Used for Mercury-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│             🌡️ Mercury Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'mercury doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from mercury_cli.personality import active_personality_name
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `mercury cron list`, then pin the "
        "intended values with `mercury cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (mercury_cli/plugins_cmd.py, mercury_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `mercury config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``mercury config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``mercury config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``mercury config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   mercury config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   mercury config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``mercury config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``mercury config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    mercury config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    mercury config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    try:
    ...
    # so a fresh `mercury config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `mercury config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `mercury config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: mercury config get <key> [--json]")
            print()
            print("Examples:")
            print("  mercury config get model")
            print("  mercury config get terminal.backend")
            print("  mercury config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: mercury config set [--force] <key> <value>")
            print()
            print("Examples:")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: mercury config unset <key>")
            print()
            print("Examples:")
            print("  mercury config unset model")
            print("  mercury config unset terminal.backend")
            print("  mercury config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        print("  mercury config get <key>          Print a resolved config value")
        print("  mercury config set <key> <value>   Set a config value")
        print("  mercury config unset <key>        Remove a config value")
