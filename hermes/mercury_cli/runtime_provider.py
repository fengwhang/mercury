...
from mercury_cli import config as _config_mod
from mercury_cli.providers import custom_provider_aliases, custom_provider_slug
from mercury_constants import OPENROUTER_BASE_URL
from mercury_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`mercury_cli.config.load_config`.

    Deliberately NOT a module-level ``from mercury_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``mercury_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``mercury_cli.config.load_config`` OR
    ``mercury_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from mercury_cli.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # host_mandated_api_mode clause in mercury_cli/providers.py so the
...
_VALID_API_MODES = {
    ...
    # instead of Mercury' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from mercury_cli.config import _canonical_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from mercury_cli.models import azure_foundry_model_api_mode
        ...
        if configured_provider == provider and pool_url_is_default:
            ...
            if cfg_base_url:
                base_url = cfg_base_url
        configured_mode = _parse_api_mode(model_cfg.get("api_mode"))
        from mercury_cli.models import opencode_provider_family
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/mercury_cli/test_runtime_provider_resolution.py
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'mercury model' or "
...
def resolve_runtime_provider(
    *,
    ...
    from mercury_cli.config import is_provider_enabled, load_config
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from mercury_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Mercury canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Mercury-compatible importers).
