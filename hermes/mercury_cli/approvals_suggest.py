"""``mercury approvals suggest`` — mine approval history into allowlist proposals.

Mercury has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.mercury/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, mercury self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/mercury home prefixes and collapse whitespace.
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  mercury approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.mercury/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``mercury approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``mercury approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from mercury_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: mercury approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `mercury approvals <subcommand> -h` for details."
    )
    return 1
