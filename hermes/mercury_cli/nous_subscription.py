...
# coverage category (mercury_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `mercury tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``mercury status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Mercury-managed Node dirs (Windows installer / POSIX $HERMES_HOME/node)
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from mercury_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`mercury tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``mercury tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``mercury model``'s Nous
    login, this:
