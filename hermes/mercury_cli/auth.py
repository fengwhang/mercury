"""
Multi-provider authentication system for Mercury.
...
# httpx is imported lazily: it costs ~30ms at import time and mercury_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("mercury_cli.auth.httpx.Client", ...) keeps working in tests.
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from mercury_cli import __version__ as _HERMES_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _HERMES_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"mercury-cli/{_HERMES_CLI_VERSION}"
...
OAUTH_OVER_SSH_DOCS_URL = "https://mercury-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Mercury' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Mercury finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `mercury model` / `/model free`.
        api_key_env_vars=(),
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.mercury/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.mercury/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``mercury auth``.
    """
    return (
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from mercury_cli.nous_account import (
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".mercury" / "auth.json"
            try:
...
def _file_lock(
    ...
    # A stress test with 20 concurrent Mercury processes reproduced this
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # mercury-agent install tree (#25821, #93050).
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``mercury auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``mercury auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``mercury auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Mercury
    ...
    # the user initiated inside Mercury (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "mercury_pkce", "manual"}
    ...
    # explicit for users who never set Mercury up for it. Only Mercury-scoped
...
def resolve_provider(
    ...
    *,
    ...
    # Auto-detect an OpenRouter credential added via `mercury auth add openrouter`
    ...
    # `mercury auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.mercury/.env isn't silently
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    pointing Mercury at a non-production Nous Portal (e.g. a hosted agent
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: mercury auth add nous",
        provider="nous",
        code=reason,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `mercury model` walks users into a broken Qwen setup flow.
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Mercury is waiting for the OAuth callback on {redirect_uri}")
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``mercury setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``mercury auth add openai-codex``
      ...
      ``mercury auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    with _auth_store_lock():
        ...
        # ``mercury auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Mercury auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    if response.status_code == 429:
        ...
        # "run mercury auth" prompt (see issue #32790).
...
def _refresh_codex_auth_tokens(
    ...
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Mercury keeps its OWN
        ...
        # the Codex CLI (or another Mercury process) rotates the shared token,
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Mercury persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.mercury/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `mercury model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `mercury model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `mercury model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `mercury model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.mercury/auth.json even when HERMES_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``mercury auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.mercury/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `mercury model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `mercury model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Mercury (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `mercury model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``mercury model``
...
# so a new `mercury --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${HERMES_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<mercury-root>/shared/nous_auth.json`` where ``<mercury-root>`` is what
# ``get_default_hermes_root()`` returns — ``~/.mercury`` on Linux/macOS,
# ``%LOCALAPPDATA%\mercury`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's HERMES_HOME so named profiles (which
# typically live under ``<mercury-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    ``<mercury-root>/shared/``, where ``<mercury-root>`` is what
    :func:`mercury_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.mercury/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\mercury\\shared\\``, and
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Mercury root, refuse rather than silently corrupt cross-profile
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Mercury
...
def persist_nous_credentials(
    ...
    *,
    ...
    # these credentials via `mercury auth add nous --type oauth`. Best-
...
# ~350ms even on the failure path, and read-only UI surfaces (`mercury tools`, status panels,
# subscription-feature checks) call it many times per render — `mercury tools` → "All Platforms"
...
# `mercury auth login/logout/add/remove` invalidate naturally on the next call.
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``mercury doctor`` runs the live
    ...
    try:
        from mercury_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Mercury' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `mercury doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from mercury_cli.copilot_auth import (
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from mercury_cli.config import clear_model_endpoint_credentials
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`mercury_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (mercury_cli.model_selection_guards): the cost
    ...
    try:
        ...
        from mercury_cli.model_search import model_search_text
    ...
    from mercury_cli.curses_ui import format_radio_item_ansi
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Mercury auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Mercury creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``mercury auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``mercury auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Mercury auth store (~/.mercury/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Mercury login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `mercury model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``mercury status`` that just want to know
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Mercury builds still see
