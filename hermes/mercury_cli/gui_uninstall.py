"""
Mercury Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``mercury desktop`` / ``mercury gui``)
     Built inside the agent checkout under ``$HERMES_HOME/mercury-agent/``:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Mercury.app`` or ``~/Applications/Mercury.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Mercury`` (NSIS per-user)
...
the app name ("Mercury"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Mercury``
  - Windows: ``%APPDATA%\\Mercury``
  - Linux:   ``$XDG_CONFIG_HOME/Mercury`` (default ``~/.config/Mercury``)
...
``mercury uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(mercury_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return mercury_home / "mercury-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Mercury"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Mercury"
...
def source_built_gui_artifacts(mercury_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``mercury desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``mercury-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(mercury_home)
    ...
    return [
        ...
        mercury_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Mercury" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Mercury.app"),
            home / "Applications" / "Mercury.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Mercury).
            local_base / "Programs" / "Mercury",
            # Older / alternate layout some builds used.
            local_base / "mercury-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Mercury")
    else:
        ...
        # ``uninstall_gui``.
        from mercury_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `mercury desktop` installs. Its icon is
            ...
            data_base / "applications" / "Mercury.desktop",
            data_base / "icons" / "hicolor" / "scalable" / "apps" / "mercury.png",
        ]
        ...
        for size in ("256x256", "512x512", "1024x1024"):
            paths.append(data_base / "icons" / "hicolor" / size / "apps" / "mercury.png")
    return paths


def agent_is_installed(mercury_home: Path) -> bool:
    ...
    if (agent_root / "mercury_cli").is_dir():
        return True
...
def gui_is_installed(mercury_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(mercury_home):
        if p.exists():
            return True
...
def gui_install_summary(mercury_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = mercury_home if mercury_home is not None else get_hermes_home()
...
def uninstall_gui(
    mercury_home: "Path | None" = None, *, remove_userdata: bool = True
) -> "list[Path]":
    ...
    Never touches ``mercury-agent/mercury_cli`` (agent source), ``venv/``, or any
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Mercury disappears from the launcher.
        try:
            from mercury_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove mercury' or "
            "'sudo dnf remove mercury'). AppImage builds are a single file you "
