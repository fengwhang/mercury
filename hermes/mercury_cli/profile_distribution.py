"""Profile distributions — shareable, packaged Mercury profiles via git.

A distribution is a Mercury profile published as a git repository (or
...
* ``mercury profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``mercury skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``mercury profile``, not a parallel tree):

    mercury profile install <source> [--name N] [--alias] [--force] [--yes]
    mercury profile update  <name>  [--force-config] [--yes]
    mercury profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "mercury_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".mercury_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "mercury-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            mercury_requires=str(data.get("mercury_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.mercury_requires:
            out["mercury_requires"] = self.mercury_requires
        if self.author:
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Mercury {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Mercury distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``mercury profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Mercury profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Mercury distribution."
        )

    # Version check up-front so we fail fast
    check_hermes_requires(manifest.mercury_requires, mercury_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.mercury).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="mercury_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `mercury profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`mercury profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="mercury_dist_update_") as tmp:
        plan = plan_install(
