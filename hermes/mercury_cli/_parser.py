"""
Top-level argparse construction for the mercury CLI.
...
# tests/mercury_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``mercury --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``mercury_cli.relaunch`` should carry over when
...
Examples:
    ...
    mercury --tui                  Launch the modern TUI (or set display.interface: tui)
    mercury --cli                  Force the classic REPL (overrides display.interface: tui)
    ...
    mercury --resume latest        Resume the most recent session (same as -c)
    mercury --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    ...
    mercury fallback [list]        Show fallback provider chain
    mercury fallback add           Add a fallback provider (same picker as `mercury model`)
    mercury fallback remove        Remove a fallback provider from the chain
    ...
    mercury dashboard --stop       Stop running dashboard processes
    mercury dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="mercury",
        description="Mercury - AI assistant with tool-calling capabilities",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "under model.provider — use `mercury setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Mercury",
    )
    ...
    # top-level parser: when the user writes `mercury -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/mercury_cli/
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Mercury itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
