...
Design notes / invariants (same contract as ``mercury_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.MercuryCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    ``mercury_cli/heartbeat.py``). The delegation also inherits the
    ...
    """
    try:
        from mercury_cli.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from mercury_cli.goals import _warn_dropped_write
...
class LoopManager:
    ...
    def complete_tick(self, last_response: str) -> Dict[str, Any]:
        ...
        if s.until and (last_response or "").strip():
            try:
                from mercury_cli.goals import judge_goal
