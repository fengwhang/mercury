...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Mercury wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Mercury.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Mercury versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``mercury model`` wizard — not a
    ...
    while ``mercury model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Mercury itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Mercury itself discovered (``models_discovered: true``
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Mercury. They lack the tool-calling capabilities "
...
#   NousResearch/Mercury-3-Llama-3.1-70B, mercury-4-405b, openrouter/hermes3:70b
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``mercury config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from mercury_cli.model_switch import
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from mercury_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``mercury_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from mercury_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from mercury_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from mercury_cli.config import load_config
            from mercury_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from mercury_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from mercury_cli.models import should_use_ollama_native_catalog
    ...
    # mercury_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from mercury_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from mercury_cli.models import normalize_opencode_base_url
    ...
    mercury_warn = _check_hermes_model_warning(new_model)
    if mercury_warn:
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from mercury_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Mercury provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from mercury_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from mercury_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from mercury_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from mercury_cli.providers import HERMES_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from mercury_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Mercury-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for mercury_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(mercury_id)
        if (
            _alias_target
            and _alias_target != mercury_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        _canonical = mercury_id
        try:
            from providers import get_provider_profile as _gpp
            _prof = _gpp(mercury_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != mercury_id:
            continue
        if mercury_id.lower() in seen:
            continue
        if mercury_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from mercury_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(mercury_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(mercury_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        mercury_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(mercury_id)
            seen.add(mercury_id.lower())

    # --- Section 2: Mercury-only providers (HERMES_OVERLAYS) ---
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in HERMES_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        mercury_slug = _mdev_to_hermes.get(pid, pid)
        if mercury_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or mercury_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or mercury_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(mercury_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(mercury_slug)
            seen.add(pid.lower())
            seen.add(mercury_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    from mercury_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against mercury_id / mdev_id (section 1), pid / mercury_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from mercury_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://mercury-agent.nousresearch.com/docs/api/model-catalog.json so
    ...
    if "lmstudio" not in curated and (
        ...
        from mercury_cli.models import fetch_lmstudio_models
    ...
    for mercury_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip mercury_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose mercury_id matches the canonical profile name so
        ...
        # same mercury_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if mercury_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Mercury may not route yet.
        ...
        # /model picker sees the SAME list `mercury model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(mercury_id)
        if not model_ids:
        ...
        results.append({
            ...
            "is_current": (
                ...
                or mercury_id == current_provider
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if mercury_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or mercury_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(mercury_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or mercury_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if mercury_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(mercury_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(mercury_slug)
                model_ids = _ids if _ids else (curated.get(mercury_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(mercury_slug, []) or curated.get(pid, [])
        elif mercury_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `mercury model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from mercury_cli.auth import get_provider_auth_state as _nous_state
        ...
        total = len(model_ids)
        if mercury_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        _record_builtin_endpoint(mercury_slug)
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Mercury writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Mercury's own writer uses (mirrors section-4 grouping).
            ...
            # ``mercury model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``mercury model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from mercury_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``mercury model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from mercury_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`mercury_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from mercury_cli.models import fetch_openrouter_models
