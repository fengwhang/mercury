...
Usage::
    ...
    mercury profile create coder --clone  # also copy config, .env, SOUL.md, skills
...
from mercury_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See mercury_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.mercury``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   mercury-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `mercury backup` archives
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Mercury infrastructure directories that only the default profile
         (``~/.mercury``) ever contains.  Gated on ``source_dir`` actually
...
# into the export. The set below identifies the *known Mercury profile
...
def _get_default_hermes_home() -> Path:
    ...
    In Docker/custom deployments where HERMES_HOME is outside ``~/.mercury``
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``mercury``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``mercury`` profile inside ``~/.mercury/``) or get refused
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Mercury installation itself or a common system binary.  "
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Mercury home
    (:func:`mercury_constants.get_hermes_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, mercury subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a mercury subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Mercury. If we
    ...
    try:
        ...
        # not be migrated. The next `mercury doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``mercury -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``mercury profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``mercury -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``mercury profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        # Creation must not fail over this; `mercury model` still sets it later.
        pass
...
# ``config.yaml``. ``config.yaml`` is the user-facing Mercury config
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, mercury_home)`` pairs a gateway should serve.
    ...
    The returned ``mercury_home`` is the path to pass to
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``mercury update`` skips re-seeding this profile's
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Mercury profile.\n"
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`mercury profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `mercury update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `mercury profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `mercury -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``mercury profile create
    ...
    only the essential skills (e.g. ``mercury-agent``), reporting
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Mercury *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Mercury backend subcommand
    ...
    # Never terminate ourselves or a parent (e.g. `mercury -p <canon> profile
    ...
    mercury_markers = ("mercury_cli.main", "mercury-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('mercury')` resolution) spawns the
    ...
    # interpreter, not "mercury", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _HERMES_CONSOLE_SCRIPT_NAMES = frozenset({"mercury", "mercury-agent", "mercury-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Mercury process: either an entrypoint marker in argv, a
            # resolved executable named `mercury`, or a python interpreter
            # directly exec'ing a `mercury`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_hermes = (
                any(marker in joined for marker in mercury_markers)
                or exe_name == "mercury"
                or exe_name.startswith("mercury")
            )
            if not is_hermes and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                ...
                # (pyproject.toml [project.scripts]: mercury, mercury-agent,
                # mercury-acp) rather than a bare `startswith("mercury")` --
                ...
                # "mercury-notes.py" or "mercury-unrelated-tool" as the shim,
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from mercury_cli.service_manager import get_service_manager
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: mercury profile create {canon}"
        )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      first and proved unable to anticipate every non-Mercury file the
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``mercury sessions export --redact``. Runs on the *staged copy only* so the
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    it to match Mercury-owned PYTHONPATH entries written in the same
