"""Runtime-backed validation behind ``mercury plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    try:
        ...
        # removes it instead of stranding a mercury-plugin-doctor-* directory
        # until interpreter GC (or never, on a hard exit).
        temporary_home = stack.enter_context(
            tempfile.TemporaryDirectory(prefix="mercury-plugin-doctor-")
        )
    ...
    modules_before = {
        ...
        if name == "mercury_plugins" or name.startswith("mercury_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Mercury discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "mercury_plugins" or name.startswith("mercury_plugins."))
            ):
...
def _holds_plugin(path: Path) -> bool:
    ...
    confusing error: ``mercury plugins doctor`` with no argument defaults to
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        ...
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".mercury" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from mercury_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Mercury supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Mercury never auto-installs plugin dependencies; "
    ...
    if schema:
        from mercury_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Mercury' real scanner and registration path."""
    try:
