"""`mercury checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.mercury/checkpoints/``.  Actions:

    mercury checkpoints               # same as `status`
    mercury checkpoints status        # total size, project count, breakdown
    mercury checkpoints list          # per-project checkpoint counts + workdir
    mercury checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    mercury checkpoints clear [-f]    # nuke the entire base (asks first)
    mercury checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    mercury checkpoints
    mercury checkpoints prune --retention-days 3 --max-size-mb 200
    mercury checkpoints clear -f
...
from mercury_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: mercury checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``mercury checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `mercury checkpoints` → status
