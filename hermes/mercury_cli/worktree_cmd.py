"""``mercury worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``mercury_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    mercury worktree list             # audit: verdict + reason per tree
    mercury worktree prune            # reap safe trees + merged branches
    mercury worktree prune --dry-run  # show the plan, change nothing
    mercury worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `mercury worktree prune`."
        )
