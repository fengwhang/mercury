"""Safe Mercury Console command engine.

This module backs ``mercury console`` and is intentionally narrower than the
full Mercury CLI. It exposes a curated set of native adapters that can later be
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'mercury doctor'")
        and last.startswith("Run 'mercury setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from mercury_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="mercury", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `mercury "):
        return ""
    return summary
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("mercury_cli.main")
    builder = getattr(module, builder_name)
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `mercury {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `mercury {usage}`?",
        )


class HermesConsoleEngine:
    """Curated line-command executor for Mercury Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "mercury":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Mercury Console does not run shell syntax. Use one supported "
                    "Mercury command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Mercury component status.", _status)
        self.register(("version",), "version", "Show Mercury version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Mercury logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Mercury configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Mercury Console."""

        extracted = {
            "dump": (
                "mercury_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "prompt-size": (
                "mercury_cli.subcommands.prompt_size",
            ...
            "insights": (
                "mercury_cli.subcommands.insights",
            ...
            "security": (
                "mercury_cli.subcommands.security",
            ...
            "backup": (
                "mercury_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "mercury_cli.subcommands.import_cmd",
            ...
            "config": (
                "mercury_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "plugins": (
                "mercury_cli.subcommands.plugins",
            ...
            "skills": (
                "mercury_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "mercury_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "mercury_cli.subcommands.pairing",
            ...
            "webhook": (
                "mercury_cli.subcommands.webhook",
            ...
            "profile": (
                "mercury_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Mercury configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "mercury_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("mercury_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "mercury_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("mercury_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
            ...
            handler_factory=lambda fixed: _builder_handler(
                "project",
                fixed,
                "mercury_cli.projects_cmd",
                "build_parser",
                "cmd_project",
            ),
        )

        # MERCURY-OMP PATCH (B2): kanban command family removed from the
        # Mercury distribution (delegation is omp).
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Mercury Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Mercury Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`mercury {first}` is not available in Mercury Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Mercury Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Mercury Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Mercury Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Mercury Console.",
            ("skills", "publish"): "`skills publish` is not available in Mercury Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Mercury Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Mercury Console.",
            # MERCURY-OMP PATCH (B2): kanban unavailable-subcommand notes
            # removed along with the command family (delegation is omp).
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Mercury Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Mercury Console.")
...
def _version(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mercury_cli._startup_fast import print_fast_version_info
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Mercury Console.")
    ...
    from mercury_cli.logs import list_logs, tail_log
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mercury_cli.config import show_config
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from mercury_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="mercury_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from mercury_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mercury_cli.cron import cron_status
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from mercury console")
...
def run_console_repl(
    *,
    ...
    """Run the local ``mercury console`` REPL."""
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Mercury Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("mercury> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
