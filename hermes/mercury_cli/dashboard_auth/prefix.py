...
prefix (e.g. ``mission-control.tilos.com/mercury/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /mercury`` so the backend can
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``mercury.example.com`` instead of
    ``https://mercury.example.com``) — the single most common cause of
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "mercury.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/mercury"`` (no trailing slash) or ``""``
