...
from mercury_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      2. ``X-Forwarded-Prefix: /mercury`` (Mission Control deploys) — we
...
def _prefix(request: Request) -> str:
    ...
    ``mercury_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from mercury_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("mercury_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from mercury_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from mercury_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from mercury_cli.dashboard_auth import list_session_providers
    from mercury_cli.dashboard_auth.base import RefreshExpiredError
