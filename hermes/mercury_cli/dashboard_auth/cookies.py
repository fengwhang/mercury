...
Three cookies in play:
  - mercury_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - mercury_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Nous Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / mercury
  ...
  - mercury_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/mercury) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/mercury`` covers
...
setter wrote ``__Secure-mercury_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "mercury_session_at"
SESSION_RT_COOKIE = "mercury_session_rt"
...
SESSION_PROVIDER_COOKIE = "mercury_session_provider"
PKCE_COOKIE = "mercury_session_pkce"
...
SSO_ATTEMPT_COOKIE = "mercury_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /mercury`` we want ``Path=/mercury`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (mercury #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/mercury``)
...
def set_pkce_cookie(
    ...
    # showed the browser sending the intact quoted cookie while Mercury
