...
This module is imported by ``mercury_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no mercury_cli.config,
...
``mercury_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(mercury_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect HERMES_HOME?"""
    active_profile = os.path.join(mercury_root, "active_profile")
    try:
...
def container_mode_may_be_active() -> bool:
    ...
    mercury_home = os.environ.get("HERMES_HOME", "").strip()
    if mercury_home:
        if os.path.exists(os.path.join(mercury_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(mercury_home)))
        return (
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``mercury --version`` output (also used by /version).
    ...
    ``mercury version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from mercury_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from mercury_cli import __release_date__, __version__

        print(f"Mercury v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from mercury_cli.config import detect_install_method
    ...
    try:
        from mercury_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``mercury --version`` before the heavy import wall.
