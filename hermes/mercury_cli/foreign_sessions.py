...
role-alternation invariant Mercury enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Mercury SessionDB.

    Returns the new Mercury session id.  The foreign file is only read.
...
def pick_foreign_session(
    ...
    if not sys.stdin.isatty():
        print(
            ...
            "  mercury sessions import --from claude|codex <path>"
        )
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`mercury sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  {cli_invocation_name()} --resume {session_id}")
    return session_id
