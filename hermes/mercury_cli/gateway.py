...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.mercury.gateway*`` LaunchAgent — the update path needs the whole
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # mirrors the systemd branch's ``mercury-gateway*`` pattern above.
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.mercury.gateway*
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "mercury CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `mercury update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``mercury gateway status`` from falsely counting the ``mercury`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``mercury gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"mercury_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "mercury_home=" in command_lc
            and f"mercury_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``mercury update`` hung exactly there
            ...
            # watchdog probe.
            from mercury_cli._subprocess_compat import bounded_probe_run
...
def find_profile_gateway_processes(
    ...
    *,
    ...
    """Return running gateway PIDs mapped to Mercury profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def find_windows_gateway_services(
    *,
    ...
    validated profile gateway PID comes from Mercury's own PID file, and its
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m mercury_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``mercury update`` users
    # with no running gateway until they re-invoke ``mercury gateway``
    ...
    from mercury_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from mercury_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import mercury_cli.  Empty on POSIX, where
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_hermes_home` falls back to ``/root/.mercury`` — the wrong
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}mercury gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from mercury_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: mercury gateway stop")
    else:
        print(
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``mercury gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from mercury_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (mercury_cli/gateway.py restart
    ...
    if is_windows():
        try:
            ...
            # dormant on every standard `mercury gateway install` deployment.
            from mercury_cli.gateway_windows import get_task_name
    ...
    try:
        ...
        # whole ai.mercury.gateway* fleet — not just the current profile's
    ...
    # healthy standalone gateway (no service supervisor — e.g. `mercury
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Mercury Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``mercury gateway run`` must remain interruptible from
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from mercury_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
SERVICE_DESCRIPTION = "Mercury Gateway - Messaging Platform Integration"
...
def _profile_arg(mercury_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        mercury_home: Optional explicit HERMES_HOME path. Defaults to the current
            ``get_hermes_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Mercury root to compare against. Used when
    ...
    home = Path(mercury_home or str(get_hermes_home())).resolve()
...
def _profile_arg_for_target_user(mercury_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".mercury"
    try:
        Path(mercury_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(mercury_home, default_root=target_root)
    except ValueError:
        return _profile_arg(mercury_home)
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: mercury gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``mercury setup`` (end of wizard)
    and ``mercury import``: if this host supports a service manager and no
    ...
    ``mercury gateway install --system`` explicitly (that path prompts and
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: mercury gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: mercury gateway install")
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.mercury.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.mercury.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # mercury_constants cached from before this symbol existed. See
            # _reload_mercury_constants() in mercury_cli/managed_uv.py.
            from mercury_cli.managed_uv import _reload_mercury_constants

            venv_python_path = _reload_mercury_constants().venv_python_path
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    current_default = Path.home() / ".mercury"
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    mercury_node = mercury_home / "node" / "bin"
    if _is_dir(mercury_node):
        candidates.append(str(mercury_node))
    mercury_nm = mercury_home / "node_modules" / ".bin"
    if _is_dir(mercury_nm):
        candidates.append(str(mercury_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m mercury_cli.main``, so module
    ...
    dir, or a clone that ``mercury update`` later relocates/removes — the path
...
def _systemd_watchdog_seconds(mercury_home: str | Path | None = None) -> int:
    ...
    if mercury_home is not None:
        ...
        override_token = set_hermes_home_override(mercury_home)
...
def _systemd_watchdog_service_fields(
    ...
    seconds = _systemd_watchdog_seconds(mercury_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], mercury_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Mercury-managed Node under ``$HERMES_HOME/node`` goes first when it
    ...
    *mercury_root* is the Mercury home the unit will run against. System units
    ...
    """
    from mercury_constants import (
        mercury_managed_node_tree_present,
        iter_hermes_node_dirs,
    )

    managed_node_present = mercury_managed_node_tree_present(mercury_root)
    for directory in iter_hermes_node_dirs(mercury_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target Mercury home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Mercury home is known — probing here would stat the calling
    ...
    if system:
        ...
        profile_arg = _profile_arg_for_target_user(mercury_home, home_dir)
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(mercury_home) if mercury_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m mercury_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    mercury_home = str(get_hermes_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        mercury_home
    )
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m mercury_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="HERMES_HOME={mercury_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    #
    # Under ``sudo mercury gateway … --system``, HERMES_HOME is often stripped
    # and falls back to ``/root/.mercury``. Adopting the unit's pinned home
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``mercury gateway restart``
    with ``HERMES_HOME=/tmp/mercury-e2e-<pr>`` exported; the restart path's
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../mercury_test`` path into
    ...
    if not system and (
        ...
        or '/mercury_test"' in new_unit
        or "/mercury_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # HERMES_HOME into the unit (manual E2E homes like /tmp/mercury-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Mercury install"
    )
    return True
...
def systemd_install(
    ...
    # ``sudo mercury gateway install --system --force`` would bake /root/.mercury
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}mercury gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}mercury gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `mercury gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `mercury gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}mercury gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}mercury gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}mercury gateway start{scope_flag}"
        )
...
# process invocation (each ``mercury gateway start/stop/status`` call).
...
#      background process (the `nohup mercury gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m mercury_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "mercury_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``mercury update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup mercury gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup mercury gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `mercury gateway start`)
    ...
    nofile_block = ""
    try:
        from mercury_cli.resource_limits import configured_nofile_soft_limit
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `mercury update`
...
def launchd_status(deep: bool = False):
    ...
    # Mercury PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Mercury PID file).
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: mercury gateway stop")
            else:
...
def named_profile_served_by_running_multiplexer() -> bool:
    ...
    try:
        ...
        from mercury_cli.profiles import normalize_profile_name
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``mercury gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``mercury gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``mercury gateway run`` can burn memory/CPU just to fail with "already
    ...
    if pid is None:
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                stale_home = stale.get("mercury_home", "<unknown>")
                logger.warning(
                    "PID file belongs to another profile (mercury_home=%s). "
    ...
    print("  Use 'mercury gateway restart' to replace it,")
    print("  or 'mercury gateway stop' first.")
    print("  Or use 'mercury gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Mercury gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'mercury' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/mercury/docker/entrypoint.sh before the Mercury command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `mercury gateway run` still
    ...
    # /restart) rather than through `mercury gateway restart` which already
    ...
    # `mercury gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│          🌡️ Mercury Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``mercury gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Mercury will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run mercury under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.mercury/plugins/ still require
...
def _setup_standard_platform(platform: dict):
    ...
    from mercury_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from mercury_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: mercury pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'mercury gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print_info(
        "  3. Mercury will store the returned account_id/token in ~/.mercury/.env."
    )
    print_info(
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `mercury pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `mercury pairing approve`."
        )
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``mercury_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from mercury_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        print_info(f"  Set these env vars in ~/.mercury/.env: {', '.join(required)}")
    else:
        print_info(
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo mercury gateway install --system"
                        )
                ...
                print_info(
                    "  For persistence:   tmux new -s mercury 'mercury gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: mercury gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup mercury gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`mercury_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from mercury_cli.service_manager import (
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``mercury gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``mercury gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    #
    # Prefer `sleep infinity` (matches the static main-mercury service's
    # pattern in docker/s6-rc.d/main-mercury/run, and frees the Python
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Consider running in foreground instead: mercury gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s mercury 'mercury gateway run'"
                )
                print()
            ...
            print()
            print(
                "  mercury gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s mercury 'mercury gateway run'         # persistent via tmux"
            )
            print(
                "  nohup mercury gateway run > ~/.mercury/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from mercury_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  mercury profile create <name>     # creates the s6 service slot")
                print("  mercury -p <name> gateway start   # bring it up via s6")
                print("  mercury status                    # see currently-supervised gateways")
                return
        ...
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  mercury profile delete <name>     # tears down the s6 service slot")
                print("  mercury -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `mercury -p <name> gateway
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup mercury gateway run > ~/.mercury/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s mercury 'mercury gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  mercury gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  mercury gateway install  # Install as user service")
                    print(
                        "  sudo mercury gateway install --system  # Install as boot-time system service"
                    )
