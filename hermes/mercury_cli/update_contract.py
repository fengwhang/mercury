...
``mercury update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/mercury/image-provenance.json``,
   written by the image build — see :mod:`mercury_cli.image_provenance`):
