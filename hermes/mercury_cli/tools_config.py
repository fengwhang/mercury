"""
Unified tool configuration for Mercury.
...
from mercury_cli.nous_account import format_nous_portal_entitlement_message
from mercury_cli.toolset_scope import (
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``mercury tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`mercury_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
# who want it opt in via `mercury tools` → Video Generation, which walks
...
# `mercury tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from mercury_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from mercury_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``mercury tools`` checklist actually offers
    for ``platform``.
    ...
    so ``mercury tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Mercury routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "tag": "Zig headless browser spawned by Mercury, text-only (no screenshots)",
                "env_vars": [],
                "browser_provider": "local",
...
# provider+model picker like `mercury model` — NOT this single-key prompt, so
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Mercury' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # misses the uv Mercury installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``mercury update`` and
      by ``mercury computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``mercury update`` sets
    ...
    a further ~600s wait on Windows). ``mercury computer-use install
    ...
    ``mercury update`` already prints a contextual line before its update
    ...
    ``mercury update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `mercury update` calls this
    ...
    # An installed driver that fails Mercury' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Mercury' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `mercury update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Mercury cannot use its current "
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "mercury computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `mercury computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `mercury update` (require_confirmed_update=True) keeps the
    ...
    # `mercury computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: mercury computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: mercury computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``mercury update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `mercury update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``mercury update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `mercury update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `mercury update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.mercury/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Mercury process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Mercury-managed ``$HERMES_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Mercury installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key == "lightpanda":
        # Browser Use mode drives Lightpanda directly (Mercury spawns
        ...
        if lightpanda_bin:
            ...
            if os.name == "nt":
                _print_info("    Lightpanda has no native Windows build; run Mercury under WSL2.")
        return
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Mercury-managed install)
        ...
        # (PATH -> Homebrew/Mercury-managed node -> npx) rather than a bare
        # shutil.which — Mercury-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/mercury-agent:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Mercury-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Mercury-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.mercury/config.yaml")
            else:
        ...
        try:
            from mercury_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: mercury plugins enable observability/langfuse")
        _print_info("    Restart Mercury for tracing to take effect.")
        _print_info("    Verify: mercury plugins list")
        ...
        try:
            from mercury_cli.auth import get_xai_oauth_auth_status
        ...
        try:
            ...
            _print_info("    Run later: mercury auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `mercury auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: mercury auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: mercury auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``mercury tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``mercury tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: mercury tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``mercury-discord``, which contains those
...
#:
#: Saving ``mercury tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[mercury-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``mercury tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"mercury-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (mercury-acp, mercury-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``mercury-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[mercury-cli, a2a]`` must survive filtering just
    ...
    # "mercury-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[mercury-cli, spotify]`` after enabling Spotify via ``mercury
        ...
        # through ``mercury tools`` to flip the toolset on. Only fires when
    ...
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"mercury-{platform}"
    ...
    # JSON-array string (e.g. "['memory']") from `mercury config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `mercury` instead of
    # `mercury-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `mercury update`/`mercury doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `mercury tools` "
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `mercury tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``mercury tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``mercury tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from mercury_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``mercury tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from mercury_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (mercury_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `mercury model`).
    if managed_feature:
        from mercury_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``mercury model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``mercury model`` aux-task
    ...
    """
    try:
        from mercury_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `mercury model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `mercury model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("🌡️ Mercury Tool Configuration", Colors.CYAN, Colors.BOLD))
