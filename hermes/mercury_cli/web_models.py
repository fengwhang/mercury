"""Pydantic request/response models for the Mercury dashboard web server.

Extracted verbatim from ``mercury_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from mercury_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``mercury model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `mercury import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`mercury -p <name> skills install`)
