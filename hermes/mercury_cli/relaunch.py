"""
Unified self-relaunch for Mercury CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``mercury sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``mercury`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from mercury_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched mercury."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the mercury entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("mercury")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m mercury_cli.main``.
    ...
    ``mercury.exe`` on PATH or the ``sys.executable -m mercury_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with mercury.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh mercury invocation.
    ...
    relaunch contract wants: "run mercury again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``mercury.exe`` (a Python console-script shim that wraps
    ``python -m mercury_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "mercury exited, then
    new mercury started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``mercury`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nHermes relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run mercury.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
