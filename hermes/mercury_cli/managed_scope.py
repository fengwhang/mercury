...
A system-level directory (default ``/etc/mercury``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.mercury/config.yaml`` and ``~/.mercury/.env`` on a per-leaf-key basis.

This is DISTINCT from ``mercury_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/mercury")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/mercury`` during tests so a real
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      ...
      2. ``/etc/mercury`` — POSIX default, when it exists. Ignored under pytest so
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through mercury_cli.config.load_config) to honor managed
    scope. Mirrors mercury_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from mercury_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
