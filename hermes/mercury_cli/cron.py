...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Check status:  mercury cron status", Colors.DIM))
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `mercury cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    ``mercury cron incidents [--state <s>]`` lists incidents (the stored error
    is redacted and truncated at write time, safe for terminal display);
    ``mercury cron incidents ack <id>`` closes one so its failure ping stays
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(
                color(
                    "✗ Incident ID required: mercury cron incidents ack <incident_id>",
                    Colors.RED,
                )
            )
            return 1
...
def cron_status():
    ...
    if pids or gateway_alive_via_lock:
        ...
        if hb_age is None:
            ...
            print("  If the gateway just started, wait ~60s and re-run `mercury cron status`.")
            print("  If heartbeat never appears, restart: mercury gateway restart")
            ...
            if pids:
                print(f"  PID: {', '.join(map(str, pids))}")
            print("  Cron jobs may NOT be firing. Restart: mercury gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec mercury "
                        "mercury cron ...`). Fix ownership to match the "
...
def _scripts_dir_for_cron() -> Path:
    ...
    same Mercury home the jobs were loaded from.
    """
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `mercury cron doctor` again.", Colors.DIM))
    return 1
...
def cron_notepad(args) -> int:
    """Handle ``mercury cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: mercury cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: mercury cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: mercury cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: mercury cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
