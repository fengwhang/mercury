...
``mercury_cli.config.migrate_config``. Each step is a function
...
``mercury_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``mercury_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``mercury_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``mercury_cli.config`` (e.g.
``patch("mercury_cli.config.read_raw_config", ...)``) keep working, because
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hermes_home()}/config.yaml and run `mercury setup` to "
...
def _cfg():
    """Return the live ``mercury_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `mercury curator status` has no
    ...
    #   3. Creates `~/.mercury/logs/curator/` if missing (belt-and-suspenders
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `mercury config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `mercury config set`"
                )
...
# behavior set it to true explicitly via `mercury config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from mercury_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # provider surface (`video_generate`, `mercury tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `mercury tools` → "
