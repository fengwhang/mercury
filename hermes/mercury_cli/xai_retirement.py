...
Pure logic: walks a Mercury config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `mercury doctor` and a future `mercury migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Mercury config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Mercury config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `mercury migrate xai` reports nothing to migrate
