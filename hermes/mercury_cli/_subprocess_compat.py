...
Mercury is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `mercury
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Mercury' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Mercury shells out to git from many non-interactive contexts — MCP catalog
...
def _text_names_hermes(text: str) -> bool:
    """True when *text* names Mercury at a path-segment / token boundary.

    A bare ``"mercury" in text`` substring test would also match unrelated
    ...
    that *starts with* ``mercury`` (``mercury``, ``mercury.exe``, ``mercury_cli``,
    ``mercury-agent``, ``mercury-runtime``) or the hidden-dir form
    ``.mercury``/``.mercury-runtime``.
    """
    for token in re.split(r"[\\/\s=,;\"']+", text.lower()):
        if token.startswith("mercury") or token.startswith(".mercury"):
            return True
    return False


def _process_command_is_hermes(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Mercury code."""
    try:
        import psutil
...
def pid_is_hermes(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a Mercury process. When
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``mercury update`` on slow-WMI machines
