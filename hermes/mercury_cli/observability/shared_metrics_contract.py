"""Bounded product contract for the first Mercury shared-metrics slice."""
...
SCHEMA_KEY = "mercury.metrics.schema_version"
SCHEMA_VERSION = "mercury.metrics.event.v2"
MODEL_CALL_SCOPE = "mercury.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "mercury.task_run"
TOOL_CALL_SCOPE = "mercury.tool_call"
CLIENT_ACTIVE_MARK = "mercury.client.active"
TOOL_APPROVAL_MARK = "mercury.tool_approval"
SKILL_LIFECYCLE_MARK = "mercury.skill.lifecycle"
SKILL_LOAD_MARK = "mercury.skill.load"
SUBSCRIBER_NAME = "mercury.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "mercury.client.active"
LEGACY_MODEL_CALL_METRIC = "mercury.model_call.count"
MODEL_ROUTE_METRIC = "mercury.model_route.count"
TASK_STARTED_METRIC = "mercury.task_run.started"
TASK_FINISHED_METRIC = "mercury.task_run.finished"
TOOL_CALL_METRIC = "mercury.tool_call.count"
TOOL_APPROVAL_METRIC = "mercury.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "mercury.skill.lifecycle.count"
SKILL_LOAD_METRIC = "mercury.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Mercury installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(mercury_version or "").strip()
    ...
    return {
        ...
        "mercury_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("mercury_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Mercury logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("mercury.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from mercury_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Mercury terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Mercury registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "mercury-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Mercury tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Mercury."""
