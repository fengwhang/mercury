"""Mercury update pipeline — extracted from ``mercury_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``mercury_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``mercury_cli.main`` reference — so existing call sites and test monkeypatches
that target ``mercury_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
from mercury_constants import get_default_hermes_root, venv_python_path

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``mercury_cli.main`` reference.

    Lets callers keep patching ``mercury_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``mercury_cli.main`` -> ``mercury_cli.update_cmd`` stays one-way
    at import time.
    """
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "mercury_cli.mercury_logging",
    }
)


def _purge_stale_hermes_modules() -> None:
    """Evict every cached Mercury module after the checkout changed in-place.

    ``mercury update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from mercury_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``mercury_cli.gateway`` does
    ``from mercury_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Mercury
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Mercury module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Mercury modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``mercury update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``mercury update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``mercury_cli.config_defaults``,
    ``mercury_cli.config``, and ``mercury_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``mercury_cli._subprocess_compat`` and
    ``mercury_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "mercury_cli.config_defaults",
        "mercury_cli.config",
        "mercury_cli.config_migrations",
        "mercury_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from mercury_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from mercury_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``mercury update`` historically
    ...
    try:
        ...
        from mercury_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'mercury config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'mercury config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'mercury config migrate' manually to configure."
                )
                response = "n"
    ...
    # it and warn loudly.
    try:
        from mercury_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from mercury_cli.backup import restore_cron_jobs_all_profiles
...
# Critical files that Mercury must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "mercury_cli/main.py",
    "mercury_cli/config.py",
    "mercury_cli/__init__.py",
    "mercury_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "mercury_constants.py",
)
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``mercury.exe`` has
    ...
    Skipping is safe because Mercury pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="mercury-syntax-check-") as tmpdir:
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``mercury`` startup; if any of them
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``mercury update`` can be driven by a
    ...
    a later one.
    """
    from mercury_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from mercury_constants so this can't drift
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `mercury update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `mercury update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `mercury sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    mercury sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  mercury sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``mercury update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``mercury update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: mercury curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from mercury_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    mercury dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``mercury_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.mercury-update-staging"
    backup = f"{dst}.mercury-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `mercury update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``mercury_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``mercury update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && mercury update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== mercury-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Mercury launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["mercury.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Mercury")
    print("  Desktop, a gateway, or another mercury REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `mercury` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``mercury.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.mercury-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".mercury-update-staging", ".mercury-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `mercury update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Mercury process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok:
        ...
        if not desktop_build_ok:
            print("  Run `mercury desktop` to retry the desktop rebuild.")
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from mercury_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (mercury gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Mercury by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `mercury update "
            f"--branch {branch}`, or update against main with `mercury update`."
        )
        _m().sys.exit(1)
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for mercury-agent itself
        ...
        try:
            for item in entries:
                ...
                # Mercury.exe) exists only in the LIVE tree. Swapping `apps`
    ...
    from mercury_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from mercury_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `mercury update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: mercury skills list-modified  "
    ...
    try:
        from mercury_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from mercury_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
    ...
    try:
        from mercury_cli.update_receipt import finalize_update_receipt
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``mercury update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Mercury agent unexecutable.")
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Mercury couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Mercury couldn't drop "
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from mercury_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `mercury update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `mercury update` or `mercury gateway restart`.",
            file=stream,
        )
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``mercury update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: mercury gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "mercury.exe"
    lines = ["✗ Another mercury.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Mercury Desktop, exit any open `mercury` REPLs, and")
    lines.append("  stop the gateway (`mercury gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `mercury update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from mercury_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``mercury update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Mercury Desktop backend child, an unrelated ``mercury`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``mercury tools``."""
    try:
        from mercury_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Mercury Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``mercury tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Mercury Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Mercury Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``mercury update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `mercury update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `mercury update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Mercury' editable-install extras or
    ...
    try:
        from mercury_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from mercury_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from mercury_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `mercury update` is exactly the step
...
def _npm_lockfile_changed(mercury_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `mercury
    ...
    try:
        ...
        cache_file = mercury_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(mercury_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    mercury update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `mercury update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `mercury update`.")
            failed = []
            if any(
    ...
    # Mercury profile using this checkout. Keep one per-checkout cache under the
    # shared Mercury root rather than rerunning npm once per named profile.
    ...
    # `mercury update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `mercury update`.")
        return list(labels)
    ...
    # `mercury update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.mercury/logs/update.log`` only, never the terminal.

    During ``mercury update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``mercury update --check``: fetch and report without installing.
    ...
    from mercury_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from mercury_cli.banner import _github_compare_behind
            ...
            if counted is not None:
                ...
                print(f"🌡️ Update available: {counted} {commits_word} behind {compare_branch}.")
            else:
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``mercury update`` without requiring a reinstall.  The
    ...
    ``mercury`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/mercury.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v mercury'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/mercury, code at /usr/local/lib/mercury-agent).
    fhs_link = Path("/usr/local/bin/mercury")
    ...
    path_comment = (
        "# Mercury — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``mercury-acp`` launcher next to the ``mercury`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``mercury update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``mercury-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Mercury as not installed even when it is.

    The shim simply delegates to the sibling ``mercury`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``mercury`` / ``mercury-acp``
    ...
    ``mercury_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``mercury-acp`` is already present next to the
    ``mercury`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        mercury_cmd = bin_dir / "mercury"
        acp_cmd = bin_dir / "mercury-acp"
        try:
            if not (mercury_cmd.is_file() or mercury_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of HERMES_HOME under
      ``backups/`` (restorable via ``mercury import``; the #48200 wrong-path
    ...
    try:
        ...
        try:
            from mercury_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from mercury_cli.update_receipt import record_step
    ...
    try:
        from mercury_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from mercury_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_hermes_home so the user sees ~/.mercury/...
    try:
    ...
    print(f"  Restore:  mercury import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``mercury update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``mercury update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run mercury from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.mercury-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".mercury-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The mercury.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m mercury_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``mercury
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Mercury candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "mercury.exe"}
        ):
            continue
        ...
        # this venv's path, or a `-m mercury_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "mercury_cli.main" in cmdline_low:
            try:
        ...
        # the `-m mercury_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``mercury_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``mercury.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``mercury`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `mercury` launch will complete it")
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("mercury_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("mercury", "mercury.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``mercury dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Mercury processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Mercury backend (if the Desktop app is open, close it)",
        "dashboard": "  ← mercury dashboard (stop it: mercury dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    ...
    lines.append(
        "  Close the Mercury desktop app / other Mercury terminals, then re-run:"
    )
    lines.append("    mercury update")
    lines.append("  (or use `mercury update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``mercury update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from mercury_cli._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``mercury update`` through its terminal tool.
    ...
    try:
        from mercury_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `mercury update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Mercury spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from mercury_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    try:
        ...
        if scripts_dir is not None:
            for name in ("mercury.exe", "mercury"):
                candidate = scripts_dir / name
                if candidate.is_file():
                    mercury = str(candidate)
                    break
    except Exception:
        mercury = None
    if mercury is None:
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (mercury serve --host <ip> --port <port>)."
        )
    try:
        from mercury_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning mercury-setup, and the update-in-progress marker
    ...
    the update with "Mercury is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Mercury backend (``mercury_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.mercury-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "mercury_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``mercury_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Mercury ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``mercury.exe``) is open, NOTHING legitimate
    ...
    - Only Mercury backends (``mercury_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``mercury.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.mercury-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from gateway.status import get_process_start_time
    from mercury_cli._subprocess_compat import pid_is_hermes, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            ...
            if not pid_is_hermes(
                ...
                logger.debug(
                    "Skipping taskkill of non-Mercury or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``mercury serve`` / ``mercury dashboard`` argv.
    ...
    """
    if "mercury_cli.main" not in (cmdline or "").lower():
        return False
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    mercury.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Mercury...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m mercury_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: mercury gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``mercury gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from mercury_cli.gateway import is_macos
    ...
    print("    mercury gateway status")
    if any(not name.startswith("ai.mercury.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.mercury.") for name in ordered):
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``mercury_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``mercury-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"mercury-gateway-{profile}",
        f"ai.mercury.gateway-{profile}",
        f"gateway-{profile}",
    }
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``mercury_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    command = [
        sys.executable,
        "-m",
        "mercury_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``mercury_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``mercury update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``mercury-setup.exe``) executes
...
def _portable_git_candidates() -> list:
    ...
    The Mercury-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped HERMES_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``mercury update`` must
...
def _locate_real_git() -> Optional[Path]:
    ...
    Mercury-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``mercury update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``mercury update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `mercury desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "mercury_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Mercury-managed Node on PATH: when
    # `mercury update` runs inside the desktop updater chain (Desktop →
    # mercury-setup → mercury update), the shell PATH customizations are lost,
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `mercury desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo mercury`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``mercury update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/mercury`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Mercury broken.")
    print("  This usually happens after running mercury or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    mercury update")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from mercury_cli.update_receipt import begin_update_receipt
    ...
    # running Mercury runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another mercury.exe is holding the venv shim
    ...
    # (most commonly the Desktop app's `mercury serve` backend) will keep .pyd
    ...
    # the mercury.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _ledger_backends:
                print(
                    ...
                    "Mercury backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .mercury-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Mercury is still
        ...
        if _venv_holders:
            ...
            # `mercury serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Mercury backends are leaks,
            ...
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Mercury backend process(es) "
    ...
    # process by design (a CLI `mercury update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by mercury.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Mercury windows/gateways and re-run: mercury update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Mercury gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in mercury_cli/config.py bricked
        # every user who ran ``mercury update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``mercury
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``mercury update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``mercury update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``mercury doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && mercury update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``mercury update`` can catch up even when git is already up to date.
        ...
        # marker survives and the next ``mercury`` launch finishes the install
        ...
        # symbols in mercury_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `mercury` again "
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Mercury re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.nousresearch.mercury` (repeat "
        ...
        try:
            from mercury_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        # current — so copy it straight over ~/.mercury/cache/model_catalog.json
        ...
        try:
            from mercury_cli.profiles import backfill_profile_envs
        ...
        # check `mercury curator status`. Self-stamps after printing so it
        ...
        # Self-heal the mercury-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Mercury on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("mercury-acp launcher self-heal failed: %s", e)

        # Migrate the Windows mercury launchers to the managed binary dir
        # (the default Mercury root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (mercury-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `mercury`
        ...
        # the managed clone under the default Mercury root).
        try:
            from mercury_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``mercury update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `mercury update`
                # must stay fast; `mercury computer-use install --upgrade`
        ...
        #
        # Purge stale cached Mercury modules FIRST: the import below pulls
        ...
        try:
            from mercury_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import mercury_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``mercury update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from mercury_cli.main import main; main()" update --yes'
                    )
            try:
                from mercury_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``mercury_cli.gateway`` no longer imports,
