...
from mercury_cli.sizefmt import format_bytes as _format_size
...
# Where ``mercury backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``mercury-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/mercury-agent/``
...
# File names that ``mercury import`` must never overwrite, matched by basename so
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Mercury backup slot."""
...
@contextmanager
def _backup_operation_lock(mercury_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = mercury_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Mercury backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``mercury-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/mercury-agent/`` — must be preserved.
        if part == "mercury-agent" and part != parts[0]:
            continue
        return True
...
# to the user as a hung `mercury update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # mercury_cli.sqlite_safe_read). Verification targets snapshots and
        # backup artifacts, which are offline by construction.
        from mercury_cli.sqlite_safe_read import read_header_bytes_preopen
...
def run_backup(args) -> None:
    ...
    mercury_root = get_default_hermes_root()
    ...
    try:
        with _backup_operation_lock(mercury_root):
            _run_backup_locked(args, mercury_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, mercury_root: Path) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(mercury_root, followlinks=False):
        ...
        # ``mercury-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "mercury-agent" and not is_root)
        ]
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``mercury import``, which is the
    ...
    so without the chown a ``sudo mercury import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``mercury import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Mercury' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: mercury profile list  (after installing mercury)")
        ...
        try:
            from mercury_cli.gateway import ensure_gateway_service, _is_service_running
...
# — `mercury update` snapshots this set before pulling so approved-user lists
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: mercury snapshot list"
        )
        logger.error(
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``mercury update``.
    ...
    Args:
        ...
        mercury_home: Override for the Mercury home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, mercury_home=profile_home)
        except Exception as exc:
...
def _write_full_zip_backup(out_path: Path, mercury_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(mercury_root):
            return _write_full_zip_backup_locked(out_path, mercury_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, mercury_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``mercury_root`` to ``out_path``.
...
def _pre_update_backup_dir(mercury_home: Optional[Path] = None) -> Path:
    home = mercury_home or get_hermes_home()
...
def create_pre_update_backup(
    ...
    (``mercury update``) should continue even if the backup fails.
    """
    mercury_root = mercury_home or get_default_hermes_root()
    if not mercury_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(mercury_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, mercury_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `mercury claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `mercury import` and the
