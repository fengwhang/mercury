"""``mercury dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.mercury/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.mercury/auth.json``), refreshing it if needed. Fails fast with a
     "run `mercury setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.mercury/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from mercury_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``mercury dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `mercury auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from mercury_cli.config import get_env_path
    ...
    print(
        ...
        "  `mercury dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    mercury dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from mercury_cli.auth import AuthError, resolve_nous_access_token
    from mercury_cli.config import get_env_value, is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `mercury dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `mercury setup` (or `mercury auth add nous`) first, then retry.")
        else:
    ...
    # the portal, e.g. https://mercury.example.com/auth/callback. At serve time
