...
from mercury_cli._subprocess_compat import noninteractive_git_env
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Mercury subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.mercury/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Mercury never auto-installs plugin pip
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Mercury.",
                ) from None
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `mercury plugins update {plugin_name}`."
            )
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from mercury_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `mercury plugins search <term>` to "
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Mercury plugin.",
        )
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`mercury plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `mercury plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``mercury setup`` / ``mercury plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``mercury plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    if name in LEGACY_RELAY_PLUGIN_KEYS:
        console.print(
            ...
            f"by Mercury core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."
        )
        sys.exit(1)
    ...
    # Mercury core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from mercury_cli.plugin_capabilities import parse_declared_capabilities
    ...
    for entry in _discover_all_plugins():
        ...
        if entry[5] == key or entry[0] == key:
            if entry[3] == "entrypoint":
                from mercury_cli.plugins import discover_entrypoint_manifests
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from mercury_cli.plugin_capabilities import CAPABILITY_REGISTRY
    ...
    *,
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        console.print(
            ...
            f"`mercury plugins capabilities {plugin_id}` to review and "
            f"`mercury plugins enable {plugin_id}` to grant interactively."
        )
        return False
    ...
    console.print(
        ...
        f"`mercury plugins enable {plugin_id}` to grant later.[/dim]"
    )
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``mercury plugins capabilities [<id>]`` — declared vs granted."""
    from rich.console import Console

    from mercury_cli.plugin_capabilities import granted_capabilities
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`mercury plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``mercury_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.mercury/plugins``. Include package metadata here
    so ``mercury plugins list`` can show and enable them.
    """
    from mercury_cli.plugins import ENTRY_POINTS_GROUP
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``mercury plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] mercury plugins list --plain --no-bundled")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Mercury
    ...
    try:
        from mercury_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] mercury plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``mercury plugins enable web/firecrawl`` cleared
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.mercury/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.mercury/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `mercury plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``mercury plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``mercury_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``mercury update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "mercury-plugin-update-autostash",
            )
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.mercury/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from mercury_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from mercury_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: mercury plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from mercury_cli.plugin_packs import pack_command
