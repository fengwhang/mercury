"""Install and remove the Linux desktop entry (``mercury.desktop``).

``mercury desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "mercury.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``mercury.desktop`` entry lives."""
...
def _can_import_hermes_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``mercury_cli.main`` unaided.
    ...
    try:
        result = subprocess.run(
            [key, "-I", "-c", "import mercury_cli.main"],
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``mercury_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``mercury desktop``.

    Prefer the real ``mercury`` executable (argv[0] or PATH). When Mercury
    ...
    """
    from mercury_cli.relaunch import resolve_hermes_bin
    ...
    if not _can_import_hermes_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import mercury_cli.main
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `mercury` script with
            ...
            # sys.executable is the interpreter actually running Mercury (the
...
def _resolve_hermes_bin_for_desktop_entry(
    ...
    Wraps :func:`mercury_cli.relaunch.resolve_hermes_bin` with one
    desktop-entry-specific rule: an ``argv[0]`` that points inside this
    checkout is a launch-context artifact (the repo ``mercury`` script the
    ...
    if resolve_fn is None:
        from mercury_cli.relaunch import resolve_hermes_bin as resolve_fn
    ...
    def _inside_checkout(candidate: str) -> bool:
        try:
            ...
            return False
        # The repo `mercury` script and anything else shipped in the tree is
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m mercury_cli.main` relaunch context surfaces the
    ...
    # interpreter is an external launcher (e.g. /opt/.../bin/mercury from
    ...
    if rerouted is None and primary:
        # argv[0] was checkout-internal AND PATH had no `mercury` — common
...
def _wrapper_targets_checkout(wrapper: Path, checkout_root: Path) -> bool:
    ...
    ``<checkout>/venv/bin/python <checkout>/mercury``; a venv console
    ...
    for root in roots:
        ...
        # <root>/mercury …): a path-continuation boundary is also a match.
        if root + "/" in text:
            return True
    return False
...
def _project_root() -> Path:
    """This file lives at ``<checkout>/mercury_cli/linux_desktop_entry.py``.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see Mercury' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Mercury\n"
        "GenericName=Mercury Desktop\n"
        "Comment=Launch Mercury Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    ...
    The freedesktop icon lookup finds an installed ``apps/mercury.png``
    by the unqualified name ``mercury``, so the entry can reference the
    ...
    try:
        ...
        dest = _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "mercury.png"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Mercury desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "mercury"
