"""Agent-construction and session-resume display methods for ``MercuryCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.mercury/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``MercuryCLI``. ``self.*``
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    ...
    ``mercury chat -q`` runs one turn without ever building the
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``MercuryCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'mercury model' to choose a provider, or "
                      "'mercury setup' for first-time setup.")
                return False
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``mercury model`` (which fronts Quick Setup / Nous
        ...
        _cprint("")
        _cprint("🌡️ No inference provider is configured yet — let's fix that.")
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'mercury model' or 'mercury setup' any time.")
            return False
        ...
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'mercury model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'mercury model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'mercury model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        from mercury_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (mercury sessions list).",
                        file=sys.stderr,
                    )
                else:
        ...
        try:
            ...
            from mercury_constants import partial_update_hint
