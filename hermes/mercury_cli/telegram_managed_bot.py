...
manual BotFather token copy-paste. Mercury talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.mercury-agent.nousresearch.com"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Mercury Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Mercury Telegram onboarding service.")
