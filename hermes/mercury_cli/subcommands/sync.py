"""``mercury sync`` subcommand parser — Skill Sync.

Cloned from ``mercury_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    mercury sync status                 show gate/opt-in/head state
    mercury sync pull                   pull and materialize opted-in skills
    mercury sync push                   push opted-in skills
    mercury sync now                    reconcile: pull then push
    mercury sync enable <skill>         opt a skill into sync
    mercury sync disable <skill>        opt a skill out of sync
    mercury sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    mercury sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  mercury sync status            what is synced, and from where\n"
            "  mercury sync enable my-skill   include a skill in your sync\n"
            "  mercury sync now               pull, then push\n"
            "  mercury sync propose my-skill  share a skill with your team\n"
        ),
