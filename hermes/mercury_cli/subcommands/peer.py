"""``mercury peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Mercury gateway (any machine: homelab, Spark, Mercury
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    mercury peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
    mercury peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    mercury peer dm spark/researcher "..."      # named profile (multiplexed peer)
    mercury peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
    mercury peer status spark run_abc123
...
``mercury -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.mercury/.env`` as
...
def _request(
    ...
    *,
    ...
    # The peer URL is user-registered (``mercury peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's mercury-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "mercury.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (mercury peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: mercury peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: mercury peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.mercury/.env)"
        )
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.mercury/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  mercury peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.mercury/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: mercury peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    print("Unknown peer action. See: mercury peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Mercury gateways)",
        description=(
            "Register other Mercury gateways as peers and message their agents. "
            "'mercury peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'mercury -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.mercury/.env."
        ),
        epilog=(
            "Examples:\n"
            "  mercury peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  mercury peer list\n"
            '  mercury peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  mercury peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  mercury peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  mercury peer status spark run_abc123\n"
            "  mercury peer stop spark run_abc123\n"
            "  mercury peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.mercury/.env)")
    ...
    status_p.add_argument("run_id", help="Run ID returned by 'mercury peer run'")
    status_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
    ...
    stop_p.add_argument("run_id", help="Run ID returned by 'mercury peer run'")
    stop_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
