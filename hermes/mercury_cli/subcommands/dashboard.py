"""``mercury dashboard`` / ``mercury serve`` subcommand parsers.
...
``mercury_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `mercury dashboard` / `mercury serve` cmdlines and SIGTERM them directly —
    # the same path `mercury update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Mercury web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Mercury web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Mercury web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Mercury desktop app shells (<= 0.15.x) spawn the
    # backend as `mercury dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Mercury couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Mercury Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Mercury backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Mercury backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `mercury dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.mercury/.env.
    # Nested subparser so bare `mercury dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.mercury/.env, and prints "
            "how to engage the login gate. Requires being logged in (mercury setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://mercury.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
