"""``mercury verify`` subcommand parser.

Follows the pattern of ``mercury_cli/subcommands/doctor.py``: parser built
...
# avoid paying an extra module import on every `mercury` invocation.
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "(or load the saved manifest at .mercury/environment.json), then "
    ...
    verify_parser.add_argument(
        "--save",
        action="store_true",
        help="Save the recipe as .mercury/environment.json in the project",
    )
    verify_parser.add_argument(
        "--skip-start",
        action="store_true",
