"""``mercury update`` subcommand parser.
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Mercury to the latest version",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "kind (git/docker/nix), every running Mercury service across all "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run without blocking on prompts: accepts the config-migration and stash-restore prompts, skips the fork-upstream prompt without adding a remote. API-key entry is skipped; run 'mercury config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--keep-stash",
        action="store_true",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "If the local checkout is on a different branch, mercury will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another mercury.exe is detected. The concurrent process will likely cause WinError 32 warnings. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
        "--force-venv",
        action="store_true",
