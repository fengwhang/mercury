...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Mercury Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any mercury CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--mercury-root",
        help="Override the Mercury source root used by Desktop (sets HERMES_DESKTOP_HERMES_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
    ...
    gui_parser.add_argument(
        "--identity",
        default="Mercury Local Signing",
        help="Certificate name to create/use for --setup-tcc-identity (default: Mercury Local Signing)",
    )
