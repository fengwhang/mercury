"""Dashboard process-hygiene helpers — extracted from ``mercury_cli/main.py``.
...
helpers that STAY in ``mercury_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``mercury_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``mercury_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``mercury_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``mercury dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``mercury update`` replaces files on disk, the running
    ...
    ``mercury-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Mercury Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``mercury serve`` as
    ...
    patterns = [
        ...
        # The headless backend (`mercury serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "mercury.*dashboard"`) keeps us consistent
            # with `mercury_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `mercury --profile p serve --host <ip>` contains
    # neither "mercury serve" nor "mercury_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from mercury_cli.process_identity import ledger_entries
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Mercury Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``mercury update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], mercury_home: str | None = None
) -> str:
    ...
    distinct ``…/.mercury`` roots) keep a resolved ``home:`` key so
    ...
    if mercury_home:
        try:
            home_path = Path(mercury_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(mercury_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(mercury_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``mercury update``.

    Each candidate is ``(pid, argv, mercury_home)``.  *own_home* is the
    ...
    ``mercury update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, mercury_home in candidates:
        if not argv:
            continue
        if _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and mercury_home and _normalized_home_for_compare(mercury_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, mercury_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``mercury dashboard`` / ``mercury serve`` processes.

    Called at the end of ``mercury update`` (default ``reason``) and also
    from ``mercury dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``mercury update`` path), a detected
    ``mercury-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``mercury-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``mercury update``'s systemd
    fleet-restart loop, which restarts ``mercury-serve*`` units before this
    ...
    # When the Mercury Desktop Electron app spawns this dashboard as a
    ...
    # caller asked for restarts (the `mercury update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        ...
        if already_restarted_units:
            # Already handled directly by the caller (e.g. mercury update's
        ...
        for pid in pids:
            try:
                ...
                if not pid_is_hermes(
                    ...
                    failed.append((pid, "not mercury-owned or process identity changed"))
                    continue
                result = subprocess.run(
    ...
    #    remote backend (mercury serve) under Restart=on-failure never comes
...
def _detect_concurrent_hermes_instances(
    ...
    The Mercury Desktop Electron app spawns ``mercury.EXE`` as a backend child,
    so during ``mercury update`` the user-invoked process and the desktop's
    ...
    shims (``mercury.exe`` / ``mercury-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``mercury update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``mercury.exe``) is a separate process from the Python
    ...
    # setuptools-generated mercury.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``mercury update``
    ...
    #      mercury.exe sitting *under* a non-Mercury parent (e.g. a Mercury
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        mercury serve --host 127.0.0.1 --port 0
        mercury serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "mercury" not in cmd and "mercury_cli" not in cmd:
        return False
...
# on the *remote* host for every ``mercury serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``mercury serve`` on a Mac
...
def _hermes_home_dir() -> Path:
    """Resolved Mercury home (HERMES_HOME override or ~/.mercury)."""
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{mercury_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local mercury serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``mercury serve`` backends with no parent.
