...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.mercury/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``mercury mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # mercury_cli/main.py for why the dest is renamed.
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``mercury mcp login`` and
    ``mercury mcp reauth`` so both behave identically for a single server.
    """
    ...
    #
    # force_interactive_oauth: `mercury mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Mercury desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `mercury mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``mercury mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``mercury mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: mercury mcp reauth <name>   |   mercury mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def mcp_command(args):
    ...
    if action == "picker":
        from mercury_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from mercury_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from mercury_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `mercury plugin`.
        from mercury_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("mercury mcp                                    Open the catalog picker (default)")
        _info("mercury mcp catalog                            List Nous-approved MCPs")
        _info("mercury mcp install <name>                     Install a catalog MCP")
        _info("mercury mcp serve                              Run as MCP server")
        _info("mercury mcp add <name> --url <endpoint>        Add a custom MCP server")
        ...
        _info("mercury mcp list                               List configured servers")
        ...
        _info("mercury mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
