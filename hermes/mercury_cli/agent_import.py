"""mercury import-agent — import Claude Code / Codex CLI setups into Mercury.

Usage:
    mercury import-agent                       # auto-detect ~/.claude or ~/.codex
    mercury import-agent claude-code           # import from ~/.claude
    mercury import-agent codex                 # import from ~/.codex
    mercury import-agent claude-code --dry-run # preview only, no changes
    mercury import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``mercury claw migrate`` /
...
deliberately via ``mercury setup`` or config.yaml.
"""
...
# Same entry delimiter as the Mercury memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `mercury config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`mercury config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Mercury command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Mercury glob.
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Mercury equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        mercury_srv["headers"] = kept_headers
                    for k in headers:
...
def import_agent_command(args) -> None:
    """Handle ``mercury import-agent`` (invoked from mercury_cli.main)."""
    from mercury_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: mercury import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: mercury import-agent claude-code   or   mercury import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│         🌡️ Mercury — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: mercury import-agent "
    ...
    print_info("Secrets:     never imported — run 'mercury setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: mercury import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'mercury setup' "
               "to configure providers, or add them to ~/.mercury/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from mercury_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'mercury setup' or ~/.mercury/.env.")
        print()
