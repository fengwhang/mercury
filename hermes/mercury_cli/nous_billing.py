...
billing screens drive. Companion to ``mercury_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Mercury already holds for inference
...
# mercury_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `mercury portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
