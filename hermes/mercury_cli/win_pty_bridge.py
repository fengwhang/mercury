"""Windows ConPTY bridge for the `mercury dashboard` chat tab.

Drop-in counterpart to ``mercury_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``mercury_cli.web_server`` consumes: ``spawn``, ``read``,
