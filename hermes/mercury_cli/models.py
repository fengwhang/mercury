...
from mercury_cli.urllib_security import open_credentialed_url, url_origin
...
_HERMES_USER_AGENT = f"mercury-cli/{_HERMES_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from mercury_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from mercury_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Mercury-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see mercury_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Mercury that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Mercury that doesn't ship
...
#
# Some vendors expose several Mercury provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `mercury model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``mercury model``, the
...
# In-repo fallback for the model Mercury silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from mercury_cli.model_catalog import get_default_model_from_cache
...
# ``mercury model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in mercury_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``mercury auth add openai-codex`` without
    ``mercury model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``mercury model`` picker offers first. For metered aggregators
...
# copy, `mercury -p`, a cron job, or a freshly booted gateway answers
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from mercury_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Mercury lands on when none is selected.
            desc = "default"
        else:
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    env_base = None
    try:
        from mercury_cli.auth import _nous_inference_env_override
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from mercury_cli.providers import custom_provider_slug
...
def _resolve_static_model_alias(
    ...
    try:
        from mercury_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``mercury auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.mercury/.env``.
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from mercury_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Mercury release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``mercury model`` shows.
            from mercury_cli.providers import is_official_openai_host
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # env var, so it must change the fingerprint too or `mercury config set
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Mercury' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from mercury_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"mercury-agent/{_HERMES_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=mercury"
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `mercury moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Mercury will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Mercury
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Mercury actually ships.  (#46850)
            #
            ...
            # `mercury chat` already accepts these models via
