...
configured for "cli" in `mercury tools`.
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"mercury -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "mercury -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"mercury -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "mercury -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "mercury -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "mercury -z: --provider requires --model (or HERMES_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"mercury -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("mercury -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``mercury -z`` / oneshot mode.

    Oneshot bypasses ``MercuryCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from mercury_cli import model_switch as _ms
    ...
    # MercuryCLI._init_agent's wait — it builds AIAgent directly here, so the
