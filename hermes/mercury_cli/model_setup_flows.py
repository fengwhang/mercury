"""Per-provider model-selection wizard flows for ``mercury setup`` / ``mercury model``.

Extracted from ``mercury_cli/main.py`` as part of the god-file decomposition
campaign (``~/.mercury/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from mercury_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``mercury_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from mercury_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``mercury_cli.main`` at import time -> no import cycle.
"""
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from mercury_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from mercury_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.mercury/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `mercury setup` from scratch. OpenRouter
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.mercury/.env by hand.
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `mercury moa configure <name>` first.")
        return
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``mercury auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from mercury_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from mercury_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from mercury_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from mercury_cli.auth import _save_model_choice, deactivate_provider
    from mercury_cli.config import (
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from mercury_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from mercury_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from mercury_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Mercury will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Mercury will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from mercury_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from mercury_cli.auth import _save_model_choice, deactivate_provider
    from mercury_cli.config import load_config, normalize_extra_headers, save_config
...
def _model_flow_copilot(config, current_model=""):
    ...
    from mercury_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from mercury_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from mercury_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from mercury_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from mercury_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `mercury auth`)
    from mercury_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from mercury_cli.config import load_config, save_config, get_env_value
    from mercury_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.mercury/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``mercury_cli.auth`` so it stays in
    ...
    from mercury_cli.auth import ZAI_ENDPOINTS
