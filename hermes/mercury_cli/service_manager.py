...
the existing module-level functions in mercury_cli.gateway and
mercury_cli.gateway_windows directly. This protocol is a thin facade
...
``mercury gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``mercury gateway start/stop/restart``).
    """
    ...
    # `mercury gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged mercury
    ...
    detection always fails. Since every Mercury runtime call inside the
    container drops to mercury via ``s6-setuidgid``, that silent failure
...
# in ``mercury_cli.gateway`` (systemd/launchd) and ``mercury_cli.gateway_windows``
...
# active (set via the ``mercury -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in mercury_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from mercury_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from mercury_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from mercury_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from mercury_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in mercury_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from mercury_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from mercury_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from mercury_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from mercury_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``mercury_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `mercury profile create`
# runs inside the container (Phase 4). Static services (main-mercury, dashboard)
...
# UID/GID of the in-image ``mercury`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the mercury user.
    ...
    (the ``mercury`` user — UID 10000 — running every Mercury runtime
    ...
    is dispatched as the mercury user.
    ...
    with mercury ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           mercury:mercury, 0755 (parent must already exist)
    ``svc_dir/event/``                     mercury:mercury, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 mercury:mercury, 0755
    ``svc_dir/supervise/event/``           mercury:mercury, 03730
    ``svc_dir/supervise/control``          mercury:mercury, 0660    (FIFO)
    ...
    slot's supervise/ was mercury-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the mercury user already — directory is mercury-
    ...
    # when the parent slot's supervise/ is mercury-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``mercury -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`mercury profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-mercury, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HERMES_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HERMES_HOME=/data/mercury`` is honored at run
          ...
          4. Drops to the mercury user and exec's
             ``mercury -p <profile> gateway run`` (or just ``mercury
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``mercury gateway
        ...
        ``mercury_cli.gateway`` maps that empty string to the
        ...
        allocator in ``mercury_cli.profiles._allocate_gateway_port``).
        ...
        # profile-redirect guard in mercury_cli.main._apply_profile_override,
        ...
        # (a stray ``mercury gateway run`` from a shell, an agent action, or
        ...
        if profile == "default":
            gateway_cmd = "mercury gateway run --replace"
        else:
            gateway_cmd = f"mercury -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid mercury {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        — so a container started with ``-e HERMES_HOME=/data/mercury``
        gets its logs under /data/mercury/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``mercury logs`` reads and what
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock as mercury when
            # this script starts as root. Never chown or unlink mercury-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is mercury-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded mercury-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid mercury mkdir -p "$log_dir"\n'
            f'  s6-setuidgid mercury rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid mercury s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``mercury gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``mercury -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with mercury ownership
            ...
            # the mercury user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is mercury-owned (see ``_seed_supervise_skeleton``), and on
