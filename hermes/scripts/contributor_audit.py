...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Mercury\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (mercury-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "mercury@nousresearch.com",
    "mercury-audit@example.com",
    "nousbot@nousresearch.com",
    "mercury@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "NousResearch/mercury-agent",
                "--state", "merged",
