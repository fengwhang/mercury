"""Run a real Mercury CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_hermes_executable(mercury_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "mercury",
        Path(".venv") / "Scripts" / "mercury.exe",
    ):
        candidate = mercury_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("mercury")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Mercury executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Mercury source checkout containing .venv/bin/mercury",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["mercury.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['mercury.client.active']}"
        )
    [model] = by_name["mercury.model_route.count"]
    expected_model = {
        "name": "mercury.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['mercury.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["mercury.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['mercury.task_run.started']}"
        )
    [terminal] = by_name["mercury.task_run.finished"]
    ...
    [tool] = by_name["mercury.tool_call.count"]
    ...
    lifecycle = by_name["mercury.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["mercury.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Mercury development environment requires jsonschema"
        ) from exc
    ...
    if metrics["mercury.client.active"] != [
        {
            "name": "mercury.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['mercury.client.active']}"
        )
    [model] = metrics["mercury.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['mercury.model_route.count']}"
        )
    [terminal] = metrics["mercury.task_run.finished"]
    ...
    [tool] = metrics["mercury.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["mercury.skill.lifecycle.count"]
    ...
    loads = metrics["mercury.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    mercury_repo = args.mercury_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    mercury = _resolve_hermes_executable(mercury_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="mercury-relay-shared-metrics-"))
    home = root / "mercury-home"
    ...
    (root / "mercury.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "mercury.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Mercury exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Mercury model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Mercury did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Mercury did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from mercury_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "mercury.shared_metrics.v2.schema.json",
    )

    print("Mercury -> NeMo Relay shared-metrics smoke test passed")
