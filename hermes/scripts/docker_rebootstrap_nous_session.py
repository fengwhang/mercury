...
Background
----------
A Nous bootstrap session (client_id ``mercury-cli-vps``) can take a terminal
...
Design constraints
------------------
- Pure stdlib, no mercury_cli imports: runs early in the boot hook, before the
...
BOOTSTRAP_CLIENT_ID = "mercury-cli-vps"
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``mercury_cli.auth.get_nous_session_validity``:
