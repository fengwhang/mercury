#!/usr/bin/env python3
"""Live test harness for Mercury's Tool Search feature.
...
# Force-isolate the test environment BEFORE any mercury imports.
ORIGINAL_HOME = os.environ.get("HERMES_HOME")
ORIGINAL_AUTH = Path.home() / ".mercury" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.mercury/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.mercury/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="mercury_ts_live_"))
    mercury_home = home_dir / ".mercury"
    mercury_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, mercury_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".mercury" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, mercury_home / ".env")
        ...
        load_hermes_dotenv(mercury_home=str(Path.home() / ".mercury"))
    ...
    (mercury_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return mercury_home
