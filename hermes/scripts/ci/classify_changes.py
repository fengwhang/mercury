...
# tree generates llms.txt — the index every LLM (Mercury included, via the
# mercury-agent skill) reads to learn what Mercury can do — and
...
_MCP_CATALOG_FILES = {"mercury_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Mercury-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``mercury serve`` backend, the Docker image)
