...
When Mercury runs in TUI mode, the gateway child process communicates with
...
# ``get_hermes_home() / "plugins"`` (user) and ``./.mercury/plugins/`` (project,
# gated behind ``HERMES_ENABLE_PROJECT_PLUGINS``) — see
# ``mercury_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``mercury_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import mercury_constants (this script
    ...
    # ./.mercury/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_hermes_home() / "plugins"]
    if os.environ.get("HERMES_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".mercury" / "plugins")
