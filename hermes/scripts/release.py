...
VERSION_FILE = REPO_ROOT / "mercury_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@mercury.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.mercury": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (mercury journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "mercury.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.mercury/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows mercury-shim resolution + prefer --update on recovery; #52378)
    ...
    "mercury@marian.local": "Schrotti77",
    ...
    "274096618+mercury-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mercury@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on mercury mcp add)
    ...
    "mercury@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@mercury-agent": "teknium1",
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: mercury-eval community link)
    ...
    "vigo@mercury": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.mercury@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Mercury runtime files)
    ...
    "zhchl@mercury-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Mercury
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "mercury@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/mercury-agent",
    ...
    lines.append(f"# Mercury v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Mercury. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Mercury Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Mercury v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Mercury v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Mercury v{new_version} ({calver_date})' "
