#!/usr/bin/env python3
"""Drive the Mercury TUI under HERMES_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.mercury/perf.log on exit.
...
bypasses the mercury_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HERMES_PERF_LOG     (default ~/.mercury/perf.log)
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "mercury-ink" / "src"
