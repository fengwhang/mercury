...
What it does
------------
1. Spawns a SCRATCH dashboard (``mercury dashboard``) bound to loopback on a
   free port, with an ISOLATED ``HERMES_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.mercury.dashboard / live
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        cmd = [
            python, "-m", "mercury_cli.main", "dashboard",
