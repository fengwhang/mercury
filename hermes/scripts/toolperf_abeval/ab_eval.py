...
measured in the production session DB) through `mercury chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    HERMES_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Mercury home with credentials for the
...
(tracker: NousResearch/mercury-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "mercury_cli.main", "chat", "--query", q,
