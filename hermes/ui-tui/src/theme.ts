...
export function buildPalette(seeds: ThemeSeeds, isLight: boolean): ThemeColors {
  ...
  return {
    ...
    // sessionLabel: HERMES-OMP PATCH (drive fix) — PINK by default (the
    // Mercury session identity color), overridable by any skin's
    // session_label key or the ui_label accent. sessionBorder keeps
    // tracking muted ("same role, same colour", #11300).
    sessionLabel: '#FF69B4',
    sessionBorder: muted,
...
export const LIGHT_SEEDS: ThemeSeeds = {
  // HERMES-OMP PATCH (orangered theme, drive fix): light seeds re-authored
  // from the DARK orangered identity (not gold) — liftForContrast equivalents
  // for #FF4500/#FF6347/#B22222 against #ffffff at 4.5:1.
  accent: '#B33A00',
  bg: '#ffffff',
  border: '#8B3A2E',
  error: '#C14240',
