...
_HERMES_CORE_TOOLS = [
    ...
    # Kanban multi-agent coordination — MERCURY-OMP PATCH (B2): removed from
    # the Mercury distribution (delegation is omp; see
    # tools/omp_delegation.py). The `kanban` toolset entry below is deleted;
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `mercury tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``mercury tools`` → Video Generation."
        ),
    ...
    # MERCURY-OMP PATCH (B2): the `kanban` toolset is removed from the Mercury
    # distribution (delegation is omp; see tools/omp_delegation.py). Stale
    # configs that still list `kanban` under enabled toolsets are skipped
    # silently by validate_toolset.
    ...
    # Coding posture (base Mercury — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in mercury_cli/tools_config.py).
        "posture": True,
    },
    ...
    # the `mercury send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "mercury-gateway": {
        ...
        "tools": [],
        "includes": ["mercury-telegram", "mercury-discord", "mercury-whatsapp", "mercury-slack", "mercury-signal", "mercury-bluebubbles", "mercury-homeassistant", "mercury-email", "mercury-sms", "mercury-mattermost", "mercury-matrix", "mercury-dingtalk", "mercury-feishu", "mercury-wecom", "mercury-wecom-callback", "mercury-weixin", "mercury-qqbot", "mercury-webhook", "mercury-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``mercury-*`` bundle's platform-specific tools, excluding core.
    ...
    Bundle nesting is one level deep in practice (only ``mercury-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (mercury-<name>).
        ...
        if include_registry and name.startswith("mercury-"):
            platform_name = name[len("mercury-"):]
            try:
