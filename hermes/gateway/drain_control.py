...
Why the epoch (NS-570). ``HERMES_HOME`` is a **durable** store — on Mercury
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``mercury gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    durable Fly volume on Mercury Cloud) and the lifecycle action that triggered
