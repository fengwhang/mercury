...
class PlatformEntry:
    ...
    # status displays (``mercury setup``, ``mercury status``, the dashboard
    ...
    # Env vars this platform needs (for ``mercury setup`` display).
    ...
    # built-ins).  Used by ``mercury gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `mercury`
        # invocation -- including plain `mercury chat`, which never touches any
        ...
        # `mercury setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``mercury setup``/``gateway status``, channel
