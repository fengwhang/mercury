"""Relay/connector support package for the Mercury gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``mercury gateway enroll`` writes them to
    ``~/.mercury/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``mercury gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from mercury_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Mercury:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``mercury gateway enroll`` CLI. Three modes, in precedence order:
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.mercury/.env``.
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``mercury gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'mercury'}"
