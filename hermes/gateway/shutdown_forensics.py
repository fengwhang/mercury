...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if mercury_home_str:
            takeover_path = Path(mercury_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(mercury_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(
    ...
    upgraded mercury-agent but never re-ran ``mercury setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../mercury-gateway.service"
    ...
    # that's the common case for mercury.
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by mercury_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
