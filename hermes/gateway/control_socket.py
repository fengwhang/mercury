...
(the updater, `mercury serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, mercury_home, code_sha/code_version
...
- Windows: named pipe ``\\\\.\\pipe\\mercury-gateway-<home-hash>`` served via
...
Consumers (``mercury update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    ...
    return rf"\\.\pipe\mercury-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"mercury-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.mercury")
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "mercury_home": record.get("mercury_home"),
        "profile": _profile_label_for_home(record.get("mercury_home") or ""),
