...
# ``sessions_dir / f"{session_id}.json"`` in mercury_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `mercury tools` (opt-in, default
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Mercury session "
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.mercury/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.mercury/state.db "
                "and are shown by `mercury sessions list` and `/sessions`. "
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from mercury_state import CompressionSessionClosedError
