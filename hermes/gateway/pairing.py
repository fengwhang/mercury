...
# even if a context-local override (see mercury_constants.set_hermes_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``mercury pairing`` CLI) re-imports this module later with the final
...
# working (NousResearch/mercury-agent#93449).
#
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``mercury_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from mercury_cli.config import save_env_value, remove_env_value
...
class PairingStore:
    ...
    as ``mercury -p <name> pairing ...``. This keeps multiplex gateways and
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            # ``mercury -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `mercury` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> mercury pairing approve ...` as root, "
                    "re-run with `docker exec -u mercury <container> ...` and "
                    "chown the existing file to the mercury user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``mercury
