...
from mercury_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Mercury commands.
    ...
    # MERCURY-OMP PATCH (B2 phase 2): _handle_kanban_command() deleted — the
    # /kanban gateway handler (run_slash + create auto-subscribe) served the
    # removed kanban engine; the command is unknown on every gateway surface
    # (registry-derived) and delegation = omp (/omp <task>).
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Mercury version."""
        from mercury_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # via the shared single-owner parser (mercury_cli.model_switch).
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from mercury_cli.context_switch_guard import (
                        ...
                        from mercury_cli.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from mercury_cli.route_identity import should_clear_context_pin_async
                        ...
                        from mercury_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Mercury default runtime
        ...
        from mercury_cli import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        # Persist the selection only — mercury_cli.personality never writes
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from mercury_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `mercury cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from mercury_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Mercury itself changed. ``--stat`` limits output to the summary.
    ...
    def _reasoning_picker_choices(self, current_effort: str) -> list:
        ...
        from mercury_constants import VALID_REASONING_EFFORTS
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.mercury/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``mercury skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hermes_home
        from mercury_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `mercury skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.mercury/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Mercury is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while Mercury is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while Mercury is busy."
            return EphemeralReply(
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from mercury_cli.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from mercury_cli.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="mercury_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from mercury_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (mercury_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `mercury bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Mercury')}"
        ...
        mercury_cmd = _resolve_hermes_bin()
        if not mercury_cmd:
            return t("gateway.update.mercury_cmd_not_found")
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `mercury update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through mercury_cmd (venv\Scripts\mercury.exe): the shim
