...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.mercury to the active profile home."""
    ...
    if raw == "~/.mercury":
        return mercury_home
    if raw.startswith("~/.mercury/"):
        return mercury_home / raw.removeprefix("~/.mercury/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return mercury_home / path
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    ...
    if raw_text == "~/.mercury" or raw_text.startswith("~/.mercury/"):
