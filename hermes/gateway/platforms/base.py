...
# Audio file extensions Mercury recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Mercury but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Mercury' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Mercury-created Telegram private-chat
...
from mercury_constants import get_default_hermes_root, get_hermes_dir, get_hermes_home
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from mercury_cli.config import load_config_readonly as _load_config
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Mercury root.
...
def _media_delivery_allowed_roots() -> List[Path]:
    ...
    # MERCURY-OMP PATCH (B2 p2): kanban attachment roots removed (kanban dead).
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Mercury profile and shared Mercury root both contain control
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.mercury
    ...
    for mercury_root in (_HERMES_HOME, _HERMES_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(mercury_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Mercury
    secrets (``~/.mercury/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Mercury cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.mercury/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Mercury cache dirs (``/root/.mercury/...``), the session's
    ...
    # In-process gateways (Desktop backend, `mercury serve`) may not have
    ...
    # are longer prefixes, so /root/.mercury/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.mercury/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.mercury"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    Mercury-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the Mercury root (~/.mercury/.env,
    ...
    # ``MEDIA:~/.mercury/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?mercury\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+mercury[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Mercury commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Mercury sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(mercury --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Mercury home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Mercury home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.mercury/media/generated/stale.png"}
