...
_ROOM_RETENTION_REQUEST_KEY = (
    RequestKey("mercury.room_run_retention_until", float)
    if RequestKey is not None
    else "mercury.room_run_retention_until"
)
...
async def _handle_runs(
    ...
    *,
                ...
                with self._profile_scope(request_profile):
                    try:
                        ...
                        session_tokens = self._bind_api_server_session(
                            ...
                            # X-Mercury-Session-Id equivalent) exactly like
    ...
    response_headers = (
        {"X-Mercury-Session-Key": gateway_session_key} if gateway_session_key else {}
    )
    return web.json_response(
...
async def _handle_run_approval(
    ...
    *,
    ...
    return web.json_response({
        "object": "mercury.run.approval_response",
        "run_id": run_id,
        "choice": choice,
        **({"request_id": request_id} if request_id else {}),
        "resolved": resolved,
...
async def _handle_steer_run(
    ...
    *,
    ...
    return web.json_response({"object": "mercury.run.steer", "run_id": run_id, "accepted": True})
