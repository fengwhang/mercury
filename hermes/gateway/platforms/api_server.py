...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Mercury-Session-Id header; opt-in long-term memory scoping via X-Mercury-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Mercury-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists mercury-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Mercury sessions
- POST /api/sessions               — create an empty Mercury session
...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    """
    try:
        from mercury_cli.profiles import profile_matches_home
...
_BROWSER_CONTROL_WS_PROTOCOL = "mercury-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "mercury-browser-control-ticket."
...
def _hermes_version() -> str:
    """Return the canonical Mercury version string.

    ``mercury_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``mercury_agent-*.dist-info`` after a source update until the environment is
...
# config.yaml accept (mercury_constants.VALID_REASONING_EFFORTS); wire-level
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from mercury_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``mercury-agent``)
    ...
    ``direct_model_requests`` config value here, while Mercury-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Mercury-aware and is
    always honored.
    """
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # mercury_state._WAL_INCOMPAT_MARKERS).
        from mercury_state import apply_wal_with_fallback
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Mercury-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`mercury_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from mercury_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "mercury-agent")
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `mercury peer dm mini/researcher` answered by the mini's default
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Mercury-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Mercury-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Mercury-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Mercury-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Mercury-Session-Key requires API key authentication. "
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``mercury-agent``) means "use
        ...
        400s ("mercury-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "mercury-agent") is
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "mercury_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "mercury_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        by the client (via ``X-Mercury-Session-Key``).  Unlike ``session_id``
        ...
        # mercury_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "mercury-agent", "version": _hermes_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list mercury-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Mercury provider/model inventory.
        ...
        Mercury provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Mercury version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "mercury.api_server.capabilities",
            "platform": "mercury-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Mercury AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Mercury-Session-Id",
                "session_key_header": "X-Mercury-Session-Key",
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        try:
            ...
            # under the Mercury home.
            try:
                from mercury_state import get_hermes_home
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Mercury sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `mercury peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `mercury peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Mercury session row.
        ...
        # (e.g. "provider::mercury-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "mercury-agent") is nulled out
        # there — a bare "mercury-agent" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "mercury.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "mercury.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "mercury.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "mercury.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Mercury-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Mercury-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "mercury.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Mercury-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Mercury-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "mercury.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Mercury-Session-Key.  This
        # is independent of X-Mercury-Session-Id: the key persists across
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``mercury.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Mercury-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Mercury-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["mercury"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Mercury-Completed"] = "false"
            response_headers["X-Mercury-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Mercury-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Mercury-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Mercury-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if gateway_session_key:
            sse_headers["X-Mercury-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="mercury.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Mercury-Session-Id header.  Without this,
        ...
        response_headers = {"X-Mercury-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Mercury-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `mercury cron run` / cronjob(action='run', prompt=...) — same length
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Mercury agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Mercury-Session-Id header) can track compression-
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Mercury-Relay
            ...
            # that is the exact surface the mercury-0day campaign abused to write
            # ~/.mercury/config.yaml and plant persistence. Sandboxing (Docker /
