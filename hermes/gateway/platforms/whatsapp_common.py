...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "🌡️ *Mercury*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    When the install tree is read-only (e.g., Docker /opt/mercury), this function
    ...
    mercury_home_bridge = mercury_home / "scripts" / "whatsapp-bridge"
    ...
    if mercury_home_bridge.exists():
        return mercury_home_bridge

    # Mirror the bridge source to HERMES_HOME
    try:
        mercury_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            mercury_home_bridge,
            dirs_exist_ok=False,
