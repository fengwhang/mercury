...
# Inbound media cache lives under the user's mercury dir so it survives
...
def check_whatsapp_cloud_requirements() -> bool:
    ...
    for Graph API calls (outbound). Both ship with mercury-agent's default
...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        a generic audio attachment. Mercury TTS produces MP3, so we try
    ...
    async def _build_message_event_from_cloud(
        ...
        """Convert a Cloud-API message object into a Mercury MessageEvent.
