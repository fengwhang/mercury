...
from mercury_constants import get_hermes_home, _get_platform_default_hermes_home
...
# Mirrors mercury_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (mercury_constants + stdlib only).
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    label from the persisted ``mercury_home`` for locks written before the
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Mercury' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Mercury gateway lifecycle subcommand from a command line.
    ...
    test also matched ``mercury_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\mercury-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Mercury's
    ...
    for token in tokens:
        ...
        if basename in ("mercury-gateway", "mercury-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "mercury_cli.main" in joined
        or "mercury_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("mercury", "mercury.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `mercury gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Mercury-owned runtime records
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``mercury_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"mercury_home={home_lc}" in command_lc
        )
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "mercury_home=" in command_lc and f"mercury_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        "mercury_home": str(_canonical_hermes_home(_get_process_hermes_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``mercury_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `mercury update` (and the
...
def _pid_record_belongs_to_current_profile(
    ...
    """Return True when the PID record's ``mercury_home`` matches the current process.

    PID records written by ``_build_pid_record()`` include the gateway's
    ``mercury_home`` at write time. If a profile gateway was started (or recorded)
    ...
    Records that predate the ``mercury_home`` field (pre-#74872 gateways) are
    accepted conservatively (no field → assume current profile).
    """
    if not isinstance(record, dict):
        return False
    record_home = record.get("mercury_home")
    if not record_home:
        # Records without mercury_home belong to a pre-#74872 gateway;
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `mercury status` show per-profile
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``mercury_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    # verify the persisted record's mercury_home matches the current process.
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    # is inferable; readers fall back to deriving it from mercury_home.
...
def _get_takeover_marker_path(mercury_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``mercury_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = mercury_home or _get_process_hermes_home()
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``mercury gateway stop`` on Windows would be misclassified as an
...
def _validated_scoped_lock_gateway_owner(
    ...
    raw_home = record.get("mercury_home")
    ...
    pid_record_home = pid_record.get("mercury_home")
