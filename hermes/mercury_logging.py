"""Centralized logging setup for Mercury.
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``mercury_cli.web_server``,
    ``mercury_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Mercury (TUI, gateway, ``hy_memory`` server, MCP
...
def _install_session_record_factory() -> None:
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.mercury_home = str(get_hermes_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.mercury_home = ""  # type: ignore[attr-defined]
        return record
...
COMPONENT_PREFIXES = {
    ...
    # ``mercury logs --component gateway``.
    "gateway": ("gateway", "mercury_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("mercury_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "mercury_cli.web_server",
        "mercury_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    home = mercury_home or get_hermes_home()
    from mercury_constants import mkdir_under_hermes_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the mercury group. However, both ``_open()`` (initial creation)
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Mercury home.
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "mercury_home", "")
        try:
...
# the module header): when several Mercury processes log to the same file, an
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="mercury-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse mercury_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when mercury_cli.config isn't importable (bare
        # mercury_logging consumers).
        try:
            from mercury_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
