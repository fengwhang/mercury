"""Run the read-tool eval through the REAL Mercury AIAgent.
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    mercury_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".mercury"
    mercury_home.mkdir(parents=True)
    build_workspace(ws)

    old_env = dict(os.environ)
    os.environ["HERMES_HOME"] = str(mercury_home)
    ...
    try:
        ...
        shutil.rmtree(mercury_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.mercury/.env; set +a  — then relaunch."
        )
