"""ACP auth helpers — detect and advertise Mercury authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "mercury-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Mercury runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Mercury can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Mercury.
    ...
    not have Mercury provider credentials configured yet, so Mercury always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Mercury using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Mercury provider",
            description=(
                "Open Mercury' interactive model/provider setup in a terminal. "
                "Use this when Mercury has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
