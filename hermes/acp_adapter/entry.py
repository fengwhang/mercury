"""CLI entry point for the mercury-agent ACP adapter.

Loads environment variables from ``~/.mercury/.env``, configures logging
...
try:
    ...
    # shadowing Mercury's own modules — ``mercury acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    mercury_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from HERMES_HOME (default ``~/.mercury``)."""
    from mercury_cli.env_loader import load_hermes_dotenv

    mercury_home = get_hermes_home()
    loaded = load_hermes_dotenv(mercury_home=mercury_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", mercury_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="mercury-acp",
        description="Run Mercury as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Mercury version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Mercury provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.mercury/node/ "
...
def _run_check() -> None:
    ...
    print("Mercury ACP check OK")


def _run_setup() -> None:
    from mercury_cli.main import main as mercury_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "mercury", "model"]
        mercury_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting mercury-agent ACP adapter")
