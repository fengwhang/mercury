...
This is an additive Mercury extension surfaced under ACP ``_meta.mercury`` so
...
compression rotates the internal Mercury head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.mercury.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_hermes_session_id: The live internal Mercury DB session id
    ...
    Returns:
        A dict suitable for ``{"mercury": {"sessionProvenance": <dict>}}`` under
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"mercury": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"mercury": {"sessionProvenance": prov}}
