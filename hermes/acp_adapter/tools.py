"""ACP tool-call helpers for mapping mercury tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map mercury tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
_POLISHED_TOOLS = {
    ...
    # MERCURY-OMP PATCH (B2): the kanban tools are removed from the ACP
    # polished list (delegation is omp; see tools/omp_delegation.py).
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a mercury tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Mercury tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Mercury tool result clearly failed.
    ...
    # Mercury core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Mercury read_file output is line-numbered with `|`. If we send it as raw
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given mercury tool invocation.
