...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with mercury-agent)
2. User plugins: ``$HERMES_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``mercury_agent.plugins``
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``mercury_agent.plugins``
    ...
    * **Provider targets only.** The ``mercury_agent.plugins`` group is shared
    ...
    try:
        from mercury_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    # 0. Pip-installed plugins — entry points in the ``mercury_agent.plugins``
    ...
    # 1. Bundled plugins — shipped with mercury-agent.
