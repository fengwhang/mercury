...
Set via `mercury model` (picker) or `mercury setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `mercury auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `mercury auth add openai-codex` |
| qwen-oauth | OAuth | `mercury auth add qwen-oauth` |
| minimax-oauth | OAuth | `mercury auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`mercury auth`).
Fallback chain when the primary fails: `mercury fallback add|remove|list`.
...
#   mercury config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
