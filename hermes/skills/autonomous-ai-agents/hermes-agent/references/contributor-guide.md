...
Config: `~/.mercury/config.yaml` (settings), `~/.mercury/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
never hardcode `~/.mercury`. For custom/local-only tools, write a plugin in
`~/.mercury/plugins/` instead of editing core — see the developer docs.
