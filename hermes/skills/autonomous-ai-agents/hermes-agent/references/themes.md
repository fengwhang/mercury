...
`~/.mercury/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- Write access to the Hermes home dir — `~/.mercury` by default, or `$HERMES_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `mercury config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   mercury config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `mercury config set display.skin default` (or they can `/skin default`).
...
palette and resets the background. `mercury skin set background "#08201f"` changes
only the background; `mercury skin use <name>` / `mercury skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.mercury`** when a profile is active — resolve the real home
  from `$HERMES_HOME` first, falling back to `~/.mercury`.
...
- **Never hand-edit `config.yaml` to activate.** Use `mercury config set
...
- **You apply it, not the user.** `mercury config set display.skin <name>` is
...
- Run `mercury config get display.skin` and confirm it reports `<name>`.
