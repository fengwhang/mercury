...
the Hermes CLI, TUI, and desktop app. This skill drives the `mercury pets` CLI
...
Use the `terminal` tool to run `mercury pets <subcommand>`.

## Quick Reference

| Goal | Command |
| --- | --- |
| Browse the gallery | `mercury pets list` (add a substring to filter: `mercury pets list cat`) |
| List installed pets | `mercury pets list --installed` |
| Install a pet | `mercury pets install <slug>` (add `--select` to make it active) |
| Set the active pet | `mercury pets select <slug>` (omit slug for a picker) |
| Resize the pet everywhere | `mercury pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate in terminal | `mercury pets show [slug] [--cycle] [--state run]` |
| Disable the pet | `mercury pets off` |
| Remove a pet | `mercury pets remove <slug>` |
| Diagnose setup | `mercury pets doctor` |

## Procedure

1. Find a pet: `mercury pets list <query>` and note its `slug`.
2. Install + activate: `mercury pets install <slug> --select`.
3. Preview it: `mercury pets show` (Ctrl+C to stop).
4. Confirm setup: `mercury pets doctor` — shows the resolved pet, configured
...
- `scale` (float) — on-screen size of the native 192×208 frames (default 0.33,
  ...
  `mercury pets scale <factor>`, the `/pet scale` slash command, or the desktop
  Appearance slider.
...
- The petdex npm CLI installs to `~/.codex/pets`; Hermes uses its own
  profile-scoped `<HERMES_HOME>/pets/` instead — install through `mercury pets`.

## Verification

- `mercury pets doctor` reports `✓ ready` when a pet is installed, selected,
