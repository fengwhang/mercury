...
system (`~/.mercury/plugins/`) is otherwise documented separately.
...
1. Create `$HERMES_HOME/desktop-plugins/<name>/plugin.js` from
   ...
   `~/.mercury/...` by default, or `~/.mercury/profiles/<profile>/...` under a
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.mercury/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
