...
What Mercury has that *feels* like OAuth — `mercury login --provider nous`
opening a browser, user signs in, token lands in `~/.mercury/auth.json` — is a
...
1. Read Hermes's existing Portal credential out of `~/.mercury/auth.json`.
...
pitch in the Tool Gateway announcement is specifically about Mercury's
