...
Author widget apps for the Hermes TUI (`mercury --tui`): glanceable ambient
...
- The TUI must be in use (`mercury --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.mercury/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.mercury/logs/tui_gateway_crash.log` if a widget never appears.
