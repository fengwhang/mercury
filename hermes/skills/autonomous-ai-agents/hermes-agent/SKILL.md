---
name: hermes-agent
description: "Use, configure, theme, extend, and orchestrate Mercury (hybrid distribution of Hermes by Nous Research and omp by can1357)."
version: 3.2.0
...
You are running on Mercury — a hybrid distribution of Hermes by Nous Research and omp by can1357. The hermes half is this agent (conversation, memory, skills, platforms, scheduling); the omp half is the delegate coding engine (delegate_task subagents, /omp command). Hermes' own docs remain the authoritative reference for the hermes half; omp's docs (omp.sh) for the omp half.

Upstream context: Hermes Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Hermes works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.
...
- CLI commands: `mercury --help`, `hermes <command> --help`, and `hermes_cli/main.py`
...
# Other surfaces
mercury desktop                 # launch the native desktop app (alias: mercury gui)
mercury dashboard               # web admin panel + embedded chat
mercury proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```

## Key Paths

```
~/.mercury/config.yaml       Main configuration (settings — never secrets)
~/.mercury/.env              API keys and secrets ONLY (under $HERMES_HOME if set)
$HERMES_HOME/skills/        Installed skills
~/.mercury/skins/            Custom themes (see references/themes.md)
~/.mercury/desktop-plugins/  Desktop app UI plugins (see references/desktop-plugins.md)
~/.mercury/tui-widgets/      TUI widget apps (see references/tui-widgets.md)
~/.mercury/pets/             Installed pet mascots (see references/petdex.md)
~/.mercury/state.db          Canonical session store (SQLite + FTS5)
~/.mercury/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
...
Profiles use `~/.mercury/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$HERMES_HOME` — never hardcode `~/.mercury`.
...
| Delegation, cron, curator | `references/background-systems.md` |
| MCP servers (add, catalog, `mercury mcp`) | `references/native-mcp.md` |
...
Two theming rules that hold even without loading the reference: **you apply skins yourself** (`mercury config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`mercury skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.
...
- **"delegate_task is capped at N" reports** — in Mercury, delegate children run as omp one-shots with native recursion; there are no hermes-style cap paths. If a model reports a cap, it is self-limiting.
...
- **Desktop app** (`mercury desktop` / `mercury gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`mercury dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `mercury --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`mercury --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`mercury proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Never hand-edit `config.yaml` for the user** — use `mercury config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
