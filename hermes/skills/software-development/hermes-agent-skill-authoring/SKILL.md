...
1. **User-local:** `~/.mercury/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
...
- Don't use for: personal skills in `~/.mercury/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `mercury skills install official/<category>/<skill>`.
...
- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.mercury/skills/`.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.mercury/skills/`, not the repo tree. Use `write_file`.
