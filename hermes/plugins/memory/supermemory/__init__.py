...
_DEFAULT_CONTAINER_TAG = "mercury"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=mercury"
...
def _load_supermemory_config(mercury_home: str) -> dict:
    config = _default_config()
    config_path = Path(mercury_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, mercury_home: str) -> None:
    config_path = Path(mercury_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "mercury"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Mercury writes into the "Mercury" Space in the Supermemory
        ...
        merged = {"sm_source": "mercury", **(metadata or {})}
...
def _resolve_container_tag_for_setup(mercury_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(mercury_home)
...
def _probe_supermemory_connection(api_key: str, mercury_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(mercury_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(mercury_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `mercury memory setup`.
    ...
    def save_config(self, values, mercury_home):
        ...
        _save_supermemory_config(sanitized, mercury_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, mercury_home)
    ...
    def post_setup(self, mercury_home: str, config: dict) -> None:
        ...
        from mercury_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, mercury_home=mercury_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from mercury_constants import get_hermes_home
        self._hermes_home = kwargs.get("mercury_home") or str(get_hermes_home())
