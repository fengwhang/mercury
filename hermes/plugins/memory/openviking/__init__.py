...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-mercury/{_HERMES_VERSION}"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Mercury slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Mercury to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Mercury refused the connection."
        ) from exc
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("HERMES_HOME", "")).expanduser() if os.environ.get("HERMES_HOME") else Path.home() / ".mercury"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Mercury did not start "
    ...
    try:
        ...
        # The Mercury Desktop backend can include the Mercury venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Mercury venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Mercury venv,
        # aborting `mercury update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Mercury will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Mercury session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Mercury only", "write values only to Mercury .env"),
    ...
    _print_openviking_ready("Connection saved to Mercury .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See mercury-agent#28296 review.
    ...
    def post_setup(self, mercury_home: str, config: dict) -> None:
        ...
        from mercury_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        mercury_home_path = Path(mercury_home)
        env_path = mercury_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        mercury_home = str(kwargs.get("mercury_home") or "").strip()
        if not mercury_home:
            try:
                from mercury_constants import get_hermes_home
                mercury_home = str(get_hermes_home())
            except Exception:
                mercury_home = str(Path.home() / ".mercury")
        self._hermes_home = mercury_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Mercury will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.mercury/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts mercury (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Mercury will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Mercury' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Mercury canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See mercury-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/mercury/.overview.md).
