...
the ``mercury`` integration, and applies a chosen manifest to the user's bank
...
def fetch_hermes_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``mercury`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "mercury" in (e.get("integrations") or [])]
