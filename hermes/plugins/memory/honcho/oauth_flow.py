...
``mercury://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Mercury OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "mercury-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``mercury-desktop`` / ``mercury-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Mercury.</div>"
)
