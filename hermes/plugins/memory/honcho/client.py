...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Mercury instances)
...
from mercury_cli.profiles import _get_default_hermes_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Mercury profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Mercury profile.
    ...
    Resolution order:
      ...
      4. Fallback: ``"mercury"`` (default profile)
    """
    ...
    # but applying it before profile resolution makes every named Mercury
    # profile share that same host.  Keep named profiles isolated; only the
    # default Mercury profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.mercury/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.mercury
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Mercury profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.mercury block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Mercury-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Mercury session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Mercury session title (from /title command; non-per-session)
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'mercury honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.mercury_home) if config.mercury_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/mercury_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'mercury honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `mercury honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                mercury_cfg = load_config()
                honcho_cfg = mercury_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
