...
Handles: mercury honcho setup | status | sessions | map | peer
"""
...
from mercury_cli.config import cfg_get
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "mercury" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Mercury profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'mercury honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'mercury honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `mercury update` -- no output, no exceptions.
    """
    try:
        from mercury_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Mercury profile."""
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        mercury_host.get("pinUserPeer"),
        mercury_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in mercury_host:
        aliases_src = mercury_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in mercury_host:
        prefix_src = mercury_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(mercury_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom mercury_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    mercury_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        mercury_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(mercury_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = mercury_host.get("userPeerAliases")
    prefix = mercury_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(mercury_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(mercury_host)
    mercury_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        mercury_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        mercury_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/mercury (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Mercury persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = mercury_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            mercury_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(mercury_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'mercury honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'mercury honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'mercury honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(mercury_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'mercury honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = mercury_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        mercury_host["peerName"] = new_peer

    current_ai = mercury_host.get("aiPeer") or cfg.get("aiPeer", "mercury")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        mercury_host["aiPeer"] = new_ai

    current_workspace = mercury_host.get("workspace") or cfg.get("workspace", "mercury")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        mercury_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Mercury GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, mercury_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Mercury gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = mercury_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(mercury_host)
            mercury_host["pinUserPeer"] = False
            if prior_aliases:
                mercury_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                mercury_host, current_prefix, prefix_from_root,
            ...
            if merged:
                mercury_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                mercury_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                mercury_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = mercury_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        mercury_host["observationMode"] = new_obs
    else:
        mercury_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(mercury_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        mercury_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        mercury_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = mercury_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        mercury_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = mercury_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        mercury_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                mercury_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(mercury_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            mercury_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        mercury_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        mercury_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        mercury_host["dialecticReasoningLevel"] = new_reasoning
    else:
        mercury_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = mercury_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        mercury_host["sessionStrategy"] = new_strat

    mercury_host["enabled"] = True
    mercury_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        mercury_config = load_config()
        mercury_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(mercury_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: mercury config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    mercury honcho status     -- show full config")
    print("    mercury honcho mode       -- change recall/observation mode")
    print("    mercury honcho tokens     -- tune context and dialectic budgets")
    print("    mercury honcho peer       -- update peer names")
    print("    mercury honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if mercury_cli internals move."""
    try:
        from mercury_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(mercury_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    mercury_host["apiKey"] = cred.access_token
    mercury_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        mercury_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Mercury profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("mercury.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: mercury honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'mercury honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'mercury honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: mercury honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        mercury = hosts.get(_host_key(), {})
        user = mercury.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = mercury.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = mercury.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = mercury.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Mercury' identity in Honcho. Seed with 'mercury honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: mercury honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: mercury honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = mercury.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = mercury.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = mercury.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Mercury asks Honcho's AI peer a question")
        ...
        print("\n  Set with: mercury honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'mercury honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'mercury honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    mercury honcho identity --show        — show both peer representations")
        print("    mercury honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Mercury + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Mercury\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Mercury persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  mercury honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'mercury honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'mercury honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  mercury honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'mercury'")
        print("  with Honcho configured and no prior session history.")
        print("  (Mercury calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    mercury honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'mercury honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Mercury, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'mercury honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    mercury honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  mercury honcho identity <path/to/SOUL.md>")
    ...
    print("    Mercury:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Mercury:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Mercury:   Honcho observes every message and updates representations")
    ...
    print("    Mercury:   per-session by default — each run gets its own session")
    print("              Map a custom name:  mercury honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. mercury honcho setup              — configure API key (required)")
        print("  2. mercury honcho migrate            — re-run this walkthrough")
    else:
        print("  1. mercury honcho status             — verify Honcho connection")
        print("  2. mercury                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. mercury honcho identity --show    — verify AI peer representation")
        print("  4. mercury honcho tokens             — tune context and dialectic budgets")
        print("  5. mercury honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'mercury memory setup'...\n")
        from mercury_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``mercury honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``mercury honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to mercury memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Mercury Honcho",
    )
