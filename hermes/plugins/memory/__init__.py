...
3. Project-local providers: ``./.mercury/plugins/<name>/``, opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``mercury_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "mercury_agent.memory_providers"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.mercury/plugins/`` or None if unavailable or not opted in.
    ...
    """
    try:
        from mercury_cli.plugins import _env_enabled

        if not _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".mercury" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($HERMES_HOME/plugins/<name>/)
    # 3. Project-local providers (./.mercury/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``mercury <provider>`` subcommands — working, but a second-class citizen next
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from mercury_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``mercury memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every mercury startup.
        """
        if self._context is None:
            from mercury_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
