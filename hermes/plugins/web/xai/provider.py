...
``{title, url, description, position}`` rows every other Mercury web
provider produces.
...
prefers Mercury-managed xAI Grok OAuth (via ``mercury auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.mercury/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``mercury tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `mercury auth` to sign in with "
