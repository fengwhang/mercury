"""Meta Model API (Muse Spark) provider plugin for Mercury.
...
Bundled from https://github.com/albertodepaola/mercury-meta-provider. Mercury'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to mercury' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Mercury' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in mercury_cli/providers.py and the retention hint in
