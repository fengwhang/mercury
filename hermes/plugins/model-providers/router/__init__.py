"""Ramp Router (router.com) provider plugin for Mercury.
...
* **Responses API is the native wire.** Router serves ``GET /v1/models``
  ...
  in ``mercury_cli/providers.py`` keep every path on the native wire —
...
``mercury_cli/models.py``: cache-only lookups on the per-request hot path
...
def _resolve_api_key() -> str:
    ...
    raw environment when the mercury_cli helper is unavailable (e.g. stripped
    test environments).
    """
    resolvers = []
    try:
...
def _warm_efforts_async() -> None:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        # Match the canonical caps warmer (mercury_cli/models.py): a mid-suite
...
router = RouterProfile(
    ...
    base_url=_base_url(),
    auth_type="api_key",
    # Identify Mercury traffic to the gateway (Router attributes coding-agent
    ...
    default_headers={"User-Agent": f"Mercury-Agent/{_HERMES_VERSION}"},
