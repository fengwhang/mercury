...
gateway (which Mercury uses to bill Browser Use sessions to a Nous
...
Auth env vars (one of)::
    ...
    # OR a managed Nous gateway entry (configured via 'mercury setup')
"""
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the mercury tools picker: the "Browser Use" row now
