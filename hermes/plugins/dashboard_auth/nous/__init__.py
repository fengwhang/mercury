...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``mercury-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``mercury_session_pkce``
cookie.
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"mercury_session_pkce\"]``
...
def register(ctx) -> None:
    ...
    engagement layer (``mercury_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "'agent:{instance_id}') when it deploys a Mercury "
