...
A standards-compliant OpenID Connect Relying Party for the ``mercury dashboard``
...
:class:`~mercury_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``mercury_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~mercury_cli.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/mercury/   # required
          client_id: mercury-dashboard                              # required
          scopes: "openid profile email"                           # optional
          # client_secret: set ONLY for a confidential client. It is a
          # credential — prefer the env var / ~/.mercury/.env over config.yaml.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "mercury_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Mercury does not call
...
def register(ctx) -> None:
    ...
    # canonical home is the env var / ~/.mercury/.env; config.yaml is supported
