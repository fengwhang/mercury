"""`mercury meet node ...` subcommand tree.

Wired into the existing ``mercury meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``mercury meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="mercury-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``mercury meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             mercury meet node approve <name> ws://<host>:{args.port} {token}")
        try:
