"""
Microsoft Teams platform adapter for Mercury.
...
Requires:
    ...
    manually: ``<mercury-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Mercury owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Mercury"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        mercury_action = data.get("mercury_action", "")
        session_key = data.get("session_key", "")

        if not mercury_action or not session_key:
        ...
        choice = choice_map.get(mercury_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="mercury_approve",
            data={**btn_data_base, "mercury_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="mercury_approve",
                data={**btn_data_base, "mercury_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="mercury_approve",
                    data={**btn_data_base, "mercury_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="mercury_approve",
            data={**btn_data_base, "mercury_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Mercury\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.mercury/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Mercury
