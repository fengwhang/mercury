"""
``mercury photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Mercury
...
# wiring imports this module for `mercury --help`).
...
# when called later (gateway start, mercury status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `mercury photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Mercury')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``mercury photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Mercury" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.mercury/.env,
    ...
    try:
        from mercury_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  mercury gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `mercury photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`mercury photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.mercury/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `mercury photon telemetry on` / `mercury photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.mercury/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  mercury gateway restart")
    return 0
...
#
# `mercury gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``mercury photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `mercury gateway setup` wizard."""
