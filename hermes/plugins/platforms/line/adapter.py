"""
LINE Messaging API platform adapter for Mercury.
...
to Mercury. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            mercury_home = Path(get_hermes_home()).resolve()
        except Exception:
            mercury_home = Path.home().joinpath(".mercury").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``mercury status`` even before the adapter is instantiated."""
...
def interactive_setup() -> None:
    ...
    try:
        from mercury_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("mercury_cli.config not available; set LINE_* vars manually in ~/.mercury/.env")
        return
