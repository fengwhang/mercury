...
In single-bot mode (what Mercury currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="mercury-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Mercury gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``mercury_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"mercury_action": action_name, "approval_id": approval_id},
                }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "mercury_update_prompt_action": answer,
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Mercury does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        mercury_action = action_value.get("mercury_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("mercury_update_prompt_action")
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("mercury_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("mercury_update_prompt_action", "") or "").strip().lower()
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Mercury webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Mercury' SessionSource fields.
    ...
    async def _connect_websocket(self) -> None:
        ...
        self._ws_client = FeishuWSClient(
            ...
            # See https://github.com/NousResearch/mercury-agent/issues/50656
...
# Called by `mercury gateway setup` via _setup_feishu() in mercury_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=mercury&tp=mercury"
    else:
        qr_url += "?from=mercury&tp=mercury"
    return {
...
# dict in mercury_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `mercury setup` to install Feishu support."}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in mercury_cli/gateway.py and the static
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mercury setup` to install Feishu support.",
