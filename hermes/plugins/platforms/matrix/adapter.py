...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Mercury command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Mercury accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Mercury ``/command`` text."""
...
from mercury_constants import get_hermes_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Mercury
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "mercury"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Mercury" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in mercury_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces mercury_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via mercury_cli.gateway.get_env_value so setup-status
