...
# trigger its own recovery path. Refs: NousResearch/mercury-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: NousResearch/mercury-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Mercury-created private-chat topic lanes are marked with
        ...
        queued follow-ups after a gateway restart) prefer the Mercury topic's
        ``message_thread_id`` so they stay in the active topic lane (#87051);
        ``direct_messages_topic_id`` is only used when no topic thread
        resolves, since the native DM-topic id does not match the Mercury
        ...
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            ...
            if reply_to_message_id is None:
                ...
                # active topic lane: prefer the Mercury topic thread id when it
    ...
    # older PTB/clients. Streaming edits stay on Mercury' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Mercury must skip rich delivery up front and use the legacy MarkdownV2
        ...
        try:
            if app and app.updater and app.updater.running:
                try:
                    ...
                    # Refs: NousResearch/mercury-agent#58270
                    await _await_with_thread_deadline(
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `mercury update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Mercury or OpenClaw instance is running "
            "with this token, then restart the gateway with 'mercury gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                from mercury_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                if not self._bot:
                    return
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  Mercury defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Mercury process (shared mount, same machine, etc.).
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``mercury model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from mercury_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from mercury_cli.models import PROVIDER_GROUPS
    ...
    async def _handle_inline_query(
        ...
        Mercury default), so most skill commands can never appear in the
    ...
    # Scripts live in ~/.mercury/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".mercury" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@mercury_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Mercury bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Mercury inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Mercury inbox (#52363).
        #
...
# mercury_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in mercury_cli/gateway.py.
    """
    from mercury_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mercury setup` to install Telegram support.",
