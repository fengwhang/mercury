"""SimpleX Chat platform adapter (Mercury plugin).
...
``plugins/platforms/simplex/``. The Mercury plugin loader scans the
...
discoverable and ``mercury setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Mercury — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``mercury setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.mercury/.env`` via
    ``mercury_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "mercury_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.mercury/.env"
        )
        return
