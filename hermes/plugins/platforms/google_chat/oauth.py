...
try:
    from mercury_constants import display_hermes_home, get_hermes_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where mercury_constants isn't importable
...
# ``ls ~/.mercury/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `mercury setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `mercury setup` to repair the managed installation, then retry.")
        return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Mercury (native attachment delivery)"
    )
