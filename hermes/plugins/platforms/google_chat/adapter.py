...
# ``model_tools`` import time, so ``mercury status``, ``mercury chat``, etc.
...
# ``mercury_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Mercury is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a mercury MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "mercury-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from mercury_constants import get_hermes_home as _get_hermes_home
            _hermes_home = _get_hermes_home()
        except (ModuleNotFoundError, ImportError):
            _hermes_home = _Path.home() / ".mercury"
        ...
        # they don't sit in the chat forever as "Mercury is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERMES_HOME", str(_Path.home() / ".mercury"))
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a mercury MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Mercury is thinking…' marker message.
        ...
        "Mercury is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Mercury is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Mercury is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Mercury is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``mercury setup``.

    The setup wizard at ``mercury_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.mercury/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. mercury-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.mercury/.env")
    print_info("Restart the gateway: mercury gateway restart")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Mercury plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mercury setup` to install Google Chat support.",
        ...
        # the "Mercury is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Mercury is thinking…' marker message "
