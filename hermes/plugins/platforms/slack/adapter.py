...
# identify HermesAgent traffic — matching other Mercury outbound surfaces
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``mercury config set`` and Railway persist YAML
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|mercury>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `mercury slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `mercury gateway setup` "
                "or in the active profile's ~/.mercury/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `mercury gateway setup` "
                "or add it to your active profile's ~/.mercury/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `mercury gateway setup` "
                "or in the active profile's ~/.mercury/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `mercury gateway setup` "
                "or add it to your active profile's ~/.mercury/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            # Mercury has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Mercury can "
                    "be removed from the Slack app manifest via "
                    "`mercury slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /mercury prefix). A single regex
            ...
            # manifest (see `mercury slack manifest`). In Socket Mode, Slack
            ...
            from mercury_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/mercury$")
            ...
            for _action_id in (
                "mercury_approve_once",
                "mercury_approve_session",
                "mercury_approve_always",
                "mercury_deny",
            ):
            ...
            for _action_id in (
                "mercury_confirm_once",
                "mercury_confirm_always",
                "mercury_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("mercury_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^mercury_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("mercury_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'mercury slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Mercury handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Mercury session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Mercury owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message_impl(
        ...
        # the shape produced by peer Mercury agents in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "mercury_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "mercury_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "mercury_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "mercury_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "mercury_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``mercury_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``mercury_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"mercury_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "mercury_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "mercury_confirm_once": "once",
            "mercury_confirm_always": "always",
            "mercury_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "mercury_approve_once": "once",
            "mercury_approve_session": "session",
            "mercury_approve_always": "always",
            "mercury_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "mercury_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/mercury <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/mercury
        ...
        if slash_name in {"mercury", ""}:
            # Legacy /mercury <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from mercury_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/mercury   ``).
        ...
        # questions via "/mercury <question>" must produce public replies so
...
# ``mercury_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``mercury_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from mercury_cli.config import get_env_value, remove_env_value, save_env_value
    from mercury_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            from mercury_cli.slack_cli import _build_full_manifest
            from mercury_constants import get_hermes_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Mercury",
                bot_description="Your Mercury agent on Slack",
            )
            ...
            print_info(
                "   Re-run `mercury slack manifest --write` anytime to refresh after "
                "Mercury adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `mercury update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``mercury_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mercury setup` to install Slack support.",
        # Interactive setup wizard — replaces mercury_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in mercury_cli/gateway.py.
