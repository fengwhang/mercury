"""ntfy platform adapter (Mercury plugin).
...
a Mercury dependency.

This adapter ships as a Mercury platform plugin under
``plugins/platforms/ntfy/``. The Mercury plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "mercury-in"              # subscribe topic (incoming)
          publish_topic: "mercury-out"     # optional — defaults to topic
...
_ECHO_TAG = "mercury-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``mercury cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Mercury dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `mercury gateway status` without
