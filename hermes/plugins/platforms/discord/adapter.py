...
def _needs_server_members_intent(
    ...
    """Return True when Mercury must request Discord's Server Members intent.
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://mercury-agent.nousresearch.com/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.mercury/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Mercury gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Mercury requested (#79430): Message
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Mercury response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Mercury work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Mercury/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:mercury|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Mercury pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.mercury/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Mercury to the latest version")
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Mercury has an LLM-generated session title and can safely rename
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Mercury: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Mercury semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Mercury session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Mercury handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Mercury to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Mercury needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Mercury needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Mercury could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``mercury pairing approve``).
    ...
    # so users approved via ``mercury pairing approve`` can interact with
...
# is not in this process (e.g. ``mercury cron`` running standalone) and no live
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``mercury_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mercury setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # mercury_cli/setup.py::_setup_discord function.  Same shape as Teams.
