"""langfuse — Mercury plugin for Langfuse observability.

Traces Mercury conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Mercury plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``mercury plugins enable
observability/langfuse`` or ``mercury tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``mercury tools`` or ~/.mercury/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Mercury plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `mercury tools` and configure Langfuse "
        ...
        # short-lived-process gap (kanban workers / mercury chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older Mercury paths only expose ``task_id``/``session_id``. Newer paths
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Mercury usage into Langfuse usage and cost maps."""
    ...
    # Mercury estimate rather than summing components because it also includes
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Mercury turn",
                tags=["mercury", "langfuse"],
            ):
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``mercury chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Mercury branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Mercury also has a turn-scoped
    ...
    # messages is a list only for legacy Mercury branches that fired
    # pre_llm_call with API messages directly. Current Mercury fires
...
def register(ctx) -> None:
    ...
    # Mercury versions.  pre_api_request / post_api_request fire per API
