...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=mercury``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`mercury_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``mercury tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from mercury the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `mercury tools` → Image "
                    "Generation → DeepInfra to configure, or `mercury setup` "
                    "to add the key."
                ),
        ...
        from mercury_cli.models import deepinfra_base_url
