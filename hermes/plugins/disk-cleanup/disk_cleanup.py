"""disk_cleanup — ephemeral file cleanup for Mercury.
...
Scope: strictly HERMES_HOME and /tmp/mercury-*
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HERMES_HOME or ``/tmp/mercury-*``.
...
def quick() -> Dict[str, Any]:
    ...
    # durable state trees.  Some installs place the Mercury checkout, venv,
    ...
    try:
        for top in mercury_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under HERMES_HOME (e.g. /tmp/mercury-*) — fall through.
        pass
