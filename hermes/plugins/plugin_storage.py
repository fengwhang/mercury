...
``<mercury home>/plugins/<name>/``. That tree is the plugin *install* dir —
``mercury plugins remove`` deletes it and ``mercury plugins update`` git-pulls
...
``<mercury home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Mercury.
...
# Mirrors the plugin-name shape `mercury plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<mercury home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`mercury_constants.get_hermes_home` on every call. Don't cache the
