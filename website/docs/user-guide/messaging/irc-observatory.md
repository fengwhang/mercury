...
- **ircd.json** — `~/.mercury/observatory/ircd.json`, written once:
  ...
  (`127.0.0.1:6670`), TLS bouncer (`127.0.0.1:6697`), history limit.
  Hand-editable, never overwritten; delete it (or `reset` in the
  wizard) to re-provision from scratch.
- **TLS material** — `observatory/tls/` holds a private CA plus a
  server certificate (825-day validity, regenerated by a fresh
  provision only): the TLS bouncer serves strict clients that
  refuse plaintext.
...
2. In your IRC client, add a server with the host and port from the
   card in their own fields: plain IRC with TLS **off** (port 6670),
   any nickname you like, and the server/bouncer password from
   `~/.mercury/.env` (`IRC_BOUNCER_PASSWORD`, never printed).
   Strict clients that demand TLS (Goguma on iOS leaves no off
   switch on some paths) use the card's TLS port (6697) instead:
   same rooms, same password — trust the observatory CA once
   (`~/.mercury/observatory/tls/ca.crt`, installed as a profile on
   the phone) and connect.
